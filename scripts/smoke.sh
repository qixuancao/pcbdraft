...
SMOKE_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/pcb-agent-smoke.XXXXXX")
cleanup() {
    rm -rf -- "$SMOKE_ROOT"
}
...
if [[ "$REAL_CODEX" == "1" ]]; then
    ...
    # touching the system demo so pcb-agent's ambiguity behavior remains strict.
    ...
    timeout 960s uv run ./pcb-agent review "$SELECTED" --output "$RUNS_DIR" --timeout 900
else
