...
def make_directory(path: Path, *, parents: bool = True) -> Path:
    ...
    try:
        ...
        raise PcbAgentError(f"cannot create private directory: {path}") from exc
    return path
...
def atomic_write_bytes(path: Path, data: bytes, *, mode: int = 0o600) -> None:
    ...
    if not path.parent.exists():
        ...
        raise PcbAgentError(f"atomic write parent is not a directory: {path.parent}")
    descriptor = -1
    temporary_name = ""
    try:
        ...
        raise PcbAgentError(f"atomic write failed: {path}") from exc
    finally:
        if descriptor >= 0:
            os.close(descriptor)
        if temporary_name:
...
def read_bytes_limited(path: Path, limit: int) -> bytes:
    try:
        ...
        raise PcbAgentError(f"cannot read artifact: {path}") from exc
    if size > limit:
        raise PcbAgentError(f"artifact exceeds {limit} byte limit: {path.name}")
    try:
...
def read_text_limited(path: Path, limit: int) -> str:
    try:
        ...
        raise PcbAgentError(f"artifact is not UTF-8 text: {path.name}") from exc
