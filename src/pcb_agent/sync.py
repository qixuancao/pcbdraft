...
class SyncPreview:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema": "pcb-agent-kicad-sync-preview",
            "version": 1,
...
def apply_kicad_import(
    ...
    *,
    ...
    transaction_parent = project.root.parent / ".pcb-agent-transactions"
    ...
    receipt: dict[str, Any] = {
        "schema": "pcb-agent-project-transaction",
        "version": 1,
        "kind": "kicad_import",
        "status": "preparing",
    ...
    try:
        ...
        atomic_write_json(receipt_path, receipt)
        lock_parent = project.root.parent / ".pcb-agent-locks"
...
def undo_kicad_import(transaction_value: str | Path) -> Path:
    ...
    if (
        ...
        or receipt.get("schema") != "pcb-agent-project-transaction"
    ):
    ...
    with ResourceLock(project, project.parent / ".pcb-agent-locks", timeout=10.0):
...
def _fsync_directory(path: Path) -> None:
    try:
        ...
        raise PcbAgentError(f"cannot fsync transaction directory: {path}") from exc
    try:
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
