"""Loopback-only HTTP application exposing the shared CopperWright service."""
...
from .application import ApplicationService, sanitize_user_text
...
class CopperWrightHTTPServer(ThreadingHTTPServer):
    daemon_threads = True
    ...
    def __init__(
        ...
        super().__init__(address, CopperWrightHandler)
...
class CopperWrightHandler(BaseHTTPRequestHandler):
    server: CopperWrightHTTPServer
    protocol_version = "HTTP/1.1"
    server_version = "CopperWright"
    sys_version = ""
    _STATIC: ClassVar[dict[str, tuple[str, str]]] = {
        "/": ("index.html", "text/html; charset=utf-8"),
        "/assets/app.css": ("app.css", "text/css; charset=utf-8"),
        "/assets/app.js": ("app.js", "text/javascript; charset=utf-8"),
        "/assets/copperwright-mark.png": ("copperwright-mark-128.png", "image/png"),
    }
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/api/bootstrap":
                self._json_response(
                    {
                        "schema": "copperwright-browser-bootstrap",
                        "version": 1,
        ...
        except (PcbAgentError, ValueError) as exc:
    ...
    def _valid_mutation_request(self) -> bool:
        ...
        if origin not in self.server.allowed_origins:
            ...
            return False
        supplied = self.headers.get("X-CopperWright-CSRF", "")
    ...
    def _serve_static(self, path: str) -> None:
        ...
        resource = files("pcb_agent").joinpath("web", resource_name)
    ...
    def _open_kicad(self, project_id: str) -> None:
        ...
        if not executable:
            raise PcbAgentError("KiCad desktop application is unavailable")
        ...
        try:
            ...
            raise PcbAgentError("failed to start KiCad desktop application") from exc
...
def create_app_server(
    *,
    ...
    provider: str = "auto",
) -> CopperWrightHTTPServer:
    if host not in {"127.0.0.1", "localhost", "::1"}:
        raise ValidationError(
            "CopperWright app only binds to an explicit loopback host"
        )
    ...
    return CopperWrightHTTPServer((host, port), service)
...
def run_app(
    *,
    ...
    print(f"CopperWright app: {server.base_url}", flush=True)
