...
def canonical_output_parent(value: str | None) -> Path:
    ...
    try:
        ...
        raise CopperWrightError(f"cannot prepare output parent: {parent}") from exc
    if not parent.is_dir():
        raise CopperWrightError(f"output parent is not a directory: {parent}")
    return parent
    ...
    for _ in range(8):
        ...
        try:
            ...
            raise CopperWrightError(
                f"cannot create run directory under: {parent}"
            ) from exc
    raise CopperWrightError("could not allocate a unique run id")
...
def ensure_output_outside_project(project: Path, output_parent: str | None) -> None:
    ...
    try:
        ...
        raise CopperWrightError(f"cannot resolve output parent: {raw}") from exc
    if candidate == project or candidate.is_relative_to(project):
        raise CopperWrightError("output parent must be outside the source project")
