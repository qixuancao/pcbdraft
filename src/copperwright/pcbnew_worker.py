...
def _load_job(path):
    ...
    if job["schema"] != "copperwright-pcbnew-job" or job["version"] != 1:
...
def _actual_layers(count):
    if count < 2 or count > 4:
        raise ValueError("worker supports 2-4 copper layers")
    return [pcbnew.F_Cu, *[4 + 2 * index for index in range(count - 2)], pcbnew.B_Cu]
...
def inspect_job(job):
    ...
    return {
        "schema": "copperwright-pcbnew-result",
        "version": 1,
        "mode": "inspect",
...
def build_job(job, output_path):
    ...
    if title:
        ...
        block.SetCompany("Generated by CopperWright")
        block.SetComment(
