...
def inspect_native_board(
    ...
    *,
    ...
    job = {
        "schema": "copperwright-pcbnew-job",
        "version": 1,
        "mode": "inspect_board",
        "design_id": design.design_id,
        "board_path": str(source),
    }
    with tempfile.TemporaryDirectory(prefix="copperwright-board-inspect-") as temporary:
    ...
    if result.get("mode") != "inspect_board":
        raise CopperWrightError("pcbnew worker returned a malformed board inspection")
...
def generate_pcb(
    ...
    *,
    ...
    if result.get("mode") != "build" or not isinstance(result.get("sha256"), str):
        raise CopperWrightError("pcbnew worker returned a malformed build receipt")
    ...
    if actual_hash != result["sha256"]:
        raise CopperWrightError("pcbnew worker board hash does not match its receipt")
    ...
    if project_hash != result.get("project_sha256"):
        raise CopperWrightError("pcbnew worker project hash does not match its receipt")
...
def _parse_reference_planes(value: Any) -> tuple[dict[str, Any], ...]:
    ...
    if not isinstance(value, list) or len(value) != 1:
        raise CopperWrightError("pcbnew worker did not return one reference plane")
    plane = value[0]
    if not isinstance(plane, dict) or set(plane) != required:
        raise CopperWrightError("pcbnew worker returned a malformed reference plane")
    ...
    if (
        ...
        raise CopperWrightError("pcbnew worker reference-plane evidence is invalid")
    return (dict(plane),)
...
def inspect_footprints(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="copperwright-inspect-") as temporary:
    ...
    if result.get("mode") != "inspect" or not isinstance(
        ...
        raise CopperWrightError("pcbnew worker returned a malformed inspection result")
    ...
    if set(inspections) != expected:
        raise CopperWrightError("pcbnew inspection omitted or added components")
...
def _run_worker(
    ...
    *,
    ...
    if not worker.is_file() or worker.is_symlink():
        raise CopperWrightError("trusted pcbnew worker is unavailable")
    with tempfile.TemporaryDirectory(prefix="copperwright-worker-") as temporary:
        ...
        if result.timed_out:
            raise CopperWrightError("isolated pcbnew worker timed out")
        if result.output_limited:
            raise CopperWrightError("isolated pcbnew worker exceeded its output bound")
    ...
    if (
        ...
        or value.get("schema") != "copperwright-pcbnew-result"
        or value.get("version") != 1
    ):
        raise CopperWrightError("isolated pcbnew worker result schema is invalid")
    return value
...
def _system_python(value: str | Path | None) -> Path:
    ...
    try:
        ...
        raise CopperWrightError("system Python for pcbnew is unavailable") from exc
    if (
        not resolved.is_file()
        or not resolved.is_absolute()
        or not shutil.which(str(resolved))
    ):
        raise CopperWrightError("system Python for pcbnew is not executable")
    return resolved
