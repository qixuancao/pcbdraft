    ...
    for member in _iter_members(root, max_members=TREE_MEMBER_LIMIT):
        ...
        try:
            ...
            raise CopperWrightError(
                f"cannot inspect project member: {relative}"
            ) from exc
...
def sha256_file(path: Path, *, max_bytes: int | None = None) -> str:
    ...
    try:
        ...
        raise CopperWrightError(f"cannot hash project file: {path}") from exc
...
def copy_project(source: Path, destination: Path) -> None:
    if destination.exists():
        raise CopperWrightError(f"staging destination already exists: {destination}")
    try:
        shutil.copytree(source, destination, symlinks=True, copy_function=shutil.copy2)
        privatize_tree(destination)
    except OSError as exc:
        raise CopperWrightError("failed to create private staging copy") from exc
