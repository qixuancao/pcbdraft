...
def _export(
    *,
    ...
    except (OSError, CopperWrightError):
...
def _parse_netlist(data: bytes) -> dict[str, Any]:
    try:
        ...
        raise CopperWrightError(
            "KiCad schematic netlist export is invalid XML"
        ) from exc
...
def _parse_stats(data: bytes) -> Any:
    try:
        ...
        raise CopperWrightError(
            "KiCad board statistics export is invalid JSON"
        ) from exc
...
def _parse_ipcd356(data: bytes) -> dict[str, Any]:
    try:
        ...
        raise CopperWrightError("KiCad IPC-D-356 export is not UTF-8") from exc
...
def _fit_prompt_context(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if size() > PROMPT_CONTEXT_LIMIT:
        raise CopperWrightError("semantic KiCad context exceeds the prompt limit")
    return context
...
def _managed_semantic_context(project_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return context
    except (CopperWrightError, ValidationError, KeyError, TypeError) as exc:
        return {
            "available": False,
