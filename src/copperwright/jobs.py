"""Crash-visible bounded background jobs for the local browser application."""
...
JOB_SCHEMA = "copperwright-application-job"
JOB_VERSION = 1
...
class JobRunner:
    ...
    def __init__(self, service: ApplicationService, *, workers: int = 2) -> None:
        if workers < 1 or workers > 4:
            raise ValidationError("browser job workers must be between 1 and 4")
        self.service = service
        self._pool = ThreadPoolExecutor(
            max_workers=workers, thread_name_prefix="copperwright-job"
        )
    ...
    def submit(
        ...
        *,
        ...
        if any(job["status"] in _ACTIVE for job in existing):
            raise ValidationError("project already has an active browser job")
    ...
    def _normalize_args(action: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        allowed = {"text", "timeout"} if action == "message" else {"timeout"}
        if set(args) - allowed:
