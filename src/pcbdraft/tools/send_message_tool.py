...
def _handle_list():
    ...
    try:
        from pcbdraft.services.messaging.channel_directory import (
...
def _handle_react(args, remove=False):
    ...
    runner = None
    try:
        from pcbdraft.services.messaging.run import _gateway_runner_ref
...
def _handle_send(args):
    ...
    if not is_builtin and entry is None:
        return tool_error(f"Unknown or unregistered plugin platform: {platform_name}")
    try:
        platform = Platform(platform_name)
    except (ValueError, KeyError):
        return tool_error(f"Unknown platform: {platform_name}")
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(
                    f"Platform '{platform_name}' is not configured. Set up credentials in the PCBDraft runtime config or environment variables."
                )
        else:
            return tool_error(
                f"Platform '{platform_name}' is not configured. Set up credentials in the PCBDraft runtime config or environment variables."
    ...
    from pcbdraft.services.messaging.platforms.base import BasePlatformAdapter
    ...
    if not chat_id:
        ...
        if not home and platform_name == "weixin":
            ...
            if wx_home:
                from pcbdraft.services.messaging.config import HomeChannel
        ...
        if home:
            ...
            return tool_error(
                ...
                f"or set {home_env} to the channel ID in the runtime environment."
            )
    ...
    if platform_name == "slack" and chat_id:
        ...
        if _slack_dm_target.startswith("U") and _SLACK_USER_ID_RE.fullmatch(
            _slack_dm_target
        ):
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                from pcbdraft.services.messaging.mirror import mirror_to_session

                source_label = get_session_env(
                    "PCBDRAFT_RUNTIME_SESSION_PLATFORM", "cli"
                )
                user_id = (
                    get_session_env("PCBDRAFT_RUNTIME_SESSION_USER_ID", "") or None
                )
...
def _parse_target_ref(platform_name: str, target_ref: str):
    ...
    if platform_name == "telegram":
        ...
        from pcbdraft.agent.extensions.bundled.platforms.telegram.telegram_ids import (
...
def resolve_send_target(
    ...
    try:
        from pcbdraft.services.messaging.channel_directory import resolve_channel_name
...
def _get_cron_auto_delivery_target():
    ...
    platform = (
        get_session_env("PCBDRAFT_RUNTIME_CRON_AUTO_DELIVER_PLATFORM", "")
        .strip()
        .lower()
    )
    chat_id = get_session_env("PCBDRAFT_RUNTIME_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = (
        get_session_env("PCBDRAFT_RUNTIME_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        or None
    )
    return {
        "platform": platform,
...
async def _send_to_platform(
    ...
    from pcbdraft.services.messaging.platforms.base import (
        BasePlatformAdapter,
        utf16_len,
    )
    ...
    try:
        from pcbdraft.agent.extensions.bundled.platforms.telegram.adapter import (
            TelegramAdapter,
        )
    ...
    if platform == Platform.FEISHU and media_files:
        from pcbdraft.agent.extensions.manager import discover_plugins as _dp_feishu
        from pcbdraft.services.messaging.platform_registry import (
            platform_registry as _pr_feishu,
        )
    ...
    if platform == Platform.SLACK and media_files:
        from pcbdraft.agent.extensions.manager import discover_plugins as _dp_slack
        from pcbdraft.services.messaging.platform_registry import (
            platform_registry as _pr_slack,
        )
    ...
    if platform == Platform.WHATSAPP and media_files:
        from pcbdraft.agent.extensions.manager import discover_plugins as _dp_wa
        from pcbdraft.services.messaging.platform_registry import (
            platform_registry as _pr_wa,
        )
...
async def _send_telegram(
    ...
    try:
        ...
        try:
            from pcbdraft.services.messaging.platforms.base import resolve_proxy_url
        ...
        _tg_caption = None
        from pcbdraft.services.messaging.platforms.base import utf16_len as _utf16_len
        ...
        for media_path, is_voice in media_files:
            ...
            try:
                with open(media_path, "rb") as f:
                    ...
                    if _tg_caption is not None and not (
                        ext in _VOICE_EXTS and is_voice
                    ):
                        media_kwargs["caption"] = _tg_caption
                        media_kwargs["parse_mode"] = send_parse_mode
                    if (
                        ext in _VOICE_EXTS and is_voice
                    ) or ext in _TELEGRAM_SEND_AUDIO_EXTS:
                        try:
                            from pcbdraft.agent.extensions.bundled.platforms.telegram.adapter import (
                            ...
                            duration = await asyncio.to_thread(
                                _probe_voice_duration_seconds, media_path
                            )
                        ...
                        if _is_telegram_thread_not_found(
                            ...
                            if not _has_html and media_kwargs.get("caption"):
                                try:
                                    ...
                                    media_kwargs["caption"] = _strip_mdv2(
                                        media_kwargs["caption"]
                                    )
                                except Exception:
                                    pass
                            if ext in _IMAGE_EXTS and not force_document:
...
async def _resolve_slack_user_target(token, chat_id):
    ...
    try:
        ...
        async def resolve_user_name(session, name):
            ...
            for _page in range(20):
                ...
                if not data.get("ok"):
                    return (
                        None,
                        f"Slack users.list error: {data.get('error', 'unknown')}",
                    )
            ...
            if len(matches) > 1:
                return (
                    None,
                    f"Slack user '@{name}' matched multiple Slack users. Use a Slack user ID instead.",
                )
        ...
        async with aiohttp.ClientSession(
            ...
            if not dm_id:
                return None, _error(
                    "Slack conversations.open did not return a DM channel ID"
                )
            return dm_id, None
    except Exception as e:
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    from pcbdraft.services.messaging.platforms.signal_format import markdown_to_signal
    from pcbdraft.services.messaging.platforms.signal_rate_limit import (
...
async def _send_matrix_via_adapter(
    ...
    try:
        from pcbdraft.agent.extensions.bundled.platforms.matrix.adapter import (
            MatrixAdapter,
        )
    except ImportError:
        return {
...
async def _send_bluebubbles(extra, chat_id, message):
    ...
    try:
        from pcbdraft.services.messaging.platforms.bluebubbles import (
            BlueBubblesAdapter,
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``PCBDRAFT_RUNTIME_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``PCBDRAFT_RUNTIME_HOME`` which has no ``gateway.pid``, so the gateway-running check
    ...
    if platform and platform != "local":
        return True
    try:
        from pcbdraft.core.runtime_process import is_gateway_running
...
async def _send_qqbot(pconfig, chat_id, message):
    ...
    from pcbdraft.services.messaging.config import _getenv

    extra = pconfig.extra or {}
    appid = extra.get("app_id") or _getenv("QQ_APP_ID", "")
    secret = (
        pconfig.token or extra.get("client_secret") or _getenv("QQ_CLIENT_SECRET", "")
    )
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    """
    try:
        from pcbdraft.services.messaging.platforms.yuanbao import (
            get_active_adapter,
            send_yuanbao_direct,
        )
    except ImportError:
