...
# When present in PCBDRAFT_RUNTIME_HOME, sync_skills() is a no-op so neither the
...
def _get_bundled_dir() -> Path:
    ...
    Checks PCBDRAFT_RUNTIME_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    """
    try:
        from pcbdraft.tools.skill_usage import read_suppressed_names
...
def _discover_bundled_skills(bundled_dir: Path) -> list[tuple[str, Path]]:
    ...
    for skill_md in bundled_dir.rglob("SKILL.md"):
        ...
        if is_excluded_skill_path(skill_md.relative_to(bundled_dir), root=bundled_dir):
            continue
        skill_dir = skill_md.parent
...
def _find_installed_skill_dir_by_name(
    skill_dir_name: str,
    installed_index: dict[str, list[Path]] | None = None,
) -> Path | None:
...
def _backfill_optional_provenance(quiet: bool = False) -> list[str]:
    ...
    backfilled: list[str] = []
    changed = False
    installed_dir_index: dict[str, list[Path]] | None = None
    for skill_md in sorted(optional_dir.rglob("SKILL.md")):
        if is_excluded_skill_path(skill_md):
            continue
        src = skill_md.parent
...
def _recover_renamed_skill(
    ...
    *user-deleted*: the old directory is stranded forever and never receives
    ...
    for candidate in active_index.get(skill_name, []):
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Explicitly restore the bundled version of {skill_name} "
        ...
        if not quiet:
            print(
                f"  → {skill_name} (moved {rel} → {dest.relative_to(SKILLS_DIR).as_posix()})"
            )
        return rel
    return None
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    if (PCBDRAFT_RUNTIME_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
            print(
    ...
    active_index: dict[str, list[Path]] | None = None
    hub_paths: set[str] | None = None
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Explicitly reset the bundled version of {skill_name} "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``PCBDRAFT_RUNTIME_HOME/skills/`` to prevent the catastrophic wipe of
    ...
    # path join, a missing ``PCBDRAFT_RUNTIME_HOME`` default, a malicious
    ...
    # ``PCBDRAFT_RUNTIME_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Manage hub-installed skills through the configured skills integration.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future bundled skill sync runs "
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Inspect hub-installed skills through the configured skills integration."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes PCBDRAFT_RUNTIME_HOME/.no-bundled-skills so the
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = PCBDRAFT_RUNTIME_HOME / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            PCBDRAFT_RUNTIME_HOME.mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "in PCBDraft.\n"
                "Delete this file to re-enable the next bundled skill sync.\n",
                encoding="utf-8",
            )
            changed = not existed
            message = (
            ...
            changed = existed
            message = (
                "Opted back in. The next bundled skill sync will re-seed bundled skills."
                if changed
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (PCBDRAFT_RUNTIME_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print(f"Syncing bundled skills into {PCBDRAFT_RUNTIME_HOME / 'skills'} ...")
