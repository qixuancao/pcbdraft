...
def _hint_command_not_found(command: str, output: str) -> str | None:
    ...
    m = re.search(
        r"(?:bash: line \d+: |bash: |sh: \d*:? ?)?([\w.+-]+): command not found", output
    )
    if not m:
        return None
    missing = m.group(1)
...
def _hint_module_not_found(command: str, output: str) -> str | None:
    ...
    m = re.search(
        r"(?:ModuleNotFoundError|ImportError): No module named '?([\w.]+)", output
    )
    if not m:
        return None
    return (
...
def _hint_gh_rate_limit(command: str, output: str) -> str | None:
