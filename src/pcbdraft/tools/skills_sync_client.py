...
def parse_sync_manifest(data: bytes) -> dict[str, bool] | None:
...
def resolve_identity() -> dict[str, Any]:
    ...
    owner = (
        claims.get("sub") or claims.get("privy_did") or claims.get("tid") or "unknown"
    )
...
# sync.base_url (or the PCBDRAFT_RUNTIME_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> str | None:
    ...
    Order: PCBDRAFT_RUNTIME_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("PCBDRAFT_RUNTIME_SYNC_BASE_URL")
...
# precedence as base_url: the PCBDRAFT_RUNTIME_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   PCBDRAFT_RUNTIME_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   PCBDRAFT_RUNTIME_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   PCBDRAFT_RUNTIME_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def _sync_config_bool(env_var: str, config_key: str, *, default: bool) -> bool:
    ...
    try:
        ...
        logger.debug(
            "skills_sync_client: config sync.%s read failed: %s", config_key, e
        )
    return default
...
def sync_feature_enabled() -> bool:
    ...
    ``PCBDRAFT_RUNTIME_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    ...
    """
    return _sync_config_bool("PCBDRAFT_RUNTIME_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``PCBDRAFT_RUNTIME_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.
    ...
    return _sync_config_bool(
        "PCBDRAFT_RUNTIME_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``PCBDRAFT_RUNTIME_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.
    ...
    """
    return _sync_config_bool(
        "PCBDRAFT_RUNTIME_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False
    )
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    """
    try:
        from pcbdraft.agent.skill_utils import is_external_skill_path
        from pcbdraft.tools.skill_usage import (
            _find_skill_dir,
            is_bundled,
            is_hub_installed,
        )
...
def list_synced_skill_names() -> list[str]:
    ...
    - **opt-out (Hermes Cloud "on by default"):** every *eligible* skill syncs
      ...
      deployment sets (via ``PCBDRAFT_RUNTIME_SYNC_DEFAULT_OPT_IN``) so a user's skills
      follow them with no per-skill setup.

    Sorted, deduped.
    """
    try:
        from pcbdraft.tools.skill_usage import load_usage
    except Exception:
        return []
    usage = load_usage() or {}
...
def _all_local_skill_names() -> list[str]:
    ...
    try:
        ...
        for skill_md in root.rglob("SKILL.md"):
            ...
            name: str | None = None
            try:
                from pcbdraft.tools.skill_usage import _read_skill_name
...
def stable_device_id() -> str:
    ...
    # declaratively via PCBDRAFT_RUNTIME_SYNC_DEVICE_NAME, so a hosted instance shows a
    ...
    env_name = (os.environ.get("PCBDRAFT_RUNTIME_SYNC_DEVICE_NAME") or "").strip()
...
class SyncConflict(RuntimeError):
    ...
    def __init__(self, actual: str | None):
        ...
        self.actual: str | None = actual or None
        super().__init__(
...
class SyncClient:
    ...
    def capabilities(self) -> dict[str, Any]:
        ...
        if r.status_code != 200:
            raise SyncError(
                f"capabilities failed: {r.status_code}", status=r.status_code
            )
        return r.json()
    ...
    def get_object(
        self, obj_hash: str, *, org_scope: bool = False
    ) -> tuple[str, bytes]:
    ...
    def put_objects(
        ...
        *,
        ...
        if r.status_code not in (200, 201):
            raise SyncError(
                f"put_objects failed: {r.status_code}", status=r.status_code
            )
        return r.json() if r.content else {}

    def cas_ref(self, name: str, from_hash: str | None, to_hash: str) -> dict[str, Any]:
    ...
    if not path.exists():
        ...
        if legacy.exists():
            try:
                ...
                logger.debug(
                    "skills_sync_client: legacy sync state migrate failed: %s", e
                )
...
def write_sync_state(data: dict[str, Any]) -> None:
    ...
    try:
        ...
        fd, tmp = tempfile.mkstemp(
            dir=str(path.parent), prefix=".sync_state_", suffix=".tmp"
        )
        try:
...
def _skill_rel_path(skill_name: str) -> PurePosixPath | None:
    """Return the skill's path relative to ~/.hermes/skills/ (posix), or None."""
    try:
        from pcbdraft.tools.skill_usage import _find_skill_dir
    except Exception:
        return None
    skill_dir = _find_skill_dir(skill_name)
    if skill_dir is None:
...
def snapshot_profile(
    ...
    for name in sorted(set(skill_names)):
        ...
        try:
            tree_hash = build_tree(
                skill_dir, objects, max_object_bytes=max_object_bytes
            )
    ...
    manifest_hash = objects.add(KIND_BLOB, build_sync_manifest_bytes(manifest_map))
...
def _build_root_tree(
    node: dict[str, Any], objects: ObjectSet, *, manifest_hash: str | None = None
) -> str:
...
def _merge_skill(base: str | None, ours: str | None, theirs: str | None) -> str:
...
def maybe_push_skills(*, message: str = "pcbdraft skill sync") -> dict[str, Any] | None:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (PCBDRAFT_RUNTIME_SYNC_ENABLED)
        ...
        logger.debug(
            "skills_sync_client: maybe_push_skills failed: %s", e, exc_info=True
        )
        return None


def maybe_pull_skills() -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.debug(
            "skills_sync_client: maybe_pull_skills failed: %s", e, exc_info=True
        )
        return None
...
def list_org_skill_names() -> list[str]:
    ...
    names: list[str] = []
    try:
        from pcbdraft.agent.skill_utils import read_active_org_id
...
def _read_org_head(client: SyncClient, org_id: str) -> str | None:
...
def _org_baseline_path(org_id: str) -> Path:
    ...
    from pcbdraft.agent.skill_utils import ORG_BASELINE_FILE
...
def list_locally_modified_org_skills(org_id: str | None = None) -> list[str]:
...
def _write_active_org_marker(org_id: str) -> None:
    ...
    try:
        from pcbdraft.agent.skill_utils import ORG_ACTIVE_MARKER
...
def _write_org_provenance(org_id: str, data: dict[str, Any]) -> None:
    ...
    try:
        from pcbdraft.agent.skill_utils import ORG_PROVENANCE_FILE
...
def maybe_pull_org_skills() -> dict[str, Any] | None:
