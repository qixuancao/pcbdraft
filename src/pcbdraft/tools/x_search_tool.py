...
from pcbdraft.tools.xai_http import (
    pcbdraft_xai_user_agent,
...
def _get_x_search_reasoning_effort() -> str | None:
    cfg = _load_x_search_config()
...
def _resolve_xai_bearer() -> tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `pcbdraft connect` "
