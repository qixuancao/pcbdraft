...
This module provides the glue:
    - ``PCBDraftTokenStorage``: persists tokens/client-info to disk so they
...
from pcbdraft.core.runtime_environment import secure_parent_dir
...
def _ensure_sdk_loaded() -> bool:
    ...
    if not _SDK_CLASSES:
        try:
            ...
            from mcp.shared.auth import OAuthClientInformationFull as _InfoFull
            from mcp.shared.auth import OAuthClientMetadata as _ClientMeta
            from mcp.shared.auth import OAuthMetadata as _Meta
            from mcp.shared.auth import OAuthToken as _Token
        except ImportError:
...
_USER_SKIPPED_SENTINEL = "__pcbdraft_user_skipped__"
...
def _get_token_dir(runtime_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses PCBDRAFT_RUNTIME_HOME so each profile gets its own OAuth tokens.
    Layout: ``PCBDRAFT_RUNTIME_HOME/mcp-tokens/``
    """
    from pcbdraft.core.runtime_environment import get_runtime_home

    base = Path(runtime_home) if runtime_home is not None else Path(get_runtime_home())
...
def _cached_redirect_port(storage: "PCBDraftTokenStorage | None") -> int | None:
...
def _cached_redirect_uri(storage: "PCBDraftTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `pcbdraft doctor` and (re)authorize through the configured MCP integration, "
...
# ---------------------------------------------------------------------------
# PCBDraftTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class PCBDraftTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        PCBDRAFT_RUNTIME_HOME/mcp-tokens/<server_name>.json         -- tokens
        PCBDRAFT_RUNTIME_HOME/mcp-tokens/<server_name>.client.json   -- client info
        PCBDRAFT_RUNTIME_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, runtime_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._runtime_home = Path(runtime_home) if runtime_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._runtime_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._runtime_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._runtime_home) / f"{self._server_name}.meta.json"
    ...
    async def get_client_info(self) -> "OAuthClientInformationFull | None":
        ...
        try:
            ...
            if getattr(info, "client_secret", None) and data.get(
    ...
    async def set_client_info(self, client_info: "OAuthClientInformationFull") -> None:
        ...
        if data.get("client_secret") and data.get("token_endpoint_auth_method") in (
            None,
            "none",
            "",
        ):
    ...
    def restore(
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._runtime_home)
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                (
                    ...
                    "<p>You can close this tab and return to PCBDraft.</p></body></html>"
                )
                if code
                else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  Run `pcbdraft doctor` for local configuration diagnostics.\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Use the configured MCP integration later to "
...
PCBDraftOAuthClientProvider: Any = None


def _get_pcbdraft_oauth_provider_class() -> type | None:
    global PCBDraftOAuthClientProvider
    if PCBDraftOAuthClientProvider is not None:
        return PCBDraftOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _PCBDraftOAuthClientProvider(OAuthClientProvider):
    ...
    _PCBDraftOAuthClientProvider.__name__ = "PCBDraftOAuthClientProvider"
    _PCBDraftOAuthClientProvider.__qualname__ = "PCBDraftOAuthClientProvider"
    PCBDraftOAuthClientProvider = _PCBDraftOAuthClientProvider
    return PCBDraftOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = PCBDraftTokenStorage(server_name, runtime_home=runtime_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "PCBDraftTokenStorage | None" = None,
) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "PCBDraft")
    scope = cfg.get("scope")
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize MCP server %s through its configured integration; diagnostics: pcbdraft doctor",
            storage._server_name,
            old_client_id,
            new_client_id,
            storage._server_name,
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"the configured MCP authorization flow for {server_name} first to complete "
    ...
    provider_class = _get_pcbdraft_oauth_provider_class()
