...
from pcbdraft.tools.computer_use.browser_route import CuaTypedBrowserRoute
...
# fetches the latest release, so a `PCBDRAFT_RUNTIME_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `PCBDRAFT_RUNTIME_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "PCBDRAFT_RUNTIME_CUA_DRIVER_CMD"
...
def _cua_configured_permission_mode() -> str:
    ...
    """
    raw = (
        str(_computer_use_cfg().get("permission_mode", "standard") or "")
        .strip()
        .lower()
    )
...
def _cua_capability_manifest() -> str | None:
...
def _linux_x11_active_window_id() -> int | None:
...
class _EmbeddedCuaDaemon:
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        self.capability_manifest: str | None = None
    ...
    def start(self) -> None:
        if self._process is not None and self._process.poll() is None:
            return
        from pcbdraft.tools.environments.local import _sanitize_subprocess_env
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    try:
        ...
        return driver_cmd, _mcp_args_with_overlay_flag(
            list(_CUA_DRIVER_ARGS), driver_cmd=driver_cmd
        )
...
def _candidate_cua_driver_commands(override: str | None = None) -> list[str]:
    ...
    ``PCBDRAFT_RUNTIME_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
...
def resolve_cua_driver_cmd(override: str | None = None) -> str | None:
    """Resolve the cua-driver executable for every runtime/status surface.

    A supplied override (or ``PCBDRAFT_RUNTIME_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_runtime_contract_status(binary: str | None = None) -> dict[str, Any]:
...
def cua_driver_update_check(*, timeout: float | None = None) -> dict[str, Any] | None:
...
def _maybe_repair_runtime_contract(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    ``PCBDRAFT_RUNTIME_CUA_DRIVER_CMD`` override is authoritative even when broken, and
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._owned_standard_runtime_socket: str | None = None
        self._transport_generation = 0
        self._transport_reset_callback: Any | None = None
    ...
    def _call_tool_via_cli(
        ...
        try:
            for attempt in range(attempts):
                ...
                if "daemon is not running" in out or "daemon is not running" in (
                    proc.stderr or ""
                ):
                    raise RuntimeError(
            ...
            images: list[str] = []
            data: Any = None
            structured: dict | None = parsed if isinstance(parsed, dict) else None
            is_error = False
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        if permission_mode not in {"standard", "bounded", "unrestricted"}:
            raise ValueError(
                f"unsupported cua-driver permission mode: {permission_mode}"
            )
        self.permission_mode = permission_mode
        if permission_mode == "unrestricted":
            ...
            self._embedded_daemon: _EmbeddedCuaDaemon | None = _EmbeddedCuaDaemon(
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            ...
            if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip():
                repair = (
                    "Update the binary selected by PCBDRAFT_RUNTIME_CUA_DRIVER_CMD or "
        ...
        if self._session._started:
            ...
            if max_dim:
                try:
                    ...
                    logger.debug(
                        "cua-driver set_config(max_image_dimension) failed: %s", e
                    )
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        if target:
            ...
            self._snapshot_tokens = {}
            self._last_app = (
                target["app_name"] or app
            )  # retained for back-compat diagnostics
            self._last_target = {
                "pid": self._active_pid,
                "window_id": self._active_window_id,
            }
    ...
    def set_agent_cursor_enabled(
        self, enabled: bool, *, cursor_id: str | None = None
    ) -> ActionResult:
    ...
    def get_agent_cursor_state(self, *, cursor_id: str | None = None) -> dict[str, Any]:
    ...
    def call_tool(
        self, name: str, args: dict[str, Any] | None = None, *, timeout: float = 30.0
    ) -> dict[str, Any]:
