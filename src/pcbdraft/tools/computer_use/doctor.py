...
def _mcp_rpc(
    proc: subprocess.Popen, msg_id: int, method: str, params: Any = None
) -> dict[str, Any]:
...
def _drive_health_report(
    ...
    *,
    ...
    if not isinstance(result, dict):
        raise RuntimeError(
            f"health_report result was not an object: {type(result).__name__}"
        )
...
def _compose_fallback_report(
    ...
    *,
    ...
    if probes.get("init_version"):
        ...
        ver_msg = (
            f"cua-driver {ver_value}"
            if ver_status == "pass"
            else (ver_value or "version unknown")
        )
    ...
    perms = (
        probes.get("permissions")
        if isinstance(probes.get("permissions"), dict)
        else None
    )
...
def _print_text_report(
    ...
    *,
    ...
    if (
        cli_v
        and report_v
        and str(report_v) not in str(cli_v)
        and str(cli_v) not in str(report_v)
    ):
...
def run_doctor(
    ...
    *,
    ...
    Honors `PCBDRAFT_RUNTIME_CUA_DRIVER_CMD` via the shared runtime resolver, so the
