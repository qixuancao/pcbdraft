...
def _config_preauthorized(action: str, args: dict[str, Any]) -> bool:
    ...
    if args.get("profile_mode") != "existing_profile":
        return False
    try:
        from pcbdraft.tools.computer_use.cua_backend import _cua_grant_existing_profile
...
def _get_backend(session_id: str = "") -> ComputerUseBackend:
    ...
    while True:
        ...
        stale_lock: threading.RLock | None = None
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "PCBDRAFT_RUNTIME_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    from pcbdraft.tools.computer_use.cua_backend import CuaDriverBackend
                    ...
                    raise RuntimeError(
                        f"Unknown PCBDRAFT_RUNTIME_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        from pcbdraft.tools.computer_use.vision_routing import (
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$PCBDRAFT_RUNTIME_HOME/cache/vision/``
...
def _format_elements(elements: list[UIElement], max_lines: int = 40) -> list[str]:
    ...
    for e in elements[:max_lines]:
        ...
        where = (
            ...
            if _bounds_unknown(e.bounds)
            else f"@ {e.bounds}"
        )
        out.append(
            f"  #{e.index} {e.role} {label!r} {where}"
...
def _capture_lost_detail(
    ...
    if truncated_elements:
        return True
    return any(len(e.label) > _MAX_ELEMENT_LABEL_CHARS for e in visible_elements)
...
def check_computer_use_requirements() -> bool:
    ...
    if sys.platform not in ("darwin", "win32", "linux"):
        return False
    from pcbdraft.tools.computer_use.cua_backend import cua_driver_binary_available

    return cua_driver_binary_available()


def get_computer_use_schema() -> dict[str, Any]:
    from pcbdraft.tools.computer_use.schema import COMPUTER_USE_SCHEMA
