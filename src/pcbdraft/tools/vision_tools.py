...
def _load_auxiliary_client() -> None:
    ...
    if async_call_llm is None or extract_content_or_reasoning is None:
        from pcbdraft.model.auxiliary_client import async_call_llm as _acl
        from pcbdraft.model.auxiliary_client import extract_content_or_reasoning as _ecr
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("PCBDRAFT_RUNTIME_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: PCBDRAFT_RUNTIME_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("PCBDRAFT_RUNTIME_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
async def _validate_image_url_async(url: str) -> bool:
    ...
    if not _image_url_shape_ok(url):
        return False
    from pcbdraft.tools.url_safety import async_is_safe_url
    ...
    async with client.stream("GET", url, headers=headers) as response:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "Could not delete partial download: %s",
                    tmp_destination,
                    exc_info=True,
                )
            raise
...
async def _download_image(
    ...
    async def _ssrf_redirect_guard(response):
        ...
        """
        from pcbdraft.tools.url_safety import (
            async_is_safe_url,
    ...
    for attempt in range(max_retries):
        try:
            ...
            from pcbdraft.tools.url_safety import create_ssrf_safe_async_client
...
def _build_scale_note(
    ...
    if scale_info:
        ...
        parts.append(
            f"Image downscaled from {ow}x{oh} to {nw}x{nh} for vision; {factor_clause}"
        )
    if crop_offset:
        parts.append(
...
def _resize_image_for_vision(
    ...
    try:
        ...
        try:
            from pcbdraft.tools.lazy_deps import ensure as _ensure_dep
...
async def _vision_analyze_native(
    ...
    try:
        ...
        from pcbdraft.tools.image_source import (
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $PCBDRAFT_RUNTIME_HOME/cache/vision/ and cleaned up
...
async def _materialize_video_from_terminal_backend(
    video_source: str, task_id: str | None
) -> Path:
