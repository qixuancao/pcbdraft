...
def _make_pcbdraft_provider_class() -> type | None:
    ...
    class PCBDraftMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            from pcbdraft.tools.mcp_tool import sdk_httpx
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._runtime_home = key[0]
    ...
    def _build_provider(
        ...
        from pcbdraft.tools.mcp_oauth import apply_oauth_provider_defaults
    ...
    def remove(
        ...
        *,
        ...
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, runtime_home), None)

        from pcbdraft.tools.mcp_oauth import remove_oauth_tokens

        remove_oauth_tokens(server_name, runtime_home=runtime_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, runtime_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, runtime_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        """
        from pcbdraft.tools.mcp_oauth import _get_token_dir, _safe_filename

        entry = self._entries.get(self._key(server_name, runtime_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = (
                _get_token_dir(runtime_home) / f"{_safe_filename(server_name)}.json"
            )
            try:
