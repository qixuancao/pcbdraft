...
from pcbdraft.agent.file_safety import is_write_denied as _shared_is_write_denied
from pcbdraft.tools.binary_extensions import BINARY_EXTENSIONS
...
def normalize_read_pagination(
    ...
    """
    from pcbdraft.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from pcbdraft.tools.tool_output_limits import get_max_line_length
    ...
    def _escape_shell_arg(self, arg: str) -> str:
        ...
        """
        from pcbdraft.tools.environments.local import _bash_safe_path
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        """
        from pcbdraft.tools.environments.local import _IS_WINDOWS, _msys_to_windows_path
    ...
    def _detect_file_line_ending(
        self, path: str, pre_content: str | None = None
    ) -> str | None:
    ...
    def _try_read_utf16(
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('PCBDRAFT_RUNTIME_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
            ...
            "           'content': '\\n'.join(sel)}\n"
            "    print('PCBDRAFT_RUNTIME_UTF16:OK')\n"
        ...
        marker = stdout.find("PCBDRAFT_RUNTIME_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("PCBDRAFT_RUNTIME_UTF16:OK") :].strip()
        try:
    ...
    def _unicode_variant_match(self, path: str) -> str | None:
        ...
        if len(candidates) == 1:
            return (
                os.path.join(dir_path, candidates[0])
                if dir_path != "." or "/" in path
                else candidates[0]
            )
        return None
    ...
    def _suggest_similar_files(self, path: str) -> ReadResult:
        ...
        if ls_result.exit_code == 0 and ls_result.stdout.strip():
            for f in ls_result.stdout.strip().split("\n"):
                ...
                if (
                    score == 0
                    and difflib.SequenceMatcher(None, lower_name, lf).ratio() >= 0.8
                ):
                    score = 50
    ...
    def patch_replace(
        ...
        if error or match_count == 0:
            ...
            from pcbdraft.tools.fuzzy_match import is_already_applied
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from pcbdraft.tools.patch_parser import apply_v4a_operations, parse_v4a_patch
    ...
    def _check_lint_delta(
        self, path: str, pre_content: str | None, post_content: str | None = None
    ) -> LintResult:
        """
    ...
    def _lsp_local_only(self) -> bool:
        ...
        if env is None:
            ...
            return False
        try:
            from pcbdraft.tools.environments.local import LocalEnvironment
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        if not ext:
            return False
        try:
            from pcbdraft.agent.lsp.servers import SERVERS
        except Exception:  # noqa: BLE001
            return False
        ext_lower = ext.lower()
        for srv in SERVERS:
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        if not self._lsp_local_only():
            return False
        try:
            from pcbdraft.agent.lsp import get_service
        except Exception:  # noqa: BLE001
            return False
        try:
            svc = get_service()
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        if (
            ...
            try:
                from pcbdraft.agent.lsp.range_shift import build_line_shift
        ...
        if not diagnostics:
            return ""
        try:
            from pcbdraft.agent.lsp.reporter import report_for_file, truncate
    ...
    def _try_multi_path_search(
        ...
        for p in existing:
            if target == "files":
                ...
                sub = self._search_content(
                    pattern, p, file_glob, limit, offset, output_mode, context
                )
            if sub.error:
                continue
        ...
        note = (
            f"path contained {len(parts)} entries; searched {len(existing)} that exist"
        )
        if missing:
    ...
    def _search_content(
        ...
        if (
            ...
            if hint:
                result.warning = (
                    hint if not result.warning else f"{result.warning} {hint}"
                )
    ...
    def _search_with_grep(
        ...
        cmd_parts = [
            "grep",
            "-rnHE",
        ]  # -H forces filenames; -E matches rg regex behavior
