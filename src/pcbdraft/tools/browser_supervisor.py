...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    """
    try:
        from pcbdraft.agent.redact import redact_cdp_url
...
DIALOG_BRIDGE_HOST = "pcbdraft-dialog-bridge.invalid"
...
(() => {
  if (window.__pcbdraftDialogBridgeInstalled) return;
  window.__pcbdraftDialogBridgeInstalled = true;
  const ENDPOINT = "http://pcbdraft-dialog-bridge.invalid/";
