...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``PCBDRAFT_RUNTIME_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/hermes`` read-only), setting
  ``PCBDRAFT_RUNTIME_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (PCBDRAFT_RUNTIME_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
...
_LAZY_TARGET_ENV = "PCBDRAFT_RUNTIME_LAZY_INSTALL_TARGET"
...
def _lazy_install_target() -> Path | None:
...
def _ensure_target_ready(target: Path) -> str | None:
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``PCBDRAFT_RUNTIME_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("PCBDRAFT_RUNTIME_DISABLE_LAZY_INSTALLS") == "1":
        return _lazy_install_target() is not None

    return True


def _unsupported_feature_reason(feature: str) -> str | None:
...
def _core_constraints_file() -> Path | None:
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # deployment sets PCBDRAFT_RUNTIME_MANAGED=true *and* PCBDRAFT_RUNTIME_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from pcbdraft.model.configuration import get_managed_system
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> str | None:
    ...
    (default install, ``PCBDRAFT_RUNTIME_HOME`` overrides, profile installs) and
...
def install_specs(
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``PCBDRAFT_RUNTIME_DISABLE_LAZY_INSTALLS=1``) and sets
      ``PCBDRAFT_RUNTIME_LAZY_INSTALL_TARGET``, installs are redirected to the writable
