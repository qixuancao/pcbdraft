...
def _default_input_samplerate(sd) -> int:
    ...
    try:
        ...
        rate = (
            ...
            else getattr(info, "default_samplerate", None)
        )
    ...
    try:
        with open("/proc/version", "r", encoding="utf-8") as f:
            if "microsoft" in f.read().lower():
                if has_forwarded_audio:
                    notices.append(
                        "Running in WSL with a reachable PulseAudio/PipeWire sound server"
                    )
...
# ============================================================================
_DEFAULT_BEEP_VOLUME = (
    0.3  # Backward-compatible default (matches prior hardcoded value)
)
...
def thinking_sound_enabled() -> bool:
    ...
    try:
        ...
        if isinstance(voice_cfg, dict):
            return is_truthy_value(voice_cfg.get("thinking_sound", True), default=True)
    except Exception:
        pass
    return True
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``TerminalApp._voice_start_recording``). A value <= 0
        ...
        def _callback(indata, frames, time_info, status):
            ...
            if self._on_silence_stop is not None:
                ...
                if not should_fire and self._max_duration_reached(elapsed):
                    logger.info(
                        "Max recording length reached (%.0fs), auto-stopping",
...
def transcribe_recording(wav_path: str, model: str | None = None) -> dict[str, Any]:
    ...
    """
    from pcbdraft.tools.transcription_tools import MAX_FILE_SIZE, transcribe_audio
...
def _transcribe_wav_in_chunks(
    ...
    *,
    ...
    from pcbdraft.tools.transcription_tools import transcribe_audio
...
def _wsl_powershell_tts_available() -> bool:
    ...
    """
    return bool(
        _is_wsl2_env() and shutil.which("powershell.exe") and shutil.which("ffmpeg")
    )
    ...
    for cmd in players:
        ...
        if exe:
            try:
                ...
                logger.debug(
                    "System player %s exited with code %d, trying next", cmd[0], rc
                )
            except subprocess.TimeoutExpired:
    ...
    try:
        with sd.InputStream(
            ...
            while not should_stop():
                ...
                trigger = max(
                    float(threshold or SILENCE_RMS_THRESHOLD * 2), _floor * 8.0
                )
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("PCBDRAFT_RUNTIME_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    PCBDRAFT_RUNTIME_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
...
def check_voice_requirements() -> dict[str, Any]:
    ...
    stt_available = stt_enabled and (
        native_stt_available or command_stt_config is not None or plugin_stt_available
    )
