...
CHECKPOINT_PATH = get_runtime_home() / "processes.json"
...
def _is_supervised_gateway_process() -> bool:
    ...
    try:
        ...
        from pcbdraft.services.messaging.restart import is_gateway_supervisor_process
...
class ProcessSession:
    ...
    systemd_unit: str = (
        ""  # transient scope unit name when spawned under systemd-run (#70716)
    )
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        try:
            from pcbdraft.tools.async_delegation import restore_undelivered_completions
    ...
    def _safe_host_start_time(pid: int | None) -> int | None:
        ...
        if not pid:
            return None
        try:
            from pcbdraft.core.runtime_process import get_process_start_time
    ...
    def spawn_local(
        ...
        from pcbdraft.tools.terminal_tool import (
            _rewrite_compound_background as _rewrite_bg,
        )
            ...
            while pty.isalive():
                try:
                    ...
                    if chunk:
                        ...
                        text = (
                            chunk if isinstance(chunk, str) else decoder.decode(chunk)
                        )
                        if text:
                            _append_text(text)
                except EOFError:
    ...
    def read_log(
        ...
        if offset is None and limit > 0:
            ...
            observed_completion_output = total_lines == 0 or (
                bool(selected) and stop == total_lines
            )
...
def _format_async_delegation(evt: dict) -> str:
    ...
    if evt.get("is_batch") or isinstance(batch_results, list):
        ...
        for r in sorted(results, key=lambda x: x.get("task_index", 0)):
            ...
            icon = (
                "⚠"
                if r_truncated
                else ("✓" if r_status in ("completed", "success") else "✗")
            )
            lines.append("")
    ...
    lines.append(
        f"Status: {status}   API calls: {api_calls}   Duration: {duration}s{_trunc}"
    )
    lines.append("--- RESULT ---")
...
def _delegation_attribution_line(evt: dict) -> "str | None":
    ...
    if not task_id.startswith("sa-"):
        return None
    try:
        from pcbdraft.tools.delegate_tool import get_subagent_attribution
