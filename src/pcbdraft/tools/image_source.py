...
async def _download_to_bytes(url: str) -> bytes:
    ...
    from pcbdraft.tools.vision_tools import _download_image
...
def _media_cache_roots() -> list:
    """Agent-managed media cache directories under PCBDRAFT_RUNTIME_HOME (host side).
...
def _permitted_host_read_target(p: Path, ctx: ResolveContext) -> Path | None:
    ...
    from pcbdraft.tools.credential_files import from_agent_visible_cache_path
...
def _ensure_container_env(task_id: str | None) -> None:
    ...
    if not task_id:
        return
    try:
        from pcbdraft.tools.terminal_tool import ensure_task_env
...
async def _resolve_container_fallback(
    ...
    try:
        ...
        raise NotAnImage(
            f"sandbox returned non-image data for '{p}': {exc}", src=src
        ) from exc
...
def _finalize(
    ...
    """
    from pcbdraft.tools.vision_tools import _detect_image_mime_type_from_bytes
    ...
    if sniffed is not None:
        if "image" not in permitted:
            raise NotAnImage(
                "source is an image, but this argument takes a video",
                src=src,
                origin=origin,
            )
    ...
    if "video" in permitted:
        ...
        raise NotAnImage(
            "source is not a recognized video (mp4 expected)", src=src, origin=origin
        )
...
def _detect_video_mime(data: bytes, src: str) -> str | None:
    ...
    from pcbdraft.tools.vision_tools import _detect_video_mime_type
...
async def resolve_local_source_to_data_url(
    src: str, task_id: str | None, *, permitted: tuple = ("image",)
) -> str:
