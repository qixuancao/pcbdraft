...
from pcbdraft.tools.environments.base import BaseEnvironment, _pipe_stdin
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from pcbdraft.model.configuration import OPTIONAL_ENV_VARS
...
def _inject_context_runtime_home(env: dict) -> None:
    ...
    try:
        from pcbdraft.core.runtime_environment import get_runtime_home_override

        value = get_runtime_home_override()
        if value:
            env["PCBDRAFT_RUNTIME_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (PCBDRAFT_RUNTIME_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(
    ...
    try:
        from pcbdraft.tools.env_passthrough import is_env_passthrough as _is_passthrough
        from pcbdraft.tools.env_passthrough import (
    ...
    _inject_context_runtime_home(sanitized)

    from pcbdraft.core.runtime_environment import apply_subprocess_home_env
...
def _scrub_delegated_child_kanban_env(env: dict[str, str]) -> dict[str, str]:
    ...
    try:
        from pcbdraft.agent.delegation_context import (
...
def pcbdraft_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    _inject_context_runtime_home(env)
    from pcbdraft.core.runtime_environment import apply_subprocess_home_env
    ...
    # copies os.environ, whose PCBDRAFT_RUNTIME_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's PCBDRAFT_RUNTIME_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    propagation (``PCBDRAFT_RUNTIME_HOME`` / subprocess ``HOME`` contract) and the
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      ``PCBDRAFT_RUNTIME_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Hermes home override into ``PCBDRAFT_RUNTIME_HOME`` and apply the
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``PCBDRAFT_RUNTIME_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs PCBDRAFT_RUNTIME_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("PCBDRAFT_RUNTIME_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale PCBDRAFT_RUNTIME_GIT_BASH_PATH pointing at one) must not
    ...
    # PCBDRAFT_RUNTIME_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "PCBDRAFT_RUNTIME_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
...
def _managed_runtime_path_entries() -> list[str]:
    ...
    - ``$PCBDRAFT_RUNTIME_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$PCBDRAFT_RUNTIME_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_runtime_home()`` is profile-scoped and a managed tree can appear
...
def _build_pcbdraft_repo_root_aliases(
    ...
    ``gateway_windows._preserve_runtime_home_path`` maps a physical path under
    the resolved PCBDRAFT_RUNTIME_HOME back onto the configured PCBDRAFT_RUNTIME_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of PCBDRAFT_RUNTIME_HOME as Hermes-owned.
    ...
    # lexically the same way get_default_runtime_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of PCBDRAFT_RUNTIME_HOME.
    ...
    for home in home_candidates:
        ...
        try:
            if repo_candidate.resolve(strict=True) == resolved_root.resolve(
                strict=True
            ):
                add(repo_candidate)
        except OSError:
...
#: the configured PCBDRAFT_RUNTIME_HOME spelling (which may be a junction to another
#: drive — see ``hermes_cli/gateway_windows.py::_preserve_runtime_home_path``).
...
_in_venv: bool = getattr(sys, "base_prefix", sys.prefix) != sys.prefix or hasattr(
    sys, "real_prefix"
)
...
def _strip_pcbdraft_owned_pythonpath(env: dict) -> None:
    ...
    for entry in pp.split(os.pathsep):
        ...
        # resolved and unresolved (PCBDRAFT_RUNTIME_HOME/junction) spellings count as
        # Hermes-owned.
        if not should_strip:
            should_strip = any(
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``PCBDRAFT_RUNTIME_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under PCBDRAFT_RUNTIME_HOME.  Using
