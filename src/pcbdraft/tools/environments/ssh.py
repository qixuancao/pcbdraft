...
from pcbdraft.tools.environments.file_sync import (
    FileSyncManager,
    iter_sync_files,
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "pcbdraft-ssh"
        ...
        self._remote_runtime_home = f"{self._remote_home.rstrip('/')}/.pcbdraft/runtime"

        self._ensure_remote_dirs()
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(
                f"{self._remote_home}/.pcbdraft/runtime"
            ),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create the remote .pcbdraft/runtime tree in one SSH call."""
        base = f"{self._remote_home}/.pcbdraft/runtime"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="pcbdraft-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .pcbdraft/runtime/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths matching the upload-side _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.pcbdraft/runtime".lstrip("/")
        ssh_cmd = self._build_ssh_command()
