...
# PCBDRAFT_RUNTIME_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("PCBDRAFT_RUNTIME_DEBUG_INTERRUPT"))
    ...
    def _maybe_spill(self, text: str) -> None:
        ...
        try:
            if self._spill_fh is None:
                from pcbdraft.tools.spill_safety import ensure_spill_dir, open_exclusive
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {PCBDRAFT_RUNTIME_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_runtime_home() / "sandboxes"
...
# the FIRST session's PCBDRAFT_RUNTIME_SESSION_ID into the snapshot makes every LATER
...
# PCBDRAFT_RUNTIME_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# with one of these prefixes (or is PCBDRAFT_RUNTIME_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = "^declare -x (PCBDRAFT_RUNTIME_SESSION_|PCBDRAFT_RUNTIME_UI_SESSION_ID|PCBDRAFT_RUNTIME_CRON_AUTO_DELIVER_|PCBDRAFT_RUNTIME_CRON_SESSION)"
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``PCBDRAFT_RUNTIME_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!PCBDRAFT_RUNTIME_SESSION_*} ${!PCBDRAFT_RUNTIME_CRON_AUTO_DELIVER_*} "
        # AI_AGENT / PCBDRAFT_RUNTIME_AGENT are per-command attribution markers
        ...
        "AI_AGENT PCBDRAFT_RUNTIME_AGENT "
        f"PCBDRAFT_RUNTIME_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    ...
    def _snapshot_excluded_passthrough_names(self) -> tuple[str, ...]:
        ...
        try:
            ...
            if is_multiplex_active():
                from pcbdraft.tools.env_passthrough import get_all_passthrough
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"PCBDRAFT_RUNTIME_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_runtime_home() / "cache" / "terminal-output"
                spill_path = (
                    spill_dir
        ...
        # --- Debug tracing (opt-in via PCBDRAFT_RUNTIME_DEBUG_INTERRUPT=1) -------------
    ...
    def execute(
        ...
        *,
        ...
        if rewrite_compound_background:
            from pcbdraft.tools.terminal_tool import _rewrite_compound_background
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from pcbdraft.tools.terminal_tool import _transform_sudo_command
