...
from pcbdraft.tools.environments.base import _file_mtime_key
...
_FORCE_SYNC_ENV = "PCBDRAFT_RUNTIME_FORCE_FILE_SYNC"
...
def _credential_host_paths() -> set[str]:
    ...
    try:
        from pcbdraft.tools.credential_files import get_credential_file_mounts
    except Exception:
        return set()
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``PCBDRAFT_RUNTIME_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, runtime_home: Path | None = None) -> None:
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(runtime_home=runtime_home)

    def _sync_back_transaction(self, runtime_home: Path | None = None) -> None:
        ...
        lock_path = (runtime_home or get_runtime_home()) / ".sync.lock"
