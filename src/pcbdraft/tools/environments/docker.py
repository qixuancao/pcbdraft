...
def find_docker() -> str | None:
    ...
    Resolution order:
    1. ``PCBDRAFT_RUNTIME_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("PCBDRAFT_RUNTIME_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using PCBDRAFT_RUNTIME_DOCKER_BINARY override: %s", override)
        return override
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``PCBDRAFT_RUNTIME_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    try:
        from pcbdraft.agent.proxy_sources import iron_proxy as ip
    ...
    # through the proxy.  Keep the PCBDRAFT_RUNTIME_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"PCBDRAFT_RUNTIME_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
    ...
    while i < len(extra_args):
        ...
        if any(arg.startswith(f"{flag}=") for flag in env_flags):
            if arg.startswith("--env-file="):
                ...
                if name in critical_names:
                    collisions.append(name)
        elif arg in network_flags or any(
            arg.startswith(f"{flag}=") for flag in network_flags
        ):
            collisions.append(arg)
        i += 1
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        from pcbdraft.tools.environments.base import get_sandbox_dir
        ...
        if egress_env_overrides:
            try:
                from pcbdraft.model.configuration import (
                    load_config as _load_cfg_for_collision,
                )
            ...
            try:
                from pcbdraft.agent.proxy_sources import iron_proxy as _ip_for_mappings
            ...
            if _collisions:
                ...
                logger.warning(
                    ...
                    "will NOT route through the proxy.",
                    _msg,
                )
        ...
        try:
            from pcbdraft.model.configuration import (
                load_config as _load_cfg_for_precedence,
            )

            _enforce_egress_merge = bool(
                (_load_cfg_for_precedence().get("proxy") or {}).get(
        ...
        if _egress_node_append:
            ...
            if append_token in _CA_MODE_FLAGS:
                dropped = [
                    ...
                    if t in _CA_MODE_FLAGS and t != append_token
                ]
                if dropped:
                    logger.warning(
                ...
                existing_tokens = [
                    t
                    for t in existing_tokens
                    if t not in _CA_MODE_FLAGS or t == append_token
                ]
        ...
        if egress_env_overrides:
            ...
            if _extra_collisions:
                ...
                logger.warning(
                    "%s  Extra Docker args may bypass egress isolation.",
                    _msg,
                )
        ...
        if not reused:
            ...
            logger.info(
                "Started container %s (%s)", container_name, self._container_id[:12]
            )
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        for key in sorted(forward_keys):
            ...
            if value is not None:
                ...
                and _ENV_VAR_NAME_RE.fullmatch(key)
            ):
                unset_names.add(key)
