...
def _is_pcbdraft_provider_credential(name: str) -> bool:
    ...
    if _is_pcbdraft_internal_secret(name):
        return True
    return name in _PCBDRAFT_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_pcbdraft_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register PCBDraft provider "
                "credential %r (blocked by _PCBDRAFT_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                if _is_pcbdraft_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register PCBDraft "
                        "provider credential %r from config.yaml (blocked "
                        "by _PCBDRAFT_PROVIDER_ENV_BLOCKLIST). Operator "
