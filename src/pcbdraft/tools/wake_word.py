...
def ensure_tflite_runtime() -> bool:
    ...
    try:
        from ai_edge_litert import (
            interpreter as _litert,  # type: ignore[import-not-found]
        )
    except ImportError:
        return False
...
def enrolled_profile_phrases() -> dict[str, str]:
    ...
    try:
        from pcbdraft.interfaces.tui.profiles import get_profile_dir, list_profiles
    ...
    if isinstance(info, dict):
        ...
        if isinstance(hostapi_index, (int, float)):
            ...
            try:
                ...
                hostapi_name = (
                    hostapi.get("name") if isinstance(hostapi, dict) else None
                )
                if hostapi_name:
    ...
    def __init__(self, cfg: dict[str, Any]):
        ...
        if framework == "tflite" and not ensure_tflite_runtime():
            ...
            if not ensure_tflite_runtime():
                ...
                logger.warning(
                    "wake word: no tflite runtime available — falling back to onnx"
                )
                framework = "onnx"
...
# PCBDRAFT_RUNTIME_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
def _sherpa_model_root() -> Path:
    ...
    return get_runtime_home() / "cache" / "wakewords"


def _ensure_sherpa_model(root: Path | None = None) -> Path:
...
class _SherpaKwsEngine(_Engine):
    ...
    def __init__(self, cfg: dict[str, Any]):
        ...
        self.last_match: tuple[str, str] | None = None
...
def _tts_ready() -> bool:
    ...
    """
    try:
        from pcbdraft.tools.tts_tool import _get_provider, _load_tts_config
...
def check_wake_word_requirements(cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if provider not in ("porcupine", "sherpa", "sherpa-onnx", "kws", "open"):
        ...
        if framework == "tflite":
            tflite_ok = (
                ...
                or lazy_deps.is_available("wake.openwakeword.tflite")
                or lazy_ok
            )

    if (
        provider == "porcupine"
        and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip()
    ):
        key_ok = False
        ...
        hint = (
            "Microphone capture needs sounddevice + numpy and a working audio device."
        )
    elif not stt_ok or not tts_ok:
        missing = " and ".join(
            name
            for name, ok in (("speech-to-text", stt_ok), ("text-to-speech", tts_ok))
            if not ok
        )
        hint = (
...
class WakeWordDetector:
    ...
    def _run(self, ready: threading.Event, startup_errors: list[BaseException]) -> None:
        ...
        logger.info(
            "wake word: listening (frame=%d, rate=%d, external=%s)",
            frame_length,
            SAMPLE_RATE,
            self.external_audio,
        )
        ...
        # as silent.
        silent_alert_frames = max(
            1, int(_SILENCE_ALERT_SECONDS * SAMPLE_RATE / max(1, frame_length))
        )
        try:
...
def _lock_path() -> Path:
    ...
    return get_default_runtime_root() / "runtime" / "wake-word.lock"
