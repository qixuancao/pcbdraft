...
from pcbdraft.tools import mcp_sampling_handler as _mcp_sampling_handler
from pcbdraft.tools import mcp_server_configuration as _mcp_server_configuration
from pcbdraft.tools import mcp_server_task as _mcp_server_task
from pcbdraft.tools import mcp_tool_call as _mcp_tool_call
...
_mcp_sampling_handler.configure_mcp_sampling_runtime(namespace=lambda: globals())
_mcp_server_task.configure_mcp_server_task_runtime(namespace=lambda: globals())
...
async def _paginate_full_list(
    list_method, items_attr: str, server_name: str, cache_meta_out: dict | None = None
):
...
# Compatibility exports for MCP content-block handling.
_is_reserved_mcp_meta_key = _mcp_content._is_reserved_mcp_meta_key
_strip_reserved_meta_keys = _mcp_content._strip_reserved_meta_keys
_mcp_image_extension_for_mime_type = _mcp_content._mcp_image_extension_for_mime_type
_cache_mcp_image_block = _mcp_content._cache_mcp_image_block
_MCP_RESOURCE_MAX_BYTES = _mcp_content._MCP_RESOURCE_MAX_BYTES
_MCP_RESOURCE_MAX_B64_CHARS = _mcp_content._MCP_RESOURCE_MAX_B64_CHARS
_mcp_resource_filename = _mcp_content._mcp_resource_filename
_cache_mcp_audio_block = _mcp_content._cache_mcp_audio_block
_render_mcp_resource_block = _mcp_content._render_mcp_resource_block
...
# Compatibility exports for extracted connection-front policy.
InvalidMcpUrlError = _mcp_connection_policy.InvalidMcpUrlError
NonMcpEndpointError = _mcp_connection_policy.NonMcpEndpointError
_unwrap_exception_group = _mcp_connection_policy._unwrap_exception_group
_contains_only_cancellation = _mcp_connection_policy._contains_only_cancellation


def _classify_mcp_failure(exc: BaseException) -> str:
    return _mcp_connection_policy._classify_mcp_failure(
        exc,
        is_auth_error=_is_auth_error,
    )


_validate_remote_mcp_url = _mcp_connection_policy._validate_remote_mcp_url
_resolve_client_cert = _mcp_connection_policy._resolve_client_cert
_resolve_identity_header = _mcp_connection_policy._resolve_identity_header


def _apply_identity_header(server_name: str, config: dict, headers: dict) -> dict:
    return _mcp_connection_policy._apply_identity_header(
        server_name,
        config,
        headers,
        resolve_identity_header=_resolve_identity_header,
    )


_make_redirect_header_stripper = _mcp_connection_policy._make_redirect_header_stripper


def _format_connect_error(exc: BaseException) -> str:
    return _mcp_connection_policy._format_connect_error(
        exc,
...
SamplingHandler = _mcp_sampling_handler.SamplingHandler
...
MCPServerTask = _mcp_server_task.MCPServerTask
...
_connect_server_claim: contextvars.ContextVar[
    Callable[[MCPServerTask], None] | None
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from pcbdraft.tools.mcp_oauth import OAuthNonInteractiveError
...
def _get_connected_server_for_call(server_name: str) -> MCPServerTask | None:
...
_make_tool_handler = _mcp_tool_call._make_tool_handler
...
_normalize_mcp_input_schema = _mcp_tool_schema._normalize_mcp_input_schema
...
_UTILITY_CAPABILITY_METHODS = _mcp_tool_discovery._UTILITY_CAPABILITY_METHODS
...
# #18051.
_UTILITY_CAPABILITY_ATTRS = _mcp_tool_discovery._UTILITY_CAPABILITY_ATTRS
...
_select_utility_schemas = _mcp_tool_discovery._select_utility_schemas
...
_register_server_tools = _mcp_tool_discovery._register_server_tools


_CachedMCPTool = _mcp_tool_discovery._CachedMCPTool


_register_from_cache_sync = _mcp_tool_discovery._register_from_cache_sync
...
def register_mcp_servers(servers: dict[str, dict]) -> list[str]:
    ...
    lazy_server_count = 0
    try:
        from pcbdraft.tools.mcp_schema_cache import config_fingerprint, get_cached_entry
    ...
    from pcbdraft.tools.interrupt import is_interrupted as _is_interrupted
...
def refresh_agent_mcp_tools(
    ...
    *after* that snapshot — a slow HTTP/OAuth server that misses the bounded
    ...
    """
    from pcbdraft.tools.dispatch import get_tool_definitions
    from pcbdraft.tools.registry import registry
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    ...
    try:
        ...
        if callable(get_mem_schemas):
            ...
            from pcbdraft.agent.memory_manager import memory_provider_tools_enabled
