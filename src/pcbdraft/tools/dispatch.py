...
from pcbdraft.tools.toolsets import resolve_toolset, validate_toolset
...
def _is_dispatcher_owned_worker() -> bool:
    """False when PCBDRAFT_RUNTIME_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_runtime_home() under the active profile.
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by PCBDRAFT_RUNTIME_KANBAN_TASK and
        ...
        from pcbdraft.tools.toolsets import get_all_toolsets
    ...
    if "execute_code" in available_tool_names:
        from pcbdraft.tools.code_execution_tool import (
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from pcbdraft.tools import discord_tool as _dt
    ...
    try:
        from pcbdraft.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    try:
        from pcbdraft.tools.tool_search import assemble_tool_defs
        from pcbdraft.tools.tool_search import load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from pcbdraft.model.configuration import load_config as _load
        ...
        if config_ctx is None and base_url:
            try:
                from pcbdraft.model.model_metadata import get_cached_context_length
...
from pcbdraft.tools.registry import _MAX_TOOL_ERROR_CHARS as _TOOL_ERROR_MAX_LEN
...
def coerce_tool_args(tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        from pcbdraft.tools.schema_sanitizer import unrename_tool_args
...
def _tool_result_observer_fields(
    ...
    try:
        ...
        pass
    try:
        from pcbdraft.agent.display import _detect_tool_failure
...
def _emit_post_tool_call_hook(
    *,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from pcbdraft.interfaces.tui.lifecycle import has_hook, invoke_hook
...
def handle_function_call(
    ...
    try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from pcbdraft.tools.file_tools import notify_other_tool_call
