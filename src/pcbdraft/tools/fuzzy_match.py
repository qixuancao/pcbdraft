...
def _format_match_locations(
    content: str, matches: list[tuple[int, int]], cap: int = 5
) -> str:
    ...
    for strategy_name, strategy_fn in strategies:
        ...
        if matches:
            ...
            if (
                replace_all
                and len(matches) > 1
                and strategy_name in _SIMILARITY_STRATEGIES
            ):
                return (
                    content,
                    0,
...
def _detect_backslash_doubling(
    matched_regions: str, old_string: str, new_string: str
) -> str | None:
...
def format_no_match_hint(
    error: str | None, match_count: int, old_string: str, content: str
) -> str:
