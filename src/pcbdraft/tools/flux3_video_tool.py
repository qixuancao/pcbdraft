...
async def _call_gateway(
    ...
    if response.status_code >= 400:
        ...
        message = (
            error.get("message")
            or f"the gateway refused the request (HTTP {response.status_code})"
        )
    ...
    for field, permitted in _MEDIA_FIELDS.items():
        ...
        if isinstance(value, list):
            if len(value) > _MAX_IMAGES:
                raise ValueError(
                    f"{field} takes at most {_MAX_IMAGES} images; you passed {len(value)}."
                )
            ...
            prepared[field] = list(
                await asyncio.gather(
                    *(_deliver_media(entry, permitted, task_id) for entry in value)
                )
            )
        else:
...
async def _deliver_media(value, permitted: tuple, task_id: str | None):
    ...
    try:
        resolved = await resolve_image_source(
            value, ResolveContext(task_id=task_id), permitted=permitted
        )
    ...
    if uploader is None:
        raise ValueError(
            "Local file uploads are not available in this build; pass a URL instead."
        )
...
def _download_read_timeout(started: float) -> float:
    ...
    """
    left = (
        _CALL_BACKSTOP_SECONDS - (time.monotonic() - started) - _DOWNLOAD_GRACE_SECONDS
    )
...
async def _download_video(url: str, save_to, started: float) -> tuple:
    ...
    timeout = httpx.Timeout(
        _DOWNLOAD_CONNECT_TIMEOUT_SECONDS, read=_download_read_timeout(started)
    )
    ...
    try:
        ...
        if size < _MIN_PLAUSIBLE_VIDEO_BYTES:
            raise ValueError(
                f"the download returned only {size} bytes, which is not a video"
            )
...
def _delivers_as_an_attachment() -> bool:
    ...
    """
    try:
        from pcbdraft.agent.session_context import session_is_messaging_surface
...
def _default_directory():
    ...
    is not: an operator running PCBDRAFT_RUNTIME_MEDIA_DELIVERY_STRICT=1 delivers only
...
async def _submit(mode: str, args: dict) -> str:
    ...
    return await _call_gateway(
        "POST", f"{endpoints['base_url']}/generations", _submit_args(mode, args)
    )
...
async def _poll_until_done(url: str, save_to, started: float) -> str:
    ...
    while True:
        ...
        if _is_transport_error(raw):
            ...
            gap = (
                _POLL_GAP_SECONDS
                if throttled_for is None
                else max(throttled_for, _POLL_GAP_SECONDS)
            )
...
TEXT_TO_VIDEO_SCHEMA = {
    ...
    "description": (
        ...
        "bfl_flux3_get_result. " + _OVERRIDE_SENTENCE
    ),
    "parameters": {
        "type": "object",
