...
def _normalize_temperature_ranges(text: str) -> str:
    ...
    text = re.sub(
        ...
        lambda m: (
            f"{m.group(1).replace(chr(0x2212), '-')} to {m.group(2).replace(chr(0x2212), '-')} degrees Celsius"
        ),
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        ...
        lambda m: (
            f"{m.group(1).replace(chr(0x2212), '-')} to {m.group(2).replace(chr(0x2212), '-')} degrees Fahrenheit"
        ),
        text,
        flags=re.IGNORECASE,
    )
...
def normalize_symbols_for_tts(text: str) -> str:
    ...
    text = re.sub(
        r"(?<=\d)\s*km\s*/\s*h\b", " kilometres per hour", text, flags=re.IGNORECASE
    )
    text = re.sub(
        r"(?<=\d)\s*km/h\b", " kilometres per hour", text, flags=re.IGNORECASE
    )
    ...
    text = re.sub(
        r"NZ\$\s*([\d,]*\d(?:\.\d+)?)",
        r"\1 New Zealand dollars",
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        r"A\$\s*([\d,]*\d(?:\.\d+)?)",
        r"\1 Australian dollars",
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(
        r"US\$\s*([\d,]*\d(?:\.\d+)?)", r"\1 US dollars", text, flags=re.IGNORECASE
    )
...
def smooth_whitespace_for_tts(text: str) -> str:
    ...
    add_sentence_pauses = (
        sum(1 for raw_line in raw_lines if raw_line.replace(_HEAD, "").strip()) > 1
    )
    lines: list[str] = []
