...
def _blocked_url_in_code(code: str) -> str | None:
    ...
    from pcbdraft.tools.browser_tool import evaluate_url_safety
...
def _base_subprocess_env() -> dict:
    from pcbdraft.tools.browser_tool import _build_browser_env
...
def default_downgrade_notice() -> str | None:
    ...
    try:
        ...
        stamp = Path(get_runtime_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
...
def _managed_bin_dir() -> str | None:
    """Hermes' own bin dir ($PCBDRAFT_RUNTIME_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from pcbdraft.core.runtime_environment import get_runtime_home

        return str(Path(get_runtime_home()) / "bin")
    except Exception as e:  # pragma: no cover — defensive
        logger.debug("Could not resolve managed bin dir: %s", e)
        return None


def _user_local_bin_dir() -> str | None:
...
def _find_cli() -> list[str] | None:
    ...
    MANAGED-FIRST resolution: Hermes' own ``$PCBDRAFT_RUNTIME_HOME/bin`` copy — the
...
def install_cli(timeout_s: int = 600) -> tuple[bool, str]:
    ...
    into ``$PCBDRAFT_RUNTIME_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
...
def _workspace_dir(task_id: str | None) -> str | None:
    ...
    try:
        ...
        path = Path(get_runtime_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _native_screenshot_result(
    ...
    try:
        ...
        from pcbdraft.tools.vision_tools import (
...
def _resolve_backend_cdp(
    env: dict, task_id: str | None, session_name: str = ""
) -> str | None:
...
def browser_exec(
    ...
    from pcbdraft.tools.registry import tool_error, tool_result
...
def _description_header() -> str:
    ...
    try:
        from pcbdraft.tools.vision_tools import _should_use_native_vision_fast_path
