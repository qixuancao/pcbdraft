...
def ledger_path() -> Path:
    return get_runtime_home() / "skills" / ".curator_ledger.jsonl"


def blobs_dir() -> Path:
    return get_runtime_home() / ".curator_backups" / "blobs"
...
def _validate_entry_paths(entry: dict[str, Any]) -> str | None:
    """All paths in an entry must live under PCBDRAFT_RUNTIME_HOME. Defense in depth —
