...
# match a safe prefix, the operational PCBDRAFT_RUNTIME_ allowlist, or (on Windows) an
...
#
# NB: the broad "PCBDRAFT_RUNTIME_" prefix was deliberately removed (#27303) — it leaked
# PCBDRAFT_RUNTIME_*-named config that lacks a secret substring (e.g. PCBDRAFT_RUNTIME_BASE_URL,
# PCBDRAFT_RUNTIME_KANBAN_DB, PCBDRAFT_RUNTIME_*_WEBHOOK).  The child only needs the few
# location/profile vars in _HERMES_CHILD_ALLOWED below; PCBDRAFT_RUNTIME_RPC_SOCKET /
# PCBDRAFT_RUNTIME_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
_SAFE_ENV_PREFIXES = (
    "PATH",
    "HOME",
    "USER",
...
# Operational PCBDRAFT_RUNTIME_* vars the child legitimately needs by exact name — these
...
_HERMES_CHILD_ALLOWED = frozenset(
    {
        ...
        "PCBDRAFT_RUNTIME_DELEGATED_CHILD_CONTEXT",
    }
)
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational PCBDRAFT_RUNTIME_* vars (_HERMES_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret PCBDRAFT_RUNTIME_* vars dropped by the tightened allowlist (#27303). The
    # broad "PCBDRAFT_RUNTIME_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like PCBDRAFT_RUNTIME_KANBAN_DB / PCBDRAFT_RUNTIME_BASE_URL), but a sandbox script
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> str | None:
    ...
    try:
        ...
        if m:
            missing = m.group(1)
            available = sorted(
                SANDBOX_ALLOWED_TOOLS & set(enabled_tools or SANDBOX_ALLOWED_TOOLS)
            )
        ...
        m = re.search(
            r"NameError: name '(json_parse|shell_quote|retry)' is not defined", window
        )
        if m:
            return (
...
def _connect():
    ...
    PCBDRAFT_RUNTIME_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["PCBDRAFT_RUNTIME_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("PCBDRAFT_RUNTIME_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
def _rpc_server_loop(
    ...
    """
    from pcbdraft.tools.dispatch import handle_function_call
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"PCBDRAFT_RUNTIME_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PCBDRAFT_RUNTIME_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("PCBDRAFT_RUNTIME_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    from pcbdraft.tools.terminal_tool import _docker_has_host_access, _get_env_config
    ...
    from pcbdraft.tools.approval import check_execute_code_guard
    ...
    # selector from PCBDRAFT_RUNTIME_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  PCBDRAFT_RUNTIME_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["PCBDRAFT_RUNTIME_RPC_SOCKET"] = rpc_endpoint
        child_env["PCBDRAFT_RUNTIME_RPC_TOKEN"] = rpc_token
        ...
        # PCBDRAFT_RUNTIME_TIMEZONE is an internal Hermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("PCBDRAFT_RUNTIME_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("PCBDRAFT_RUNTIME_TIMEZONE", None)
...
def _resolve_child_cwd(mode: str, staging_dir: str, task_id: str = "") -> str:
    ...
    if task_id:
        ...
        try:
            from pcbdraft.tools.terminal_tool import get_session_cwd
        ...
        try:
            from pcbdraft.tools.file_tools import _registered_task_cwd_override
