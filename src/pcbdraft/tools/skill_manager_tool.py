...
from pcbdraft.agent.skill_utils import parse_frontmatter as _parse_frontmatter
from pcbdraft.core.runtime_environment import display_runtime_home, get_runtime_home
from pcbdraft.core.runtime_utils import atomic_write_text, is_truthy_value
...
def mark_background_review_skill_read(path: Path) -> None:
    ...
    """
    try:
        from pcbdraft.tools.skill_provenance import is_background_review
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch PCBDRAFT_RUNTIME_HOME and
    ...
    live profile-scoped PCBDRAFT_RUNTIME_HOME on every call.
    """
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from pcbdraft.agent.skill_utils import get_all_skills_dirs
...
def _pinned_guard(name: str) -> str | None:
    ...
    """
    try:
        from pcbdraft.tools import skill_usage
...
def _curator_consolidation_delete_guard(
    name: str, absorbed_into: str | None
) -> dict[str, Any] | None:
...
def _find_skill(name: str) -> dict[str, Any] | None:
    ...
    {"path": Path} or None.
    """
    from pcbdraft.agent.skill_utils import get_all_skills_dirs, is_excluded_skill_path
...
def _maybe_auto_propose_org_edit(name: str, skill_path: Path) -> str | None:
    ...
    """
    try:
        from pcbdraft.agent.skill_utils import is_org_mirror_path
...
def _attach_lint_findings(result: dict[str, Any], skill_md: Path) -> None:
    ...
    """
    try:
        from pcbdraft.tools.skill_linter import (
            lint_skill,  # local import: optional path
        )
...
def _patch_skill(
    ...
    from pcbdraft.tools.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from pcbdraft.tools.fuzzy_match import format_no_match_hint
...
def _delete_skill(name: str, absorbed_into: str | None = None) -> dict[str, Any]:
    ...
    if curator_pass:
        try:
            from pcbdraft.tools.skill_usage import archive_skill
...
def _maybe_debounced_sync_push(skill_name: str) -> None:
    ...
    try:
        from pcbdraft.tools.skill_usage import is_sync_enabled
    ...
    def _fire():
        try:
            from pcbdraft.tools.skills_sync_client import maybe_push_skills
...
def skill_manage(
    ...
    try:
        from pcbdraft.tools import skill_ledger as _ledger
    ...
    if result.get("success"):
        ...
        try:
            from pcbdraft.tools.skill_provenance import is_background_review
            from pcbdraft.tools.skill_usage import bump_patch, forget, record_created
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_runtime_home()}/skills/; existing skills can be modified wherever they live.\n\n"
