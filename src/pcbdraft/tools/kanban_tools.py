...
running under the dispatcher (env var ``PCBDRAFT_RUNTIME_KANBAN_TASK`` set) or when
...
from pcbdraft.interfaces.tui.goals import judge_goal
...
def _is_delegated_child_context() -> bool:
    try:
        from pcbdraft.agent.delegation_context import is_delegated_child_context
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever PCBDRAFT_RUNTIME_KANBAN_* is present but not ours."""
    try:
        from pcbdraft.agent.delegation_context import is_dispatcher_owned_worker_context
...
def _reject_delegated_child_mutation(tool_name: str) -> str | None:
    ...
    inherited PCBDRAFT_RUNTIME_KANBAN_* env vars are not proof of dispatcher ownership.
...
def _check_kanban_mode() -> bool:
    ...
    1. ``PCBDRAFT_RUNTIME_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    ...
    if _is_delegated_child_context():
        return False
    if os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK") and _is_dispatcher_owned_worker():
        return True
...
def _default_task_id(arg: str | None) -> str | None:
    ...
    if not _is_dispatcher_owned_worker():
        ...
        return None
    env_tid = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK")
    return env_tid or None


def _worker_run_id(task_id: str) -> int | None:
    """Return this worker's dispatcher run id when it is scoped to task_id."""
    if os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _enforce_worker_task_ownership(tid: str) -> str | None:
    ...
    A process spawned by the dispatcher has ``PCBDRAFT_RUNTIME_KANBAN_TASK`` set
    ...
    ``PCBDRAFT_RUNTIME_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: str | None = None):
    ...
    (``PCBDRAFT_RUNTIME_KANBAN_DB`` → ``PCBDRAFT_RUNTIME_KANBAN_BOARD`` env → current symlink
...
def _goal_judge_available() -> bool:
    ...
    """
    try:
        from pcbdraft.model.auxiliary_client import get_text_auxiliary_client
...
def _goal_mode_handoff_rejection(task, evidence: str) -> str | None:
...
#   - No-op outside dispatcher-spawned worker context (no ``PCBDRAFT_RUNTIME_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``PCBDRAFT_RUNTIME_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``PCBDRAFT_RUNTIME_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``PCBDRAFT_RUNTIME_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_RUN_ID")
            run_id: int | None
            try:
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    (``PCBDRAFT_RUNTIME_KANBAN_TASK`` set) and ``agent`` exposes ``steer``. Returns True
    ...
    by ``PCBDRAFT_RUNTIME_PROFILE``) are skipped to avoid echoing itself.
    """
    tid = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK")
    ...
    own = (os.environ.get("PCBDRAFT_RUNTIME_PROFILE") or "").strip()
    fresh = [
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set PCBDRAFT_RUNTIME_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # PCBDRAFT_RUNTIME_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = os.environ.get("PCBDRAFT_RUNTIME_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("PCBDRAFT_RUNTIME_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets PCBDRAFT_RUNTIME_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    # Prefer the request-scoped api_server origin binding: PCBDRAFT_RUNTIME_SESSION_ID
    ...
    from pcbdraft.tools.async_delegation import _current_origin_session_id

    session_id = (
        args.get("session_id")
        or _current_origin_session_id()
        or os.environ.get("PCBDRAFT_RUNTIME_SESSION_ID")
    )
    ...
    try:
        ...
        try:
            ...
            if _inherit_project and project_id is None:
                _self_tid = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("PCBDRAFT_RUNTIME_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``PCBDRAFT_RUNTIME_SESSION_PLATFORM``,
      ``PCBDRAFT_RUNTIME_SESSION_CHAT_ID``, and ``PCBDRAFT_RUNTIME_SESSION_CHAT_TYPE`` are set in
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``PCBDRAFT_RUNTIME_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        platform = get_session_env("PCBDRAFT_RUNTIME_SESSION_PLATFORM", "")
        chat_id = get_session_env("PCBDRAFT_RUNTIME_SESSION_CHAT_ID", "")
        if not platform or not chat_id:
            # TUI / desktop fallback: platform/chat_id ContextVars are
            # cleared for TUI sessions, but the parent process exports
            # PCBDRAFT_RUNTIME_SESSION_KEY into the subprocess env. Treat that
            # as a "tui" subscription so the TUI notification poller
            # (tui_gateway/server.py) can pick it up.
            #
            # PCBDRAFT_RUNTIME_SESSION_ID is intentionally NOT a fallback here:
            ...
            # poller keys on PCBDRAFT_RUNTIME_SESSION_KEY.
            session_key = get_session_env(
                "PCBDRAFT_RUNTIME_SESSION_KEY", ""
            ) or os.environ.get("PCBDRAFT_RUNTIME_SESSION_KEY", "")
            if not session_key:
                return False  # CLI / cron / test — no persistent channel
            platform = "tui"
            chat_id = session_key
        is_gateway_session = platform != "tui"
        chat_type = get_session_env("PCBDRAFT_RUNTIME_SESSION_CHAT_TYPE", "") or None
        delivery_mode = "notify+wake" if is_gateway_session else None
        thread_id = get_session_env("PCBDRAFT_RUNTIME_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("PCBDRAFT_RUNTIME_SESSION_USER_ID", "") or None
        user_id_alt = (
            get_session_env("PCBDRAFT_RUNTIME_SESSION_USER_ID_ALT", "") or None
        )
        message_id = get_session_env("PCBDRAFT_RUNTIME_SESSION_MESSAGE_ID", "") or ""
        notifier_profile = get_session_env(
            "PCBDRAFT_RUNTIME_SESSION_PROFILE", ""
        ) or os.environ.get("PCBDRAFT_RUNTIME_PROFILE")
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to PCBDRAFT_RUNTIME_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: PCBDRAFT_RUNTIME_KANBAN_DB env → "
    "PCBDRAFT_RUNTIME_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to PCBDRAFT_RUNTIME_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
