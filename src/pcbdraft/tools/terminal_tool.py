...
# ---------------------------------------------------------------------------
# display_runtime_home imported lazily at call site (stale-module safety during hermes update)
# =============================================================================
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("PCBDRAFT_RUNTIME_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("PCBDRAFT_RUNTIME_SESSION_KEY", "")
    if session_key:
...
from pcbdraft.tools.approval import check_all_command_guards as _check_all_guards_impl
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("PCBDRAFT_RUNTIME_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (PCBDRAFT_RUNTIME_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (PCBDRAFT_RUNTIME_INTERACTIVE=1 or a registered sudo password callback):
    ...
    should_prompt_for_sudo = (
        env_var_enabled("PCBDRAFT_RUNTIME_INTERACTIVE") or has_sudo_prompt_callback
    )
...
from pcbdraft.tools.environments.base import EnvironmentConnectionError
from pcbdraft.tools.environments.docker import DockerEnvironment as _DockerEnvironment
from pcbdraft.tools.environments.local import LocalEnvironment as _LocalEnvironment
from pcbdraft.tools.environments.managed_modal import (
    ManagedModalEnvironment as _ManagedModalEnvironment,
)
from pcbdraft.tools.environments.modal import ModalEnvironment as _ModalEnvironment
from pcbdraft.tools.environments.singularity import (
    SingularityEnvironment as _SingularityEnvironment,
)
from pcbdraft.tools.environments.ssh import SSHEnvironment as _SSHEnvironment
...
def _maybe_reap_docker_orphans(container_config: dict[str, Any]) -> None:
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    try:
        from pcbdraft.tools.environments.docker import (
...
def _get_env_config() -> dict[str, Any]:
    ...
    from pcbdraft.model.configuration import _is_ssh_remote_tilde_cwd
...
def _create_environment(
    ...
    if env_type == "local":
        ...
        from pcbdraft.tools.environments.daytona import (
            DaytonaEnvironment as _DaytonaEnvironment,
        )
        ...
        from pcbdraft.tools.environments.vercel_sandbox import (
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    for task_id, env in envs_to_stop:
        ...
        try:
            from pcbdraft.tools.file_tools import clear_file_ops_cache
...
def terminal_tool(
    ...
    try:
        ...
        from pcbdraft.tools.approval import get_current_session_key
        ...
        if os.environ.get("_HERMES_GATEWAY") == "1":
            ...
            def _read_script_in_env(script_path: str) -> str | None:
        ...
        if env_type == "local":
            from pcbdraft.tools.self_repo_guard import (
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    from pcbdraft.agent.session_context import (
                        async_delivery_supported as _async_ok,
                    )
                    from pcbdraft.agent.session_context import get_session_env as _gse
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("PCBDRAFT_RUNTIME_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("PCBDRAFT_RUNTIME_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse(
                                "PCBDRAFT_RUNTIME_SESSION_THREAD_ID", ""
                            )
                            _gw_user_id = _gse("PCBDRAFT_RUNTIME_SESSION_USER_ID", "")
                            _gw_user_name = _gse(
                                "PCBDRAFT_RUNTIME_SESSION_USER_NAME", ""
                            )
                            _gw_message_id = _gse(
                                "PCBDRAFT_RUNTIME_SESSION_MESSAGE_ID", ""
                            )
            ...
            if _approved_run:
                from pcbdraft.tools.interrupt import clear_current_thread_interrupt
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled(
                    "PCBDRAFT_RUNTIME_INTERACTIVE"
                ):
                    output += (
            ...
            from pcbdraft.tools.tool_output_limits import get_max_bytes
            ...
            from pcbdraft.agent.redact import redact_terminal_output
            ...
            if returncode != 0 and not exit_note:
                try:
                    from pcbdraft.tools.terminal_hints import annotate_failure
                ...
                try:
                    from pcbdraft.tools.terminal_hints import annotate_masked_success
            ...
            if spill_file_path:
                try:
                    ...
                    from pcbdraft.tools.spill_safety import write_text_exclusive
                    ...
                    logger.debug(
                        "spill redaction failed; dropping spill handle", exc_info=True
                    )
                    try:
                        ...
                        pass
            try:
                from pcbdraft.agent.verification_evidence import record_terminal_result
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from pcbdraft.tools.environments.docker import find_docker
