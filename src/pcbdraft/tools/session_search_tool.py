...
# Third-party integrations tag their sessions with PCBDRAFT_RUNTIME_SESSION_SOURCE=tool;
...
def _session_end_reason(db, session_id: str) -> str | None:
...
def _discover(
    ...
    current_lineage_root = (
        _resolve_lineage(db, current_session_id) if current_session_id else None
    )
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "SOURCE-FIRST LIMIT\n\n"
        "  This tool searches PCBDraft conversation history only. It is not evidence "
        ...
        "reformat it as a markdown link or wrap it in backticks. PCBDraft renders "
        ...
        "(`deploy*`).\n\n"
        "WHEN TO USE\n\n"
        "  Reach for this on questions about PCBDraft conversation history itself, such "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "profile": {
                "type": "string",
                "description": (
                    "Optional. Read sessions from another PCBDraft profile's database "
