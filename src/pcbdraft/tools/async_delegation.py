...
def _capture_routing_origin() -> dict[str, Any]:
    ...
    try:
        ...
        for evt_key, env_name in (
            ("scope_id", "PCBDRAFT_RUNTIME_SESSION_SCOPE_ID"),
            ("user_id", "PCBDRAFT_RUNTIME_SESSION_USER_ID"),
            ("user_name", "PCBDRAFT_RUNTIME_SESSION_USER_NAME"),
        ):
...
def recover_abandoned_delegations() -> int:
    ...
    try:
        from pcbdraft.core.runtime_process import _pid_exists, get_process_start_time
    except Exception:
        return 0
    now = time.time()
    recovered = 0
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``PCBDRAFT_RUNTIME_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``PCBDRAFT_RUNTIME_SESSION_CHAT_ID`` binding survives child
    ...
    try:
        ...
        if get_session_env("PCBDRAFT_RUNTIME_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("PCBDRAFT_RUNTIME_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
...
def dispatch_async_delegation(
    *,
    ...
    try:
        ...
        # get_runtime_home() under the right profile.
...
def _begin_finalization(
    delegation_id: str,
) -> tuple[dict[str, Any], Callable[[], None] | None] | None:
