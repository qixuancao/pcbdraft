...
from pcbdraft.tools import file_state
...
def _build_child_agent(
    ...
    from pcbdraft.agent.delegation_context import delegated_child_context
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = (
                    get_session_env("PCBDRAFT_RUNTIME_UI_SESSION_ID", "") or None
                )
            except Exception:
        ...
        _owner_agent_session_id = str(
            getattr(child, "_parent_session_id", "") or ""
    ...
    _worktree_info: dict[str, str] | None = None
    ...
    def _attach_worktree(entry_dict: dict[str, Any]) -> None:
        ...
        if _worktree_info is None:
            return
        try:
            from pcbdraft.tools import subagent_worktree

            entry_dict["worktree"] = subagent_worktree.finalize_subagent_worktree(
                _worktree_info
            )
        except Exception as e:
            ...
            try:
                from pcbdraft.tools import subagent_worktree as _sw
    ...
    try:
        ...
        if _get_worktree_isolation():
            try:
                ...
                if subagent_worktree.local_backend_active():
                    _parent_cwd = None
                    try:
                        from pcbdraft.tools.terminal_tool import get_session_cwd as _gsc
            ...
            if _worktree_info is not None:
                try:
                    from pcbdraft.tools.terminal_tool import record_session_cwd as _rsc
                ...
                from pcbdraft.tools.subagent_worktree import build_worktree_context_note
        ...
        if isinstance(_output_schema, dict):
            from pcbdraft.tools.delegation_output_schema import (
                build_retry_message,
                validate_output,
            )
        ...
        entry["cost_status"] = (
            _cost_status
            if isinstance(_cost_status, str) and _cost_status
            else "unknown"
        )
        if status == "failed":
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from pcbdraft.agent.extensions.manager import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    from pcbdraft.tools.delegation_output_schema import coerce_output_schema
    ...
    from pcbdraft.tools.delegation_live_log import (
    ...
    # PCBDRAFT_RUNTIME_SESSION_ID ContextVar and os.environ with the subagent's internal
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("PCBDRAFT_RUNTIME_UI_SESSION_ID", "")
    except Exception:
    ...
    for i, t in enumerate(task_list):
        ...
        if _task_schema is not None:
            from pcbdraft.tools.delegation_output_schema import append_output_contract
    ...
    if background:
        ...
        from pcbdraft.tools.async_delegation import dispatch_async_delegation_batch
        ...
        try:
            from pcbdraft.agent.session_context import async_delivery_supported
        ...
        if not _async_ok:
            ...
            # PCBDRAFT_RUNTIME_SESSION_ID here would return the subagent's internal id.
        ...
        try:
            ...
            _source = get_session_env("PCBDRAFT_RUNTIME_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("PCBDRAFT_RUNTIME_UI_SESSION_ID", "")
                or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and PCBDRAFT_RUNTIME_SESSION_KEY is not in the CLI
        ...
        if dispatch.get("status") == "dispatched":
            ...
            _sids = [getattr(_c, "_subagent_id", None) for _c in _child_agents]
...
def _load_config() -> dict:
    ...
    PCBDRAFT_RUNTIME_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``PCBDRAFT_RUNTIME_IGNORE_USER_CONFIG=1`` (``hermes chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("PCBDRAFT_RUNTIME_IGNORE_USER_CONFIG") == "1"
...
DELEGATE_TASK_SCHEMA = {
    ...
    # redirect PCBDRAFT_RUNTIME_HOME.
    "description": (
