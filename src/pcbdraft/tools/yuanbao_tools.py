...
def _get_active_adapter():
    ...
    try:
        from pcbdraft.services.messaging.platforms.yuanbao import get_active_adapter
...
async def search_sticker(query: str = "", limit: int = 10) -> dict:
    ...
    """
    from pcbdraft.services.messaging.platforms.yuanbao_sticker import search_stickers
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（PCBDRAFT_RUNTIME_SESSION_CHAT_ID）。
    ...
    from pcbdraft.services.messaging.platforms.yuanbao_sticker import (
        get_random_sticker,
        get_sticker_by_id,
        get_sticker_by_name,
    )

    target = (chat_id or "").strip() or get_session_env(
        "PCBDRAFT_RUNTIME_SESSION_CHAT_ID", ""
    )
    if not target:
        return {
            "success": False,
...
def _check_yuanbao():
    ...
    try:
        ...
        if get_session_env("PCBDRAFT_RUNTIME_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
...
async def _handle_yb_send_dm(args, **kw):
    ...
    for item in raw_media:
        if isinstance(item, dict):
            media_files.append(
                (item.get("path", ""), bool(item.get("is_voice", False)))
            )
