...
def _resolve_key(env_var: str, provider_id: str) -> str:
    ...
    """
    try:
        from pcbdraft.tools.tts_tool import _resolve_provider_key
...
def register(
    name: str,
) -> Callable[[type[StreamingTTSProvider]], type[StreamingTTSProvider]]:
...
class OpenAIStreamer(StreamingTTSProvider):
    ...
    def stream(self, text: str) -> Iterator[bytes]:
        ...
        client = OpenAI(
            ...
            base_url=(
                self.section.get("base_url") or get_env_value("OPENAI_BASE_URL") or None
            ),
        )
...
def _capped(chunks: Iterator[bytes], label: str) -> Iterator[bytes]:
    ...
    for chunk in chunks:
        ...
        if total > _STREAM_SENTENCE_BYTE_CAP:
            logger.warning(
                "%s exceeded %d bytes for one sentence; truncating",
                label,
...
class GeminiStreamer(StreamingTTSProvider):
    ...
    def stream(self, text: str) -> Iterator[bytes]:
        ...
        api_key = _resolve_key("GEMINI_API_KEY", "gemini") or _resolve_key(
            "GOOGLE_API_KEY", "gemini"
        )
        model = (
            str(self.section.get("model", DEFAULT_GEMINI_TTS_MODEL)).strip()
            or DEFAULT_GEMINI_TTS_MODEL
        )
        voice = (
            str(self.section.get("voice", DEFAULT_GEMINI_TTS_VOICE)).strip()
...
class XAIStreamer(StreamingTTSProvider):
    ...
    async def _async_frames(self, text: str):
        ...
        from pcbdraft.tools.tts_tool import DEFAULT_XAI_VOICE_ID
        from pcbdraft.tools.xai_http import resolve_xai_http_credentials
        ...
        voice = (
            str(self.section.get("voice_id", DEFAULT_XAI_VOICE_ID)).strip()
