...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <PCBDRAFT_RUNTIME_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  PCBDRAFT_RUNTIME_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: str | None, session_id: str | None) -> Path:
    ...
    if directory_override:
        ...
        base = Path(get_runtime_home()) / "hook_outputs"
...
def spill_if_oversized(
    ...
    *,
    ...
    try:
        ...
        from pcbdraft.tools.spill_safety import ensure_spill_dir, write_text_exclusive
