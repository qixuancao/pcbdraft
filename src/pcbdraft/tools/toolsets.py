...
_PCBDRAFT_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (PCBDRAFT_RUNTIME_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (PCBDRAFT_RUNTIME_KANBAN_TASK env "
    ...
    "pcbdraft": {
        "description": "PCBDraft PCB design tools - plan, generate, validate, repair, and release KiCad projects",
        "tools": [
            "pcb_plan_request",
...
def resolve_toolset(
    ...
    if name in {"all", "*"}:
        ...
        for toolset_name in get_toolset_names():
            ...
            resolved = resolve_toolset(
                toolset_name, visited.copy(), include_registry=include_registry
            )
    ...
    for included_name in toolset.get("includes", []):
        included_tools = resolve_toolset(
            included_name, visited, include_registry=include_registry
        )
    ...
    if external_call:
        ...
        _resolve_toolset_memo[(name, include_registry, registry_id, generation)] = list(
            result
        )
    return result
