...
CHECKPOINT_BASE = get_runtime_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("PCBDRAFT_RUNTIME_CHECKPOINT_TIMEOUT", 30)))
...
def _save_ledger(store: Path, dir_hash: str, ledger: dict[str, dict]) -> None:
    ...
    try:
        ...
        logger.debug(
            "Failed to save agent-write ledger for %s", dir_hash, exc_info=True
        )
...
def _pre_v2_shadow_repos(base: Path) -> list[dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "PCBDRAFT_RUNTIME_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``PCBDRAFT_RUNTIME_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> str | None:
    ...
    # Compat marker for tests that look at PCBDRAFT_RUNTIME_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "PCBDRAFT_RUNTIME_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> dict:
        ...
        if not ok:
            return {
                "success": False,
                "error": f"Could not compute changed files: {err}",
            }
    ...
    def restore(
        ...
        if safe and not file_path:
            ...
            if not plan.get("success"):
                return {
                    "success": False,
                    "error": plan.get("error", "Safe-restore plan failed"),
                }
