...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _runtime_home() / "skills"
...
_DYNAMIC_PATH_RESOLVERS = {
    "PCBDRAFT_RUNTIME_HOME": _runtime_home,
...
def _referenced_support_paths(skill_md: str) -> set[str] | None:
...
def _ssrf_safe_http_get(url: str, *, timeout: int = 20) -> httpx.Response:
    ...
    from pcbdraft.tools.url_safety import create_ssrf_safe_client
...
def _guarded_http_get(url: str, *, timeout: int = 20) -> httpx.Response | None:
    ...
    from pcbdraft.tools.url_safety import SSRFConnectionBlocked
...
def github_provider_for(repo: str) -> str | None:
...
class GitHubSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> SkillBundle | None:
        ...
        if tree is not None:
            ...
            for rel_path in sorted(referenced):
                ...
                if item is None:
                    logger.warning(
                        "Referenced skill support file is missing: %s", item_path
                    )
                    return None
                if item.get("type") != "blob" or item.get("mode") == "120000":
                    logger.warning(
                        "Rejected non-regular file in skill bundle: %s", item_path
                    )
                    return None
            ...
            for rel_path in referenced:
                content = self._fetch_file_bytes(
                    repo, f"{skill_path.rstrip('/')}/{rel_path}"
                )
                if content is None:
                    return None
                files[rel_path] = content
        ...
        return SkillBundle(
            ...
            metadata={
                "source_url": (
                    ...
                    if revision
                    else f"https://github.com/{repo}/{skill_path}"
                ),
    ...
    def _fetch_file_bytes(self, repo: str, path: str) -> bytes | None:
...
class SkillsShSource(SkillSource):
    ...
    def _discover_identifier(
        self, identifier: str, detail: dict | None = None
    ) -> str | None:
        parts = identifier.split("/", 2)
        if len(parts) < 3:
            return None
    ...
    def _detail_to_metadata(
        self, canonical: str, detail: dict | None
    ) -> dict[str, Any]:
        parts = canonical.split("/", 2)
...
class ClawHubSource(SkillSource):
    ...
    def enrich_owners(self, skills: list[SkillMeta], max_workers: int = 30) -> int:
        ...
        def _fetch(meta: SkillMeta) -> str | None:
...
class BrowseShSource(SkillSource):
    ...
    def _resolve_skill_md_url(self, slug: str, item: dict) -> str | None:
        ...
        if source_url and base_url_host_matches(
            source_url, "raw.githubusercontent.com"
        ):
            return source_url
        return None
    ...
    def _fetch_from_live_repo(self, rel: str) -> SkillBundle | None:
        ...
        for item in entries:
            ...
            if (
                ...
                or "__pycache__" in rel_file.split("/")
            ):
                continue
            content = github._fetch_file_bytes(self.OFFICIAL_REPO, item_path)
            if content is None:
                logger.warning(
                    "Live-repo optional skill fetch failed for %s", item_path
                )
                return None
        ...
        logger.info(
            "Optional skill '%s' fetched from live repo (not in local checkout)", rel
        )
        return SkillBundle(
            name=rel.rsplit("/", 1)[-1],
            files=files,
...
def _category_skill_dirs(directory: Path) -> list[str]:
    ...
    for entry in directory.iterdir():
        ...
        for skill_md in entry.rglob("SKILL.md"):
            if is_excluded_skill_path(skill_md.relative_to(directory), root=directory):
                continue
...
def install_from_quarantine(
    ...
    try:
        from pcbdraft.tools.skill_usage import record_installed
...
def _load_pcbdraft_index() -> dict | None:
    ...
    We cache it locally for PCBDRAFT_RUNTIME_INDEX_TTL seconds to avoid repeated
