...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("PCBDRAFT_RUNTIME_YOLO_MODE", ""))
...
# os.environ["PCBDRAFT_RUNTIME_INTERACTIVE"] races: one session's restore in `finally`
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("PCBDRAFT_RUNTIME_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("PCBDRAFT_RUNTIME_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("PCBDRAFT_RUNTIME_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("PCBDRAFT_RUNTIME_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("PCBDRAFT_RUNTIME_CRON_SESSION")
...
def _is_single_query_approval_context() -> bool:
    ...
    answer approval prompts, but it still exports ``PCBDRAFT_RUNTIME_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(
            get_session_env("PCBDRAFT_RUNTIME_SINGLE_QUERY_SESSION", "")
        )
    except Exception:
        return env_var_enabled("PCBDRAFT_RUNTIME_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set PCBDRAFT_RUNTIME_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind PCBDRAFT_RUNTIME_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds PCBDRAFT_RUNTIME_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("PCBDRAFT_RUNTIME_GATEWAY_SESSION"):
        return True
...
def _resolve_cli_approval_callback(approval_callback=None):
    ...
    try:
        from pcbdraft.tools.terminal_tool import _get_approval_callback
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``PCBDRAFT_RUNTIME_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $PCBDRAFT_RUNTIME_HOME, or by the resolved absolute
...
def _save_blocked_payload(command: str) -> str | None:
    ...
    try:
        ...
        script_dir = get_runtime_home() / "cache" / "blocked-scripts"
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    """
    from pcbdraft.tools.ansi_strip import strip_ansi
    ...
    # it tracks HOME / PCBDRAFT_RUNTIME_HOME even when those are set after this module is
    ...
    command = _rewrite_resolved_runtime_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / PCBDRAFT_RUNTIME_HOME such as ``/``,
...
def _rewrite_resolved_runtime_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.hermes/``.

    Resolves the active ``PCBDRAFT_RUNTIME_HOME`` at call time (and its symlink-resolved
    ...
    ``$PCBDRAFT_RUNTIME_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from pcbdraft.core.runtime_environment import get_runtime_home

        home = get_runtime_home().expanduser()
        candidates = [
            str(home),
...
def _command_parser_limit_exceeded(command: str) -> bool:
    ...
    if len(command) > _MAX_SEPARATOR_FREE_COMMAND_CHARS and not any(
...
def _shell_tokens_with_spans(segment: str, start: int):
    ...
    while i < len(segment):
        ...
        inert_single_quoted = (raw.startswith("'") and raw.endswith("'")) or (
...
def _command_detection_variants(command: str):
    ...
    if re.search(r"(?:[A-Za-z]:|\\\\)[\\\\]", command) or re.search(
        r"[A-Za-z]:\\", command
    ):
...
def _prompt_dangerous_approval_inner(
    command: str,
    description: str,
    timeout_seconds: int,
    allow_permanent: bool = True,
    ...
    *,
    ...
    try:
        ...
        if "PCBDRAFT_RUNTIME_SPINNER_PAUSE" in os.environ:
            del os.environ["PCBDRAFT_RUNTIME_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    ...
    into one place:
      - process-scoped ``--yolo`` / ``PCBDRAFT_RUNTIME_YOLO_MODE`` (frozen at import time
...
def _get_single_query_approval_mode() -> str:
    ...
    try:
        ...
        config = load_config_readonly()
        mode = (
            str(cfg_get(config, "approvals", "single_query_mode", default="deny"))
            .lower()
            .strip()
        )
        if mode in {"approve", "off", "allow", "yes"}:
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            PCBDRAFT_RUNTIME_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export PCBDRAFT_RUNTIME_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set PCBDRAFT_RUNTIME_INTERACTIVE or "
                "PCBDRAFT_RUNTIME_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix,
                pattern_key,
                description,
            )
        ...
        logger.warning(
            "%s (pattern: %s): %s — set PCBDRAFT_RUNTIME_INTERACTIVE or "
            "PCBDRAFT_RUNTIME_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix,
            pattern_key,
            description,
        )
    ...
    if is_gateway or env_var_enabled("PCBDRAFT_RUNTIME_EXEC_ASK"):
        ...
        # can see (PCBDRAFT_RUNTIME_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``PCBDRAFT_RUNTIME_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from pcbdraft.agent.extensions.manager import get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from pcbdraft.tools.environments.base import touch_activity_if_due
    ...
    def _poll() -> None:
        if touch_activity_if_due is not None:
            touch_activity_if_due(
                activity_state, "waiting for plugin approval transport"
            )
    ...
    hook_choice = (
        result.choice if result.failure is None else f"transport_{result.failure}"
    )
    _fire_approval_hook(
        "post_approval_response",
        command=request.command,
...
def _await_coalesced_leader(
    session_key: str, leader, approval_data: dict, *, surface: str = "gateway"
):
...
def _await_gateway_decision(
    ...
    with _lock:
        for existing in _gateway_queues.get(session_key, []):
            data = existing.data
            if data.get("command") == approval_data.get("command") and list(
                data.get("pattern_keys") or []
...
def check_all_command_guards(
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("PCBDRAFT_RUNTIME_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # PCBDRAFT_RUNTIME_EXEC_ASK routes through the gateway decision loop (no human
    ...
    try:
        ...
        try:
            from pcbdraft.model.configuration import load_config_readonly as _load_cfg
...
def check_execute_code_guard(
    ...
    # Ask-mode (PCBDRAFT_RUNTIME_EXEC_ASK) still takes this path even when INTERACTIVE
