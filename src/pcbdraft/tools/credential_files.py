...
def register_credential_file(
    ...
    *relative_path* is relative to ``PCBDRAFT_RUNTIME_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside PCBDRAFT_RUNTIME_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because PCBDRAFT_RUNTIME_HOME is exactly where
    ...
    # Reject absolute paths — they bypass the PCBDRAFT_RUNTIME_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to PCBDRAFT_RUNTIME_HOME)",
            relative_path,
        )
        return False

    host_path = runtime_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape PCBDRAFT_RUNTIME_HOME.
    from pcbdraft.tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, runtime_home)
    ...
    # inside PCBDRAFT_RUNTIME_HOME and therefore pass the containment check above.
