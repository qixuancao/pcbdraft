...
def auth_json_path():
    """Return the Hermes auth store path, respecting PCBDRAFT_RUNTIME_HOME overrides."""
    return get_runtime_home() / "auth.json"
...
def read_nous_access_token() -> str | None:
    ...
    try:
        from pcbdraft.model.auth import resolve_nous_access_token
...
def is_managed_nous_gateway_url(
    ...
    return bool(actual.scheme) and (actual.scheme, actual.netloc) == (
        expected.scheme,
        expected.netloc,
    )
...
def build_managed_media_uploader(
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        headers = managed_gateway_auth_headers(
            server_url, gateway_builder, token_reader
        )
        if not headers:
