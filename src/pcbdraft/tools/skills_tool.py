...
def _skills_scan_signature(dirs_to_scan, disabled) -> tuple:
    ...
    """
    from pcbdraft.agent import skill_utils as _skill_utils
...
SKILLS_DIR = PCBDRAFT_RUNTIME_HOME / "skills"
...
def _skills_dir() -> Path:
    ...
    set PCBDRAFT_RUNTIME_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped PCBDRAFT_RUNTIME_HOME on every call.
    """
...
def _skill_lookup_path_error(name: str) -> str | None:
    ...
    """
    from pcbdraft.tools.path_security import has_traversal_component
...
def load_env() -> dict[str, str]:
    """Load profile-scoped environment variables from PCBDRAFT_RUNTIME_HOME/.env."""
    env_path = get_runtime_home() / ".env"
...
def skill_matches_platform(frontmatter: dict[str, Any]) -> bool:
    ...
    """
    from pcbdraft.agent.skill_utils import skill_matches_platform as _impl
...
def skill_matches_environment(frontmatter: dict[str, Any]) -> bool:
    ...
    """
    from pcbdraft.agent.skill_utils import skill_matches_environment as _impl
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set PCBDRAFT_RUNTIME_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (PCBDRAFT_RUNTIME_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("PCBDRAFT_RUNTIME_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("PCBDRAFT_RUNTIME_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_runtime_home()}/.env manually."
...
def _parse_frontmatter(content: str) -> tuple[dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from pcbdraft.agent.skill_utils import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> str | None:
    ...
    try:
        from pcbdraft.agent.skill_utils import get_external_skills_dirs
...
def _get_disabled_skill_names() -> set[str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from pcbdraft.agent.skill_utils import get_disabled_skill_names
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``PCBDRAFT_RUNTIME_SESSION_PLATFORM``.
    """
    try:
        from pcbdraft.agent.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str | None = None) -> bool:
    ...
    2. ``PCBDRAFT_RUNTIME_PLATFORM`` environment variable
    3. ``PCBDRAFT_RUNTIME_SESSION_PLATFORM`` from gateway session context
    """
    try:
        ...
        resolved_platform = (
            platform
            or os.getenv("PCBDRAFT_RUNTIME_PLATFORM")
            or _get_session_platform()
        )
...
def _find_all_skills(*, skip_disabled: bool = False) -> list[dict[str, Any]]:
    ...
    # (_skills_dir() resolves the LIVE profile PCBDRAFT_RUNTIME_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    if file_path:
        from pcbdraft.tools.path_security import (
    ...
    if preprocess:
        try:
            from pcbdraft.agent.skill_preprocessing import preprocess_skill_content
...
def _plugin_skill_linked_files(skill_root: Path) -> dict[str, list[str]] | None:
    from pcbdraft.tools.path_security import validate_within_dir
...
def skill_view(
    ...
    try:
        ...
        from pcbdraft.agent.skill_utils import iter_skill_index_files
        ...
        if skill_md is not None and project_dirs:
            from pcbdraft.agent.skill_utils import is_quarantined_project_skill
        ...
        if file_path and skill_dir:
            ...
            try:
                from pcbdraft.tools.skill_manager_tool import (
        ...
        if available_env_names:
            try:
                from pcbdraft.tools.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from pcbdraft.tools.credential_files import register_credential_files
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from pcbdraft.tools.skill_usage import bump_use, bump_view
