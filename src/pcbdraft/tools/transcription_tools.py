...
LOCAL_STT_COMMAND_ENV = "PCBDRAFT_RUNTIME_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "PCBDRAFT_RUNTIME_LOCAL_STT_LANGUAGE"
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``PCBDRAFT_RUNTIME_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _run_ffmpeg_stt_encode(
    ffmpeg: str, input_path: str, output_path: str, *, audio_filter: str | None = None
) -> None:
...
#
# The single-env-var ``PCBDRAFT_RUNTIME_LOCAL_STT_COMMAND`` escape hatch is preserved
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set PCBDRAFT_RUNTIME_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from pcbdraft.agent.extensions.manager import has_hook, invoke_hook
...
def _validate_audio_file_size(audio_path: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        if model is None:  # defensive: load failed without raising
            return {
                "success": False,
                "transcript": "",
                "error": "Local whisper model failed to load",
            }
...
def _convert_caf_to_wav(file_path: str) -> str | None:
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``PCBDRAFT_RUNTIME_LOCAL_STT_LANGUAGE`` (env).
...
_CLOUD_TRIM_MIN_RESULT_SECONDS = (
    0.3  # all-silence guard floor: never upload ~empty audio
)
...
def _cloud_trim_settings(stt_config: dict[str, Any]) -> tuple[bool, int, int]:
    ...
    try:
        threshold_db = int(
            cfg.get("cloud_trim_threshold_db", _CLOUD_TRIM_THRESHOLD_DB_DEFAULT)
        )
    except (TypeError, ValueError):
...
def _trim_silence_for_cloud_stt(
    ...
    try:
        _run_ffmpeg_stt_encode(
            ffmpeg, file_path, trimmed_path, audio_filter=filter_expr
        )
...
def _transcribe_prepared_audio(
    ...
    trim_cleanup_dir: str | None = None
