...
from pcbdraft.tools.binary_extensions import (
    has_binary_extension,
    has_opaque_document_extension,
    is_pdf_path,
)
from pcbdraft.tools.file_operations import (
    ShellFileOperations,
...
def _expand_tilde(path: str) -> str:
    ...
    if not path or "~" not in path:
        return path
    try:
        from pcbdraft.core.runtime_environment import get_subprocess_home
...
def _uses_container_paths(task_id: str = "default") -> bool:
    try:
        from pcbdraft.tools.terminal_tool import _CONTAINER_BACKENDS
...
def _registered_task_cwd_override(task_id: str = "default") -> str | None:
    ...
    """
    try:
        from pcbdraft.tools.terminal_tool import resolve_task_overrides
...
_real_runtime_home_cached: str | None = None
_real_runtime_home_loaded = False


def _get_real_runtime_home() -> str | None:
    """Return the realpath of the authoritative Hermes home (cached)."""
    global _real_runtime_home_cached, _real_runtime_home_loaded
    if _real_runtime_home_loaded:
        return _real_runtime_home_cached
    _real_runtime_home_loaded = True
    try:
        from pcbdraft.core.runtime_environment import get_runtime_home
...
def _protected_instruction_reason(
    ...
    *,
    ...
    real_home = _get_real_runtime_home()
...
def _request_protected_instruction_approval(
    ...
    try:
        import pcbdraft.tools.approval as _approval
    except Exception:
...
def _check_approval_required_write(
    ...
    """
    try:
        from pcbdraft.agent.file_safety import is_write_approval_required
    except Exception:
        return None
...
def read_file_tool(
    ...
    try:
        ...
        from pcbdraft.tools.read_extract import (
            ANYDOC_EXTENSIONS,
        ...
        # and block credential stores under PCBDRAFT_RUNTIME_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == PCBDRAFT_RUNTIME_HOME (e.g. "auth.json") still hits the
...
def _mark_verification_stale(
    ...
    if not paths:
        return
    try:
        from pcbdraft.agent.coding_context import project_facts_for
        from pcbdraft.agent.verification_evidence import mark_workspace_edited
...
def _check_file_reqs():
    ...
    from pcbdraft.tools import check_file_requirements
