...
if TYPE_CHECKING:
    from pcbdraft.tools.file_operations import PatchResult
    ...
    for op in operations:
        ...
        if op.operation == OperationType.UPDATE:
            ...
            if not src_err and dst_err:
                pending_content[op.new_path] = (
                    src_content if src_content is not None else ""
                )
                pending_content.pop(op.file_path, None)
                removed_paths.add(op.file_path)
...
def _apply_add(
    op: PatchOperation, file_ops: Any
...
def _apply_update(
    ...
    if _write_file_accepts_pre_content(file_ops):
        write_result = file_ops.write_file(
    ...
    return (
        ...
        getattr(write_result, "lint", None),
    )
