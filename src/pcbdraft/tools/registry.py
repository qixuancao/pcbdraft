...
def discover_builtin_tools(tools_dir: Path | None = None) -> list[str]:
    ...
    for path in sorted(tools_path.glob("*.py")):
        ...
        if registers:
            module_names.append(f"pcbdraft.tools.{path.stem}")
...
def _discovery_cache_path() -> Path | None:
    ...
    try:
        ...
        return Path(get_runtime_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
def _save_discovery_cache(cache: dict[str, list]) -> None:
    ...
    try:
        from pcbdraft.core.runtime_utils import (
            atomic_json_write,  # stdlib+yaml only; no cycle
        )
...
def check_fn_cache_scope() -> str | None:
...
def get_cached_check_fn_result(fn: Callable) -> bool | None:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved PCBDRAFT_RUNTIME_HOME. A
        ...
        self._plugin_override_policy: dict[
            tuple[str | None, str], _PluginOverridePolicy
        ] = {}
        ...
        self._plugin_module_scopes: dict[str, set[str | None]] = {}
    ...
    def current_scope_key() -> str:
        ...
        return runtime_home_key()

    def _merged_tools(self, scope: str | None = None) -> dict[str, ToolEntry]:
    ...
    def restore_plugin_override_policy(
        ...
        previous: _PluginOverridePolicy | None,
        *,
        scope: str | None = None,
    ) -> bool:
    ...
    def _plugin_scope_of(self, module_namespace: str) -> str | None:
    ...
    def plugin_scope_for_module(self, module_namespace: str) -> str | None:
    ...
    def plugin_scope_for_callable(self, callback: Callable) -> str | None:
        ...
        *,
        ...
        with self._lock:
            ...
            for toolset in affected_toolsets:
                surviving = [
                    entry
                    for entry in self._merged_tools(scope).values()
                    if entry.toolset == toolset
                ]
                check_fn = next(
    ...
    def dispatch(
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            ...
            try:
                from pcbdraft.tools.dispatch import _sanitize_tool_error
    ...
    def get_max_result_size(
        ...
        if default is not None:
            return default
        from pcbdraft.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
