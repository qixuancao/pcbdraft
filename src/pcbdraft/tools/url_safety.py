...
def _global_allow_private_urls() -> bool:
    ...
    1. ``PCBDRAFT_RUNTIME_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    if get_runtime_home_override() is not None:
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("PCBDRAFT_RUNTIME_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``PCBDRAFT_RUNTIME_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
...
def _safe_connect_scheme(
    host: str, port: int, schemes_by_origin: dict[tuple[str, int], str]
) -> str:
...
def _install_ssrf_guard_on_async_transport(
    ...
    if pool is None or not hasattr(pool, "_network_backend"):
        raise SSRFConnectionBlocked(
            "Unsupported async httpx transport cannot be made SSRF-safe"
        )
...
def _install_ssrf_guard_on_transport(
    ...
    if pool is None or not hasattr(pool, "_network_backend"):
        raise SSRFConnectionBlocked(
            "Unsupported httpx transport cannot be made SSRF-safe"
        )
