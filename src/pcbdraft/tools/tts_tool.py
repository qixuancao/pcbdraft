...
def get_env_value(name, default=None):
    ...
    """
    try:
        from pcbdraft.model.configuration import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    """
    try:
        from pcbdraft.tools.tool_backend_helpers import resolve_provider_secret
    ...
    try:
        if _response_has_explicit_stream(response):
            ...
            content = vars(response).get(
                "content", getattr(type(response), "content", b"")
            )
...
def _pack_audio_files_for_delivery(
    ...
    for path in audio_paths:
        ...
        if current and (
            current_size + size > profile.target_file_bytes or suffix != current_suffix
        ):
...
def _sniff_audio_container(path: str) -> str:
    ...
    """
    from pcbdraft.tools.audio_container import sniff_container
...
def _repair_ogg_container(file_str: str) -> str:
    ...
    try:
        ...
        logger.warning(
            ...
            "file is delivered with its real format",
            file_str,
            honest,
        )
        return honest
...
def _concat_audio_files(
    ...
    *,
    ...
    concat_path = destination.with_name(
        f".{destination.name}.{uuid.uuid4().hex}.concat.txt"
    )
...
def _generate_deepinfra_tts(
    ...
    from pcbdraft.model.catalog import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: dict[str, Any]) -> str:
    ...
    from pcbdraft.tools.xai_http import resolve_xai_http_credentials
...
def _generate_gemini_tts(
    ...
    if response.status_code != 200:
        ...
        try:
            if raw_body:
            ...
            elif not _response_has_explicit_stream(response) and callable(
                ...
                err = {}
            detail = (
                err.get("message") or raw_body.decode("utf-8", errors="replace")[:300]
            )
        except Exception:
...
def _tts_cache_get_or_load(
    cache: dict[str, Any], key: str, load: Callable[[], Any]
) -> Any:
...
def _text_to_speech_single(
    ...
    *,
    ...
    tts_config = (
        tts_config_override if tts_config_override is not None else _load_tts_config()
    )
    ...
    platform = get_session_env("PCBDRAFT_RUNTIME_SESSION_PLATFORM", "").lower()
    ...
    if output_path:
        ...
        if is_write_denied(str(file_path)) or is_write_approval_required(
            str(file_path)
        ):
            return json.dumps(
                {
...
def text_to_speech_tool(
    ...
    if output_path:
        ...
        if is_write_denied(str(base_path)) or is_write_approval_required(
            str(base_path)
        ):
            return json.dumps(
                {
...
class _SyncSentencePipeline:
    ...
    def __init__(self, stop_event: threading.Event, *, lookahead: int = 2):
        ...
        self._queue: queue.Queue[tuple[str, Future] | None] = queue.Queue(
    ...
    def _synthesize_to_tmp(self, cleaned: str) -> str | None:
        if self._stop.is_set():
            return None
        tmp_path = None
        try:
...
def stream_tts_to_speaker(
    ...
    sync_pipeline: _SyncSentencePipeline | None = None
    ...
    try:
        ...
        if streamer is not None:
            ...
            if platform.system() == "Darwin":
                ...
                try:
                    ...
                    logger.debug(
                        "sounddevice not available, streamer→tempfile: %s", exc
                    )
                    output_stream = None
                except Exception as exc:
        ...
        _audio_queue: queue.Queue[queue.Queue[bytes | None] | None] = queue.Queue()
        ...
        def _playback_worker() -> None:
            ...
            if output_stream is not None:
                ...
                try:
                    from pcbdraft.tools.voice_mode import mark_audio_output_active
                except Exception:
                ...
                while True:
                    ...
                    while True:
                        ...
                        if chunk is None:
                            break
                        _chunks.append(chunk)
                    _play_via_tempfile(iter(_chunks), stop_event, streamer.sample_rate)
...
if __name__ == "__main__":
    ...
    print(
        f"    API Key:  {'set' if _resolve_provider_key('ELEVENLABS_API_KEY', 'elevenlabs') else 'not set'}"
    )
    print(
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_runtime_home()}/audio_cache/<timestamp>.mp3",
            },
            "speed": {
                "type": "number",
