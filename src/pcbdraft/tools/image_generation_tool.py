...
def _load_fal_client() -> Any:
    ...
    if fal_client is not None:
        return fal_client
    from pcbdraft.tools.fal_common import import_fal_client
...
def _agent_visible_cache_path(host_path: str, env: Any) -> str | None:
    ...
    try:
        from pcbdraft.tools.credential_files import map_cache_path_to_container
...
def _dispatch_to_plugin_provider(
    ...
    try:
        ...
        if reference_image_urls is not None:
            from pcbdraft.agent.image_gen_provider import normalize_reference_images
...
def _maybe_route_managed_krea(
    ...
    try:
        from pcbdraft.agent.extensions.bundled.image_gen.krea import (
...
def _confine_source_images(
    ...
    try:
        ...
        if isinstance(reference_image_urls, (list, tuple)):
            reference_image_urls = [
                _run_async(
                    resolve_local_source_to_data_url(ref, task_id, permitted=permitted)
                )
