...
def _get_runtime_home() -> str:
    """Return the Hermes home directory, respecting PCBDRAFT_RUNTIME_HOME env var."""
    return str(get_runtime_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_runtime_home(), ".tirith-install-failed")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $PCBDRAFT_RUNTIME_HOME/bin/tirith.
