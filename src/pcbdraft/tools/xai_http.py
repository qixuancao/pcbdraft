...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_runtime_home() / "auth.json"
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> str | None:
    ...
    try:
        ...
        marker_dir = get_runtime_home() / "state"
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``PCBDRAFT_RUNTIME_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    ...
    override = (
        str(
            get_env_value("PCBDRAFT_RUNTIME_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    honors ``PCBDRAFT_RUNTIME_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
    ...
    try:
        ...
        api_key = resolve_provider_secret(
            "XAI_API_KEY", "xai", env_getter=get_env_value
        )
    except ImportError:  # pragma: no cover — helpers are in-repo
