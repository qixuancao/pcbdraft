    ...
    *,
    ...
    try:
        ...
        if exc.permanent:
            ...
            logger.warning(
                "Honcho OAuth %s failed for host %s: %s", op_label, host, exc
            )
        return None
    except Exception as exc:
        _refresh_failure_at[key] = time.monotonic()
...
def force_refresh_token(path: Path, host: str) -> str | None:
    ...
    with _refresh_lock, _config_refresh_lock(path):
        ...
        if (
            cached is not None
            and cred.access_token != cached[1]
            and not cred.is_expired(now=now)
        ):
            _expiry_cache[key] = (cred.expires_at, cred.access_token)
            return cred.access_token
        rotated = _rotate_and_persist(
            path, host, key, cred, now=now, op_label="forced refresh"
        )
        if rotated is None:
            return None
        logger.info(
            "Honcho OAuth token force-refreshed for host %s after an auth failure", host
        )
