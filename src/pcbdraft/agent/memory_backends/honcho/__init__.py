...
Config: Uses the existing Honcho config chain:
  1. $PCBDRAFT_RUNTIME_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def post_setup(self, runtime_home: str, config: dict) -> None:
        ...
        from pcbdraft.agent.memory_backends.honcho.cli import cmd_setup
        ...
        with self._init_lock:
            ...
            def _run() -> None:
                ...
                try:
                    ...
                    self._manager = None
                    logger.warning(
                        "Honcho background session init failed: authentication rejected"
                    )
                except Exception as e:
                    self._init_error = str(e)
                    self._manager = None
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        from pcbdraft.agent.memory_backends.honcho.client import get_honcho_client
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_runtime_home() / "memories")
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        _skip_base = self._injection_frequency == "first-turn" and self._turn_count > 1
        ...
        if not _skip_base:
            ...
            if _first_base_fetch and self._manager:
                ...
                def _fetch_base() -> None:
                    try:
                        ctx = (
                            self._manager.get_prefetch_context(
    ...
    def _consume_pending_dialectic(self) -> str:
        ...
        if (
            ...
            return ""
        return (
            dialectic_result if (dialectic_result and dialectic_result.strip()) else ""
        )
    ...
    def _run_dialectic_depth(
        self, query: str, *, use_query_rewrite: bool = True
    ) -> str:
        ...
        for i in range(self._dialectic_depth):
            ...
            if i == 0:
                ...
                if not prior_results:
                    ...
                    logger.debug(
                        "Honcho dialectic depth %d: pass %d has no non-empty prior — "
        ...
        def _sync():
            try:
                ...
                if clean_assistant_content:
                    for chunk in self._chunk_message(
                        clean_assistant_content, msg_limit
                    ):
...
def register(ctx) -> None:
    ...
    from pcbdraft.agent.memory_backends.query_rewrite import rewrite_memory_query
