...
Resolution order for config file:
  1. $PCBDRAFT_RUNTIME_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. PCBDRAFT_RUNTIME_HONCHO_HOST env var (explicit override)
    ...
    """
    explicit = os.environ.get("PCBDRAFT_RUNTIME_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    """
    local_path = get_runtime_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_runtime_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # resolve_config_path()/get_runtime_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $PCBDRAFT_RUNTIME_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        ...
        native_base_url = (
            endpoint_block.get("baseUrl") if isinstance(endpoint_block, dict) else None
        )
        base_url = _sanitize_url(
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_runtime_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_runtime_home) silently
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.runtime_home) if config.runtime_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/runtime_home) identity reappears with a
    ...
    with _client_slots_lock:
        ...
        if slot is None:
            stale = [
                k
                for k in _client_slots
                if k != key
                and (
                    (k[:3] if k[0] == "ambient" else (k[0], k[1], k[5], k[6]))
                    == identity
                )
            ]
            for k in stale:
    ...
    def _build() -> Honcho:
        ...
        if _is_local:
            ...
            _host_block_local = _host_block(
                _raw, config.host
            )  # uses dot-form legacy fallback (#37436)
            _explicit_key = bool(_host_block_local.get("apiKey") or _raw.get("apiKey"))
