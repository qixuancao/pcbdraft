...
def resolve_endpoints(
    ...
    return OAuthEndpoints(
        ...
        device_authorization_url=os.environ.get(
            "HONCHO_OAUTH_DEVICE_AUTH_URL", default_device
        ),
    )
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 - stdlib API name
            ...
            if captured["error"]:
                ...
                page = _CALLBACK_ERROR_HTML.format(
                    error=_html.escape(captured["error"])
                )
    ...
    while True:
        ...
        if status == 200:
            if not body.get("access_token"):
                raise DeviceFlowError(
                    "invalid_response", "token response missing access_token"
                )
            return body
        error = str(body.get("error") or f"http_{status}")
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local PCBDRAFT_RUNTIME_HOME override can't reach.
