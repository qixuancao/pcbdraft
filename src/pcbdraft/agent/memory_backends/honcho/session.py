...
from pcbdraft.agent.memory_backends.honcho.oauth import redact_tokens as _redact_tokens
...
class HonchoSessionManager:
    ...
    def _clear_auth_failure(self) -> None:
        if self._auth_failure is not None:
            logger.info(
                "Honcho authentication recovered; memory sync and recall resumed"
            )
    ...
    def _force_reauth(self) -> bool:
        ...
        try:
            from pcbdraft.agent.memory_backends.honcho import oauth
            from pcbdraft.agent.memory_backends.honcho.client import reset_honcho_client
        ...
        if not auth_dead:
            try:
                ...
                if existing_messages and len(existing_messages) > 1:
                    timestamps = [
                        m.created_at for m in existing_messages if m.created_at
                    ]
    ...
    def migrate_memory_files(self, session_key: str, memory_dir: str) -> bool:
        ...
        for filename, upload_name, description, target_peer_id, target_kind in files:
            ...
            try:
                ...
                logger.error(
                    "Honcho memory migration stopped after %s: auth failed", filename
                )
                break
            except Exception as e:
        ...
        *,
        ...
        try:
            ...
            def _peer_context() -> Any:
                peer = self._get_or_create_peer(peer_id)
                return (
                    peer.context(**context_kwargs) if context_kwargs else peer.context()
                )
        ...
        if not representation:
            try:
                ...
                representation = (
                    self._authed_call("peer representation fetch", _peer_representation)
                    or ""
                )
            except HonchoAuthError:
                raise
    ...
    def search_context(
        ...
        try:
            ...
            logger.debug(
                "Honcho message search failed (peer_perspective=%s): %s", peer_id, e
            )
    ...
    def delete_conclusion(
        ...
        try:
            ...
            self._authed_call(
                "conclusion delete",
                lambda: self._conclusions_scope(session, target_peer_id).delete(
                    conclusion_id
                ),
            )
