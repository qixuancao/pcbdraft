...
def _save_supermemory_config(values: dict, runtime_home: str) -> None:
    config_path = Path(runtime_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    ...
    def get_profile(
        self, query: str | None = None, *, container_tag: str | None = None
    ) -> dict:
        tag = container_tag or self._container_tag
    ...
    def forget_memory(
        self, memory_id: str, *, container_tag: str | None = None
    ) -> None:
        tag = container_tag or self._container_tag
...
def _resolve_container_tag_for_setup(
    runtime_home: str, *, identity: str = "default"
) -> str:
    config = _load_supermemory_config(runtime_home)
...
def _probe_supermemory_connection(
    api_key: str, runtime_home: str, *, identity: str = "default"
) -> dict:
    config = _load_supermemory_config(runtime_home)
    ...
    status = {
        ...
        "container_tag": _resolve_container_tag_for_setup(
            runtime_home, identity=identity
        ),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        ...
        # All other options are documented for $PCBDRAFT_RUNTIME_HOME/supermemory.json
    ...
    def save_config(self, values, runtime_home):
        ...
        _save_supermemory_config(sanitized, runtime_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, runtime_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._runtime_home = kwargs.get("runtime_home") or str(get_runtime_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._runtime_home)
    ...
    def get_tool_schemas(self) -> list[dict[str, Any]]:
        ...
        if not self._enable_custom_containers:
            return with_kebab_aliases(
                [STORE_SCHEMA, SEARCH_SCHEMA, FORGET_SCHEMA, PROFILE_SCHEMA]
            )
