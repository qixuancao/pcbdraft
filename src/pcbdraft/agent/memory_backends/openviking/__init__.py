...
from pcbdraft.core.runtime_utils import atomic_json_write, env_var_enabled
...
_SYNC_TRACE_ENV = "PCBDRAFT_RUNTIME_OPENVIKING_SYNC_TRACE"
...
class _VikingClient:
    ...
    def _authenticated_json(self, path: str) -> dict:
        ...
        resp = self._httpx.get(self._url(path), headers=headers, timeout=3.0)
        return self._parse_response(resp)
...
def _openviking_endpoint_is_always_blocked(candidate: str) -> bool:
    ...
    """
    from pcbdraft.tools.url_safety import is_always_blocked_url
...
def _normalize_openviking_url(url: str) -> str:
    ...
    try:
        ...
        if parsed.scheme.lower() not in {"http", "https"} or not parsed.hostname:
            raise ValueError(
                "OpenViking endpoints must use http:// or https:// with a host."
            )
...
def _resolve_connection_settings(provider_config: dict | None = None) -> dict:
    ...
    return {
        ...
        if account_env is not None
            ...
            ovcli_values.get("account"),
            _clean_config_value(provider_config.get("account")),
        ),
        "user": user_env
...
def _validate_openviking_reachability(endpoint: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if hasattr(client, "health_payload"):
            ...
            return (
                False,
                "OpenViking server responded, but its /health response is not valid OpenViking.",
            )
        elif client.health():
            return True, ""
    except Exception as e:
...
def _local_openviking_port_is_open(host: str, port: int) -> bool:
    ...
    try:
        with socket.create_connection(
            (host, port), timeout=_LOCAL_OPENVIKING_PROBE_TIMEOUT
        ):
            return True
    except OSError:
        return False
        ...
        if not api_key_type and is_local:
            credential_choice = select(
                ...
                [
                    (
                        "User API key",
                        "recommended; server derives account/user automatically",
                    ),
                    ("Root API key", "requires account and user IDs"),
                    (
                        "No API key",
                        "only for explicitly unauthenticated local development",
                    ),
                ],
                default=0,
                cancel_returns=cancelled,
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values: dict[str, Any], runtime_home: str) -> None:
    ...
    def post_setup(self, runtime_home: str, config: dict) -> None:
        ...
        from pcbdraft.interfaces.tui.memory_setup import (
            _CANCELLED,
            _curses_select,
        ...
        env_path = runtime_home_path / ".env"
        ...
        *,
        ...
        with self._runtime_start_lock:
            ...
            if start_state != _LOCAL_SERVER_STARTED:
                ...
                self._client = None
                status_message = f"{start_message} OpenViking memory is starting in the background and will attach when ready."
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        runtime_home = str(kwargs.get("runtime_home") or "").strip()
        if not runtime_home:
            try:
                from pcbdraft.core.runtime_environment import get_runtime_home
    ...
    def _ensure_client_locked(self) -> _VikingClient | None:
        ...
        if config_unchanged:
            with self._runtime_start_lock:
                if self._runtime_start_pending or (
                    self._runtime_start_thread and self._runtime_start_thread.is_alive()
                ):
                    return self._client
    ...
    def _pending_session_dir(self) -> Path | None:
        if not self._runtime_home:
            return None
        return Path(self._runtime_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Path | None:
        if not self._runtime_home:
            return None
        return Path(self._runtime_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _claim_owner_run_for_recovery(
    ...
    def _mark_session_pending(self, sid: str) -> None:
        ...
        if self._run_lock_path is None:
            logger.debug(
                "Could not safely mark OpenViking session %s pending without a run lock",
                sid,
            )
            return
        try:
    ...
    def _recover_pending_sessions(self) -> None:
        ...
        for owner_run_id, sids in pending_by_owner.items():
            recoverable, owner_lock_file = self._claim_owner_run_for_recovery(
                owner_run_id
            )
            if not recoverable:
                continue
                ...
                try:
                    for pending_sid in pending_sids:
                        with self._deferred_commit_lock:
                            if (
                                self._shutting_down
                                or pending_sid in self._deferred_commit_sids
                            ):
                                continue
        ...
        *,
        ...
        try:
            ...
            if clear_missing and _status_code_from_error(e) == 404:
                self._clear_pending_session(sid)
                logger.debug(
                    "OpenViking pending session %s no longer exists; dropped marker",
                    sid,
                )
                return False
    ...
    def _recall_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "max_injected_chars": self._setting_int(
                ...
                cfg.get(
                    "recall_max_injected_chars", _DEFAULT_RECALL_MAX_INJECTED_CHARS
                ),
                default=_DEFAULT_RECALL_MAX_INJECTED_CHARS,
                minimum=100,
                maximum=50000,
    ...
    def _truncate_profile_content(cls, content: str, max_units: int) -> str:
        ...
        def _head_only() -> str:
            ...
            return (
                f"{head}{marker}"
                if head
                else cls._take_token_prefix(content, max_units)
            )
    ...
    def _build_session_start_memory_block(
        ...
        *,
        ...
        placeholder_count = (
            int(bool(profile)) + int(bool(preferences)) + int(bool(entities))
        )
                ...
                while next_batch_index < len(batch_messages):
                    if _sync_trace_enabled():
                        logger.info(
                            ...
                            next_batch_index,
                            json.dumps(
                                batch_messages[next_batch_index], ensure_ascii=False
                            ),
                        )
            ...
            def _post_turn(client: _VikingClient) -> None:
                ...
                if batch_messages:
                    while next_batch_index < len(batch_messages):
                        ...
                        payload = {
                            "messages": batch_messages[next_batch_index:batch_end]
                        }
                        if _sync_trace_enabled():
                            logger.info(
