...
def _get_user_plugins_dir() -> Path | None:
    """Return ``$PCBDRAFT_RUNTIME_HOME/plugins/`` or None if unavailable."""
    try:
        from pcbdraft.core.runtime_environment import get_runtime_home

        d = get_runtime_home() / "plugins"
...
def _get_project_plugins_dir() -> Path | None:
    ...
    """
    try:
        from pcbdraft.agent.extensions.manager import _env_enabled
...
def _entry_point_package_dir(entry_point) -> Path | None:
    ...
    if entry_point is None:
        return None
    try:
        from pcbdraft.agent.extensions.manager import resolve_module_origin
...
def load_memory_provider(
    ...
    *,
    ...
    (``$PCBDRAFT_RUNTIME_HOME/plugins/<name>/``), and pip entry-point providers.
    ...
    *,
    ...
    if hasattr(loaded, "register"):
        collector = _ProviderCollector(
            entry_point.name, register_skills=register_skills
        )
    ...
    logger.debug(
        "Memory provider entry point '%s' loaded no provider", entry_point.name
    )
    return None
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        try:
            ...
            logger.debug(
                "Memory provider '%s' failed to register skill: %s", self.name, exc
            )
