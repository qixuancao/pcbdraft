...
try:
    from pcbdraft.agent.memory_backends.holographic import holographic as hrr
except ImportError:
...
class MemoryStore:
    ...
    def __init__(
        ...
        if db_path is None:
            ...
            db_path = str(get_runtime_home() / "memory_store.db")
    ...
    def _init_db(self) -> None:
        ...
        # gracefully on NFS/SMB/FUSE-mounted PCBDRAFT_RUNTIME_HOME (same issue as
    ...
    def search_facts(
        ...
        with self._lock:
            ...
            from pcbdraft.agent.memory_backends.holographic.retrieval import (
                FactRetriever,
            )
    ...
    def _rebuild_bank(self, category: str) -> None:
        ...
        with self._lock:
            ...
            vectors = [
                hrr.bytes_to_phases(row["hrr_vector"], dim=self.hrr_dim) for row in rows
            ]
    ...
    def release_all_under(cls, directory: "str | Path") -> int:
        ...
        with cls._shared_guard:
            ...
            doomed = [
                key for key in cls._shared if os.path.normcase(key).startswith(root)
            ]
            for key in doomed:
