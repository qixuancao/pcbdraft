...
Config in $PCBDRAFT_RUNTIME_HOME/config.yaml (profile-scoped):
  plugins:
    hermes-memory-store:
      db_path: $PCBDRAFT_RUNTIME_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_runtime_home()}/memory_store.db"
        return [
            {
                "key": "db_path",
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _runtime_home = str(get_runtime_home())
