...
from pcbdraft.agent.memory_backends.mem0._oss_providers import (
    EMBEDDER_PROVIDERS,
    KNOWN_DIMS,
    LLM_PROVIDERS,
    VECTOR_PROVIDERS,
...
def _setup_platform(runtime_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    provider.save_config(provider_config, runtime_home)
...
def _setup_selfhosted(runtime_home: str, config: dict, flags: dict[str, str]) -> None:
...
def _setup_oss(runtime_home: str, config: dict, flags: dict[str, str]) -> None:
...
def _prompt_api_key(label: str, env_var: str, runtime_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    if not existing:
        env_path = Path(runtime_home) / ".env"
        if env_path.exists():
...
def _setup_oss_interactive(runtime_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], runtime_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(
            f"{embedder_def['label']} embedder", embedder_def["env_var"], runtime_home
        )
        if key:
...
def post_setup(runtime_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(runtime_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(runtime_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(runtime_home, config, flags)
        return
