...
Configuration
-------------
Secret (lives in $PCBDRAFT_RUNTIME_HOME/.env or the environment):
...
Behavioral settings (live in $PCBDRAFT_RUNTIME_HOME/mem0.json, set via `hermes memory
setup`):
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def post_setup(self, runtime_home: str, config: dict) -> None:
        from pcbdraft.agent.memory_backends.mem0._setup import post_setup

        post_setup(runtime_home, config)
    ...
    def _create_backend(self):
        ...
        try:
            if self._mode == "oss":
                from pcbdraft.agent.memory_backends.mem0._backend import OSSBackend

                return OSSBackend(self._config.get("oss", {}))
            if self._host:
                from pcbdraft.agent.memory_backends.mem0._backend import (
                    SelfHostedBackend,
                )

                return SelfHostedBackend(self._api_key, self._host)
            from pcbdraft.agent.memory_backends.mem0._backend import PlatformBackend
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $PCBDRAFT_RUNTIME_HOME/mem0.json) —
