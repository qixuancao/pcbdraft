...
def apply_template(
    api_url: str, bank_id: str, api_key: str | None, manifest: dict
) -> None:
...
def probe_existing_customization(
    api_url: str, bank_id: str, api_key: str | None
) -> bool:
...
def run_template_step(
    *,
    ...
    items = [
        (e.get("name", e["id"]), (e.get("description") or "")[:72]) for e in entries
    ]
    items.append(("Blank", "Start with an empty memory bank"))
    idx = select(
        "  Starter memory template", items, default=0, cancel_returns=cancelled
    )
    ...
    try:
        ...
        log(
            f"  ✓ Applied '{entry.get('name', entry['id'])}' template to bank '{bank_id}'"
        )
        return entry["id"]
    except Exception as e:
        log(
            f"  ⚠ Could not apply template ({e}). You can apply one later from "
