...
to ``<runtime_home>/moa-traces/<session_id>.jsonl``. The record is the TRUE
...
def _traces_enabled_and_dir() -> Path | None:
    ...
    ``moa.trace_dir`` overrides the default ``<runtime_home>/moa-traces/``.
    """
    try:
    ...
    if override:
        ...
        base = get_runtime_home() / "moa-traces"
    return base
