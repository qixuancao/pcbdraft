...
from pcbdraft.agent.skill_preprocessing import load_skills_config as _load_skills_config
from pcbdraft.agent.skill_preprocessing import (
...
def _resolve_skill_commands_platform() -> str | None:
    ...
    Resolves from (in order) ``PCBDRAFT_RUNTIME_PLATFORM`` env var and
    ``PCBDRAFT_RUNTIME_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("PCBDRAFT_RUNTIME_PLATFORM") or get_session_env(
            "PCBDRAFT_RUNTIME_SESSION_PLATFORM"
        )
    except Exception:
        resolved_platform = os.getenv("PCBDRAFT_RUNTIME_PLATFORM")
...
def _resolve_skill_commands_home() -> str:
    ...
    ``skills.external_dirs`` (via ``set_runtime_home_override``), but the
...
def _load_skill_payload(
    ...
    try:
        from pcbdraft.agent.skill_utils import normalize_skill_lookup_name
        from pcbdraft.tools.skills_tool import SKILLS_DIR, skill_view
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_runtime_home()}/config.yaml):"]
...
def _build_skill_message(
    ...
    from pcbdraft.tools.skills_tool import SKILLS_DIR
...
def build_skill_invocation_message(
    ...
    try:
        from pcbdraft.tools.skill_usage import bump_use
...
def build_preloaded_skills_prompt(
    ...
    a deployment's ``PCBDRAFT_RUNTIME_TUI_SKILLS`` env var could force-load a skill an
