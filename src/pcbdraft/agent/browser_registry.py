...
def register_provider(provider: BrowserProvider, *, scope: str | None = None) -> None:
    ...
    with _lock:
        target = (
            _providers if scope is None else _scoped_providers.setdefault(scope, {})
        )
...
def list_providers(*, scope: str | None = None) -> list[BrowserProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or runtime_home_key(), {}))
        items = list(merged.values())
    return sorted(items, key=lambda p: p.name)


def get_provider(name: str, *, scope: str | None = None) -> BrowserProvider | None:
    """Return the provider registered under *name*, or None."""
    if not isinstance(name, str):
        return None
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or runtime_home_key(), {}).get(
            key
...
def registry_generation(*, scope: str | None = None) -> tuple[int, int]:
    ...
    active_scope = scope or runtime_home_key()
    with _lock:
...
def _resolve(configured: str | None) -> BrowserProvider | None:
    ...
    plugins added under ``<PCBDRAFT_RUNTIME_HOME>/plugins/browser/<vendor>/`` are subject
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(runtime_home_key(), {}))
