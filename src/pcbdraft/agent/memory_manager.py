...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_PCBDRAFT_CORE_TOOLS)
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile PCBDRAFT_RUNTIME_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped PCBDRAFT_RUNTIME_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``runtime_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_runtime_home()`` themselves.
        """
        if "runtime_home" not in kwargs:
            from pcbdraft.core.runtime_environment import get_runtime_home

            kwargs["runtime_home"] = str(get_runtime_home())
        for provider in self._providers:
            try:
