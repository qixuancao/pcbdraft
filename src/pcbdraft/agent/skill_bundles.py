...
Storage
-------
Bundles live in ``<PCBDRAFT_RUNTIME_HOME>/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``PCBDRAFT_RUNTIME_HOME``). Each file looks like::
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under PCBDRAFT_RUNTIME_HOME.

    Honors ``PCBDRAFT_RUNTIME_BUNDLES_DIR`` for tests; falls back to
    ``<PCBDRAFT_RUNTIME_HOME>/skill-bundles``.
    """
    override = os.environ.get("PCBDRAFT_RUNTIME_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_runtime_home() / "skill-bundles"
...
def build_bundle_invocation_message(
    ...
    from pcbdraft.agent.skill_commands import _build_skill_message, _load_skill_payload
