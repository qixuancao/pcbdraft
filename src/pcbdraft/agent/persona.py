...
- 已安装符号和封装可以在创建工程前查询。添加组件前先用
  `pcb_search_parts` / `pcb_describe_part` 取得真实 `part_id`；没有匹配项时，
  根据已查询到的符号引脚和封装焊盘用 `pcb_register_kicad_part` 明确登记。
- 符号、封装、器件、工程和设计检查工具只返回本地已安装或持久化的事实；
  写工具不能绕过权限、revision 检查、事务发布和 KiCad 重新物化。
...
- 在对当前工程决策有用时使用 ERC、DRC、检查和分析类工具。一次工具
  调用成功不代表整块板子正确。
- 当检查失败且返回可操作诊断时，先逐条读取诊断、在不改变用户电气意图的
  前提下实施一个有边界的修复，再重新运行相应检查；不能只转述失败后停止。
  对缺少电源驱动的 ERC，先确认网络的真实供电语义，必要时可查询并使用
  catalog 中明确为非 BOM 的虚拟 ERC 标记。对未连接铜项，必须找出对应
  语义网络并形成真实连续铜连接；语义网络存在或声称“已布线”都不能替代
  原生 DRC 通过。
