...
from pcbdraft.core.runtime_environment import get_config_path, get_skills_dir, is_termux
...
def read_active_org_id(skills_dir: Path) -> str | None:
...
def org_id_of_path(path, skills_dir: Path) -> str | None:
...
def is_excluded_skill_path(path, *, root: Path | None = None) -> bool:
...
def is_skill_support_path(path, *, root: Path | None = None) -> bool:
...
def _detect_environment(env: str) -> bool:
    ...
    PCBDRAFT_RUNTIME_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``PCBDRAFT_RUNTIME_KANBAN_TASK`` / ``PCBDRAFT_RUNTIME_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("PCBDRAFT_RUNTIME_KANBAN_TASK") or os.getenv(
            "PCBDRAFT_RUNTIME_KANBAN_BOARD"
        ):
        ...
        if _owns_dispatcher_task:
            ...
            try:
                from pcbdraft.tools.kanban_tools import _profile_has_kanban_toolset
...
def get_disabled_skill_names(platform: str | None = None) -> set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``PCBDRAFT_RUNTIME_PLATFORM`` or
            ``PCBDRAFT_RUNTIME_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        ...
        or get_session_env("PCBDRAFT_RUNTIME_SESSION_PLATFORM")
    )
...
def get_external_skills_dirs() -> list[Path]:
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against PCBDRAFT_RUNTIME_HOME, not cwd
        if not p.is_absolute():
            p = (runtime_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_untrusted_project_skills_root() -> tuple[Path, int] | None:
...
#
# The scan cache lives under PCBDRAFT_RUNTIME_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_runtime_home() / "cache" / "project_skill_scans"
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    if cached is not None:
        return cached
    try:
        from pcbdraft.tools.skills_guard import scan_skill_cached
