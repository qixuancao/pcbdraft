...
from pcbdraft.agent.turn_context import drop_stale_api_content
    ...
    *,
    ...
    if pool_provider and current_provider != pool_provider:
        ...
        if current_provider == "custom" and pool_provider.startswith("custom:"):
            try:
                from pcbdraft.model.credential_pool import get_custom_provider_pool_key
    ...
    try:
        ...
        if pool is not None and pool.has_available():
            ...
            if entry is not None:
                ...
                from pcbdraft.model.credential_pool import CUSTOM_POOL_PREFIX
        ...
        from pcbdraft.agent.chat_completion_helpers import _reset_stale_streak

        _reset_stale_streak(agent)

        # Undo the fallback's identity rewrite so the prompt is
        # byte-identical to the stored copy again (prefix cache match).
        from pcbdraft.agent.chat_completion_helpers import rewrite_prompt_model_identity
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        if env_var_enabled("PCBDRAFT_RUNTIME_DUMP_REQUEST_STDOUT"):
            print(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            ...
            from pcbdraft.model.configuration import load_config as _load_moa_cfg
    ...
    from pcbdraft.model.anthropic_adapter import _model_name_is_kimi_family
    ...
    if is_anthropic_wire and is_minimax_route:
        from pcbdraft.model.model_metadata import _model_name_suggests_minimax_m3
    ...
    from pcbdraft.agent.prompt_caching import ALIBABA_FAMILY_PROVIDERS, is_qwen_model
...
def switch_model(agent, new_model, new_provider, api_key="", base_url="", api_mode=""):
    ...
    turn-scoped).
    """
    from pcbdraft.model.provider_config import determine_api_mode
    ...
    try:
        ...
        from pcbdraft.model.configuration import load_config as _sm_load_config
...
def invoke_tool(
    ...
    try:
        from pcbdraft.agent.extensions.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from pcbdraft.agent.extensions.manager import _dispatch_pre_tool_call_hooks
    ...
    if function_name == "todo":
        ...
        def _execute(next_args: dict) -> Any:
            from pcbdraft.tools.setup_mcp_tool import setup_mcp_tool as _setup_mcp_tool
    ...
    from pcbdraft.agent.extensions.middleware import run_tool_execution_middleware
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    """
    from pcbdraft.agent.message_sanitization import apply_reasoning_content_policy
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    ...
    * Switching TO a strict provider that rejects the field (Mistral,
    ...
    removed.
    """
    from pcbdraft.agent.message_sanitization import reapply_reasoning_echo
