...
Language resolution order:
    ...
    2. ``PCBDRAFT_RUNTIME_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``PCBDRAFT_RUNTIME_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("PCBDRAFT_RUNTIME_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "PCBDRAFT_RUNTIME_BUNDLED_LOCALES points to a non-directory path (%s); "
    ...
    source_dir = (
        Path(__file__).resolve().parent.parent / "interfaces" / "tui" / "locales"
    )
    return source_dir
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("PCBDRAFT_RUNTIME_LANGUAGE")
    if env_lang:
