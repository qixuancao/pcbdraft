...
The manifest lives at ``<project>/.pcbdraft/environment.json`` and is the
...
from pcbdraft.agent.verify.recipes import Recipe, detect_recipe

MANIFEST_VERSION = 1
_MANIFEST_RELPATH = Path(".pcbdraft") / "environment.json"
...
def load_manifest(root: Path) -> Recipe | None:
    ...
    fresh detection instead of breaking ``python -m pcbdraft.agent.verify``.
    """
