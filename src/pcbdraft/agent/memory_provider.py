...
class MemoryProvider(ABC):
    ...
    def save_config(self, values: dict[str, Any], runtime_home: str) -> None:
        ...
        ``runtime_home`` is the active PCBDRAFT_RUNTIME_HOME directory path.
    ...
    def backup_paths(self) -> list[str]:
        """Return extra on-disk paths this provider stores OUTSIDE PCBDRAFT_RUNTIME_HOME.

        ``hermes backup`` only walks PCBDRAFT_RUNTIME_HOME, so any provider state kept
