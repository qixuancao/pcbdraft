...
from pcbdraft.core.runtime_utils import safe_json_loads
...
def _resolve_skill_manage_paths(args: dict) -> list[Path]:
    ...
    from pcbdraft.tools.skill_manager_tool import _find_skill, _resolve_skill_dir
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("PCBDRAFT_RUNTIME_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def _get_cute_tool_message(
    ...
    if tool_name == "delegate_task":
        ...
        if _action in ("list", "steer", "stop"):
            ...
            return _wrap(
                f"┊ 🔀 delegate  {_trunc(f'{_action} {_sid}'.strip(), 35)}  {dur}"
            )
        tasks = args.get("tasks")
        if tasks and isinstance(tasks, list):
