...
from pcbdraft.agent.display import KawaiiSpinner
from pcbdraft.agent.error_classifier import FailoverReason, classify_api_error
from pcbdraft.agent.message_metadata import append_message
...
from pcbdraft.agent.turn_context import (
    _compression_warrants_another_preflight_pass,
    build_turn_context,
    compose_user_api_content,
    reanchor_current_turn_user_idx,
)
from pcbdraft.agent.turn_retry_state import TurnRetryState
...
# DEFAULT_DB_PATH = get_runtime_home() / "state.db" breaks tests that
# monkeypatch get_runtime_home to return a str).
_STALE_MARKER_RE = re.compile(r"^\[[A-Za-z_][A-Za-z0-9_.-]*\]$")
from pcbdraft.agent import empty_response_guard as _empty_guard
from pcbdraft.agent.process_bootstrap import _install_safe_stdio
from pcbdraft.agent.prompt_caching import (
    build_prompt_cache_plan,
    effective_cache_ttl,
    strip_anthropic_cache_control,
    strip_anthropic_tool_cache_control,
)
from pcbdraft.agent.repetition_guard import is_repetition_dominated
from pcbdraft.agent.retry_utils import (
    adaptive_rate_limit_backoff,
    is_zai_coding_overload_error,
    jittered_backoff,
    zai_coding_overload_retry_ceiling,
)
from pcbdraft.agent.trajectory import has_incomplete_scratchpad

# Bind before the turn starts so a source-tree swap cannot load a skewed
# finalizer at turn end.
from pcbdraft.agent.turn_finalizer import finalize_turn
from pcbdraft.core.runtime_environment import PARTIAL_STREAM_STUB_ID
from pcbdraft.core.runtime_logging import set_session_context
from pcbdraft.core.runtime_utils import base_url_host_matches, env_var_enabled
...
from pcbdraft.model.usage_pricing import estimate_usage_cost, normalize_usage
from pcbdraft.tools.skill_provenance import set_current_write_origin
...
def _billing_or_entitlement_message(
    *,
    ...
    try:
        from pcbdraft.model.billing_links import build_billing_block
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        ...
        _bot_stale = False
        try:
            from pcbdraft.tools.bot_mode_probe import (
                BOT_CHAT_TITLE,
            ...
            try:
                from pcbdraft.agent.system_prompt import _agent_home
        ...
        from pcbdraft.agent.system_prompt import restore_plugin_prompt_sections
        ...
        from pcbdraft.agent.system_prompt import reconstruct_static_prefix
    ...
    try:
        from pcbdraft.model.credits_tracker import seed_credits_at_session_start
...
def _get_continuation_prompt(
    is_partial_stub: bool, dropped_tools: list[str] | None = None
) -> str:
...
def _peel_moa_guidance(
    ...
    """
    from pcbdraft.agent.moa_loop import peel_reference_guidance
...
def _apply_context_engine_selection(
    ...
    *,
    ...
    try:
        from pcbdraft.agent.context_engine import ContextEngine as _CE
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from pcbdraft.interfaces.tui.moa_config import decode_moa_turn
    ...
    while (
        ...
        if _pre_api_steer:
            ...
            for _si in range(len(messages) - 1, -1, -1):
                ...
                if isinstance(_sm, dict) and _sm.get("role") == "tool":
                    from pcbdraft.agent.prompt_builder import format_steer_marker
        ...
        if moa_config:
            try:
                from pcbdraft.agent.message_content import (
                    flatten_message_text as _flatten_mt,
                )
                from pcbdraft.agent.moa_loop import (
                    _preset_temperature,
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    from pcbdraft.agent.nous_rate_guard import (
            ...
            try:
                ...
                if env_var_enabled("PCBDRAFT_RUNTIME_DUMP_REQUESTS"):
                ...
                if agent.provider == "nous":
                    try:
                        from pcbdraft.agent.nous_rate_guard import clear_nous_rate_limit
                ...
                if retry_count >= max_retries:
                    ...
                    from pcbdraft.agent.thinking_timeout_guidance import (
                        is_thinking_timeout,
                    )
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    from pcbdraft.services.session_db import classify_persistence_error
                ...
                # timeout (PCBDRAFT_RUNTIME_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    from pcbdraft.agent.verification_stop import (
                ...
                try:
                    ...
                    if (
                        ...
                        if coding is None:
                            from pcbdraft.agent.coding_context import is_coding_context
                ...
                try:
                    from pcbdraft.agent.kanban_stop import build_kanban_stop_nudge
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
