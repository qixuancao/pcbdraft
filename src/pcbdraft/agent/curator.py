...
def _state_file() -> Path:
    return get_runtime_home() / "skills" / ".curator_state"
...
def apply_automatic_transitions(now: datetime | None = None) -> dict[str, int]:
    ...
    """
    from pcbdraft.tools import skill_usage as _u
    ...
    for row in _u.curated_report():
        ...
        if anchor <= archive_cutoff and current != _u.STATE_ARCHIVED:
            ...
            try:
                from pcbdraft.tools.skill_ledger import (
                    reset_ledger_actor,
                    set_ledger_actor,
                )
...
def _reports_root() -> Path:
    ...
    that bypasses both.
    """
    root = get_runtime_home() / "logs" / "curator"
    try:
