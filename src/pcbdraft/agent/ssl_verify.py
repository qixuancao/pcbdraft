...
def _coerce_insecure(ssl_verify: Any) -> bool:
    ...
    return bool(
        ...
        and ssl_verify.strip().lower() in {"false", "0", "no", "off"}
    )
...
def resolve_httpx_verify(
    *,
    ...
    3. ``PCBDRAFT_RUNTIME_CA_BUNDLE``, ``SSL_CERT_FILE``, ``REQUESTS_CA_BUNDLE``,
    ...
    effective_ca = (
        (ca_bundle or "").strip()
        or os.getenv("PCBDRAFT_RUNTIME_CA_BUNDLE", "").strip()
