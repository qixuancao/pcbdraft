...
from pcbdraft.agent.extensions.middleware import (
    OBSERVER_SCHEMA_VERSION,
    VALID_MIDDLEWARE,
)
from pcbdraft.core.registration_lifecycle import replacement_coordinator
...
from pcbdraft.core.runtime_utils import env_var_enabled, fast_safe_load
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``PCBDRAFT_RUNTIME_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("PCBDRAFT_RUNTIME_BUNDLED_PLUGINS")
    if env_override:
...
_PLUGINS_DEBUG = os.getenv("PCBDRAFT_RUNTIME_PLUGINS_DEBUG", "").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When PCBDRAFT_RUNTIME_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    if force:
        _PLUGINS_DEBUG = os.getenv(
            "PCBDRAFT_RUNTIME_PLUGINS_DEBUG", ""
        ).strip().lower() in {
            "1",
            "true",
            "yes",
    ...
    logger.debug(
        "PCBDRAFT_RUNTIME_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
def _detect_kind_from_source(source_text: str) -> str | None:
...
def _read_source_from_origin(origin: str | None, limit: int = 8192) -> str:
...
def resolve_module_origin(module_name: str) -> str | None:
...
class PluginRegistration:
    ...
    _on_dispose: Callable[[PluginRegistration], None] | None = field(
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_runtime_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        import pcbdraft.model.configuration as config_mod
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from pcbdraft.interfaces.tui.platform_actions import PlatformActions
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from pcbdraft.agent.plugin_llm import PluginLlm
    ...
    def subagent_lifecycle(self) -> Any:
        ...
        if self._subagent_lifecycle is None:
            from pcbdraft.agent.subagent_lifecycle import (
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``PCBDRAFT_RUNTIME_HOME`` via
    ...
    def call_mcp(
        ...
        from pcbdraft.tools.mcp_tool import _make_tool_handler
    ...
    def register_context_engine(self, engine) -> PluginRegistration | None:
        ...
        from pcbdraft.agent.context_engine import ContextEngine
    ...
    def register_context_reference(self, provider) -> None:
        ...
        """
        from pcbdraft.agent.context_references import ContextReferenceProvider as _CRP
    ...
    @_serialized_replacement
    def register_image_gen_provider(self, provider) -> PluginRegistration | None:
        ...
        tool calls.
        """
        from pcbdraft.agent.image_gen_provider import ImageGenProvider
        from pcbdraft.agent.image_gen_registry import (
            register_provider,
    ...
    @_serialized_replacement
    def register_video_gen_provider(self, provider) -> PluginRegistration | None:
    ...
    @_serialized_replacement
    def register_web_search_provider(self, provider) -> PluginRegistration | None:
    ...
    @_serialized_replacement
    def register_browser_provider(self, provider) -> PluginRegistration | None:
    ...
    @_serialized_replacement
    def register_secret_source(self, source) -> PluginRegistration | None:
        ...
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        """
        from pcbdraft.agent.secret_sources.base import SecretSource
        from pcbdraft.agent.secret_sources.registry import (
            register_source,
            restore_registration,
            snapshot_registration,
        )
    ...
    @_serialized_replacement
    def register_tts_provider(self, provider) -> PluginRegistration | None:
    ...
    @_serialized_replacement
    def register_transcription_provider(self, provider) -> PluginRegistration | None:
        ...
        **but only when**:
        ...
        backends).
        """
        from pcbdraft.agent.transcription_provider import TranscriptionProvider
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        from pcbdraft.interfaces.tui.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_redaction_patterns(self, patterns) -> int:
        ...
        """
        from pcbdraft.agent.redact import register_redaction_patterns as _register
    ...
    def register_skill(
        ...
        """
        from pcbdraft.agent.skill_utils import _NAMESPACE_RE
...
class PluginManager:
    ...
    def __init__(self, scope_key: str | None = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or runtime_home_key()
        self.home_path = Path(self.scope_key)
        self._discovery_lock = threading.RLock()
        self._plugins: dict[str, LoadedPlugin] = {}
        from pcbdraft.agent.observability import BUILTIN_HOOKS, BUILTIN_MIDDLEWARE

        self._builtin_hooks = BUILTIN_HOOKS
        self._builtin_middleware = BUILTIN_MIDDLEWARE
        ...
        # — i.e. per (runtime_home, plugin_id) — and every release/restore
    ...
    def _unload_scoped(
        ...
        if unload_all:
            ...
            if self._plugin_platform_names:
                raise RuntimeError(
                    "Messaging platform registrations are not supported by PCBDraft"
                )
            ...
            if preledger_tools:
                try:
                    from pcbdraft.tools.registry import registry as tool_registry
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("PCBDRAFT_RUNTIME_SAFE_MODE"):
                logger.info("PCBDRAFT_RUNTIME_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _re_register_shell_hooks_after_force(self) -> None:
        ...
        try:
            from pcbdraft.agent.shell_hooks import re_register_config_hooks
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from pcbdraft.interfaces.tui.approval_transport import (
            RegisteredApprovalTransport,
        )
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_runtime_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_runtime_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> list[PluginManifest]:
        ...
        if _env_enabled("PCBDRAFT_RUNTIME_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set PCBDRAFT_RUNTIME_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from pcbdraft.interfaces.tui.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from pcbdraft.interfaces.tui.agent_plugins import (
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from pcbdraft.tools.registry import registry as _registry
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from pcbdraft.interfaces.tui.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_runtime_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> list[Any]:
        ...
        callbacks = (
            *self._builtin_hooks.get(hook_name, ()),
    ...
    def has_hook(self, hook_name: str) -> bool:
        ...
        return bool(self._builtin_hooks.get(hook_name) or self._hooks.get(hook_name))
    ...
    def has_middleware(self, kind: str) -> bool:
        ...
        return bool(self._builtin_middleware.get(kind) or self._middleware.get(kind))
    ...
    def invoke_middleware(self, kind: str, **kwargs: Any) -> list[Any]:
        ...
        """
        callbacks = (
            *self._builtin_middleware.get(kind, ()),
            *self._middleware.get(kind, ()),
        )
        results: list[Any] = []
        for cb in callbacks:
            try:
...
# multiple profiles via different PCBDRAFT_RUNTIME_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_runtime_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# hermes_constants.set_runtime_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_runtime_home() / "plugins"`` and some
    ...
    ``set_runtime_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Hermes home instead of
    being one process-wide singleton.
    """
    try:
        return get_runtime_home().expanduser().resolve()
    except Exception:
        return get_runtime_home().expanduser()
...
def get_plugin_manager() -> PluginManager:
    ...
    profile switch — via ``PCBDRAFT_RUNTIME_HOME`` or the context-local
    ``set_runtime_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=runtime_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_runtime_home() / "cache" / "plugin_toolset_keys.json"
...
def _read_plugin_keys_cache() -> dict | None:
    try:
        import json as _json
...
def _get_pre_tool_call_directive_details(
    ...
    from pcbdraft.interfaces.tui.lifecycle import invoke_hook as invoke_lifecycle_hook
