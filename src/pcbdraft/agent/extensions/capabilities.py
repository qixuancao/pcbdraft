...
CAPABILITY_REGISTRY: dict[str, CapabilitySpec] = {
    ...
    for spec in (
        ...
        CapabilitySpec(
            ...
            description=(
                "Route its LLM calls through the host's built-in auxiliary task lanes"
            ),
        ),
        CapabilitySpec(
    ...
    for item in raw:
        ...
        if cap in VALID_CAPABILITY_IDS:
            ...
            logger.warning(
                ...
                plugin_name,
                cap,
                ", ".join(sorted(VALID_CAPABILITY_IDS)),
            )
    return out
...
def granted_capabilities(
    plugin_id: str, config: Mapping[str, Any] | None = None
) -> frozenset:
...
def plugin_capability_granted(
    ...
    * the legacy ``allow_*`` config key is set (deprecated, still honored so
    ...
    if spec is None:
        ...
        return False
    entry = _plugin_entry(plugin_id, config)
    if capability in granted_capabilities(
        plugin_id, config={"plugins": {"entries": {plugin_id: entry}}}
    ):
...
def record_consent(
    ...
    logger.info(
        ...
        plugin_id,
        ",".join(entry[GRANTED_KEY]) or "(none)",
