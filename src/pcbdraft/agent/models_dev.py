...
def _get_cache_path() -> Path:
    ...
    return get_runtime_home() / "models_dev_cache.json"
...
def _get_etag_path() -> Path:
    ...
    return get_runtime_home() / "models_dev_cache.etag"
...
def _default_override_context(provider: str) -> int | None:
...
def _provider_override_section(provider: str) -> dict[str, Any] | None:
...
def _explicit_model_override(provider: str, model: str) -> dict[str, Any] | None:
...
def _default_model_override(provider: str) -> dict[str, Any] | None:
...
def _override_int(override: dict[str, Any], key: str) -> int | None:
...
def _override_context_window(provider: str, model: str) -> int | None:
...
def get_model_capabilities(
    ...
    if entry is not None:
        ...
        ctx = limit.get("context")
        context_window = (
            int(ctx) if isinstance(ctx, (int, float)) and ctx > 0 else 200000
        )

        out = limit.get("output")
        max_output_tokens = (
            int(out) if isinstance(out, (int, float)) and out > 0 else 8192
        )
...
def list_provider_models(provider: str, *, allow_network: bool = True) -> list[str]:
    ...
    """
    from pcbdraft.model.catalog import normalize_provider
...
def get_model_info(
    ...
    def _from_override_alone() -> ModelInfo | None:
