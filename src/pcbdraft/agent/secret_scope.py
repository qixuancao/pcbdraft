...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the PCBDRAFT_RUNTIME_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset(
    {
        ...
        "PCBDRAFT_RUNTIME_GATEWAY_LOCK_DIR",
        "PCBDRAFT_RUNTIME_MAX_ITERATIONS",
        "PCBDRAFT_RUNTIME_MAX_TOKENS",
...
_GLOBAL_ENV_PREFIXES = (
    "PCBDRAFT_RUNTIME_KANBAN_",
    "PCBDRAFT_RUNTIME_TELEGRAM_",  # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> dict[str, str]:
    ...
    from pcbdraft.model.configuration import _parse_env_value
...
def build_profile_secret_scope(runtime_home: Path) -> dict[str, str]:
    ...
    try:
        from pcbdraft.model.env_loader import get_secret_source_values
