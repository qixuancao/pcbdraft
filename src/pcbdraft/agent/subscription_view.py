...
class SubscriptionState:
    ...
    org_id: str | None = None  # org.id from the NAS response
    role: str | None = (
...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``PCBDRAFT_RUNTIME_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
def subscription_manage_url(
    state: SubscriptionState, tier_id: str | None = None
) -> str | None:
...
def _format_dollars_grouped(value: Decimal | None) -> str:
...
def is_upgrade(state: SubscriptionState, tier_id: str) -> bool:
    ...
    if cur_id is not None and cur_id in orders:
        ...
        cur_order = next(
            (t.tier_order or 0 for t in (state.tiers or ()) if t.is_current), 0
        )
...
def dev_fixture_subscription_state() -> SubscriptionState | None:
    """Return a fixture :class:`SubscriptionState` for ``PCBDRAFT_RUNTIME_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE``.
    """
    name = (
        (os.getenv("PCBDRAFT_RUNTIME_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    )
    if not name:
        return None
    ...
    return SubscriptionState(
        logged_in=False,
        error=f"unknown PCBDRAFT_RUNTIME_DEV_SUBSCRIPTION_FIXTURE: {name}",
    )
