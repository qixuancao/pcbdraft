...
from pcbdraft.model.model_metadata import estimate_tokens_rough
...
_SENSITIVE_PCBDRAFT_DIRS = (Path("skills") / ".hub",)
    ...
    if provider is not None:
        try:
            ...
            if plugin_content is not None:
                return (
                    None,
                    f"📌 {ref.raw} ({estimate_tokens_rough(plugin_content)} tokens)\n{plugin_content}",
                )
        except Exception as exc:
...
async def _default_url_fetcher(url: str) -> str:
    from pcbdraft.tools.web_tools import web_extract_tool
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    runtime_home = get_runtime_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(runtime_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(runtime_home / rel for rel in _SENSITIVE_PCBDRAFT_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError(
            "path is a sensitive credential or internal PCBDraft path and cannot be attached"
        )
    ...
    # message text into reference expansion, so a runtime `@file:.../auth.json` from a
...
def _agent_visible_path(path: Path) -> str:
    ...
    try:
        ...
        from pcbdraft.tools.terminal_tool import _ensure_terminal_env_bridged

        _ensure_terminal_env_bridged()
        from pcbdraft.tools.credential_files import to_agent_visible_cache_path
