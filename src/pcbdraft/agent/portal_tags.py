...
def pcbdraft_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=hermes-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=pcbdraft-client-v{_pcbdraft_version()}"
...
def conversation_tag(session_id: str) -> str:
    ...
    used for runtime ``sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> list[str]:
    ...
    """
    tags = ["product=pcbdraft", pcbdraft_client_tag()]
