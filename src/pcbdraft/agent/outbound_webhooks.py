...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``PCBDRAFT_RUNTIME_SAFE_MODE=1`` still skips registration, matching
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/hermes-events
          ...
          secret_env: PCBDRAFT_RUNTIME_OUTBOUND_WEBHOOK_SECRET
...
_delivery_queue: queue.Queue[dict[str, Any] | None] = queue.Queue(
...
def register_from_config(cfg: dict[str, Any] | None) -> list[WebhookTarget]:
    ...
    if env_var_enabled("PCBDRAFT_RUNTIME_SAFE_MODE"):
        logger.info(
            "PCBDRAFT_RUNTIME_SAFE_MODE=1 — outbound webhook registration skipped"
        )
        return []
...
def iter_configured_targets(cfg: dict[str, Any] | None) -> list[WebhookTarget]:
...
def _parse_single_target(index: int, raw: Any) -> WebhookTarget | None:
...
def _resolve_secret(index: int, raw: dict[str, Any]) -> str | None:
