...
def finalize_turn(
    ...
    *,
    ...
    """
    from pcbdraft.agent.conversation_loop import logger
    ...
    if iteration_limit_fallback:
        ...
        _kanban_task = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK")
        if _kanban_task:
    ...
    if failed and str(_turn_exit_reason) == "session_persistence_failed":
        ...
        if "failure_reason" not in result:
            ...
            result["failure_reason"] = "session_persistence_failed:" + (
                _cause or "unknown"
            )
