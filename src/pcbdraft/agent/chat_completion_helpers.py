...
from pcbdraft.agent.errors import EmptyStreamError
from pcbdraft.agent.message_content import flatten_message_text
from pcbdraft.agent.message_metadata import append_message, stamp_message_timestamp
from pcbdraft.agent.message_sanitization import (
    _repair_tool_call_arguments,
    _sanitize_surrogates,
)
from pcbdraft.agent.reasoning_summaries import separate_glued_reasoning_blocks
from pcbdraft.agent.stream_single_writer import (
    claim_stream_writer,
    stream_writer_is_current,
)
from pcbdraft.agent.turn_context import substitute_api_content
...
from pcbdraft.model.gemini_native_adapter import is_native_gemini_base_url
from pcbdraft.model.model_metadata import is_local_endpoint
from pcbdraft.model.timeouts import (
    get_provider_request_timeout,
    get_provider_stale_timeout,
)
from pcbdraft.tools.terminal_tool import is_persistent_env
...
def _provider_stream_error_from_text(
    ...
    *,
    ...
    for event in _parse_provider_sse_events(text):
        ...
        if not (
            is_http_error or is_structured_error_event or is_bare_error_finish_payload
        ):
            continue
...
def _prompt_cache_scope_for_agent(agent) -> str | None:
    ...
    """
    try:
        from pcbdraft.agent.prompt_cache_scope import resolve_prompt_cache_scope_safe
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("PCBDRAFT_RUNTIME_STREAM_STALE_GIVEUP", 5)
...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("PCBDRAFT_RUNTIME_STREAM_STALE_TIMEOUT", 180.0)
...
def _resolve_direct_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    ``PCBDRAFT_RUNTIME_API_CALL_STALE_TIMEOUT`` → reasoning-model floor → context-size
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # activity. Operators can tune via PCBDRAFT_RUNTIME_CODEX_TTFB_TIMEOUT_SECONDS and
    # PCBDRAFT_RUNTIME_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # limit. Tunable via PCBDRAFT_RUNTIME_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float(
        "PCBDRAFT_RUNTIME_CODEX_HARD_TIMEOUT_SECONDS", 1500.0
    )
    ...
    # PCBDRAFT_RUNTIME_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("PCBDRAFT_RUNTIME_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float(
            "PCBDRAFT_RUNTIME_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0
        )
        _ttfb_strict = os.environ.get(
            "PCBDRAFT_RUNTIME_CODEX_TTFB_STRICT", ""
        ...
        if (
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set PCBDRAFT_RUNTIME_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
        ...
        _ttfb_cap = _env_float("PCBDRAFT_RUNTIME_CODEX_TTFB_MAX_SECONDS", 120.0)
        if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
            logger.info(
                "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set PCBDRAFT_RUNTIME_CODEX_TTFB_MAX_SECONDS to tune.",
                _ttfb_timeout,
                _ttfb_cap,
                f"{_est_tokens_for_codex_watchdog:,}",
            )
    ...
    _codex_idle_timeout = _env_float(
        "PCBDRAFT_RUNTIME_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(
    ...
    if agent.api_mode == "codex_responses":
        ...
        from pcbdraft.agent.native_compaction import (
    ...
    _is_nvidia = base_url_host_matches(
        agent._base_url_lower, "integrate.api.nvidia.com"
    )
    _is_kimi = (
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects PCBDRAFT_RUNTIME_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
        from pcbdraft.agent.redact import redact_sensitive_text
...
def try_activate_fallback(agent, reason: FailoverReason | None = None) -> bool:
    ...
    from pcbdraft.agent.backend_identity import BackendIdentity, should_skip_candidate
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        summary_extra_body = {}
        try:
            from pcbdraft.model.auxiliary_client import OMIT_TEMPERATURE as _OMIT_TEMP
            from pcbdraft.model.auxiliary_client import _fixed_temperature_for_model
        except Exception:
        ...
        if _is_nous:
            from pcbdraft.agent.portal_tags import nous_portal_tags as _portal_tags
...
def cleanup_task_resources(agent, task_id: str) -> None:
    ...
    try:
        headed = False
        try:
            from pcbdraft.tools.browser_tool import _is_headed_mode
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
        ...
        def _bedrock_call():
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(
                        "plugin reasoning stream observer check failed", exc_info=True
                    )
        ...
        try:
            ...
            while t.is_alive():
                ...
                if _stale_elapsed > _bedrock_stale_timeout:
                    ...
                    # crosses PCBDRAFT_RUNTIME_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # wins over the PCBDRAFT_RUNTIME_API_TIMEOUT env default if the user set it.
        ...
        _base_timeout = (
            ...
            else env_float("PCBDRAFT_RUNTIME_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # PCBDRAFT_RUNTIME_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float(
                "PCBDRAFT_RUNTIME_STREAM_READ_TIMEOUT", 120.0
            )
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode PCBDRAFT_RUNTIME_STREAM_READ_TIMEOUT.
            if (
    ...
    def _call_anthropic(request_client):
        ...
        from pcbdraft.agent import relay_llm
        from pcbdraft.model.anthropic_adapter import sanitize_anthropic_kwargs
    ...
    def _call():
        ...
        _max_stream_retries = env_int("PCBDRAFT_RUNTIME_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float(
            "PCBDRAFT_RUNTIME_STREAM_STALE_TIMEOUT", 180.0
        )
    ...
    # unless the user explicitly set PCBDRAFT_RUNTIME_STREAM_STALE_TIMEOUT; override the
    # local ceiling with PCBDRAFT_RUNTIME_LOCAL_STREAM_STALE_TIMEOUT (documented in
    ...
    if (
        ...
        # env var ``PCBDRAFT_RUNTIME_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        _stream_stale_timeout = env_float(
            "PCBDRAFT_RUNTIME_LOCAL_STREAM_STALE_TIMEOUT", _local_default
        )
        logger.debug(
