...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from pcbdraft.interfaces.tui.moa_config import coerce_privacy_filter
    ...
    *,
    ...
    try:
        ...
        _effective_max_tokens = (
            _slot_max_tokens if _slot_max_tokens is not None else max_tokens
        )
        extra_headers = None
        ...
        from pcbdraft.model.auxiliary_client import _normalize_aux_provider
...
def _run_references_parallel(
    ...
    *,
    ...
    """
    from pcbdraft.model.usage_pricing import CanonicalUsage
            ...
            for future in done:
                ...
                if progress_callback is not None:
                    try:
                        ...
                        Exception
                    ) as exc:  # pragma: no cover - display must never break
...
def _successful_references(
    ...
    return [
        output for output in reference_outputs if not _is_failed_reference(output[1])
    ]
...
def _failed_reference_labels(
    ...
    return [
        label
        for label, text, _accounting in reference_outputs
...
class MoAChatCompletions:
    ...
    def __init__(
        self, preset_name: str, reference_callback: Any = None, agent: Any = None
    ):
    ...
    def consume_and_save_trace(
        ...
        if not pending or "aggregator_input_messages" not in pending:
            return
        try:
            from pcbdraft.agent.moa_trace import save_moa_turn
    ...
    def _call_prepared_aggregator(
        ...
        try:
            ...
            _cache_disabled = (
                getattr(_agent, "_cache_disabled", None) if _agent is not None else None
            )
            ...
            logger.warning(
                ...
                "request (cache misses expected): %s",
                exc,
            )
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        from pcbdraft.model.configuration import get_config_path, load_config
        ...
        if _refs_from_cache:
            ...
            if privacy_mode:
                _trace_refs = [
                    (
                        label,
                        _redact_reference_text(text),
                        _redact_trace_accounting(acct),
                    )
                    for label, text, acct in reference_outputs
                ]
            else:
            ...
            try:
                from pcbdraft.agent.moa_trace import slot_metrics
            ...
            for _idx, (_label, _text, _accounting) in enumerate(
