...
def _run_bounded_on_exit_thread(fn: Callable[[], Any], timeout: float) -> Any:
    ...
    worker = threading.Thread(target=_target, daemon=True, name="relay-scope-op-exit")
    worker.start()
    worker.join(timeout)
    if worker.is_alive():
        raise TimeoutError(
            ...
            if _SEGMENTS_CONFIG is None:
                ...
                try:
                    from pcbdraft.services.messaging.run import (
                        _load_gateway_config,  # late import
                    )

                    telemetry = (_load_gateway_config().get("gateway") or {}).get(
                        "telemetry"
                    ) or {}
...
class RelayRuntime:
    ...
    def run_in_session(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            return _run_bounded_on_exit_thread(lambda: context.run(invoke), timeout)
        try:
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        try:
            try:
                _scope_op_executor().submit(self.relay.subscribers.flush).result(
                    timeout=_SCOPE_OP_TIMEOUT
                )
            except RuntimeError:
...
class RelaySessionCoordinator:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        """
        try:
            if not (isinstance(lease.host, RelayRuntime) and lease.session is not None):
                return
            session = lease.session
            with session.lock:
