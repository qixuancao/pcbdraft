...
def compute_session_context_breakdown(
    ...
    from pcbdraft.model.model_metadata import estimate_messages_tokens_rough
...
def _bytes_to_tokens(size: int | None) -> int | None:
    if size is None:
        return None
    return (int(size) + 3) // 4
...
def compute_context_details(agent: Any) -> dict[str, Any]:
    ...
    """
    from pcbdraft.agent.system_prompt import build_system_prompt_parts
    from pcbdraft.interfaces.tui.prompt_size import (
    ...
    if skills_block:
        for entry in _compute_skills_breakdown(skills_block):
            skills.append(
                {
                    "name": entry.get("name", ""),
                    "index_tokens": _bytes_to_tokens(entry.get("index_line_bytes"))
                    or 0,
...
def render_context_category_lines(payload: dict[str, Any]) -> list[str]:
    ...
    if context_max > 0:
        ...
        lines.append(
            f"{_FREE_GLYPH} {'Free space':<{width}} {free:>9,} tokens {pct:>5.1f}%"
        )
    return lines
