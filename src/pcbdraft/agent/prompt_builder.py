...
from pcbdraft.tools.threat_patterns import scan_for_threats as _scan_for_threats
...
DEFAULT_AGENT_IDENTITY = (
    "You are PCBDraft, an AI assistant for PCB design and engineering. "
...
PCBDRAFT_RUNTIME_AGENT_HELP_GUIDANCE = (
    "When the user needs help configuring or using PCBDraft, use the installed "
...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $PCBDRAFT_RUNTIME_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $PCBDRAFT_RUNTIME_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${PCBDRAFT_RUNTIME_KANBAN_BRANCH:-wt/$PCBDRAFT_RUNTIME_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$PCBDRAFT_RUNTIME_KANBAN_BRANCH` a deterministic "
...
def build_environment_hints() -> str:
    ...
    extra = (os.getenv("PCBDRAFT_RUNTIME_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_runtime_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("PCBDRAFT_RUNTIME_PLATFORM") or os.environ.get(
        "PCBDRAFT_RUNTIME_SESSION_PLATFORM"
    )
    if platform:
        return platform

    session_context = sys.modules.get("pcbdraft.agent.session_context")
    get_session_env = (
...
def build_skills_system_prompt(
    ...
    # PCBDRAFT_RUNTIME_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_runtime_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        from pcbdraft.agent.skill_utils import get_project_skills_dirs
        ...
        if _home_token is not None:
            reset_runtime_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if project_dirs:
        from pcbdraft.agent.skill_utils import iter_project_skill_files
            ...
            for skill_file in iter_project_skill_files(proj_dir):
                try:
                    ...
                    entry = _build_snapshot_entry(
                        skill_file, proj_dir, frontmatter, desc
                    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "For PCBDraft setup and troubleshooting, consult its own command help "
            "and project documentation when available.\n"
...
def load_soul_md(
    ...
    """Load SOUL.md from PCBDRAFT_RUNTIME_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the PCBDRAFT_RUNTIME_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from pcbdraft.model.configuration import ensure_runtime_home

        ensure_runtime_home()
    except Exception as e:
        logger.debug(
            "Could not ensure PCBDRAFT_RUNTIME_HOME before loading SOUL.md: %s", e
        )

    _home = Path(home_override) if home_override is not None else get_runtime_home()
...
def build_context_files_prompt(
    ...
    SOUL.md from PCBDRAFT_RUNTIME_HOME is independent and always included when present.
    ...
    from pcbdraft.agent.runtime_cwd import _is_install_tree
    ...
    # SOUL.md from PCBDRAFT_RUNTIME_HOME only — skip when already loaded as identity
    if not skip_soul:
