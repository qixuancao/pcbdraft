...
if TYPE_CHECKING:
    from pcbdraft.agent.rate_limit_tracker import RateLimitState
...
from pcbdraft.agent.context_compressor import ContextCompressor
from pcbdraft.agent.iteration_budget import IterationBudget
from pcbdraft.agent.memory_manager import StreamingContextScrubber
from pcbdraft.agent.process_bootstrap import _install_safe_stdio
from pcbdraft.agent.session_activity import ActivityProvenance
from pcbdraft.agent.subdirectory_hints import SubdirectoryHintTracker
from pcbdraft.agent.think_scrubber import StreamingThinkScrubber
...
from pcbdraft.core.runtime_utils import base_url_host_matches, is_truthy_value
...
def _ra():
    ...
    """
    import pcbdraft.agent.loop as run_agent
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    try:
        from pcbdraft.model.provider_config import (
            PCBDRAFT_RUNTIME_OVERLAYS,
            get_provider,
        )

        overlay = PCBDRAFT_RUNTIME_OVERLAYS.get(provider)
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$PCBDRAFT_RUNTIME_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_runtime_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$PCBDRAFT_RUNTIME_HOME`` just means the notice
...
def init_agent(
    ...
    if credential_pool is not None:
        try:
            from pcbdraft.model.credential_pool import credential_pool_matches_provider
    ...
    agent._cache_ttl = "5m"
    try:
        from pcbdraft.agent.agent_runtime_helpers import cache_ttl_means_disabled
        from pcbdraft.model.configuration import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind PCBDRAFT_RUNTIME_DEV_CREDITS.
    agent._credits_state = None
    agent._credits_session_start_micros = None
    # Threshold-notice latch (L4): active sticky-notice keys + the crossing gates.
    from pcbdraft.model.credits_tracker import new_credits_latch
    ...
    setup_logging(runtime_home=_ra()._runtime_home)
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if (
                ...
                try:
                    from pcbdraft.model.auth import build_minimax_oauth_token_provider
            ...
            from pcbdraft.model.anthropic_adapter import _is_oauth_token as _is_oat
        ...
        try:
            from pcbdraft.model.configuration import (
                load_config_readonly as _load_br_cfg,
            )
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                from pcbdraft.model.auxiliary_client import build_or_headers

                client_kwargs["default_headers"] = build_or_headers()
            elif base_url_host_matches(effective_base, "integrate.api.nvidia.com"):
                from pcbdraft.model.auxiliary_client import build_nvidia_nim_headers
            ...
            from pcbdraft.model.auxiliary_client import resolve_provider_client
            ...
            if _routed_client is not None:
                ...
                if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                    ...
                    for _fb in _fb_entries:
                        try:
                            from pcbdraft.model.fallback_config import (
        ...
        try:
            from pcbdraft.agent.ssl_guard import verify_ca_bundle_with_fallback
    ...
    from pcbdraft.agent.agent_runtime_helpers import sync_credential_pool_entry_id
    ...
    # A multiplexed gateway may enter a different PCBDRAFT_RUNTIME_HOME after
    ...
    try:
        from pcbdraft.tools.registry import registry as _snapshot_registry
    ...
    # worker (kanban_show tool is present iff PCBDRAFT_RUNTIME_KANBAN_TASK is set).
    # Resolving the ~835-token block once here avoids re-running the
    # membership test + reference on every system-prompt rebuild
    # (init + each context compression).
    from pcbdraft.agent.prompt_builder import KANBAN_GUIDANCE
    ...
    try:
        from pcbdraft.model.configuration import load_config_readonly as _load_sess_cfg
    ...
    from pcbdraft.tools.checkpoint_manager import CheckpointManager
    ...
    try:
        from pcbdraft.tools.approval import _YOLO_MODE_FROZEN
    ...
    try:
        from pcbdraft.model.configuration import (
            load_config_readonly as _load_agent_config,
        )
    ...
    if not skip_memory or _memory_toolset_requested:
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                from pcbdraft.tools.memory_tool import MemoryStore
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                from pcbdraft.agent.memory_manager import (
    ...
    from pcbdraft.agent.empty_response_guard import resolve_guard_settings
    ...
    if agent._environment_probe:
        try:
            from pcbdraft.tools.env_probe import warm_environment_probe_async
    ...
    from pcbdraft.core.runtime_utils import is_truthy_value as _is_truthy
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from pcbdraft.model.auth import (
                    resolve_provider as resolve_auth_provider,
                )
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from pcbdraft.model.configuration import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        try:
            from pcbdraft.agent.extensions.bundled.context_engine import (
                load_context_engine,
            )
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from pcbdraft.agent.extensions.manager import get_plugin_context_engine
    ...
    if (
        ...
        raise ValueError(
            ...
            f"by PCBDraft.  Choose a model with at least "
    ...
    if (
        ...
        from pcbdraft.agent.memory_manager import (
            normalize_tool_schema as _normalize_tool_schema,
        )
