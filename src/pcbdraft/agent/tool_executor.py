...
from pcbdraft.agent.display import build_tool_preview as _build_tool_preview
from pcbdraft.agent.display import get_cute_tool_message as _get_cute_tool_message_impl
from pcbdraft.agent.display import get_tool_emoji as _get_tool_emoji
from pcbdraft.agent.display import (
...
from pcbdraft.tools.terminal_tool import get_active_env
from pcbdraft.tools.thread_context import propagate_context_to_thread
from pcbdraft.tools.tool_result_storage import (
    enforce_turn_budget,
...
def _ensure_file_checkpoint(
    ...
    from pcbdraft.tools.file_tools import _resolve_path_for_task
...
def _authorization_gate_lock_timeout() -> float:
    ...
    """
    try:
        from pcbdraft.tools.approval import human_wait_ceiling
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``PCBDRAFT_RUNTIME_CONCURRENT_TOOL_TIMEOUT_S`` env var
    remains the back-compat bridge, and ``0``/negative still disables the bound.
    """
    from pcbdraft.agent.deadline import resolve_timeout

    return resolve_timeout(
        "tools.concurrent_batch",
        default=_DEFAULT_CONCURRENT_TOOL_TIMEOUT_S,
        env_var="PCBDRAFT_RUNTIME_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
def _emit_terminal_post_tool_call(
    ...
    *,
    ...
    try:
        from pcbdraft.tools.dispatch import _emit_post_tool_call_hook
...
def _tool_search_scoped_names(agent) -> frozenset:
    ...
    """
    try:
        import pcbdraft.tools.dispatch as model_tools
        from pcbdraft.tools import tool_search as _ts
...
class _ConcurrentToolAuthorizationGate:
    ...
    def __init__(
        ...
        *,
        ...
        self._lock_timeout = (
            _authorization_gate_lock_timeout() if lock_timeout is None else lock_timeout
        )
    ...
    def _human_wait_seconds(self) -> float:
        try:
            from pcbdraft.tools.approval import human_wait_seconds
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    from pcbdraft.agent import relay_tools
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``PCBDRAFT_RUNTIME_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
...
def _begin_tool_execution(
    ...
    *,
    ...
    try:
        from pcbdraft.tools.environments.base import set_activity_callback
...
def execute_tool_calls_concurrent(
    ...
    *,
    ...
    for i, (tc, name, args, middleware_trace, block_result, _scope_block) in enumerate(
        parsed_calls
    ):
    ...
    for i, (tc, name, args, middleware_trace, _parse_error, _scope_block) in enumerate(
        ...
        agent._touch_activity(
            f"tool completed: {name} ({tool_duration:.1f}s){_status_suffix}"
        )
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo":
            ...
            def _execute(next_args: dict) -> Any:
                from pcbdraft.tools.todo_tool import todo_tool as _todo_tool
            ...
            def _execute(next_args: dict) -> Any:
                ...
                from pcbdraft.tools.session_search_tool import (
            ...
            def _execute(next_args: dict) -> Any:
                ...
                from pcbdraft.tools.memory_tool import memory_tool as _memory_tool
            ...
            def _execute(next_args: dict) -> Any:
                from pcbdraft.tools.clarify_tool import clarify_tool as _clarify_tool
            ...
            def _execute(next_args: dict) -> Any:
                from pcbdraft.tools.read_terminal_tool import (
            ...
            def _execute(next_args: dict) -> Any:
                from pcbdraft.tools.read_preview_tool import (
                    read_preview_tool as _read_preview_tool,
                )
            ...
            if agent._should_emit_quiet_tool_messages():
                agent._vprint(
                    f"  {_get_cute_tool_message_impl('read_preview', function_args, tool_duration, result=function_result)}"
                )
        elif function_name == "read_window_below":

            def _execute(next_args: dict) -> Any:
                from pcbdraft.tools.read_window_tool import (
            ...
            if agent._should_emit_quiet_tool_messages():
                agent._vprint(
                    f"  {_get_cute_tool_message_impl('read_window_below', function_args, tool_duration, result=function_result)}"
                )
        elif function_name == "setup_mcp":

            def _execute(next_args: dict) -> Any:
                from pcbdraft.tools.setup_mcp_tool import (
                    setup_mcp_tool as _setup_mcp_tool,
                )
            ...
            if agent._should_emit_quiet_tool_messages():
                agent._vprint(
                    f"  {_get_cute_tool_message_impl('setup_mcp', function_args, tool_duration, result=function_result)}"
                )
            ...
            try:
                ...
                def _execute(next_args: dict) -> Any:
                    from pcbdraft.tools.dispatch import suppress_post_tool_call_hook
        ...
        _executor_must_emit_post_hook = (
            not _execution_blocked and not _execution_timed_out
        )
        ...
        agent._touch_activity(
            f"tool completed: {function_name} ({tool_duration:.1f}s){_status_suffix}"
        )
    ...
    *k+1..n* without executing them while preserving one result per
    ...
    if segments is None:
        ...
        segments = _plan_tool_batch_segments(
            assistant_message.tool_calls, execution_cwd=_exec_cwd
        )
