...
Follow the PCBDraft skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `PCBDraft`. NEVER fill it from the host
...
a privacy leak the user never opted into; the skill names itself as PCBDraft.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.pcbdraft.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through PCBDraft tools.
...
PCBDraft-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference PCBDraft tools by name in backticks: `terminal`, `read_file`,
