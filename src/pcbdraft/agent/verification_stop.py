...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``PCBDRAFT_RUNTIME_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("PCBDRAFT_RUNTIME_VERIFY_ON_STOP")
    if env is not None:
...
def _verification_snapshot(
    *,
    ...
    try:
        ...
        from pcbdraft.agent.verification_evidence import verification_status
    except Exception:
        return None
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    ...
    try:
        ...
        from pcbdraft.agent.verify.environment import manifest_path

        if manifest_path(root_path).is_file():
            return True
        from pcbdraft.agent.verify.recipes import detect_recipe
...
def build_verify_on_stop_nudge(
    *,
    ...
    try:
        from pcbdraft.agent.verify_hooks import coding_verify_guidance
