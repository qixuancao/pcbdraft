...
``hermes pause`` writes a sentinel file at ``$PCBDRAFT_RUNTIME_HOME/ESTOP``;
...
def _runtime_home() -> Path:
    """Resolve the active PCBDRAFT_RUNTIME_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active PCBDRAFT_RUNTIME_HOME."""
    return _runtime_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on PCBDRAFT_RUNTIME_HOME), report
