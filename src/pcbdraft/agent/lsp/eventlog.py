...
Grep recipe::
    ...
    tail -f "$PCBDRAFT_RUNTIME_HOME/logs/agent.log" | rg 'lsp\\['
"""
...
event_log = logging.getLogger("pcbdraft.lint.lsp")
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install the language server or set lsp.servers.<id>.command)",
        )
    else:
