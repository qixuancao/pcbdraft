...
from pcbdraft.agent.lsp import eventlog
from pcbdraft.agent.lsp.client import DIAGNOSTICS_DOCUMENT_WAIT, LSPClient
from pcbdraft.agent.lsp.servers import (
    ServerContext,
    find_server_for_file,
    language_id_for,
)
from pcbdraft.agent.lsp.workspace import clear_cache, resolve_workspace_for_file
...
class LSPService:
        ...
        if delta:
            ...
            if baseline:
                if line_shift is not None:
                    ...
                    from pcbdraft.agent.lsp.range_shift import shift_baseline
    ...
    async def _open_and_wait_async(self, file_path: str) -> list[dict[str, Any]] | None:
