...
``<PCBDRAFT_RUNTIME_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
# ``<PCBDRAFT_RUNTIME_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def _existing_binary(name: str) -> str | None:
    ...
    for staged in _native_binary_candidates(pcbdraft_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $PCBDRAFT_RUNTIME_HOME/node is not on an arbitrary process's
    ...
    if npm is None:
        ...
        return None
    staging = pcbdraft_lsp_bin_dir().parent  # <PCBDRAFT_RUNTIME_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = pcbdraft_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> str | None:
    ...
    console script.
    """
    pip_target = pcbdraft_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = pcbdraft_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
