...
* The ``bws`` binary is auto-installed into ``<runtime_home>/bin/bws`` on
...
def _encrypted_disk_cache_path(home_path: Path | None = None) -> Path:
    """Return the encrypted disk cache path under runtime_home/cache/."""
    from pcbdraft.agent.secret_sources._cache import resolve_cache_home
...
def find_bws(*, install_if_missing: bool = False) -> Path | None:
    ...
    Resolution order:
      1. ``<runtime_home>/bin/bws``  (our managed copy — preferred)
...
def _read_encrypted_disk_cache(
    *,
    ...
    try:
        ...
        key = _derive_encrypted_cache_key(access_token, salt)
        raw = AESGCM(key).decrypt(nonce, ciphertext, serialized_key.encode("utf-8"))
