...
def get_source(name: str, *, scope: str | None = None) -> SecretSource | None:
    ...
    with _REGISTRY_LOCK:
        return _SCOPED_SOURCES.get(scope or runtime_home_key(), {}).get(
            name
...
def list_sources(*, scope: str | None = None) -> list[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(scope or runtime_home_key(), {}))
...
def list_plugin_sources() -> list[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(runtime_home_key(), {}))
        return list(merged.values())
...
def _ordered_enabled_sources(
    secrets_cfg: dict, *, scope: str | None = None
) -> list[SecretSource]:
...
def _active_profile_name(home_path: Path | None) -> str:
    ...
    for env_name in ("PCBDRAFT_RUNTIME_PROFILE_NAME", "PCBDRAFT_RUNTIME_PROFILE"):
...
def apply_all(
    secrets_cfg: dict, home_path: Path, environ: MutableMapping[str, str] | None = None
) -> ApplyReport:
    ...
    enabled = _ordered_enabled_sources(secrets_cfg, scope=runtime_home_key(home_path))
    if not enabled:
        return report

    preserve_raw = secrets_cfg.get("preserve_existing")
    preserve: frozenset = (
        frozenset(n.strip() for n in preserve_raw if isinstance(n, str) and n.strip())
