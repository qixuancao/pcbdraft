...
are cached in-process and on disk under ``<runtime_home>/cache/op_cache.json``
...
# In-process cache.  The key folds in str(home_path) so a PCBDRAFT_RUNTIME_HOME switch
...
def find_op(binary_path: str = "") -> Path | None:
