...
* The requested key is passed to the child ONLY via the ``PCBDRAFT_RUNTIME_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``PCBDRAFT_RUNTIME_SECRET_KEY``) — it is never called per-key in a loop, so a
...
from pcbdraft.agent.secret_sources.bitwarden import FetchResult
...
def _run_helper(
    ...
    The key is passed as DATA via ``PCBDRAFT_RUNTIME_SECRET_KEY`` — never interpolated
...
def get_command_secret(
    *,
    ...
    ``PCBDRAFT_RUNTIME_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # PCBDRAFT_RUNTIME_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
...
class CommandSource(SecretSource):
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        try:
            timeout = float(cfg.get("helper_timeout_seconds", _COMMAND_TIMEOUT_SECONDS))
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "PCBDraft discards helper stderr so diagnostics can't leak "
                "secret material."
            )
