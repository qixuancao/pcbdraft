...
* First-use consent is gated by the allowlist under
  ``<PCBDRAFT_RUNTIME_HOME>/shell-hooks-allowlist.json``. Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``PCBDRAFT_RUNTIME_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
...
def register_from_config(
    ...
    *,
    ...
    setting.  ``PCBDRAFT_RUNTIME_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / PCBDRAFT_RUNTIME_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("PCBDRAFT_RUNTIME_SAFE_MODE"):
        logger.info("PCBDRAFT_RUNTIME_SAFE_MODE=1 — shell-hook registration skipped")
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> list[ShellHookSpec]:
    ...
    """
    from pcbdraft.agent.extensions.manager import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _spawn(spec: ShellHookSpec, stdin_json: str) -> dict[str, Any]:
    ...
    try:
        ...
        from pcbdraft.interfaces.tui._subprocess_compat import split_command_line
...
def allowlist_path() -> Path:
    ...
    return get_runtime_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / PCBDRAFT_RUNTIME_ACCEPT_HOOKS).",
            p,
            exc,
        )
...
def _prompt_and_record(
    ...
    *,
    ...
    print(
        f"\n⚠ PCBDraft is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``PCBDRAFT_RUNTIME_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("PCBDRAFT_RUNTIME_ACCEPT_HOOKS", "").strip().lower()
