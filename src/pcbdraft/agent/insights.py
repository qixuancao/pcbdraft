...
class InsightsEngine:
    ...
    def _compute_overview(
        ...
        if models:
            ...
            total_cache_write = sum(
                int(m.get("cache_write_tokens") or 0) for m in models
            )
            total_tokens = (
    ...
    def _compute_model_breakdown(
        ...
        for s in sessions:
            ...
            residual_cost = max(
                0.0,
                float(s.get("estimated_cost_usd") or 0.0)
                - totals["estimated_cost_usd"],
            )
            residual_actual = max(
                0.0,
                float(s.get("actual_cost_usd") or 0.0) - totals["actual_cost_usd"],
            )
            residual_calls = max(
