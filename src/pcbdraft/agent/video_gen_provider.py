...
as ``kind: backend``) or ``<PCBDRAFT_RUNTIME_HOME>/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _videos_cache_dir() -> Path:
    """Return ``$PCBDRAFT_RUNTIME_HOME/cache/videos/``, creating parents as needed."""
    from pcbdraft.core.runtime_environment import get_runtime_home

    path = get_runtime_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$PCBDRAFT_RUNTIME_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$PCBDRAFT_RUNTIME_HOME/cache/videos/``.
