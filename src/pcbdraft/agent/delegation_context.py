...
PCBDRAFT_RUNTIME_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's PCBDRAFT_RUNTIME_KANBAN_* vars are legitimately in os.environ (cron
...
DELEGATED_CHILD_ENV_MARKER = "PCBDRAFT_RUNTIME_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "PCBDRAFT_RUNTIME_KANBAN_TASK",
    "PCBDRAFT_RUNTIME_KANBAN_RUN_ID",
    "PCBDRAFT_RUNTIME_KANBAN_WORKSPACE",
    "PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT",
    "PCBDRAFT_RUNTIME_KANBAN_CLAIM_LOCK",
    "PCBDRAFT_RUNTIME_KANBAN_BOARD",
    "PCBDRAFT_RUNTIME_KANBAN_DB",
)
...
def delegated_child_context(session_id: str | None = None) -> Iterator[None]:
    ...
    try:
        ...
        from pcbdraft.agent.session_context import scoped_current_session_id
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``PCBDRAFT_RUNTIME_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$PCBDRAFT_RUNTIME_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``PCBDRAFT_RUNTIME_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``PCBDRAFT_RUNTIME_KANBAN_*`` vars while losing the ContextVar in
