...
def is_native_compaction_model(model: str | None) -> bool:
...
def resolve_compact_threshold(
    ...
    local = None
    try:
        if local_trigger_tokens is not None and not isinstance(
            local_trigger_tokens, bool
        ):
...
def _user_item_text(item: dict[str, Any]) -> str | None:
