...
def _runtime_home_path() -> Path:
    """Resolve the active PCBDRAFT_RUNTIME_HOME (profile-aware) without circular imports."""
    try:
        from pcbdraft.core.runtime_environment import (
            get_runtime_home,  # local import to avoid cycles
        )
...
def _runtime_root_path() -> Path:
    ...
    try:
        from pcbdraft.core.runtime_environment import (
            get_default_runtime_root,  # local import to avoid cycles
        )
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    runtime_home = _runtime_home_path()
    ...
    return {
        ...
        for p in [
            ...
            str(runtime_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(runtime_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(runtime_home / "cache" / "bws_cache.enc.json"),
            str(runtime_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved PCBDRAFT_RUNTIME_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("PCBDRAFT_RUNTIME_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> str | None:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> str | None:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside PCBDRAFT_RUNTIME_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> str | None:
    ...
    Three categories are blocked:
      ...
      * Internal Hermes cache files under ``PCBDRAFT_RUNTIME_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under PCBDRAFT_RUNTIME_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Resolve BOTH the active PCBDRAFT_RUNTIME_HOME (profile-aware) AND the global
...
#
# Hermes profiles are separate PCBDRAFT_RUNTIME_HOME dirs under
...
# Profile-scoped directories under PCBDRAFT_RUNTIME_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    ...
    """
    try:
        home_real = _runtime_home_path().resolve()
        root_real = _runtime_root_path().resolve()
...
def classify_sandbox_mirror_target(path: str) -> dict | None:
    ...
    PCBDRAFT_RUNTIME_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host PCBDRAFT_RUNTIME_HOME)
    """
