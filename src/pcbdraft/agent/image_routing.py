...
def _resolve_inference_base_url(
    ...
    try:
        from pcbdraft.model.auxiliary_client import _runtime_main_value
...
def _should_probe_ollama_vision(provider: str, base_url: str) -> bool:
    ...
    if not base_url:
        return False
    try:
        from pcbdraft.model.model_metadata import detect_local_server_type
    ...
    *,
    ...
    if not requested_provider:
        try:
            ...
            runtime_provider = (
                str(_runtime_main_value("provider") or "").strip().lower()
            )
    ...
    if _should_probe_ollama_vision(provider, base_url):
        try:
            from pcbdraft.model.model_metadata import query_ollama_supports_vision
