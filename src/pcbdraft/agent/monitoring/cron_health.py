...
def classify_cron_error(raw: Any) -> str:
    ...
    if (
        re.search(
            r"\b(?:authentication|authenticated|authenticate|authorization|authorized|authorize|unauthorized|forbidden)\b",
            text,
        )
    ...
    if any(
        value in text
        for value in ("network", "connection", "dns", "socket", "unreachable")
    ):
...
def _duration_ms(record: dict[str, Any]) -> int | None:
    start = _parse_time(record.get("started_at")) or _parse_time(
...
def project_execution_event(
    record: dict[str, Any], *, delivery_outcome: str | None = None
) -> CronExecutionEvent:
