...
def classify_gateway_error(raw: Any) -> str:
    s = str(raw or "").lower()
    if any(
        k in s for k in ("auth", "token", "unauthorized", "forbidden", "401", "403")
    ):
        return "auth_failed"
...
def _derive_busy(gateway_running: bool, gateway_state: Any, active_agents: Any) -> bool:
    try:
        from pcbdraft.core.runtime_process import derive_gateway_busy
        ...
        return bool(
            ...
            and _parse_active_agents(active_agents) > 0
        )


def _derive_drainable(gateway_running: bool, gateway_state: Any) -> bool:
    try:
        from pcbdraft.core.runtime_process import derive_gateway_drainable

        return derive_gateway_drainable(
...
def _base_attrs(
    *, profile: str, install_id: str, version: str, supervision_mode: str
) -> dict[str, str]:
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    platforms = (
        runtime.get("platforms") if isinstance(runtime.get("platforms"), dict) else {}
    )
    base = _base_attrs(
        profile=profile,
        ...
        if is_degraded:
            events.append(
                GatewayDiagnosticEvent(
                    ...
                    error_class=classify_gateway_error(
                        error_code or pdata.get("error_message")
                    ),
                    profile=profile,
                    version=version,
...
def emit_runtime_status_transition(
    ...
    try:
        ...
        old_gateway_state = (
            ...
            if (previous or {}).get("gateway_state") is not None
            else None
        )
        new_gateway_state = (
            _bounded_state(current.get("gateway_state"), allowed=_KNOWN_GATEWAY_STATES)
            if current.get("gateway_state") is not None
            else None
        )
            ...
            if new_gateway_state == "startup_failed":
                out.append(
                    GatewayDiagnosticEvent(
                        ...
                        subsystem="gateway",
                        error_class=classify_gateway_error(
                            current.get("exit_reason") or "startup_failed"
                        ),
                        error_code=classify_gateway_error(
                            current.get("exit_reason") or "startup_failed"
                        ),
                        profile=profile,
                        version=version,
        ...
        for platform, pdata in new_platforms.items():
            ...
            old_state = (
                _bounded_state(prev.get("state"), allowed=_KNOWN_PLATFORM_STATES)
                if prev.get("state") is not None
                else None
            )
            new_state = (
                _bounded_state(pdata.get("state"), allowed=_KNOWN_PLATFORM_STATES)
                if pdata.get("state") is not None
                else None
            )
            if old_state == new_state or not new_state:
                continue
            error_code = classify_gateway_error(
                pdata.get("error_code") or pdata.get("error_message")
            )
            severity = (
                "error"
                if new_state.lower() in {"fatal", "failed", "error"}
                else "warning"
            )
            out.append(
                GatewayDiagnosticEvent(
        ...
        logging.getLogger(__name__).debug(
            "gateway runtime status transition emit failed", exc_info=True
        )
...
class GatewayDiagnosticLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            logging.getLogger(__name__).debug(
                "gateway diagnostic emit failed", exc_info=True
            )
