...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``PCBDRAFT_RUNTIME_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("PCBDRAFT_RUNTIME_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> dict[str, str]:
    ...
    try:
        ...
        # the PCBDRAFT_RUNTIME_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from pcbdraft.agent.extensions.manager import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> list[str]:
    from pcbdraft.agent.extensions.manager import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Path | None:
    ...
    1. A bound PCBDRAFT_RUNTIME_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_runtime_root` (NOT ``get_runtime_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_runtime_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(
    ...
    # Some execution modes (cron) still want PCBDRAFT_RUNTIME_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the PCBDRAFT_RUNTIME_HOME ContextVar
    ...
    # PCBDRAFT_RUNTIME_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if "computer_use" in agent.valid_tool_names:
        from pcbdraft.agent.prompt_builder import computer_use_guidance
    ...
    if has_skills_tools:
        ...
        try:
            from pcbdraft.agent.coding_context import coding_compact_skill_categories
    ...
    if agent.valid_tool_names:
        try:
            from pcbdraft.agent.coding_context import coding_system_prompt_parts
    ...
    if getattr(agent, "_environment_probe", True):
        try:
            from pcbdraft.tools.env_probe import get_environment_probe_line
    ...
    # ambient PCBDRAFT_RUNTIME_HOME: on a build thread that lost the ContextVar this
    ...
    try:
        if _agent_home_path is not None:
            ...
            from pcbdraft.agent.file_safety import _resolve_active_profile_name
    ...
    # from get_default_runtime_root(): get_runtime_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_runtime_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_runtime_root())
    else:
    ...
    if active_profile == "default":
        ...
        # data sits at the ROOT (get_default_runtime_root()), which in ambient
        # profile mode is NOT get_runtime_home().
