...
# `export PCBDRAFT_RUNTIME_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in hermes_cli/main.py, gateway/run.py, and
# cli.py) or `PCBDRAFT_RUNTIME_REDACT_SECRETS=false` in the runtime .env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("PCBDRAFT_RUNTIME_REDACT_SECRETS", "true").lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def _mask_control_split_tokens(text: str, mask_fn) -> str:
    ...
    for m in _PREFIX_RE.finditer(stripped):
        ...
        if all(c in _TOKEN_BODY_CHARS or _CONTROL_CHARS_RE.match(c) for c in span) and (
            end_orig >= len(text) or text[end_orig] != "="
        ):
...
# (``security.redact_secrets: false`` / PCBDRAFT_RUNTIME_REDACT_SECRETS) applies to
