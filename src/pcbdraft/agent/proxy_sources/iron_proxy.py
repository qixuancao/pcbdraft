...
* The ``iron-proxy`` binary is auto-installed into ``<runtime_home>/bin/iron-proxy``
...
* A long-lived CA at ``<runtime_home>/proxy/ca.{crt,key}`` is generated on
  first ``hermes egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<runtime_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``hermes egress start``), pidfile
  at ``<runtime_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<runtime_home>/proxy/iron-proxy.log``;
...
# ``<runtime_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "PCBDRAFT_RUNTIME_IRON_PROXY_MGMT_KEY"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Path | None:
    """Return a path to a usable ``iron-proxy`` binary, or None.

    Resolution order:
      1. ``<runtime_home>/bin/iron-proxy``  (our managed copy — preferred)
...
def _verify_checksums_signature(tmp: Path, checksum_path: Path) -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "GPG verification (SHA-256 checksum check still enforced).",
            exc,
        )
        return False
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<runtime_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``PCBDRAFT_RUNTIME_IRON_PROXY_MGMT_KEY`` env var
...
def _read_management_token() -> str | None:
...
def build_proxy_config(
    *,
    ...
    upstream_deny_cidrs: list[str] | None = None,
    http_listen: list[str] | None = None,
) -> dict:
...
def write_proxy_config(config: dict) -> Path:
    """Serialize the config dict to ``<runtime_home>/proxy/proxy.yaml``.
        ...
        try:
            out.append(
                TokenMapping(
                    ...
                    match_headers=tuple(
                        item.get("match_headers") or ("Authorization",)
                    ),
...
def discover_provider_mappings(
    *,
    available_env_names: list[str] | None = None,
) -> list[TokenMapping]:
...
def _pid_proc_starttime(pid: int) -> str | None:
...
def _read_persisted_nonce() -> str | None:
...
def start_proxy(
    *,
    ...
    bitwarden_config: dict | None = None,
) -> ProxyStatus:
...
def stop_proxy() -> bool:
    ...
    while time.time() < deadline:
        ...
        if recycled:
            logger.warning(
                "iron-proxy pid=%s appears recycled before SIGKILL; not killing.",
                pid,
            )
        else:
            try:
...
def _read_tunnel_port_from_config() -> int | None:
...
def _read_http_listen_from_config() -> tuple[str, int] | None:
