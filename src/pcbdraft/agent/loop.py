...
def _session_source_for_agent(platform: str | None) -> str:
    try:
        ...
        source = get_session_env("PCBDRAFT_RUNTIME_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("PCBDRAFT_RUNTIME_SESSION_SOURCE", "")
...
try:
    from pcbdraft.tools.browser_tool import (
        cleanup_browser,  # type: ignore[import-not-found]
    )
except Exception:  # pragma: no cover - vendored trim: browser tooling omitted

    def cleanup_browser(*_args: Any, **_kwargs: Any) -> Any:
        return None
...
from pcbdraft.agent.api_hook_observability import (
    configure_api_hook_observability_runtime as _configure_api_hook_observability_runtime,
)
...
from pcbdraft.agent.error_normalization import (
    configure_error_normalization_runtime as _configure_error_normalization_runtime,
)
...
from pcbdraft.agent.provider_credential_refresh import (
    configure_provider_credential_refresh_runtime as _configure_provider_credential_refresh_runtime,
)
...
from pcbdraft.agent.request_client_lifecycle import (
    configure_request_client_lifecycle_runtime as _configure_request_client_lifecycle_runtime,
)
...
from pcbdraft.agent.tool_result_classification import file_mutation_result_landed
from pcbdraft.agent.trajectory import convert_scratchpad_to_think
from pcbdraft.agent.trajectory import save_trajectory as _save_trajectory_to_file
from pcbdraft.agent.turn_control import TurnControlMixin
from pcbdraft.agent.turn_control import (
    configure_turn_control_runtime as _configure_turn_control_runtime,
)
...
_configure_api_hook_observability_runtime(
    normalize_usage_fn=lambda usage, **kwargs: normalize_usage(usage, **kwargs),
    env_get=lambda name, default: os.getenv(name, default),
    is_simple_namespace=lambda value: isinstance(value, SimpleNamespace),
...
_configure_request_client_lifecycle_runtime(
...
_configure_provider_credential_refresh_runtime(namespace=lambda: globals())
_configure_error_normalization_runtime(
    coerce_api_error_detail=lambda value: AIAgent._coerce_api_error_detail(value),
    decorate_xai_entitlement_error=(
        lambda detail: AIAgent._decorate_xai_entitlement_error(detail)
    ),
    json_dumps=lambda value, **kwargs: json.dumps(value, **kwargs),
    json_loads=lambda value: json.loads(value),
    redact_text=lambda value: redact_sensitive_text(value),
    regex_search=lambda pattern, text, flags=0: re.search(pattern, text, flags),
)
_configure_session_persistence_runtime(
    classify_summary_content=(
        lambda content: ContextCompressor.classify_summary_content(content)
    ),
    compressed_summary_metadata_key=lambda: COMPRESSED_SUMMARY_METADATA_KEY,
    db_persisted_marker=lambda: _DB_PERSISTED_MARKER,
    is_ephemeral_scaffolding=lambda message: _is_ephemeral_scaffolding(message),
    is_multimodal_tool_result=(lambda content: _is_multimodal_tool_result(content)),
...
_configure_turn_control_runtime(
    debug=lambda message, *args, **kwargs: logger.debug(message, *args, **kwargs),
    hard_interrupt=(
        lambda agent, message: AIAgent.interrupt(
            agent,
            message,
            hard_cancel=True,
        )
    ),
    new_event=lambda: threading.Event(),
    print_notice=lambda text: print(text),
    request_child_hard_interrupt=(
        lambda child, message: request_hard_interrupt(child, message)
    ),
    set_thread_interrupt=(
        lambda interrupted, thread_id: _set_interrupt(interrupted, thread_id)
    ),
)
...
class AIAgent(
    ...
    TurnResultFormattingMixin,
    ErrorNormalizationMixin,
    ProviderCapabilitiesMixin,
    ProviderCredentialRefreshMixin,
    ResponseCleanupMixin,
    ClientLifecycleMixin,
    RequestClientLifecycleMixin,
    ActivityTrackingMixin,
    MemoryLifecycleMixin,
    ApiHookObservabilityMixin,
    ApiMessageHelpersMixin,
    ...
    def __init__(
        ...
        from pcbdraft.agent.agent_init import init_agent
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            try:
                from pcbdraft.tools.approval import is_session_yolo_enabled
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from pcbdraft.model.catalog import ensure_lmstudio_model_loaded
    ...
    def switch_model(
        ...
        from pcbdraft.agent.agent_runtime_helpers import switch_model
    ...
    def _cleanup_task_resources(self, task_id: str) -> None:
        ...
        from pcbdraft.agent.chat_completion_helpers import cleanup_task_resources
    ...
    # ------------------------------------------------------------------
    from pcbdraft.agent.background_review import (
    ...
    def _summarize_background_review_actions(
        ...
        from pcbdraft.agent.background_review import summarize_background_review_actions
    ...
    def _spawn_background_review(
        ...
        task_cfg = None
        if focus is None:
            from pcbdraft.agent.background_review import load_background_review_settings

            enabled, task_cfg = load_background_review_settings()
            if not enabled:
                return
        from pcbdraft.agent.background_review import spawn_background_review_thread
    ...
    def _convert_to_trajectory_format(
        ...
        from pcbdraft.agent.agent_runtime_helpers import convert_to_trajectory_format
    ...
    def _save_session_log(self, messages: list[dict[str, Any]] | None = None):
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects PCBDRAFT_RUNTIME_REDACT_SECRETS via
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``PCBDRAFT_RUNTIME_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            ...
            env = _os.environ.get("PCBDRAFT_RUNTIME_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``PCBDRAFT_RUNTIME_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            ...
            env = _os.environ.get("PCBDRAFT_RUNTIME_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _hydrate_todo_store(self, history: list[dict[str, Any]]) -> None:
        ...
        (GHSA-5g4g-6jrg-mw3g).
        """
        from pcbdraft.tools.todo_tool import MAX_TODO_RESULT_CHARS
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``PCBDRAFT_RUNTIME_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _create_openai_client(
        ...
        from pcbdraft.agent.agent_runtime_helpers import create_openai_client
    ...
    def _force_close_tcp_sockets(client: Any) -> int:
        ...
        from pcbdraft.agent.agent_runtime_helpers import force_close_tcp_sockets
    ...
    def _build_primary_client_for_active_provider(self, *, reason: str) -> Any:
        ...
        if (getattr(self, "provider", "") or "").strip().lower() == "moa":
            from pcbdraft.agent.moa_loop import build_moa_facade
    ...
    def _cleanup_dead_connections(self) -> bool:
        ...
        from pcbdraft.agent.agent_runtime_helpers import cleanup_dead_connections
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        from pcbdraft.model.codex_runtime import run_codex_create_stream_fallback
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        from pcbdraft.model.anthropic_adapter import create_anthropic_message
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        from pcbdraft.agent.chat_completion_helpers import interruptible_api_call
    ...
    def _has_stream_consumers(self) -> bool:
        ...
        try:
            from pcbdraft.agent.plugin_stream_hooks import has_stream_observer_hooks
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        from pcbdraft.agent.chat_completion_helpers import try_activate_fallback
    ...
    def _restore_primary_runtime(self) -> bool:
        ...
        from pcbdraft.agent.agent_runtime_helpers import restore_primary_runtime
    ...
    def _try_recover_primary_transport(
        ...
        *,
        ...
        from pcbdraft.agent.agent_runtime_helpers import try_recover_primary_transport
    ...
    def _build_api_kwargs(
        self, api_messages: list, tools_for_api: list | None = None
    ) -> dict:
        """Forwarder — see ``agent.chat_completion_helpers.build_api_kwargs``."""
        from pcbdraft.agent.chat_completion_helpers import build_api_kwargs
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if base_url_host_matches(
            ...
            try:
                from pcbdraft.model.catalog import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from pcbdraft.model.catalog import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from pcbdraft.model.catalog import ollama_model_supports_thinking
    ...
    def _resolve_lmstudio_summary_reasoning_effort(self) -> str | None:
        ...
        """
        from pcbdraft.model.lmstudio_reasoning import resolve_lmstudio_effort
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        from pcbdraft.agent.chat_completion_helpers import build_assistant_message
    ...
    def _needs_kimi_tool_reasoning(self) -> bool:
        ...
        """
        from pcbdraft.agent.message_sanitization import matches_reasoning_echo_family
    ...
    def _copy_reasoning_content_for_api(self, source_msg: dict, api_msg: dict) -> None:
        ...
        from pcbdraft.agent.agent_runtime_helpers import copy_reasoning_content_for_api
    ...
    def _sanitize_tool_calls_for_strict_api(
        ...
        if not isinstance(tool_calls, list):
            return api_msg
        from pcbdraft.model.transports.chat_completions import (
    ...
    def _sanitize_tool_call_arguments(
        ...
        *,
        ...
        from pcbdraft.agent.agent_runtime_helpers import sanitize_tool_call_arguments
    ...
    def _compress_context(
        ...
        *,
        ...
        from pcbdraft.agent.portal_tags import (
        ...
        try:
            ...
            try:
                from pcbdraft.core.runtime_logging import set_session_context
            ...
            # PCBDRAFT_RUNTIME_SESSION_ID to the child id after an out-of-place
    ...
    def _execute_tool_calls(
        ...
        try:
            ...
            from pcbdraft.agent.tool_dispatch_helpers import _plan_tool_batch_segments
            ...
            from pcbdraft.agent.tool_executor import execute_tool_calls_segmented
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        from pcbdraft.tools.delegate_tool import _strip_model_hidden_task_fields
        from pcbdraft.tools.delegate_tool import delegate_task as _delegate_task
    ...
    def _execute_tool_calls_concurrent(
        ...
        from pcbdraft.agent.tool_executor import execute_tool_calls_concurrent
    ...
    def _execute_tool_calls_sequential(
        ...
        from pcbdraft.agent.tool_executor import execute_tool_calls_sequential
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        from pcbdraft.agent.chat_completion_helpers import handle_max_iterations
    ...
    def run_conversation(
        ...
        from pcbdraft.agent import relay_runtime
        from pcbdraft.agent.conversation_loop import run_conversation
        from pcbdraft.agent.portal_tags import (
            reset_conversation_context,
            set_conversation_context,
        )
        from pcbdraft.agent.subagent_lifecycle import bind_subagent_parent
        from pcbdraft.interfaces.tui.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        from pcbdraft.model.aux_accounting import (
        ...
        try:
            ...
            if (
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_message = getattr(self, "_interrupt_message", None)
                        if interrupt_message:
                            interrupt_result["interrupt_message"] = interrupt_message
            ...
            from pcbdraft.model.auxiliary_client import scoped_runtime_main
    ...
    def chat(
        self, message: str, stream_callback: Callable[..., Any] | None = None
    ) -> str:
        """
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        from pcbdraft.model.codex_runtime import run_codex_app_server_turn
...
def main(
    ...
    # Handle tool listing
    if list_tools:
        from pcbdraft.tools.dispatch import get_all_tool_names, get_available_toolsets
        from pcbdraft.tools.toolsets import get_all_toolsets, get_toolset_info
