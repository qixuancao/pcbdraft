...
def canonical_output_parent(value: str | None) -> Path:
    ...
    try:
        ...
        raise PCBDraftError(f"cannot prepare output parent: {parent}") from exc
    if not parent.is_dir():
        raise PCBDraftError(f"output parent is not a directory: {parent}")
    return parent
...
def create_run(output_parent: str | None) -> tuple[str, Path]:
    ...
    for _ in range(8):
        ...
        try:
            ...
            raise PCBDraftError(f"cannot create run directory under: {parent}") from exc
    raise PCBDraftError("could not allocate a unique run id")
...
def ensure_output_outside_project(project: Path, output_parent: str | None) -> None:
    ...
    try:
        ...
        raise PCBDraftError(f"cannot resolve output parent: {raw}") from exc
    if candidate == project or candidate.is_relative_to(project):
        raise PCBDraftError("output parent must be outside the source project")
