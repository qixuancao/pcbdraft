...
from pcbdraft.core.io import privatize_tree
...
def validate_agent_tree(root: Path) -> None:
    ...
    for member in _iter_members(root, max_members=TREE_MEMBER_LIMIT):
        ...
        try:
            ...
            raise PCBDraftError(f"cannot inspect project member: {relative}") from exc
...
def sha256_file(path: Path, *, max_bytes: int | None = None) -> str:
    ...
    try:
        ...
        raise PCBDraftError(f"cannot hash project file: {path}") from exc
...
def copy_project(source: Path, destination: Path) -> None:
    if destination.exists():
        raise PCBDraftError(f"staging destination already exists: {destination}")
    try:
        shutil.copytree(source, destination, symlinks=True, copy_function=shutil.copy2)
        privatize_tree(destination)
    except OSError as exc:
        raise PCBDraftError("failed to create private staging copy") from exc
