"""Shared utility functions for PCBDraft."""
...
def atomic_replace(tmp_path: str | Path, target: str | Path) -> str:
    ...
    their runtime directory to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
    ...
    * A Windows rename contended by another open handle (winerror 5/32/33).
    ...
    try:
        ...
        if contended:
            ...
            from pcbdraft.agent.retry_utils import jittered_backoff
...
def warn_if_credential_file_broadly_readable(
    ...
    *,
    ...
    Secret-bearing files that users create by hand (or that older runtime
...
def atomic_roundtrip_yaml_save(
    ...
    * Non-dict values (lists, scalars) are overwritten wholesale — list
    ...
    Shares the fail-closed contract ``pcbdraft.model.configuration.atomic_config_write``
    ...
    ``pcbdraft.model.configuration`` itself imports from this module.
    """
    from ruamel.yaml import YAML
    from ruamel.yaml.comments import CommentedMap
    from ruamel.yaml.scalarstring import DoubleQuotedScalarString

    from pcbdraft.model.configuration import require_readable_config_before_write
