...
def _runtime_home_from_env() -> Path:
    """Resolve PCBDRAFT_RUNTIME_HOME from the process environment only.

    Reads the ``PCBDRAFT_RUNTIME_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_runtime_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_runtime_home`
    and :func:`get_process_runtime_home` so the two never drift.
    """
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``PCBDRAFT_RUNTIME_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_runtime_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[PCBDRAFT_RUNTIME_HOME fallback] PCBDRAFT_RUNTIME_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass PCBDRAFT_RUNTIME_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def runtime_home_key(path: str | Path | None = None) -> str:
    ...
    """
    candidate = Path(path) if path is not None else get_runtime_home()
...
def get_process_runtime_home() -> Path:
    ...
    process ``PCBDRAFT_RUNTIME_HOME`` env var (falling back to the platform default),
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``PCBDRAFT_RUNTIME_OPTIONAL_SKILLS``.
    """
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``PCBDRAFT_RUNTIME_OPTIONAL_MCPS``.
    """
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``PCBDRAFT_RUNTIME_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<PCBDRAFT_RUNTIME_HOME>/skills`` last-resort
    """
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%PCBDRAFT_RUNTIME_HOME%\\node`` on Windows.
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$PCBDRAFT_RUNTIME_HOME/node`` on POSIX.
    ...
    ``PCBDRAFT_RUNTIME_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
    ...
    try:
        result = subprocess.run(
            ...
            env={
                **os.environ,
                "PCBDRAFT_RUNTIME_HOME": str(get_runtime_home()),
                ...
                "PCBDRAFT_RUNTIME_NODE_SKIP_LINKS": "1",
            },
            capture_output=True,
            timeout=600,
            check=False,
...
def display_runtime_home() -> str:
    """Return a user-friendly display string for the current PCBDRAFT_RUNTIME_HOME.
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``PCBDRAFT_RUNTIME_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{PCBDRAFT_RUNTIME_HOME}/home`` when the profile-home directory exists."""
    runtime_home = (
        get_runtime_home_override()
        or (env or {}).get("PCBDRAFT_RUNTIME_HOME")
        or os.getenv("PCBDRAFT_RUNTIME_HOME")
    )
    if not runtime_home:
        return None
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    candidates: list[str] = []
    explicit = str(
        env.get("PCBDRAFT_RUNTIME_REAL_HOME")
        or os.getenv("PCBDRAFT_RUNTIME_REAL_HOME", "")
    ).strip()
    if explicit:
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{PCBDRAFT_RUNTIME_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{PCBDRAFT_RUNTIME_HOME}/home`` when it exists, preserving the
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under PCBDRAFT_RUNTIME_HOME."""
    return get_runtime_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under PCBDRAFT_RUNTIME_HOME."""
