...
from .managed import ManagedProject, load_generation_request, open_managed_project
...
def _build_checks(
    ...
    if isinstance(generation_request, AgentDesignRequest):
        ...
        if qualification is None:
            checks.append(
                CheckResult(
                    "l1.component_qualification",
                    "L1",
                    "unavailable",
                    "unknown",
                    "Generation-time component qualification evidence is not present in this older generic project.",
                    (),
                    None,
                    False,
                    True,
            ...
            checks.append(
                CheckResult(
                    "l1.footprint_pad_qualification",
                    "L1",
                    "completed",
                    "fail" if mapping_failures else "pass",
                    "Selected symbol pins map to actual pad numbers in every local footprint."
                    if not mapping_failures
                    else "One or more generation-time symbol-to-footprint pad mappings failed.",
                    (project.qualification_path.name,)
                    if project.qualification_path is not None
                    else (),
                    {
                        "failed_references": list(mapping_failures),
                        "report_sha256": qualification.sha256(),
                    },
                    True,
                    True,
                )
            ...
            checks.append(
                CheckResult(
                    "l1.component_evidence_qualification",
                    "L1",
                    "human_required" if provisional else "completed",
                    "unknown" if provisional else "pass",
                    "Local-library component records still require exact part, datasheet, ratings, lifecycle, and package review."
                    if provisional
                    else "Selected component records have at least rule-attributed evidence; physical and production qualification remain separate gates.",
                    (project.qualification_path.name,)
                    if project.qualification_path is not None
                    else (),
                    {"provisional_references": list(provisional)},
                    False,
                    True,
                )
            )
    if isinstance(generation_request, AgentDesignRequest) and project.plan is not None:
        preflight = review_agent_plan(
            generation_request,
            project.plan,
            project.design,
            graph,
        ...
        deterministic_failures = tuple(
            finding for finding in preflight.findings if finding.outcome == "fail"
        )
        checks.append(
            CheckResult(
                "l1.agent_plan_electrical_preflight",
                "L1",
                "completed",
                "fail" if deterministic_failures else "pass",
                "Deterministic generic topology and local metadata checks passed."
                if not deterministic_failures
                else "The retained generic plan has deterministic component or electrical topology failures.",
                (project.plan_path.name,) if project.plan_path is not None else (),
                {
                    "failures": [
                        {
                            "id": finding.id,
                        ...
                        for finding in deterministic_failures
                    ]
                },
                True,
                True,
...
def _check_routing(
    ...
    if reference_net_id is not None:
        ...
        stitching_contract_valid = (
            ...
            and raw_stitching >= (0 if ensure_connected else 1)
        )
        ...
        metrics["reference_plane"] = {
            ...
            "connection_policy": constraint.params.get(
                "reference_connection_policy", "explicit_stitching_minimum"
            ),
        }
