...
from pcbdraft.core.process import redact_argv, run_command
...
def verify_manufacturing_release(value: str | Path) -> ReleaseVerification:
    ...
    if (
        ...
        or receipt.get("schema") != "pcbdraft-release-receipt"
...
def _run_exports(
    ...
    for name, argv in commands:
        ...
        if remaining <= 0:
            raise PCBDraftError(f"manufacturing export deadline expired before {name}")
        result = run_command(
            argv,
            cwd=project.root,
            timeout=remaining,
        ...
        if result.timed_out:
            raise PCBDraftError(f"manufacturing export timed out: {name}")
        if result.output_limited:
            raise PCBDraftError(f"manufacturing export output limit exceeded: {name}")
