...
def resolve_billing_route(
    ...
    if provider_name == "fireworks" or base_url_host_matches(
        base_url or "", "api.fireworks.ai"
    ):
    ...
    if provider_name in {"custom", "local"} or (
...
def normalize_usage(
    ...
    *,
    ...
    if mode == "anthropic_messages" or provider_name == "anthropic":
        ...
        if not cache_write_tokens:
            cache_write_tokens = _usage_count(
                _usage_get(details, "cache_creation_input_tokens", 0) if details else 0
            )
        if not cache_write_tokens:
            cache_write_tokens = _usage_count(
                _usage_get(response_usage, "cache_creation_input_tokens", 0)
    ...
    if output_details:
        reasoning_tokens = _usage_count(
            _usage_get(output_details, "reasoning_tokens", 0)
        )
    if not reasoning_tokens:
        completion_details = _usage_get(
