...
def _parse_payment_method(raw: Any) -> PaymentMethodInfo | None:
    if not isinstance(raw, dict):
        return None
    kind = raw.get("kind")
...
def _parse_auto_reload_card(raw: Any) -> AutoReloadCard | None:
    if not isinstance(raw, dict):
        return None
    kind = raw.get("kind")
...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``PCBDRAFT_RUNTIME_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> BillingState | None:
    """Map ``PCBDRAFT_RUNTIME_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    Mirrors ``PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE``; the usage *bar* still comes from
    ``PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE`` (set both to pair a bar with a billing state).
    """
    name = (os.getenv("PCBDRAFT_RUNTIME_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return BillingState(
        logged_in=False, error=f"unknown PCBDRAFT_RUNTIME_DEV_BILLING_FIXTURE: {name}"
    )
