...
try:
    from pcbdraft.model.provider_profiles import (
        list_providers as _list_providers_for_registry,
    )
...
# Compatibility re-exports for the error formatting domain. The hooks resolve
# these legacy module globals at call time so established monkeypatch paths in
# ``pcbdraft.model.auth`` continue to affect the extracted implementation.
AuthError = _auth_error_formatting.AuthError
is_rate_limited_auth_error = _auth_error_formatting.is_rate_limited_auth_error
_parse_retry_after_seconds = _auth_error_formatting._parse_retry_after_seconds
format_auth_error = _auth_error_formatting.format_auth_error
_format_nous_entitlement_auth_error = (
    _auth_error_formatting._format_nous_entitlement_auth_error
)

_auth_error_formatting._configure_legacy_auth_hooks(
    auth_error_type=lambda: AuthError,
    rate_limited_code=lambda: CODEX_RATE_LIMITED_CODE,
    rate_limit_classifier=lambda: is_rate_limited_auth_error,
    nous_entitlement_formatter=lambda: _format_nous_entitlement_auth_error,
)
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("PCBDRAFT_RUNTIME_OAUTH_TRACE", "").strip().lower()
...
# Compatibility re-exports for credential-pool state. All hooks resolve the
# legacy module globals at call time so established monkeypatch paths continue
# to affect the extracted implementation without a reverse import.
_POOL_STATUS_FIELDS = _auth_credential_pool_store._POOL_STATUS_FIELDS
read_credential_pool = _auth_credential_pool_store.read_credential_pool
_merge_disk_cooldown_state = _auth_credential_pool_store._merge_disk_cooldown_state
write_credential_pool = _auth_credential_pool_store.write_credential_pool
suppress_credential_source = _auth_credential_pool_store.suppress_credential_source
is_source_suppressed = _auth_credential_pool_store.is_source_suppressed
unsuppress_credential_source = _auth_credential_pool_store.unsuppress_credential_source

_auth_credential_pool_store._configure_legacy_auth_hooks(
    load_auth_store=lambda: _load_auth_store(),
    load_global_auth_store=lambda: _load_global_auth_store(),
    auth_store_lock=lambda: _auth_store_lock(),
    save_auth_store=lambda store: _save_auth_store(store),
    normalize_source=lambda source: normalize_credential_source(source),
    sanitize_payload=lambda entry, provider_id: sanitize_borrowed_credential_payload(
        entry, provider_id
    ),
    now=lambda: time.time(),
    status_fields=lambda: _POOL_STATUS_FIELDS,
    merge_disk_cooldown=lambda entry, disk_entry, provider_id: (
...
def resolve_provider(
    ...
    *,
    ...
    try:
        from pcbdraft.model.provider_profiles import list_providers as _lp
    ...
    try:
        from pcbdraft.model.auxiliary_client import _scoped_key_env
    except ImportError:
        logger.warning(
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("PCBDRAFT_RUNTIME_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set PCBDRAFT_RUNTIME_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("PCBDRAFT_RUNTIME_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: dict[str, Any] | None = None) -> str:
    ...
    candidates = (
        get_env_value("PCBDRAFT_RUNTIME_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: dict[str, Any] | None = None) -> str:
    ...
    candidates = (
        get_env_value("PCBDRAFT_RUNTIME_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    save_env_value("PCBDRAFT_RUNTIME_SPOTIFY_CLIENT_ID", raw)
...
def login_spotify_command(args) -> None:
    ...
    # with "PCBDRAFT_RUNTIME_SPOTIFY_CLIENT_ID is required".
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_rate_limit:
            ...
            if stale_token and _probe_codex_quota_restored(
                ...
                if pool_token:
                    base_url = (
                        os.getenv("PCBDRAFT_RUNTIME_CODEX_BASE_URL", "")
                        .strip()
                        .rstrip("/")
                        or DEFAULT_CODEX_BASE_URL
                    )
    ...
    refresh_timeout_seconds = env_float(
        "PCBDRAFT_RUNTIME_CODEX_REFRESH_TIMEOUT_SECONDS", 20
    )
...
def _write_through_xai_oauth_to_global_root(state: dict[str, Any]) -> None:
    ...
    # ~/.hermes/auth.json even when PCBDRAFT_RUNTIME_HOME points at a profile path
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``PCBDRAFT_RUNTIME_XAI_BASE_URL``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float(
        "PCBDRAFT_RUNTIME_XAI_REFRESH_TIMEOUT_SECONDS", 20
    )
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("PCBDRAFT_RUNTIME_CA_BUNDLE")
...
#
# File lives at ${PCBDRAFT_RUNTIME_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<hermes-root>/shared/nous_auth.json`` where ``<hermes-root>`` is what
# ``get_default_runtime_root()`` returns — ``~/.hermes`` on Linux/macOS,
# ``%LOCALAPPDATA%\hermes`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's PCBDRAFT_RUNTIME_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``PCBDRAFT_RUNTIME_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hermes-root>/shared/``, where ``<hermes-root>`` is what
    :func:`hermes_constants.get_default_runtime_root` returns — so
    Linux/macOS classic installs land at ``~/.hermes/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``PCBDRAFT_RUNTIME_HOME`` deployments at
    ``<PCBDRAFT_RUNTIME_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("PCBDRAFT_RUNTIME_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from pcbdraft.core.runtime_environment import get_default_runtime_root

    return get_default_runtime_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set PCBDRAFT_RUNTIME_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        real_home_shared = (
            get_default_runtime_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set PCBDRAFT_RUNTIME_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No PCBDRAFT_RUNTIME_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if isinstance(expires_at_raw, str) and expires_at_raw:
        try:
            ...
            already_expired = parsed < datetime.now(UTC)
        except ValueError:
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        # PCBDRAFT_RUNTIME_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
    ...
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("PCBDRAFT_RUNTIME_PORTAL_BASE_URL")
...
_global_auth_store_cache: tuple[str, int, dict[str, Any]] | None = None
...
def get_api_key_provider_status(provider_id: str) -> dict[str, Any]:
    ...
    actual_local_noauth = (
        provider_id == "actual" and not api_key and is_actual_local_base_url(base_url)
    )
    ...
    return {
        ...
        or actual_local_noauth,  # compat with OAuth status shape
    }
...
def get_external_process_provider_status(provider_id: str) -> dict[str, Any]:
    ...
    command = (
        os.getenv("PCBDRAFT_RUNTIME_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("PCBDRAFT_RUNTIME_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> dict[str, Any]:
    ...
    if auth_mode == "entra_id":
        try:
            from pcbdraft.model.azure_identity_adapter import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set PCBDRAFT_RUNTIME_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from pcbdraft.model.configuration import clear_model_endpoint_credentials
...
def _prompt_model_selection(
    ...
    """
    from pcbdraft.model.catalog import _format_price_per_mtok, compute_sale_discount
    ...
    name_col = (
        max((len(m) for m in all_models), default=0) + name_pad if has_pricing else 0
    )
    ...
    try:
        ...
        from pcbdraft.model.model_search import model_search_text
    ...
    from pcbdraft.interfaces.tui.curses_ui import format_radio_item_ansi
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``PCBDRAFT_RUNTIME_PORTAL_BASE_URL`` set
