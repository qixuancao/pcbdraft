...
# * ``PCBDRAFT_RUNTIME_HOME`` / ``PCBDRAFT_RUNTIME_PROFILE`` / ``PCBDRAFT_RUNTIME_CONFIG`` /
#   ``PCBDRAFT_RUNTIME_ENV`` — Hermes runtime location flags. Writing these into
...
#
# IMPORTANT: ``PCBDRAFT_RUNTIME_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (PCBDRAFT_RUNTIME_LANGFUSE_PUBLIC_KEY,
# PCBDRAFT_RUNTIME_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset(
    {
        ...
        # NOT a PCBDRAFT_RUNTIME_* blanket: integration credentials (PCBDRAFT_RUNTIME_GEMINI_*,
        # PCBDRAFT_RUNTIME_LANGFUSE_*, PCBDRAFT_RUNTIME_SPOTIFY_*, ...) ARE allowed.
        "PCBDRAFT_RUNTIME_HOME",
        "PCBDRAFT_RUNTIME_PROFILE",
        "PCBDRAFT_RUNTIME_CONFIG",
        "PCBDRAFT_RUNTIME_ENV",
    }
)
...
_LOAD_CONFIG_CACHE: dict[
    str, tuple[int, int, int, int, dict[str, Any], dict[str, str | None]]
] = {}
...
_EXTRA_ENV_KEYS = frozenset(
    {
        ...
        "TERMINAL_SSH_KEY",
        "TERMINAL_SSH_PORT",
        # PCBDRAFT_RUNTIME_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
        ...
        "PCBDRAFT_RUNTIME_LANGFUSE_ENV",
        "PCBDRAFT_RUNTIME_LANGFUSE_RELEASE",
        "PCBDRAFT_RUNTIME_LANGFUSE_SAMPLE_RATE",
        "PCBDRAFT_RUNTIME_LANGFUSE_MAX_CHARS",
        "PCBDRAFT_RUNTIME_LANGFUSE_CAPTURE",
        "PCBDRAFT_RUNTIME_LANGFUSE_DEBUG",
        "LANGFUSE_PUBLIC_KEY",
        "LANGFUSE_SECRET_KEY",
        "LANGFUSE_BASE_URL",
        # ACP (Agent Client Protocol) keys — profile-isolable so different
        # profiles can use different ACP backends without cross-leak.
        "PCBDRAFT_RUNTIME_ACP_AUTH_METHOD",
        "PCBDRAFT_RUNTIME_ACP_AUTO_APPROVE",
        "PCBDRAFT_RUNTIME_COPILOT_ACP_COMMAND",
        "PCBDRAFT_RUNTIME_COPILOT_ACP_ARGS",
        "COPILOT_CLI_PATH",
...
from pcbdraft.interfaces.tui.default_soul import (
    DEFAULT_SOUL_MD,
...
# from `nix run` / `nix profile install` (which don't set PCBDRAFT_RUNTIME_MANAGED).
...
def get_managed_system() -> str | None:
    ...
    raw = os.getenv("PCBDRAFT_RUNTIME_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_runtime_home() / ".managed"
...
def is_managed() -> bool:
    ...
    Two signals: the PCBDRAFT_RUNTIME_MANAGED env var (set by the systemd service),
    or a .managed marker file in PCBDRAFT_RUNTIME_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Path | None = None) -> Path:
    ...
    the running interpreter, NOT of ``$PCBDRAFT_RUNTIME_HOME``, which is why a
...
def detect_install_method(project_root: Path | None = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$PCBDRAFT_RUNTIME_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. PCBDRAFT_RUNTIME_MANAGED env / .managed marker (NixOS managed mode)
    ...
    ``$PCBDRAFT_RUNTIME_HOME`` is a shared DATA directory — the Docker docs deliberately
    ...
    containerised gateway and a host install share one ``$PCBDRAFT_RUNTIME_HOME``, a
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $PCBDRAFT_RUNTIME_HOME.
    try:
    ...
    #    host install whose shared $PCBDRAFT_RUNTIME_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``hermes update``.
    try:
        method = (
            (get_runtime_home() / ".install_method")
    ...
    # detect Nix installs that don't set PCBDRAFT_RUNTIME_MANAGED (e.g. ``nix run``,
...
def stamp_install_method(method: str, project_root: Path | None = None) -> None:
    ...
    not in ``$PCBDRAFT_RUNTIME_HOME``, so that two installs sharing one data directory
...
def get_container_exec_info() -> dict | None:
    """Read container mode metadata from PCBDRAFT_RUNTIME_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or PCBDRAFT_RUNTIME_DEV=1 is set.
    ...
    container_mode_file = get_runtime_home() / ".container-mode"
...
def _resolve_pcbdraft_uid_gid() -> tuple[int | None, int | None]:
    ...
    ownership. The entrypoint chowns the top-level PCBDRAFT_RUNTIME_HOME once, but
    subdirectories created at runtime by ``ensure_runtime_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("PCBDRAFT_RUNTIME_UID", "").strip()
    gid_str = os.environ.get("PCBDRAFT_RUNTIME_GID", "").strip()
    try:
...
def _secure_dir(path):
    ...
    The mode can be overridden via the PCBDRAFT_RUNTIME_HOME_MODE environment variable
    (e.g. PCBDRAFT_RUNTIME_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse PCBDRAFT_RUNTIME_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``PCBDRAFT_RUNTIME_UID``/``PCBDRAFT_RUNTIME_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("PCBDRAFT_RUNTIME_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("PCBDRAFT_RUNTIME_CONTAINER") or os.environ.get(
        "PCBDRAFT_RUNTIME_SKIP_CHMOD"
    ):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set PCBDRAFT_RUNTIME_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into PCBDRAFT_RUNTIME_HOME, upgrading legacy empty templates.
...
def ensure_runtime_home():
    ...
    After the first successful pass for a given ``PCBDRAFT_RUNTIME_HOME`` we only re-run
    ...
    ``get_runtime_home()`` and therefore re-run for the new path.
    """
    ...
    # silently mkdir-ing the old PCBDRAFT_RUNTIME_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_runtime_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_runtime_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"PCBDRAFT_RUNTIME_HOME {home} does not exist. "
...
def warn_deprecated_cwd_env_vars(config: dict[str, Any] | None = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_runtime_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> dict[str, Any]:
    ...
    from pcbdraft.interfaces.tui.config_migrations import (
    ...
    try:
        from pcbdraft.interfaces.tui.toolset_validation import (
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = read_raw_config()
            try:
                from pcbdraft.agent.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def _strip_default_values(
    ...
    preserve_keys: set[tuple[str, ...]] | None = None,
) -> dict[str, Any]:
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``TerminalApp`` construction: the model becomes a plain
...
def is_provider_enabled(provider_cfg: dict[str, Any] | None) -> bool:
...
def cfg_get(cfg: dict[str, Any] | None, *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _runtime_home / "config.yaml"`` local variable
...
from pcbdraft.interfaces.tui.personality import (
    NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES,  # noqa: F401
)
...
def _prompt_text(value: Any) -> str:
    ...
    """
    from pcbdraft.interfaces.tui.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from pcbdraft.interfaces.tui.personality import render_personality_prompt as _render

    return _render(value)


def resolve_ephemeral_system_prompt_from_config(cfg: dict[str, Any] | None) -> str:
    ...
    still prefer ``PCBDRAFT_RUNTIME_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`hermes_cli.personality` (single owner).
    """
    from pcbdraft.interfaces.tui.personality import resolve_ephemeral_system_prompt
...
def read_user_config_raw(config_path: Path | None = None) -> dict[str, Any]:
    ...
    ``_runtime_home``, tui profile override, multi-profile probes).
    """
...
def require_readable_config_before_write(config_path: Path | None = None) -> None:
...
def load_config() -> dict[str, Any]:
    ...
    (which change ``PCBDRAFT_RUNTIME_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def save_config(
    ...
    *,
    ...
    with _CONFIG_LOCK:
        ...
        explicit_raw_paths: set[tuple[str, ...]] | None = (
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``PCBDRAFT_RUNTIME_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return (
        f"PCBDRAFT_RUNTIME_CUSTOM_{slug}_API_KEY"
        if slug
        else "PCBDRAFT_RUNTIME_CUSTOM_API_KEY"
    )
...
def get_env_value(key: str) -> str | None:
    ...
    """
    try:
        from pcbdraft.agent.secret_scope import UnscopedSecretError
...
def get_env_value_prefer_dotenv(key: str) -> str | None:
...
def show_config():
    ...
    from pcbdraft.model.auth import get_anthropic_key
    ...
    # Warn on stale PCBDRAFT_RUNTIME_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("PCBDRAFT_RUNTIME_MAX_ITERATIONS")
        if (
    ...
    try:
        from pcbdraft.interfaces.tui.personality import active_personality_name
...
def _cron_model_drift_axis_for_config_key(key: str) -> str | None:
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``PCBDRAFT_RUNTIME_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("PCBDRAFT_RUNTIME_MODEL", ""))
...
def _validate_config_key(key: str) -> tuple[bool, str | None]:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_runtime_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        from pcbdraft.model.credential_lifecycle import remove_provider_env_credential
