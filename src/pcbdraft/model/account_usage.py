...
from pcbdraft.model.anthropic_adapter import _is_oauth_token, resolve_anthropic_token
...
def build_nous_credits_snapshot(account_info) -> AccountUsageSnapshot | None:
    ...
    """
    try:
        from pcbdraft.interfaces.tui.nous_account import nous_portal_topup_url
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE selects a fixture state, /usage
...
def _snapshot_from_credits_state(state) -> AccountUsageSnapshot | None:
    ...
    so PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def redeem_codex_reset_credit(
    *,
    ...
    if code == "reset":
        ...
        try:
            from pcbdraft.model.auth import clear_codex_pool_quota_cooldowns
