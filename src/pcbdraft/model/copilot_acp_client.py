...
def _resolve_args() -> list[str]:
    raw = os.getenv("PCBDRAFT_RUNTIME_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    PCBDRAFT_RUNTIME_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set PCBDRAFT_RUNTIME_HOME explicitly if they
...
def _build_subprocess_env() -> dict[str, str]:
    ...
    env = pcbdraft_subprocess_env(inherit_credentials=True)
...
def _format_messages_as_prompt(
    ...
    sections: list[str] = [
        "You are being used as the active ACP agent backend for PCBDraft.",
        "Use ACP capabilities to complete tasks.",
        "IMPORTANT: If you take an action with a tool, you MUST output tool calls using <tool_call>{...}</tool_call> blocks with JSON exactly in OpenAI function-call shape.",
        "If no tool is needed, answer normally.",
    ]
    if model:
        sections.append(f"PCBDraft requested model hint: {model}")
    ...
    def _run_prompt(
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"PCBDRAFT_RUNTIME_COPILOT_ACP_COMMAND / PCBDRAFT_RUNTIME_COPILOT_ACP_ARGS "
                f"to a working pair."
            )
            ...
            *,
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "PCBDraft ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "point PCBDraft at it explicitly:\n"
                        "  export PCBDRAFT_RUNTIME_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
                        "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\n"
                        "directly with a Copilot subscription token) via `pcbdraft connect`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
        ...
        *,
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(
                message_id,
                -32601,
                f"ACP client method '{method}' is not supported by PCBDraft yet.",
            )
