...
_BUNDLED_PLUGINS_DIR = Path(__file__).resolve().parent / "builtins"
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$PCBDRAFT_RUNTIME_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$PCBDRAFT_RUNTIME_HOME/plugins/model-providers/`` if it exists."""
    try:
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple PCBDRAFT_RUNTIME_HOME profiles don't alias each other.
...
def _discover_entry_point_providers() -> None:
    ...
    * **Provider targets only.** The ``pcbdraft.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$PCBDRAFT_RUNTIME_HOME``) keep their
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$PCBDRAFT_RUNTIME_HOME/plugins/model-providers/<name>/``
    ...
    #    filesystem steps means a bundled or ``$PCBDRAFT_RUNTIME_HOME`` profile of the
    ...
    # 2. User plugins — under $PCBDRAFT_RUNTIME_HOME/plugins/model-providers/<name>/.
    ...
    try:
        ...
        import pcbdraft.model.provider_profiles as _pkg

        for _importer, modname, _ispkg in pkgutil.iter_modules(_pkg.__path__):
            if modname.startswith("_") or modname == "base":
                continue
            try:
                importlib.import_module(f"pcbdraft.model.provider_profiles.{modname}")
