...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    ...
    from pcbdraft.model.usage_pricing import CanonicalUsage, estimate_usage_cost
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    if not force:
        try:
            from pcbdraft.agent.conversation_compression import COMPACTION_STATUS
...
# not as MCP calls.
_INTERNAL_MCP_SERVER = "pcbdraft-tools"
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    def _stable_call_id(item: dict, name: str) -> str:
        ...
        from pcbdraft.model.transports.codex_event_projector import (
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    """
    from pcbdraft.model.transports.codex_app_server_session import (
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # in config, the /yolo session toggle, or --yolo / PCBDRAFT_RUNTIME_YOLO_MODE —
        ...
        try:
            from pcbdraft.tools.approval import is_approval_bypass_active
...
def _raise_stream_error(event: Any) -> None:
    ...
    """
    from pcbdraft.agent.loop import _StreamErrorEvent
