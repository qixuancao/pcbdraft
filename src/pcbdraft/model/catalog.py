...
def _urlopen_model_catalog_request(
    req: urllib.request.Request, *, timeout: float, ssl_context=None
):
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from pcbdraft.model.configuration import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from pcbdraft.interfaces.tui.codex_models import (
...
# $PCBDRAFT_RUNTIME_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $PCBDRAFT_RUNTIME_HOME/models_dev_cache.json directly (no network). Falls
    back to ``_XAI_STATIC_FALLBACK`` when the cache is empty or unreadable.
    """
    try:
        from pcbdraft.agent.models_dev import _load_disk_cache
...
def _nous_recommended_disk_path() -> Path:
    ...
    return get_runtime_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$PCBDRAFT_RUNTIME_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from pcbdraft.model.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
try:
    from pcbdraft.model.provider_profiles import (
        list_providers as _list_providers_for_canonical,
    )
...
PROVIDER_GROUPS: dict[str, tuple[str, str, list[str]]] = {
    ...
    "openai": (
        "OpenAI",
        "ChatGPT/Codex subscription or direct OpenAI API",
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from pcbdraft.model.model_catalog import get_default_model_from_cache
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from pcbdraft.model.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from pcbdraft.model.model_catalog import get_curated_nous_models
...
def _fireworks_pricing_from_models_dev(
    *,
    ...
    result: dict[str, dict[str, str]] = {}
    try:
        from pcbdraft.agent.models_dev import _get_provider_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from pcbdraft.model.auth import get_auth_status, has_usable_secret
...
def _resolve_static_model_alias(
    ...
    try:
        from pcbdraft.model.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def model_supports_fast_mode(model_id: str | None) -> bool:
    ...
    from pcbdraft.model.model_metadata import is_grok_46_family
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from pcbdraft.model.model_search import model_alias_canonical
...
def _openai_discovery_base_url(provider: str) -> str:
    ...
    try:
        ...
        if cfg_provider in ("openai", "openai-api") and normalize_provider(
            provider
        ) == normalize_provider(cfg_provider):
...
def provider_model_ids(
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``hermes model`` shows.
            from pcbdraft.model.provider_config import is_official_openai_host
    ...
    if normalized == "bedrock":
        try:
            from pcbdraft.model.bedrock_adapter import bedrock_model_ids_or_none
...
#
# Cache strategy:
#   - One JSON file at $PCBDRAFT_RUNTIME_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_runtime_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$PCBDRAFT_RUNTIME_HOME/auth.json`` and external
...
def ensure_lmstudio_model_loaded(
    ...
    *,
    ...
    try:
        refreshed_models = _lmstudio_fetch_raw_models(
            api_key=api_key, base_url=base_url, timeout=10
        )
    except Exception:
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_runtime_home() / "ollama_cloud_models_cache.json"
