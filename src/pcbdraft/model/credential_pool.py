...
def _exhausted_until(
    entry: PooledCredential, *, sole_credential: bool = False
) -> float | None:
...
def get_custom_provider_pool_key(
    base_url: str | None, provider_name: str | None = None
) -> str | None:
...
class CredentialPool:
    ...
    def next_available_at(self) -> float | None:
        ...
        with self._lock:
            ...
            sole_credential = (
                sum(1 for e in self._entries if e.last_status != STATUS_DEAD) <= 1
            )
    ...
    def _persist(self, *, removed_ids: list[str] | None = None) -> None:
    ...
    def _sync_codex_entry_from_auth_store(
        ...
        try:
            ...
            if store_access and (
                ...
                should_adopt = True
            elif store_refresh and store_refresh != entry_refresh and not store_access:
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``PCBDRAFT_RUNTIME_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "PCBDRAFT_RUNTIME_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "PCBDRAFT_RUNTIME_XAI_REFRESH_TIMEOUT_SECONDS"
        )
    ...
    def _select_under_lock(self) -> tuple[PooledCredential | None, list[tuple]]:
    ...
    def _select_unlocked(
    ...
    ) -> tuple[PooledCredential | None, list[tuple]]:
        ...
        available, pending_refresh = self._available_entries(
            clear_expired=True, refresh=refresh
        )
        if not available:
            self._current_id = None
    ...
    def mark_exhausted_and_rotate(
        ...
        *,
        ...
        with self._lock:
            ...
            if entry is None:
                entry = (
                    self._current_unlocked() or self._select_unlocked(refresh=False)[0]
                )
            if entry is None:
                return None
            _label = entry.label or entry.id[:8]
            ...
            if entry is None:
                if api_key_hint:
                    ...
                    entry = (
                        ...
                        or self._select_unlocked(refresh=False)[0]
                    )
            if entry is None:
                return None
            self._current_id = entry.id
    ...
    def resolve_target(
        self, target: Any
    ) -> tuple[int | None, PooledCredential | None, str | None]:
...
def _seed_from_singletons(
    ...
    try:
        from pcbdraft.model.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        try:
            from pcbdraft.model.auth import resolve_qwen_runtime_credentials
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from pcbdraft.model.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(
    ...
    try:
        from pcbdraft.model.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
