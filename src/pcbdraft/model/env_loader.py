...
def get_secret_source_values(
    runtime_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``runtime_home``."""
    home_key = str(Path(runtime_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(runtime_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        from pcbdraft.agent.secret_scope import _is_global_env, load_env_file
        from pcbdraft.agent.secret_sources.registry import apply_all
        ...
        local_env["PCBDRAFT_RUNTIME_HOME"] = str(home)
...
def reset_secret_source_cache() -> None:
    """Forget which PCBDRAFT_RUNTIME_HOME paths have already had external secrets applied.
...
def format_secret_source_suffix(env_var: str) -> str:
    ...
    try:
        from pcbdraft.agent.secret_sources.registry import get_source
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from pcbdraft.model.configuration import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    from pcbdraft.interfaces.tui import _early_recovery
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of PCBDRAFT_RUNTIME_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    owns the once-per-PCBDRAFT_RUNTIME_HOME guard, the post-apply ASCII
