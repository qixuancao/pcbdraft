...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from pcbdraft.model.auth import _resolve_api_key_provider_secret
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    """
    try:
        from pcbdraft.model.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from pcbdraft.interfaces.tui.main import _prompt_api_key
    ...
    existing_key, existing_source = _existing_api_key_for_model_flow(
        "openrouter", pconfig
    )
    if not existing_key:
    ...
    from pcbdraft.model.catalog import get_pricing_for_provider, model_ids
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from pcbdraft.core.runtime_environment import AI_GATEWAY_BASE_URL
    from pcbdraft.interfaces.tui.main import _prompt_api_key
    from pcbdraft.model.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
    ...
    from pcbdraft.model.catalog import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    from pcbdraft.model.auth import _save_model_choice, deactivate_provider
    from pcbdraft.model.configuration import load_config, save_config
    ...
    if not presets:
        print(
            "No MoA presets configured. Configure a preset in config.yaml; see `pcbdraft --help`."
        )
        return
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from pcbdraft.interfaces.tui.nous_subscription import prompt_enable_tool_gateway
    from pcbdraft.model.auth import (
        PROVIDER_REGISTRY,
        AuthError,
        _login_nous,
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from pcbdraft.model.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    selected = _prompt_model_selection(
        models, current_model=current_model or (models[0] if models else "grok-4.6")
    )
    if selected:
        _save_model_choice(selected)
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from pcbdraft.interfaces.tui.main import _DEFAULT_QWEN_PORTAL_MODELS
    from pcbdraft.model.auth import (
    ...
    from pcbdraft.model.catalog import fetch_api_models
...
def _model_flow_custom(config):
    ...
    from pcbdraft.model.catalog import probe_api_models
    ...
    if model_name:
        ...
        print(
            "Endpoint saved. Use `/model` in chat or `pcbdraft connect` to set a model."
        )
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  PCBDraft will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  PCBDraft will install it now (the preflight below "
...
def _model_flow_copilot(config, current_model=""):
    ...
    from pcbdraft.model.configuration import load_config, save_config, save_env_value
    ...
    if not api_key:
        ...
        if choice == "1":
            ...
            # Validate token type
            try:
                from pcbdraft.interfaces.tui.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates PCBDraft turns to `copilot --acp`.")
    print("  PCBDraft currently starts its own ACP subprocess for each request.")
    print("  PCBDraft uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set PCBDRAFT_RUNTIME_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from pcbdraft.model.catalog import _PROVIDER_MODELS, fetch_api_models
    from pcbdraft.model.configuration import (
        get_env_value,
        load_config,
        save_config,
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    from pcbdraft.model.auth import ProviderConfig, _resolve_api_key_provider_secret
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from pcbdraft.model.catalog import _PROVIDER_MODELS
    from pcbdraft.model.configuration import get_env_value, load_config, save_config
    ...
    if sa_path:
        ...
        print(
            "      • set VERTEX_CREDENTIALS_PATH in $PCBDRAFT_RUNTIME_HOME/.env to a service account JSON"
        )
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    or error.
    """
    from pcbdraft.interfaces.tui.main import _prompt_provider_choice
    from pcbdraft.model.auth import ZAI_ENDPOINTS
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from pcbdraft.model.gemini_native_adapter import probe_gemini_tier
        except Exception:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   PCBDraft typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with PCBDraft, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        if live_models:
            ...
            mdev_models: list = []
            try:
                from pcbdraft.agent.models_dev import list_agentic_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from pcbdraft.interfaces.tui.main import _run_anthropic_oauth_flow
