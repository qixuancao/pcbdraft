...
from pcbdraft.agent.message_metadata import PERSISTENCE_ONLY_MESSAGE_FIELDS
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``PCBDRAFT_RUNTIME_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
...
try:
    from pcbdraft.model.provider_profiles import list_providers as _list_providers
except Exception:
...
def _note_endpoint_blackholed(base_url: str) -> None:
    ...
    logger.debug(
        ...
        key,
        _ENDPOINT_BLACKHOLE_TTL_SECONDS,
    )
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_runtime_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_runtime_home() / "cache" / "openrouter_model_metadata.json"
...
def detect_local_server_type(base_url: str, api_key: str = "") -> str | None:
    ...
    if (
        cached is not None
        and (time.monotonic() - cached[1]) < _ENDPOINT_PROBE_TTL_SECONDS
    ):
        return cached[0]
    ...
    if isinstance(deepinfra_pricing, dict) and any(
        ...
        if deepinfra_pricing.get("output_tokens") is not None:
            result["completion"] = str(
                float(deepinfra_pricing["output_tokens"]) / 1_000_000
            )
        if deepinfra_pricing.get("cache_read_tokens") is not None:
            result["cache_read"] = str(
                float(deepinfra_pricing["cache_read_tokens"]) / 1_000_000
            )
        return result
...
def fetch_model_metadata(force_refresh: bool = False) -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        response = requests.get(
            OPENROUTER_MODELS_URL, timeout=(5, 10), verify=_resolve_requests_verify()
        )
...
def _get_context_cache_path() -> Path:
    ...
    return get_runtime_home() / "context_length_cache.yaml"
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or (
            "in the output" in error_lower  # OpenRouter / Nous
            and "maximum context length" in error_lower
        )
        or (
            "requested" in error_lower  # LM Studio / llama.cpp
            and "output tokens" in error_lower
        )
...
def _query_ollama_api_show_uncached(
    model: str, base_url: str, api_key: str = ""
) -> int | None:
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    def _apply_verified_bump(ctx: int, source: str) -> tuple[int, str]:
        ...
        if bumped is not None and ctx == _CODEX_OAUTH_STALE_ADVERTISED_CTX:
            logger.debug(
                "Codex OAuth context for %s: advertised %d raised to live-verified %d",
                model_bare,
                ctx,
                bumped,
            )
...
def get_model_context_length(
    ...
    if provider and model:
        try:
            from pcbdraft.agent.models_dev import _override_context_window
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from pcbdraft.model.catalog import get_copilot_model_context
    ...
    if base_url:
        ...
        _skip_ollama_probe = (
            _inferred_for_probe is not None and "ollama" not in _inferred_for_probe
        )
    ...
    if effective_provider:
        from pcbdraft.agent.models_dev import lookup_models_dev_context
...
def _estimate_message_tokens_cached(msg: Any, image_cost: int) -> int:
    try:
        ...
        return _estimate_message_tokens_without_images(msg) + _count_image_tokens(
            msg, image_cost
        )
    cached = _MSG_TOKENS_CACHE.get(key)
    if cached is not None:
        return cached[1]
    tokens = _estimate_message_tokens_without_images(msg) + _count_image_tokens(
        msg, image_cost
    )
