...
_PCBDRAFT_MODEL_WARNING = (
    "Nous Research Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with PCBDraft. They lack the tool-calling capabilities "
...
_NOUS_PCBDRAFT_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_pcbdraft_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_PCBDRAFT_NON_AGENTIC_RE.search(model_name))


def _check_pcbdraft_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_pcbdraft_non_agentic(model_name):
        return _PCBDRAFT_MODEL_WARNING
    return ""
...
def resolve_persist_behavior(
    ...
    """PCBDraft keeps one persistent provider and model for all interfaces."""
    return True
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'pcbdraft connect' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'pcbdraft doctor' — config issues detected:"
        ...
        from pcbdraft.model.provider_config import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from pcbdraft.model.provider_config import resolve_user_provider as _ruser
    ...
    if not validation.get("accepted"):
        override = False
        if user_providers:
            from pcbdraft.model.configuration import is_provider_enabled
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from pcbdraft.model.catalog import normalize_opencode_base_url
    ...
    pcbdraft_warn = _check_pcbdraft_model_warning(new_model)
    if pcbdraft_warn:
        warnings.append(pcbdraft_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from pcbdraft.model.configuration import normalize_extra_headers
...
def prewarm_picker_cache_async() -> _threading.Thread | None:
    ...
    def _warm() -> None:
        try:
            from pcbdraft.interfaces.tui.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from pcbdraft.model.catalog import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from pcbdraft.model.provider_config import ALIASES as _PROVIDER_ALIAS_TABLE
    from pcbdraft.model.provider_config import PCBDRAFT_RUNTIME_OVERLAYS
    ...
    for pcbdraft_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(pcbdraft_id)
        if (
            _alias_target
            and _alias_target != pcbdraft_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        _canonical = pcbdraft_id
        try:
            from pcbdraft.model.provider_profiles import get_provider_profile as _gpp

            _prof = _gpp(pcbdraft_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != pcbdraft_id:
            continue
        if pcbdraft_id.lower() in seen:
            continue
        if pcbdraft_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(pcbdraft_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from pcbdraft.model.auth import is_runtime_provider_routable

        if not is_runtime_provider_routable(pcbdraft_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(pcbdraft_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        pcbdraft_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(pcbdraft_id)
            seen.add(pcbdraft_id.lower())

    # --- Section 2: Hermes-only providers (PCBDRAFT_RUNTIME_OVERLAYS) ---
    _mdev_to_pcbdraft = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in PCBDRAFT_RUNTIME_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        pcbdraft_slug = _mdev_to_pcbdraft.get(pid, pid)
        if pcbdraft_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or pcbdraft_slug.lower() in _excluded_set:
            continue
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            ...
            try:
                from pcbdraft.agent.vertex_adapter import has_vertex_credentials
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or pcbdraft_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(pcbdraft_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(pcbdraft_slug)
            seen.add(pid.lower())
            seen.add(pcbdraft_slug.lower())
    ...
    for _cp in CANONICAL_PROVIDERS:
        ...
        if _cp_config and _cp_config.api_key_env_vars:
            _cp_has_creds = any(
                _scoped_key_env(ev) for ev in _cp_config.api_key_env_vars
            )
        if not _cp_has_creds:
            try:
                _cp_store = _load_auth_store()
                _cp_providers_store = (
                    _cp_store.get("providers", {}) if _cp_store else {}
                )
...
def list_authenticated_providers(
    ...
    *,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from pcbdraft.model.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    def _has_aws_sdk_creds_for_listing(slug: str) -> bool:
        ...
        if slug_norm != current_norm:
            return False
        try:
            from pcbdraft.model.bedrock_adapter import has_aws_credentials
    ...
    if "ollama-cloud" not in curated:
        from pcbdraft.model.catalog import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from pcbdraft.model.catalog import fetch_lmstudio_models
    ...
    for pcbdraft_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if slug.lower() in seen_slugs:
            continue
        if pcbdraft_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (PCBDRAFT_RUNTIME_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        ...
        if not has_creds:
            try:
                from pcbdraft.model.auth import _load_auth_store
        ...
        model_ids = cached_provider_model_ids(pcbdraft_id)
        if not model_ids:
            model_ids = curated.get(pcbdraft_id, [])
            if pcbdraft_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(pcbdraft_id, model_ids)
        ...
        results.append(
            {
                ...
                "is_current": (
                    ...
                    or pcbdraft_id == current_provider
    ...
    from pcbdraft.model.auth import PROVIDER_REGISTRY as _auth_registry
    from pcbdraft.model.provider_config import PCBDRAFT_RUNTIME_OVERLAYS

    # Build reverse mapping: models.dev ID → Hermes provider ID.
    # PCBDRAFT_RUNTIME_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    ...
    for pid, overlay in PCBDRAFT_RUNTIME_OVERLAYS.items():
        ...
        pcbdraft_slug = _mdev_to_pcbdraft.get(pid, pid)
        if pcbdraft_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or pcbdraft_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(pcbdraft_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or pcbdraft_slug in providers_store
                ):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                ...
                logger.debug(
                    "Credential pool check failed for %s: %s", pcbdraft_slug, exc
                )
        ...
        # configured.
        if not has_creds and pcbdraft_slug == "anthropic":
            try:
                ...
                pcbdraft_creds = read_pcbdraft_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (pcbdraft_creds and pcbdraft_creds.get("accessToken")) or (
        ...
        if pcbdraft_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(pcbdraft_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(pcbdraft_slug)
                model_ids = (
                    _ids
                    if _ids
                    else (curated.get(pcbdraft_slug, []) or curated.get(pid, []))
                )
            except Exception:
                model_ids = curated.get(pcbdraft_slug, []) or curated.get(pid, [])
        elif pcbdraft_slug == "nous":
            ...
            try:
                from pcbdraft.model.auth import get_provider_auth_state as _nous_state
                from pcbdraft.model.catalog import check_nous_free_tier as _nous_free
                from pcbdraft.model.catalog import (
            ...
            if not model_ids:
                model_ids = curated.get(pcbdraft_slug, []) or curated.get(pid, [])
                if pcbdraft_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(pcbdraft_slug, model_ids)
        total = len(model_ids)
        if pcbdraft_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append(
            {
                "slug": pcbdraft_slug,
                "name": get_label(pcbdraft_slug),
                "is_current": pcbdraft_slug == current_provider
        ...
        seen_slugs.add(pcbdraft_slug.lower())
        _record_builtin_endpoint(pcbdraft_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or PCBDRAFT_RUNTIME_OVERLAYS (keeps /model in sync
    # with `hermes model`).
    try:
        from pcbdraft.model.catalog import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for grp in ep_groups.values():
            ...
            if _discovery_allowed:
                try:
                    from pcbdraft.model.catalog import cached_fetch_api_models
                    ...
                    live_models = cached_fetch_api_models(
                        ...
                        timeout=1.5
                        if for_picker
                        else 5.0,  # picker: fail fast so a slow custom endpoint doesn't block /model
    ...
    if (
        ...
        try:
            ...
            _live_models = cached_fetch_api_models(
                ...
                timeout=1.5
                if for_picker
                else 5.0,  # picker: fail fast on a slow current endpoint
                cache_only=not _probe_live,
            )
            if _live_models:
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            _grp_is_current = (
                ...
                or _current_provider_norm
                in {str(alias).lower() for alias in grp.get("aliases", set())}
            ) or (
                _current_provider_norm == "custom"
                and bool(_current_base_url_norm)
...
def _prepend_moa_picker_provider(
    ...
    """
    try:
        from pcbdraft.interfaces.tui.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    """
    from pcbdraft.model.catalog import fetch_openrouter_models
