...
def _config_base_url_trustworthy_for_bare_custom(
    ...
    try:
        from pcbdraft.model.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from pcbdraft.model.catalog import copilot_model_api_mode
...
def _parse_api_mode(raw: Any) -> str | None:
    ...
    if isinstance(raw, str):
        from pcbdraft.model.configuration import _canonical_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from pcbdraft.model.catalog import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from pcbdraft.model.catalog import opencode_model_api_mode
...
def resolve_requested_provider(requested: str | None = None) -> str:
    ...
    env_provider = _getenv("PCBDRAFT_RUNTIME_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _try_resolve_from_custom_pool(
    ...
    try:
        ...
        if not pool_api_key:
            return None
        if not has_usable_secret(pool_api_key) and _loopback_hostname(
...
def find_custom_provider_identity_by_model(model: str) -> str | None:
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``PCBDRAFT_RUNTIME_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("PCBDRAFT_RUNTIME_INFERENCE_PROVIDER", "").strip()
...
def _resolve_named_custom_runtime(
    *,
    ...
    if key_cmd and not has_usable_secret((explicit_api_key or "").strip()):
        from pcbdraft.agent.command_token_source import build_command_token_provider
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("PCBDRAFT_RUNTIME_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(
                    _getenv("PCBDRAFT_RUNTIME_NOUS_TIMEOUT_SECONDS", "15")
                ),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from pcbdraft.model.configuration import is_provider_enabled, load_config
    ...
    if requested_provider == "anthropic" and base_url_host_matches(
        _eff_base, "azure.com"
    ):
        _azure_key = (
            (explicit_api_key or "").strip()
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(
                60, env_int("PCBDRAFT_RUNTIME_NOUS_MIN_KEY_TTL_SECONDS", 1800)
            )
            nous_state = {
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from pcbdraft.model.auth import resolve_minimax_oauth_runtime_credentials
