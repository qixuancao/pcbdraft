...
def usage_model_from_account(account_info: Any) -> UsageModel:
    ...
    try:
        ...
        sub_remaining = (
            _finite(getattr(access, "subscription_credits_remaining", None))
            if access
            else None
        )
        topup_remaining = (
            _finite(getattr(access, "purchased_credits_remaining", None))
            if access
            else None
        )
        total_usable = (
            _finite(getattr(access, "total_usable_credits", None)) if access else None
        )

        plan_name = getattr(sub, "plan", None) if sub is not None else None
        renews_at = (
            getattr(sub, "current_period_end", None) if sub is not None else None
        )
        monthly = (
            _finite(getattr(sub, "monthly_credits", None)) if sub is not None else None
        )
        ...
        if paid is False:
            ...
            total_spendable is not None and total_spendable < LOW_BALANCE_THRESHOLD_USD
        ):
            status = "low"
        else:
            status = "healthy"
...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    ...
    Dev override: ``PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE`` short-circuits to a fixture so
...
def _dev_fixture_usage_model() -> UsageModel | None:
    """Map ``PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE`` to a usage model for offline UX work.

    Recognized names: ``free | healthy | low | topup | depleted``. Returns
    ``None`` when the env var is unset (real portal path runs).
    """
    name = (os.getenv("PCBDRAFT_RUNTIME_DEV_CREDITS_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    if name in ("healthy", "mid"):
        return UsageModel(
            ...
            plan_bar=UsageBar(
                kind="plan", remaining_usd=14.0, total_usd=20.0, spent_usd=6.0
            ),
        )
    ...
    if name in ("topup", "top-up"):
        return UsageModel(
            ...
            topup_bar=UsageBar(
                kind="topup", remaining_usd=12.0, total_usd=12.0, spent_usd=0.0
            ),
        )
    ...
    if name == "low":
        return UsageModel(
            ...
            plan_bar=UsageBar(
                kind="plan", remaining_usd=3.4, total_usd=20.0, spent_usd=16.6
            ),
        )
    ...
    if name == "depleted":
        return UsageModel(
            ...
            plan_bar=UsageBar(
                kind="plan", remaining_usd=0.0, total_usd=20.0, spent_usd=20.0
            ),
        )
