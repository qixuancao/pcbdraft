...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    from pcbdraft.model.azure_identity_adapter import build_bearer_http_client
...
def _resolve_anthropic_pool_token() -> str | None:
    ...
    """
    try:
        from pcbdraft.model.credential_pool import AUTH_TYPE_OAUTH, load_pool
    except Exception:
        return None
...
def _convert_assistant_message(m: dict[str, Any]) -> dict[str, Any]:
    ...
    effective = (
        blocks if blocks else [{"type": "text", "text": _EMPTY_TEXT_PLACEHOLDER}]
    )
...
def _scrub_blank_text_blocks(result: list[dict[str, Any]]) -> None:
    ...
    for msg_index, msg in enumerate(result):
        ...
        if not isinstance(content, list) or not content:
            continue
        placeholder_text = (
            _EMPTY_TEXT_PLACEHOLDER if role == "assistant" else "(empty message)"
        )
...
def _is_stream_unavailable_error(exc: Exception) -> bool:
    ...
    if "invokemodelwithresponsestream" in err_lower:
        from pcbdraft.model.bedrock_adapter import is_streaming_access_denied_error
