...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    from pcbdraft.agent.native_compaction import prune_pre_checkpoint_items
...
def _preflight_codex_input_items(
    ...
    *,
    ...
    sanitize_text = (
        _neutralize_harmony_tokens if sanitize_harmony_tokens else lambda text: text
    )
            ...
            if isinstance(output, list):
                ...
                for part in output:
                    ...
                    if ptype == "input_text":
                        ...
                        if isinstance(text, str) and text:
                            cleaned.append(
                                {"type": "input_text", "text": sanitize_text(text)}
                            )
        ...
        if item_type == "message":
            ...
            for part_idx, part in enumerate(content):
                ...
                normalized_content.append(
                    {"type": "output_text", "text": sanitize_text(text)}
                )
            ...
            if isinstance(content, list):
                ...
                for part_idx, part in enumerate(content):
                    if isinstance(part, str):
                        if part:
                            validated.append(
                                {"type": text_type, "text": sanitize_text(part)}
                            )
                        continue
                    if not isinstance(part, dict):
                        raise ValueError(
                    ...
                    if ptype in {"input_text", "text", "output_text"}:
                        ...
                        if not isinstance(text, str):
                            text = str(text or "")
                        validated.append(
                            {"type": text_type, "text": sanitize_text(text)}
                        )
...
def _preflight_codex_api_kwargs(
    ...
    *,
    ...
    if is_xai_model and normalized.get("tools"):
        try:
            from pcbdraft.tools.schema_sanitizer import strip_slash_enum
