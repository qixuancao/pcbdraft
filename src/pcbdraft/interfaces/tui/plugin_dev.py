...
def _doctor_runtime(plugin_path: Path):
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                "PCBDRAFT_RUNTIME_HOME": str(home),
                "PCBDRAFT_RUNTIME_BUNDLED_PLUGINS": str(bundled),
                "PCBDRAFT_RUNTIME_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_runtime_home() / "plugins"
...
def _check_manifest_v2(report: DoctorReport, manifest: Any) -> None:
    ...
    from pcbdraft.agent.extensions.manager import SUPPORTED_MANIFEST_VERSION
    ...
    if schema:
        from pcbdraft.agent.extensions.manager import _CONFIG_SCHEMA_TYPES
