...
async def get_toolsets(profile: str | None = None):
    from pcbdraft.interfaces.tui.platforms import platform_label
    ...
    def _read():
        with _profile_scope(profile):
            ...
            if cat:
                ...
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    if name == "web" and prov.get("web_backend"):
                        ...
                        row["capabilities"] = web_provider_capabilities(
                            prov["web_backend"]
                        )
            ...
            if name == "web":
                ...
                try:
                    from pcbdraft.tools.web_tools import (
...
async def save_toolset_env(
    name: str, body: ToolsetEnvUpdate, profile: str | None = None
):
...
async def run_toolset_post_setup(
    ...
    write per-profile state must see the same PCBDRAFT_RUNTIME_HOME the rest of the
