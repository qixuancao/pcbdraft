...
from pcbdraft.interfaces.tui import web_git as _web_git  # noqa: F401 — used by handlers
...
async def gh_auth_status_route(refresh: bool = False):
    ...
    if (
        not refresh
        and _gh_auth_cache
        and time.monotonic() - _gh_auth_cache[0] < _GH_AUTH_TTL_S
    ):
...
async def git_review_list_route(
    path: str, scope: str = "uncommitted", base: str | None = None
):
...
async def git_pr_list_route(body: GitPrListBody):
    return await _git_op(
        _web_git.review_pr_list, _git_path(body.path), body.branches, body.numbers
    )
