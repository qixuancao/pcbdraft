...
from pcbdraft.interfaces.tui.web_deps import late
    ...
    for name, home in targets:
        ...
        if slices is None:
            ...
            try:
                slices = {
                    "recents": _slice(
                        db, exclude=recents_exclude_list, cap=recents_cap
                    ),
...
def _merge_by_id(
    into: dict[str, dict[str, Any]], entries: list[dict[str, Any]], child_key: str
) -> None:
    ...
    for entry in entries:
        ...
        if child_key == "sessions":
            ...
            children: dict[str, dict[str, Any]] = {
                c["id"]: c for c in existing.get(child_key) or []
            }
            _merge_by_id(children, entry.get(child_key) or [], "sessions")
            existing[child_key] = list(children.values())
        if "sessionCount" in existing:
            existing["sessionCount"] = (existing.get("sessionCount") or 0) + (
                entry.get("sessionCount") or 0
            )
...
def _merge_profile_tree(
    ...
    for project in projects:
        for lane in (
            repo for r in project.get("repos") or [] for repo in r.get("groups") or []
        ):
        ...
        repos: dict[str, dict[str, Any]] = {
            r["id"]: r for r in existing.get("repos") or []
        }
        _merge_by_id(repos, project.get("repos") or [], "groups")
        existing["repos"] = list(repos.values())
        existing["sessionCount"] = (existing.get("sessionCount") or 0) + (
            project.get("sessionCount") or 0
        )
        existing["totalTokens"] = (existing.get("totalTokens") or 0) + (
            project.get("totalTokens") or 0
        )
        existing["totalCostUsd"] = (existing.get("totalCostUsd") or 0) + (
            project.get("totalCostUsd") or 0
        )
        existing["lastActive"] = max(
            existing.get("lastActive") or 0, project.get("lastActive") or 0
        )
        previews = (existing.get("previewSessions") or []) + (
            project.get("previewSessions") or []
        )
        previews.sort(
            key=lambda s: s.get("last_active") or s.get("started_at") or 0, reverse=True
        )
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the PCBDRAFT_RUNTIME_HOME junk filters — through the
    ...
    projects = sorted(
        merged.values(), key=lambda p: p.get("lastActive") or 0, reverse=True
    )
    return {
        "projects": projects,
...
def _pr_url_from_tool_output(content: str) -> tuple[int, str] | None:
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's PCBDRAFT_RUNTIME_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    the running dashboard/gateway is scoped to (derived from PCBDRAFT_RUNTIME_HOME).
    """
    from pcbdraft.interfaces.tui import profiles as profiles_mod
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the PCBDRAFT_RUNTIME_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    try:
        from pcbdraft.interfaces.tui import profile_describer
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_runtime_home() / "profile-exports"
        try:
