...
from pcbdraft.interfaces.tui.web_deps import LateState, late
...
async def list_mcp_servers(profile: str | None = None):
    from pcbdraft.interfaces.tui.mcp_config import _get_mcp_servers
...
async def replace_mcp_servers(body: MCPServersReplace, profile: str | None = None):
    ...
    """
    from pcbdraft.interfaces.tui.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: str | None = None):
    from pcbdraft.interfaces.tui.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: str | None = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the PCBDRAFT_RUNTIME_HOME
...
async def auth_mcp_server(name: str, request: Request, profile: str | None = None):
    ...
    process_home = str(get_runtime_home().expanduser().resolve(strict=False))

    def _read():
        with _profile_scope(profile):
            return _get_mcp_servers(), str(
                get_runtime_home().expanduser().resolve(strict=False)
            )
    ...
    if not cfg.get("url"):
        raise HTTPException(
            status_code=400, detail="stdio servers authenticate via env keys, not OAuth"
        )
    if cfg.get("headers") and cfg.get("auth") != "oauth":
        raise HTTPException(
            status_code=400, detail="This server uses header/API-key auth, not OAuth"
        )
    cfg["auth"] = "oauth"
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.runtime_home == flow_home
        ...
        with _profile_scope(body.profile or profile):
            with _CONFIG_MUTATION_LOCK:
                ...
                if not isinstance(servers[name], dict):
                    raise HTTPException(
                        status_code=400, detail="Malformed server config"
                    )
...
async def list_mcp_catalog(profile: str | None = None):
    ...
    """
    try:
        from pcbdraft.interfaces.tui import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(
    body: MCPCatalogInstall, profile: str | None = None
):
    ...
    # The -p subprocess rebinds PCBDRAFT_RUNTIME_HOME-derived paths in the child.
