...
3. Running inside a container with no persistent volume mount over the
   PCBDRAFT_RUNTIME_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("pcbdraft.security_audit")
...
def _running_as_root() -> str | None:
    ...
    return (
        ...
        "full host compromise. Run PCBDraft as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(runtime_home: Path | None) -> str | None:
    ...
    return (
        ...
        "PCBDRAFT_RUNTIME_HOME data directory."
    )
...
def _network_listener_without_auth(config: dict | None) -> list[str]:
    ...
    findings: list[str] = []
    try:
        from pcbdraft.services.messaging.platforms.base import is_network_accessible
    except Exception:
        return findings
...
def run_security_audit(
    *, runtime_home: Path | None = None, config: dict | None = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(runtime_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(runtime_home=runtime_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
