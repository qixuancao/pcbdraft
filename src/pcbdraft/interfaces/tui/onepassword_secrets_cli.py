"""CLI handlers for ``internal secrets onepassword ...``.
...
the CLI through OS package managers and signed installers, so PCBDraft expects
...
from pcbdraft.agent.secret_sources import onepassword as op_src
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "PCBDraft resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]pcbdraft --help"
        '"op://Private/OpenAI/api key"[/cyan]\n'
        "  Preview:          [cyan]pcbdraft --help[/cyan]\n"
    ...
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "PCBDraft will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            '[cyan]pcbdraft --help"op://…"[/cyan]'
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]pcbdraft --help[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "run `pcbdraft --help` to turn it on.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "[cyan]pcbdraft[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "next PCBDraft invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]pcbdraft --help[/cyan] if you no longer "
        "need them."
    )
    return 0
