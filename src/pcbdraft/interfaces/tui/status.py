...
from pcbdraft.model.auth import AuthError, resolve_provider
from pcbdraft.model.catalog import provider_label
...
from pcbdraft.model.runtime_provider import resolve_requested_provider
...
def _estop_status_line():
    ...
    """
    try:
        from pcbdraft.agent.estop import get_state
    except ImportError:
        return None
    state = get_state()
    if state is None:
...
def show_status(args):
    ...
    if _effective_provider_label() == "LM Studio":
        from pcbdraft.model.catalog import probe_lmstudio_models
    ...
    if terminal_env == "ssh":
        ...
        runtime = (
            os.getenv("TERMINAL_VERCEL_RUNTIME")
            or terminal_cfg.get("vercel_runtime")
            or "node24"
        )
    ...
    jobs_file = get_runtime_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_runtime_home() / "sessions" / "sessions.json"
