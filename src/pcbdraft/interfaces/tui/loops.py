...
- This module has zero hard dependency on ``cli.TerminalApp``, the gateway
...
def parse_loop_args(text: str) -> dict[str, Any]:
    ...
    if m_times:
        try:
            ...
            result["error"] = (
                f"--times expects a positive integer, got {m_times.group(1)!r}"
            )
            return result
...
def min_interval_seconds() -> int:
    try:
        value = int(
            _loops_config().get("min_interval_seconds", DEFAULT_MIN_INTERVAL_SECONDS)
        )
        return max(5, value)
    except Exception:
...
def self_paced_floor_seconds() -> int:
    try:
        value = int(
            _loops_config().get(
                "self_paced_floor_seconds", DEFAULT_SELF_PACED_FLOOR_SECONDS
            )
        )
        return max(10, value)
    except Exception:
...
def self_paced_ceiling_seconds() -> int:
    ...
    try:
        value = int(
            _loops_config().get(
                "self_paced_ceiling_seconds", DEFAULT_SELF_PACED_CEILING_SECONDS
            )
        )
...
class LoopState:
    ...
    def cadence_label(self) -> str:
        if self.mode == "self_paced":
            live = (
                f", currently {format_interval(self.current_delay)}"
...
def load_loop(session_id: str) -> LoopState | None:
    ...
    try:
        ...
        logger.warning(
            "LoopManager: could not parse stored loop for %s: %s", session_id, exc
        )
        return None
...
class LoopManager:
    ...
    def fire_tick(self) -> str | None:
        ...
        template = (
            WAKEUP_PROMPT_WITH_UNTIL_TEMPLATE if s.until else WAKEUP_PROMPT_TEMPLATE
        )
        return template.format(
            tick=s.ticks_fired, cadence=cadence, prompt=s.prompt, until=s.until
        )
...
def dispatch_loop_command(
    ...
    *,
    ...
    if lower == "pause":
        ...
        return {
            "output": f"⏸ Loop paused: {state.prompt}\nUse /loop resume to continue.",
            "created": False,
        }
    ...
    if lower in {"stop", "clear", "cancel"}:
        had = mgr.clear()
        return {
            "output": "✓ Loop stopped." if had else "No active loop.",
            "created": False,
        }
    ...
    if (
        parsed["interval_seconds"] is not None
        and parsed["interval_seconds"] < state.interval_seconds
    ):
        lines.append(
    ...
    if state.times:
        lines.append(
            f"Runs {state.times} time{'s' if state.times != 1 else ''}, then stops."
        )
    if state.until:
        lines.append(f"Stops when: {state.until}")
    if not state.times and state.max_ticks:
        lines.append(
            f"Backstop budget: {state.max_ticks} ticks (loops.max_ticks; 0 = unlimited)."
        )
    lines.append(
        f"First wakeup {state.remaining_label()}. Controls: /loop status · pause · resume · stop."
    )
    if replacing:
