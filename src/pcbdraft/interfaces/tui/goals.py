"""Persistent session goals — the Ralph loop for PCBDraft.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, PCBDraft feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.TerminalApp`` or the gateway
...
class GoalGate:
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> GoalGate:
        ...
        return cls(
            ...
            timeout_seconds=int(
                data.get("timeout_seconds", DEFAULT_GATE_TIMEOUT_SECONDS)
                or DEFAULT_GATE_TIMEOUT_SECONDS
            ),
            max_retries=int(
                data.get("max_retries", DEFAULT_GATE_MAX_RETRIES)
                or DEFAULT_GATE_MAX_RETRIES
            ),
...
def workspace_fingerprint(cwd: str | None = None) -> str:
...
def _get_session_db() -> Any | None:
    """Return a SessionDB instance for the current PCBDRAFT_RUNTIME_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``runtime_home`` path so profile switches still pick up the right DB.
...
class GoalManager:
    ...
    def _check_gates(self) -> dict[str, Any] | None:
        ...
        for gate in state.gates:
            ...
            if unchanged:
                passed, exit_code, tail = (
                    False,
                    int(gate.last_exit_code or -1),
        ...
        if (
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.pcbdraft/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``internal -p <profile> chat -q "work kanban task <id>"``). The worker's
    ...
    while True:
        ...
        if status == "review":
            ...
            _log(
                f"kanban goal loop: task {task_id} handed off for review by worker after {turns_used} turn(s)"
            )
            return {
        ...
        if status == "changes_requested":
            _log(
                f"kanban goal loop: reviewer returned task {task_id} for changes after {turns_used} turn(s)"
            )
            return {
