...
``cli.py``'s ``TerminalApp`` class. ``TerminalApp`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        """
        from pcbdraft.tools.checkpoint_manager import format_checkpoint_list
    ...
    def _handle_diff_command(self, command: str):
        ...
        from pcbdraft.tools.working_diff import collect_working_diff
        ...
        label = {"working": "Unstaged", "staged": "Staged", "all": "All (vs HEAD)"}[
            mode
        ]
        if stat:
            print(f"\n  {label}:")
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        """
        from pcbdraft.core.runtime_environment import display_runtime_home
        from pcbdraft.interfaces.tui.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_runtime_home()}/state-snapshots/):\n")
    ...
    def _handle_stop_command(self):
        ...
        try:
            from pcbdraft.tools.async_delegation import active_count, interrupt_all
    ...
    def _handle_agents_command(self):
        ...
        try:
            from pcbdraft.tools.async_delegation import list_async_delegations
    ...
    def _handle_paste_command(self):
        ...
        from pcbdraft.interfaces.tui.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from pcbdraft.interfaces.tui.clipboard import (
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        if not pcfg or not pcfg.enabled:
            ...
            relay_fronts = False
            try:
                from pcbdraft.services.messaging.relay import relay_platform_identities
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if target.isdigit():
            ...
            from pcbdraft.interfaces.tui.main import _resolve_session_by_name_or_id
        ...
        msg_count = len(
            [m for m in self._resume_display_history if is_user_originated_turn(m)]
        )
        if self.conversation_history:
            _cprint(
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        if not sub or sub in {"status", "show"}:
            ...
            if repo_root:
                print(
                    "  /worktree new [name] — create one and move this session into it"
                )
            else:
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from pcbdraft.model.configuration import cfg_get, read_raw_config
                ...
                print(
                    f'  "{personality_prompt[:60]}{"..." if len(personality_prompt) > 60 else ""}"'
                )
        else:
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        """
        from pcbdraft.agent.pet import store
        from pcbdraft.agent.pet.manifest import ManifestError
        from pcbdraft.interfaces.tui.pets import (
            _set_active,
            _set_enabled,
            print_pet_gallery,
            set_pet_scale,
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        """
        from pcbdraft.agent.pet import store
        from pcbdraft.agent.pet.generate import orchestrate
        from pcbdraft.agent.pet.generate.imagegen import GenerationError
        from pcbdraft.interfaces.tui.pets import _set_active
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from pcbdraft.interfaces.tui.suggestions_cmd import (
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from pcbdraft.interfaces.tui.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        try:
            from pcbdraft.interfaces.tui.curator import cli_main
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from pcbdraft.interfaces.tui.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        if args and args[0].lower() in {
            ...
            from pcbdraft.interfaces.tui.write_approval_commands import (
            ...
            if out is not None:
                print(out)
                return
        from pcbdraft.interfaces.tui.skills_hub import handle_skills_slash
    ...
    def _handle_learn_command(self, cmd: str):
        ...
        """
        from pcbdraft.agent.learn_prompt import build_learn_prompt
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from pcbdraft.interfaces.tui.init_command import build_init_prompt_for_cwd
    ...
    def _handle_memory_command(self, cmd: str):
        ...
        if store is None:
            ...
            from pcbdraft.tools.memory_tool import load_on_disk_store
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        if "error" in reply.data:
            _cprint(
                f"\033[1;31mBundle subsystem unavailable: {reply.data['error']}{_RST}"
            )
            return
    ...
    def _handle_browser_command(self, cmd: str):
        ...
        if sub == "use" or sub.startswith("use "):
            ...
            from pcbdraft.tools.registry import invalidate_check_fn_cache
            ...
            if arg == "on":
                ...
                from pcbdraft.tools.browser_use_cli import BACKEND_DISABLED
        ...
        if sub.startswith("connect"):
            ...
            try:
                from pcbdraft.tools.browser_tool import cleanup_all_browsers
            ...
            if _already_open:
                ...
                if _launch.launched:
                    ...
                    if _already_open:
                        print(
                            f"   ✓ Chromium-family browser launched and listening on port {_launch_port}"
                        )
                    else:
                        print(
                            f"   ⚠ Browser launched but port {_launch_port} isn't responding yet"
                        )
                        print(
            ...
            print()
            try:
                from pcbdraft.tools.browser_use_cli import is_browser_use_cli_mode
            ...
            if current:
                ...
                try:
                    from pcbdraft.tools.browser_tool import _get_cloud_provider
                ...
                if provider is not None:
                    ...
                    try:
                        from pcbdraft.tools.browser_tool import _get_browser_engine
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from pcbdraft.interfaces.tui.heartbeat import MIN_INTERVAL_SECONDS

            _cprint(f"  Interval too small — minimum is {MIN_INTERVAL_SECONDS}s.")
            return
        if not prompt.strip():
            _cprint(
                "  Usage: /heartbeat every <interval> <prompt> — the prompt is required."
            )
            return
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        from pcbdraft.interfaces.tui.goals import parse_contract
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from pcbdraft.interfaces.tui.loops import dispatch_loop_command
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(
                f"  Custom skins: drop a YAML file in {display_runtime_home()}/skins/\n"
            )
            return
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        state = (
            f"{_Colors.GREEN}enabled{_Colors.RESET}"
            if target
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from pcbdraft.interfaces.tui.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from pcbdraft.interfaces.tui.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from pcbdraft.interfaces.tui.focus_view import format_hidden_line
    ...
    def _handle_reasoning_command(self, cmd: str):
        ...
        if len(parts) < 2:
            ...
            _cprint(
                f"  {_DIM}Usage: /reasoning <none|minimal|low|medium|high|xhigh|max|ultra|show|hide|full|clamp> [--global]{_RST}"
            )
            return
        ...
        if parsed is None:
            ...
            _cprint(
                f"  {_DIM}Valid levels: none, minimal, low, medium, high, xhigh, max, ultra{_RST}"
            )
            _cprint(f"  {_DIM}Display:      show, hide{_RST}")
            _cprint(
                f"  {_DIM}Scope:        session-scoped by default, --global to persist{_RST}"
            )
            return
        ...
        if explicit_global and save_config_value("agent.reasoning_effort", arg):
            ...
            _cprint(
                f"  {_ACCENT}✓ Reasoning effort set to '{arg}' (session only; config save failed){_RST}"
            )
        else:
            _cprint(
                f"  {_ACCENT}✓ Reasoning effort set to '{arg}' (this session — use --global to persist){_RST}"
            )
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        current = (self.config.get("display") or {}).get(
            "tui_status_indicator", DEFAULT_INDICATOR_STYLE
        )
        ...
        if save_config_value("display.tui_status_indicator", arg):
            _cprint(
                f"  {_ACCENT}✓ Busy-indicator style set to '{arg}' (saved to config){_RST}"
            )
            _cprint(f"  {_DIM}The TUI picks up the new style on its next render.{_RST}")
        else:
            _cprint(
                f"  {_ACCENT}✓ Busy-indicator style set to '{arg}' (session only){_RST}"
            )
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from pcbdraft.model.catalog import _is_anthropic_fast_model
        ...
        if explicit_global and save_config_value("agent.service_tier", saved_value):
            ...
            _cprint(
                f"  {_ACCENT}✓ {feature_name} set to {label} (session only; config save failed){_RST}"
            )
        else:
            _cprint(
                f"  {_ACCENT}✓ {feature_name} set to {label} (this session — use --global to persist){_RST}"
            )
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        from pcbdraft.interfaces.tui.debug import run_debug_share
    ...
    def _persist_wake_word_enabled(self, enabled: bool):
        ...
        if save_config_value("wake_word.enabled", enabled):
            _cprint(
                f"{_DIM}Wake word {'enabled' if enabled else 'disabled'} in config "
