...
This wedges ``internal update --check`` (hard failure) and silently degrades the
passive banner check in :mod:`pcbdraft.interfaces.tui.banner` (the fetch is swallowed, the
...
def clear_stale_git_locks(
    repo_root: Path, *, min_age_seconds: int | None = None
) -> list[str]:
    ...
    * no ``git`` process is currently running.
    ...
    cutoff = time.time() - (
        min_age_seconds if min_age_seconds is not None else STALE_LOCK_MIN_AGE_SECONDS
    )
    removed: list[str] = []
