...
def _is_pcbdraft_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for PCBDraft ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current PCBDraft gateway shapes
    ...
    try:
        ...
        return False
    return bool(
        looks_like_gateway_command_line(" ".join(str(part) for part in command))
    )
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``internal update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_pcbdraft_gateway_run_argv(argv):
        return argv
