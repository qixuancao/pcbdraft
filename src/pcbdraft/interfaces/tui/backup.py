...
from pcbdraft.interfaces.tui.sizefmt import format_bytes as _format_size
...
# PCBDRAFT_RUNTIME_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under PCBDRAFT_RUNTIME_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE PCBDRAFT_RUNTIME_HOME
...
@contextmanager
def _backup_operation_lock(runtime_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = runtime_home / ".backup.lock"
...
def _collect_memory_provider_external_paths() -> list[Path]:
    ...
    outside PCBDRAFT_RUNTIME_HOME, resolved from config only (no network, no init).
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside PCBDRAFT_RUNTIME_HOME.
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if st.st_size < 100:  # SQLite minimum viable size (header + 1 page)
        result["message"] = (
            f"too small ({st.st_size} bytes) to be a valid SQLite database"
        )
        return result
...
def copy_db_and_verify(src: Path, dst: Path) -> bool:
    ...
    if not integrity.get("valid"):
        try:
            ...
            pass
        logger.warning(
            "Backup of %s failed integrity verification: %s",
            src,
...
def run_backup(args) -> None:
    ...
    try:
        with _backup_operation_lock(runtime_root):
            _run_backup_locked(args, runtime_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, runtime_root: Path) -> None:
    ...
    print(f"Scanning {display_runtime_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(runtime_root, followlinks=False):
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(runtime_root)
    ...
    # outside PCBDRAFT_RUNTIME_HOME, so the walk above never sees it. Ask the active
    ...
    with (
        _atomic_output_path(out_path) as archive_path,
        zipfile.ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as zf,
    ):
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_runtime_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_runtime_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (runtime_root / "config.yaml").exists()
        has_env = (runtime_root / ".env").exists()
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under PCBDRAFT_RUNTIME_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_runtime_home()}."
            )
...
_QUICK_STATE_FILES = (
    ...
    # are at $PCBDRAFT_RUNTIME_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside PCBDRAFT_RUNTIME_HOME and the entry is silently
...
def _quick_snapshot_root(runtime_home: Path | None = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(runtime_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside PCBDRAFT_RUNTIME_HOME. Kept in sync with
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(runtime_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(
            f"  {len(snaps)} snapshot(s) stored in {display_runtime_home()}/state-snapshots/"
        )
...
def _write_full_zip_backup(out_path: Path, runtime_root: Path) -> Path | None:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(runtime_root):
            return _write_full_zip_backup_locked(out_path, runtime_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, runtime_root: Path) -> Path | None:
    """Write a full zip snapshot of ``runtime_root`` to ``out_path``.
...
def _pre_update_backup_dir(runtime_home: Path | None = None) -> Path:
    home = runtime_home or get_runtime_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of PCBDRAFT_RUNTIME_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<PCBDRAFT_RUNTIME_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    result = _write_full_zip_backup(out_path, runtime_root)
    if result is None:
        return None
