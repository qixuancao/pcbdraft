...
``cli.py``'s ``TerminalApp`` class. ``TerminalApp`` inherits
...
class CLIBillingMixin:
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        if usage is not None and usage.available and format_renews is not None:
            ...
            renews_display = getattr(usage, "renews_display", None) or format_renews(
                usage.renews_at
            )
            ...
            if usage.status == "free":
                _cprint(
                    f"  {_d('> Free · free models only. Run /subscription to reach paid models.')}"
                )
                printed_any = True
            elif usage.status == "low":
                _amt = (
                    f"${usage.total_spendable_usd:,.2f}"
                    if usage.total_spendable_usd is not None
                    else "under $5"
                )
        ...
        from pcbdraft.model.account_usage import nous_credits_lines
    ...
    def _subscription_overview(self, state, manage_url):
        ...
        try:
            from pcbdraft.model.billing_usage import build_usage_model
        ...
        plan_name = (
            (c.tier_name or c.tier_id) if c else (usage.plan_name if usage else None)
        )
        ...
        if not plan_name:
            ...
            usage is not None
            and u_status == "low"
            and usage.total_spendable_usd is not None
        ):
            ...
            _tail = (
                " · view only"
                if view_only
                else (f" · renews {renews_display}" if renews_display else "")
            )
        ...
        if c and c.cancel_at_period_end:
            _when = (
                format_renews(c.cancellation_effective_at)
                or "the end of the billing period"
            )
            _trans = ((c.tier_name or "your plan"), "cancels", _when)
        elif c and c.pending_downgrade_tier_name:
            _when = format_renews(c.pending_downgrade_at) or "the end of the cycle"
            _trans = (
                (c.tier_name or "your plan"),
                c.pending_downgrade_tier_name,
                _when,
            )
        _cprint("")
        if _trans:
        ...
        if (
            usage
            and getattr(usage, "has_topup", False)
            and getattr(usage, "total_spendable_usd", None) is not None
        ):
        ...
        if is_free:
            _cprint(
                f"  {_d('> Paid models need a subscription. Start one to reach them.')}"
            )
        elif u_status == "low":
            _amt = (
                f"${usage.total_spendable_usd:,.2f}"
                if usage is not None and usage.total_spendable_usd is not None
                else "under $5"
            )
    ...
    def _subscription_free_catalog(self, state, manage_url):
        ...
        if not tiers:
            ...
            self._subscription_open_portal(
                state, manage_url, verb="Start a subscription"
            )
            return
        ...
        choices = [
            (t.tier_id, format_tier_row(t), f"start {t.name} on the portal")
            for t in tiers
        ]
    ...
    def _subscription_change_menu(self, state, manage_url):
        ...
        c = state.current
        has_pending = bool(
            c and (c.cancel_at_period_end or c.pending_downgrade_tier_name)
        )
        ...
        if has_pending:
            choices = [
                (
                    "keep",
                    f"Keep {keep_name} (undo the scheduled change)",
        ...
        raw = self._prompt_text_input_modal(
            title="Manage your subscription", detail="", choices=choices
        )
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice == "change":
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        try:
            ...
            if allow_stepup:
                self._subscription_handle_scope_required(
                    state, retry=("preview", tier_id)
                )
            else:
                print(
        ...
        if effect not in ("charge_now", "scheduled"):
            ...
            _cprint(
                f"  🟡 {p.reason or 'This change cannot be confirmed here — manage it on the portal.'}"
            )
        ...
        if effect == "charge_now":
            _amt = (
                f"${p.amount_due_now_cents / 100:.2f}"
                if p.amount_due_now_cents is not None
                else None
            )
            _cprint(f"  {_b('Confirm plan change')}  {_d('· charged now')}")
            if _amt:
                _cprint(
                    f"  Upgrade to {target}. You will be charged {_amt} now (prorated)."
                )
            else:
                _cprint(
                    f"  Upgrade to {target}. You will be charged the prorated amount now."
                )
            ...
            pay_label = (
                f"Pay {_amt} & upgrade now" if _amt else "Upgrade now (prorated charge)"
            )
            ...
            _when = (
                p.effective_at[:10]
                if (p.effective_at and len(p.effective_at) >= 10)
                else "the end of the billing period"
            )
            _cprint(f"  {_b('Confirm plan change')}  {_d('· scheduled · not today')}")
            _cprint(
                f"  Change to {target} — takes effect {_when}. No charge now; you keep your current plan until then."
            )
        ...
        raw = self._prompt_text_input_modal(
            title=pay_label, detail="", choices=confirm_choices
        )
    ...
    def _subscription_confirm_cancel(self, state):
        ...
        c = state.current
        _end = (
            format_renews(c.cycle_ends_at) if (c and c.cycle_ends_at) else None
        ) or "the end of the billing period"
        print()
        _cprint(f"  {_b('Confirm cancellation')}  {_d('· scheduled · not today')}")
        _cprint(
            f"  Cancel {(c.tier_name if c else 'your plan')} — it stays active until {_end}, then won't renew."
        )
        _cprint(
            f"  {_d('You keep your remaining credits for this period. You can resume before it ends.')}"
        )
        confirm_choices = [
        ...
        raw = self._prompt_text_input_modal(
            title="Cancel subscription?", detail="", choices=confirm_choices
        )
        if self._normalize_slash_confirm_choice(raw, confirm_choices) != "yes":
    ...
    def _subscription_apply(
        self, state, action, idempotency_key=None, *, allow_stepup=True
    ):
        ...
        key = None
        if kind == "upgrade":
            from pcbdraft.model.billing_view import new_idempotency_key
        ...
        try:
            if kind == "upgrade":
                try:
                    res = (
                        post_subscription_upgrade(
                            subscription_type_id=arg, idempotency_key=key
                        )
                        or {}
                    )
                except BillingScopeRequired:
                    ...
                    if (
                        _code in ("network_error", "endpoint_unavailable")
                        or _status is None
                        or _status >= 500
                    ):
                ...
                if status == "already_on_tier":
                    ...
                    _cprint(
                        f"  {_DIM}✓ Upgraded to {name}. Your new monthly credits land in a moment.{_RST}"
                    )
                elif status == "requires_action":
                    _cprint(
                        "  🟡 This upgrade needs extra verification (3DS). Finish it on the portal."
                    )
                    if _url:
            ...
            if kind == "schedule":
                ...
                _cprint(
                    f"  {_DIM}✓ Scheduled — your plan doesn't change today. You keep it until the end of the billing period, then it switches.{_RST}"
                )
            elif kind == "cancel":
                put_subscription_pending_change(cancel=True)
                _cprint(
                    f"  {_DIM}✓ Scheduled — your plan stays active until the end of the billing period, then it cancels. Nothing changes today.{_RST}"
                )
            elif kind == "resume":
                delete_subscription_pending_change()
            ...
            if allow_stepup:
                self._subscription_handle_scope_required(
                    state, retry=action, idempotency_key=key
                )
            else:
                print(
                    "  Remote Spending still isn't active for this terminal — the authorization didn't take. Retry, or make this change on the portal."
    ...
    def _subscription_handle_scope_required(
        self, state, *, retry, idempotency_key=None
    ):
        ...
        _cprint(
            f"  {_d('To change your plan from the terminal, allow Remote Spending once. It opens your browser to authorize, then your change picks up right here.')}"
        )
        if not getattr(self, "_app", None):
            print(
        ...
        try:
            from pcbdraft.model.auth import step_up_nous_billing_scope
        ...
        if not granted:
            print(
                "  Couldn't allow Remote Spending — an org admin or owner has to approve it for this org."
            )
            return
        ...
        try:
            from pcbdraft.interfaces.tui import nous_billing as _nb
        ...
        from pcbdraft.agent.subscription_view import build_subscription_state
        ...
        if rkind == "preview":
            ...
            self._subscription_apply(
                fresh, retry, idempotency_key=idempotency_key, allow_stepup=False
            )
    ...
    def _subscription_render_upgrade_ambiguous(self, exc):
        ...
        _cprint(
            f"  {_d('Re-run /subscription to check your plan before trying again.')}"
        )
    ...
    def _billing_overview(self, state):
        ...
        if state.can_change_plan and state.cli_billing_enabled:
            if state.card is not None:
                ...
                _cprint(
                    f"  {_d('No saved card on file — “Add funds” walks you through adding one.')}"
                )
        print(f"  {'─' * 41}")
        ...
        _cprint(
            f"  {_d('Add funds now — a single charge, added to your balance today.')}"
        )
        if (
            ar is not None
            and ar.enabled
    ...
    def _usage_bar_lines(self, usage, plan_name) -> list:
        ...
        if pb is not None and pb.total_usd > 0:
            ...
            lines.append(
                f"  {label}[{bar}]  ${pb.remaining_usd:,.2f} left of ${pb.total_usd:,.2f}{pct_s}"
            )
        tb = getattr(usage, "topup_bar", None) if usage else None
        if tb is not None and tb.remaining_usd > 0:
            lines.append(
                f"  {'top-up'.ljust(8)}[{'█' * 10}]  ${tb.remaining_usd:,.2f} · never expires"
            )
        return lines
    ...
    def _billing_add_card_flow(self, state):
        ...
        _cprint(
            f"  {_d('Add a card once on the portal billing page — after that you can top up right from the terminal.')}"
        )
        choices = [
            (
                "portal",
                "Add a card on the portal",
                "opens the billing page in your browser",
            ),
            (
                "recheck",
                "I've added it — check again",
                "re-check for the card and continue",
            ),
            ("cancel", "Back", "do nothing"),
        ]
        for _ in range(8):  # bounded: portal-open plus a handful of re-checks
            raw = self._prompt_text_input_modal(
                title="Add a card", detail="", choices=choices
            )
            choice = self._normalize_slash_confirm_choice(raw, choices)
            if choice == "portal":
                self._billing_open_portal(state)
                _cprint(
                    f"  {_d('Add the card on the billing page, then pick “check again” here.')}"
                )
                continue
            if choice == "recheck":
                from pcbdraft.model.billing_view import build_billing_state
                ...
                if state.card is not None:
                    _cprint(
                        f"  {_DIM}✓ Card found: {state.card.display} — continuing.{_RST}"
                    )
                    return state
                print(
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        try:
            ...
            self._billing_handle_scope_required(
                state, amount=amount, idempotency_key=key
            )
            return
        except BillingError as exc:
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if (
            isinstance(exc, BillingRemoteSpendingRevoked)
            or code == "remote_spending_revoked"
        ):
            ...
            code in ("cli_billing_disabled", "remote_spending_disabled")
            ...
            print(
                "  🔴 Remote Spending needs approval — run /topup to allow it, then retry."
            )
        else:
            print(f"  🔴 {exc}")
        if portal_url:
    ...
    def _billing_handle_scope_required(
        self, state, *, amount=None, idempotency_key=None
    ):
        ...
        if choice != "yes":
            print(
                "  No charge made. Run /topup when you want to allow Remote Spending."
            )
            return
        print("  Opening your browser to allow Remote Spending…")
        try:
        ...
        if not granted:
            print(
                "  Couldn't allow Remote Spending — an org admin or owner has to approve it. Your card was not charged."
            )
            return
        ...
        resume_choices = [
            (
                "resume",
                f"Resume {format_money(amount)} top-up",
                "finish the held purchase",
            ),
        ...
        from pcbdraft.interfaces.tui.nous_billing import BillingError, post_charge
        from pcbdraft.model.billing_view import new_idempotency_key
