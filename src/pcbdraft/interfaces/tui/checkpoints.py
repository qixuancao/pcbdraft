...
def cmd_status(args: argparse.Namespace) -> int:
    from pcbdraft.tools.checkpoint_manager import store_status
...
def cmd_prune(args: argparse.Namespace) -> int:
    from pcbdraft.tools.checkpoint_manager import prune_checkpoints, store_status
    ...
    if delete_orphans and not args.force:
        ...
        pre_v2_orphans = [
            p for p in info.get("pre_v2_projects", []) if not p.get("exists")
        ]
        if orphans or pre_v2_orphans:
            print(
                f"This will permanently delete {len(orphans) + len(pre_v2_orphans)} "
...
def cmd_clear(args: argparse.Namespace) -> int:
    from pcbdraft.tools.checkpoint_manager import (
        CHECKPOINT_BASE,
        clear_all,
        store_status,
    )
...
def cmd_clear_legacy(args: argparse.Namespace) -> int:
    from pcbdraft.tools.checkpoint_manager import clear_legacy, store_status
