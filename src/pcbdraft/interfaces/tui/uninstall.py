...
def remove_node_symlinks(runtime_home: Path) -> list:
    ...
    """
    node_dir = (runtime_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from pcbdraft.interfaces.tui.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``PCBDRAFT_RUNTIME_HOME`` and ``PCBDRAFT_RUNTIME_GIT_BASH_PATH``
...
def _hermes_path_markers(runtime_home: Path) -> list[str]:
    """Path-entry substrings that identify Hermes-owned User-PATH entries."""
    root = str(runtime_home).rstrip("\\/")
    ...
    # Also match if PCBDRAFT_RUNTIME_HOME was customised to somewhere else — find-and-nuke
...
def remove_path_from_windows_registry(runtime_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(
            ...
            markers = _hermes_path_markers(runtime_home)
            kept: list[str] = []
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete PCBDRAFT_RUNTIME_HOME and PCBDRAFT_RUNTIME_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(
            ...
            for name in ("PCBDRAFT_RUNTIME_HOME", "PCBDRAFT_RUNTIME_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(runtime_home: Path) -> list[Path]:
...
def _is_default_runtime_home(runtime_home: Path) -> bool:
    """Return True when ``runtime_home`` points at the default (non-profile) root."""
    try:
        from pcbdraft.core.runtime_environment import get_default_runtime_root

        return runtime_home.resolve() == get_default_runtime_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its PCBDRAFT_RUNTIME_HOME directory.
    ...
    current PCBDRAFT_RUNTIME_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    ...
    # 3. Wipe the profile's PCBDRAFT_RUNTIME_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$PCBDRAFT_RUNTIME_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from pcbdraft.interfaces.tui.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(runtime_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {runtime_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(runtime_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Hermes agent at {runtime_home / 'hermes-agent'}")
        print(f"  • Your config, sessions, and secrets under {runtime_home}")
        print()
...
def run_uninstall(args):
    ...
    # offer to clean them up too instead of leaving zombie PCBDRAFT_RUNTIME_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_runtime_home(runtime_home)
    ...
    print(f"  Config:  {runtime_home / 'config.yaml'}")
    print(f"  Secrets: {runtime_home / '.env'}")
    print(
        f"  Data:    {runtime_home / 'cron/'}, {runtime_home / 'sessions/'}, {runtime_home / 'logs/'}"
    )
    print()
    ...
    # their alias wrappers, and wiping their PCBDRAFT_RUNTIME_HOME dirs. Otherwise
...
def _print_uninstall_dry_run(
    *, project_root: Path, runtime_home: Path, full_uninstall: bool
) -> None:
    ...
    if full_uninstall:
        print(f"  • Hermes config/data: {runtime_home}")
        if _is_default_runtime_home(runtime_home):
        ...
        print(f"  • Keep Hermes config/data: {runtime_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    wipe ``$PCBDRAFT_RUNTIME_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in runtime_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(runtime_home)))
        )
        ...
        log_info(
            "Removing PCBDRAFT_RUNTIME_HOME / PCBDRAFT_RUNTIME_GIT_BASH_PATH User env vars..."
        )
    ...
    removed_node_links = remove_node_symlinks(runtime_home)
    ...
    #     step-5 rmtree(runtime_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE PCBDRAFT_RUNTIME_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from pcbdraft.interfaces.tui.gui_uninstall import uninstall_gui

        gui_removed = uninstall_gui(runtime_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            ...
            if (
                runtime_home in project_root.parents
                or project_root.parent == runtime_home
            ):
    ...
    #     under PCBDRAFT_RUNTIME_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(runtime_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(runtime_home)
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile PCBDRAFT_RUNTIME_HOME dirs live under
        ...
        try:
            if runtime_home.exists():
                ...
                log_success(f"Removed {runtime_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {runtime_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {runtime_home}")
