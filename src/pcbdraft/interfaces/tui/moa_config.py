...
MOA_MARKER_PREFIX = "__PCBDRAFT_MOA_TURN_V1__"
...
def _default_reference_models() -> list[dict[str, Any]]:
    return [
        {**slot, "enabled": True} for slot in deepcopy(DEFAULT_MOA_REFERENCE_MODELS)
    ]
...
def _coerce_float_or_none(value: Any) -> float | None:
    ...
    provider default applies', matching how a single-model PCBDraft agent
...
def validate_moa_payload(raw: Any) -> list[str]:
    ...
    for name, preset in presets.items():
        ...
        if not complete_refs:
            problems.append(
                f"preset '{label}': needs at least one complete reference model"
            )
...
def _normalize_preset(raw: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "reference_temperature": _coerce_float_or_none(
            raw.get("reference_temperature")
        ),
        "aggregator_temperature": _coerce_float_or_none(
            raw.get("aggregator_temperature")
        ),
...
def exact_moa_preset_name(config: Any, text: str) -> str | None:
    ...
    ``pcbdraft.interfaces.tui/model_switch.py``) to recognize a bare ``/model <preset>``
