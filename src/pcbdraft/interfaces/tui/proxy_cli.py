...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if rotate and existing:
        ...
        try:
            ...
            console.print(
                f"  [yellow]Could not back up mappings before rotation: {exc}[/yellow]"
            )
    ...
    if restart_pref is True:
        ...
        if _sys.stdin.isatty():
            try:
                ans = (
                    input("  Restart the running proxy now with the new config? [Y/n] ")
                    .strip()
                    .lower()
                )
            except EOFError:
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if mappings:
        ...
        for m in mappings:
            ...
            lines.append(
                f"  - {m.real_env_name}: {tok} ({', '.join(m.upstream_hosts)})"
            )
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    table.add_row(
        "Process", f"pid {status.pid}" if status.pid else "[dim](stopped)[/dim]"
    )
