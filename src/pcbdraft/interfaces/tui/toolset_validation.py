...
the tool registry or launching PCBDraft (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``pcbdraft-cli`` to the non-existent ``pcbdraft``. ``resolve_toolset('pcbdraft')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``pcbdraft-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``pcbdraft`` instead of ``pcbdraft-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"pcbdraft-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `pcbdraft --help` to reconfigure."
        )
    return warnings
