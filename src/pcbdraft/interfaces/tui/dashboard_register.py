...
as ``PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID``.
...
This command:
  ...
  3. Writes ``PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL`` into ``~/.hermes/.env`` idempotently.
...
def _resolve_portal_base_url(override: str | None = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL`` env (used for testing against a
...
def _print_post_register_hint(
    *,
    ...
    from pcbdraft.model.configuration import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    PCBDRAFT_RUNTIME_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
...
def cmd_dashboard_register(args) -> None:
    ...
    from pcbdraft.model.configuration import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID
    ...
    # PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get(
        "PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL"
    )
    ...
    # client_id locally (PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(
            f"✗ Failed to write PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}"
        )
        print(
            f"  Set it manually:  PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID={client_id}"
        )
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("PCBDRAFT_RUNTIME_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # that same callback by taking PCBDRAFT_RUNTIME_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as PCBDRAFT_RUNTIME_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("PCBDRAFT_RUNTIME_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("PCBDRAFT_RUNTIME_DASHBOARD_PUBLIC_URL", public_url)
