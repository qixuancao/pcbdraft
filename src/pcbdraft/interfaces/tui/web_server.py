...
def _start_desktop_cron_ticker(
    ...
    real gateway on the same PCBDRAFT_RUNTIME_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from pcbdraft.interfaces.tui.gateway import _get_restart_drain_timeout

        return _get_restart_drain_timeout()
    except ImportError:
        from pcbdraft.services.messaging.restart import (
...
def _eager_reconcile_own_session_db() -> None:
    ...
    """
    try:
        from pcbdraft.services.session_db import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (PCBDRAFT_RUNTIME_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: threading.Event | None = None
    cron_thread: threading.Thread | None = None
    if os.getenv("PCBDRAFT_RUNTIME_DESKTOP") == "1":
        ...
        try:
            from pcbdraft.interfaces.tui.gateway import (
...
from pcbdraft.interfaces.tui.memory_oauth import (
    router as _memory_oauth_router,  # noqa: E402
)
...
# PCBDRAFT_RUNTIME_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get(
        "PCBDRAFT_RUNTIME_DASHBOARD_SESSION_TOKEN"
...
# ---------------------------------------------------------------------------
from pcbdraft.interfaces.tui.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from pcbdraft.interfaces.tui.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from pcbdraft.interfaces.tui.dashboard_auth.token_auth import token_auth_middleware
...
def _memory_provider_options() -> list[str]:
    ...
    options = [""]
    try:
        from pcbdraft.agent.memory_backends import list_memory_provider_names
...
def _custom_provider_options(
    ...
    *kind* is ``"tts"`` or ``"stt"``. The result keeps the built-in display
    ...
    if kind == "tts":
        from pcbdraft.tools.tts_tool import BUILTIN_TTS_PROVIDERS as _runtime_builtins
    else:
        from pcbdraft.tools.transcription_tools import (
    ...
    try:
        if kind == "tts":
            from pcbdraft.agent.tts_registry import (
                list_providers as _list_voice_providers,
            )
        else:
            from pcbdraft.agent.transcription_registry import (
                list_providers as _list_voice_providers,
            )
...
from pcbdraft.interfaces.tui.web_models import (
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from pcbdraft.model.catalog import _KNOWN_PROVIDER_NAMES, normalize_provider
    from pcbdraft.model.configuration import get_compatible_custom_providers
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from pcbdraft.model.catalog import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from pcbdraft.services.session_db import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "PCBDRAFT_RUNTIME_DASHBOARD_FILES_ROOT"
...
# PCBDRAFT_RUNTIME_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to PCBDRAFT_RUNTIME_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no PCBDRAFT_RUNTIME_HOME resolution, so it blocks these trees wherever
...
def _default_runtime_root_is_opt_data() -> bool:
    raw = os.environ.get("PCBDRAFT_RUNTIME_HOME", "").strip()
    if not raw:
        return False
    try:
        from pcbdraft.core.runtime_environment import get_default_runtime_root

        root = get_default_runtime_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    """
    if _default_runtime_root_is_opt_data():
        return True
    try:
        from pcbdraft.core.runtime_environment import is_container
...
def _managed_files_policy(
    ...
    # (the container/hosted layout) or when PCBDRAFT_RUNTIME_DASHBOARD_FILES_ROOT is set.
    if _default_runtime_root_is_opt_data():
        root = (
...
async def upload_chat_image(payload: ChatImageUpload, profile: str | None = None):
    ...
    ``PCBDRAFT_RUNTIME_HOME/images/`` — the same directory ``clipboard.paste`` /
    ...
    def _run():
        ...
        with _profile_scope(profile) as scoped_home:
            home = scoped_home or get_runtime_home()
...
from pcbdraft.interfaces.tui import web_git as _web_git  # noqa: E402
...
from pcbdraft.interfaces.tui.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from pcbdraft.interfaces.tui.web_routers.git import (
...
def _profile_gateway_writer_identity(
    profile_home: Path, runtime: dict | None
) -> tuple | None:
...
def _collect_profile_gateway_topology() -> dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from pcbdraft.core.runtime_process import read_runtime_status
...
# (get_default_runtime_root()) — NOT the profile-scoped PCBDRAFT_RUNTIME_HOME — so every
...
_INSTALL_ID_CACHE: dict[str, str | None] = {"value": None}
_INSTALL_ID_LOCK = threading.Lock()


def _read_or_create_install_id() -> str | None:
...
async def get_status(profile: str | None = None):
    ...
    # across that await. Status only resolves get_runtime_home() at call time
    ...
    try:
        ...
        # PCBDRAFT_RUNTIME_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        auth_flows: list[str] = []
        try:
            from pcbdraft.interfaces.tui.dashboard_auth import (
        ...
        try:
            from pcbdraft.model.auth import get_nous_session_validity
        ...
        try:
            from pcbdraft.services.messaging.readiness import _probe_state_db

            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_runtime_home())
            )
        ...
        try:
            from pcbdraft.services.messaging.memory_status import collect_memory_status

            status["memory"] = await asyncio.get_running_loop().run_in_executor(
                None,
                functools.partial(
                    collect_memory_status,
                    profile_dir if profile_dir else get_runtime_home(),
                ),
            )
        except Exception:
        ...
        # disk-exhaustion incidents). One statvfs call on PCBDRAFT_RUNTIME_HOME's
        ...
        try:
            from pcbdraft.services.messaging.disk_status import collect_disk_status
        ...
        try:
            from pcbdraft.core.runtime_environment import get_runtime_home as _ghh
            from pcbdraft.services.session_db import SessionDB as _SDB
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_runtime_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_curator_status():
    try:
        from pcbdraft.agent import curator
    except Exception as exc:
...
async def get_learning_graph(profile: str | None = None):
    ...
    def _run():
        from pcbdraft.agent.learning_graph import build_learning_graph
...
async def get_learning_node(id: str, profile: str | None = None):
    ...
    from pcbdraft.agent.learning_mutations import node_detail
...
async def delete_learning_node(body: LearningNodeRef):
    ...
    from pcbdraft.agent.learning_mutations import delete_node
...
async def update_learning_node(body: LearningNodeEdit):
    ...
    from pcbdraft.agent.learning_mutations import edit_node
...
def _get_portal_status_sync():
    ...
    auth: dict[str, Any] = {}
    try:
        from pcbdraft.model.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from pcbdraft.interfaces.tui.nous_subscription import (
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from pcbdraft.interfaces.tui.debug import build_debug_share
...
_ACTION_LOG_DIR: Path = get_runtime_home() / "logs"
...
def _spawn_hermes_action(
    ...
    *,
    ...
    action_env = {**os.environ, "PCBDRAFT_RUNTIME_NONINTERACTIVE": "1"}
    ...
    action_id = (env_overrides or {}).get("PCBDRAFT_RUNTIME_ACTION_ID")
    if action_id:
...
def _spawn_gateway_restart(profile: str | None = None) -> tuple[subprocess.Popen, bool]:
    ...
    try:
        from pcbdraft.interfaces.tui.gateway import _reap_unsupervised_gateway_orphans
...
async def gateway_drain(request: Request):
    ...
    active (no ``PCBDRAFT_RUNTIME_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
    ...
    """
    from pcbdraft.services.messaging.drain_control import (
        clear_drain_request,
        drain_requested,
        write_drain_request,
    )
...
async def update_hermes():
    ...
    try:
        proc = _spawn_hermes_action(
            ["update"],
            "hermes-update",
            env_overrides={"PCBDRAFT_RUNTIME_ACTION_ID": action_id},
        )
    except Exception as exc:
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        from pcbdraft.interfaces.tui.banner import check_for_updates

        if force:
            try:
                (get_runtime_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def transcribe_audio_upload(
    ...
    try:
        ...
        from pcbdraft.tools.voice_mode import transcribe_recording
...
async def speak_text(payload: TTSSpeakRequest, profile: str | None = None):
    ...
    try:
        from pcbdraft.tools.tts_tool import text_to_speech_tool
...
def _split_text_for_speak_stream(text: str, cap: int) -> list:
    ...
    """
    from pcbdraft.tools.tts_streaming import (
        SENTENCE_BOUNDARY_RE as _SENTENCE_BOUNDARY_RE,
    )
...
async def speak_stream_ws(ws: "WebSocket") -> None:
    ...
    def _resolve():
        ...
        from pcbdraft.tools.tts_tool import (
            _get_provider,
            _load_tts_config,
    ...
    def _produce():
        from pcbdraft.tools.tts_streaming import SentenceChunker
        from pcbdraft.tools.tts_tool import _strip_markdown_for_tts
...
from pcbdraft.interfaces.tui.web_routers import (
    sessions as _sessions_routes,  # noqa: E402
)

app.include_router(_sessions_routes.list_router)
from pcbdraft.interfaces.tui.web_routers import (
    profiles as _profiles_routes,  # noqa: E402
)
from pcbdraft.interfaces.tui.web_routers.sessions import get_sessions

app.include_router(_profiles_routes.sessions_router)
from pcbdraft.interfaces.tui.web_routers.profiles import (
    get_profiles_sessions,
    get_profiles_sessions_sidebar,
)

app.include_router(_sessions_routes.search_router)
from pcbdraft.interfaces.tui.web_routers.sessions import search_sessions
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_runtime_home() / provider.name / "config.json"
...
def _write_provider_honcho(
    ...
    from pcbdraft.agent.memory_backends.honcho.oauth import (
        ACCESS_TOKEN_PREFIX,
...
def _load_memory_provider(name: str):
    try:
        from pcbdraft.agent.memory_backends import load_memory_provider
...
def _memory_provider_manifest(name: str) -> dict[str, Any]:
    try:
        from pcbdraft.agent.memory_backends import find_provider_dir
...
def _trim_setup_output(value: str | None, limit: int = 4000) -> str:
    text = str(value or "")
    if len(text) <= limit:
        return text
    return f"{text[:limit]}\n... truncated ..."
...
def _install_memory_provider_pip_dependencies(
    ...
    # (PCBDRAFT_RUNTIME_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
    ...
    try:
        from pcbdraft.tools.lazy_deps import install_specs
...
def _read_memory_provider_existing_values(name: str) -> dict[str, Any]:
    ...
    for path in (
        runtime_home / f"{name}.json",
        runtime_home / name / "config.json",
    ):
...
def _env_lookup(env_key: str | None) -> str:
    if not env_key:
        return ""
...
def _field_visible(
    field: dict[str, Any],
    data: dict[str, Any],
    fields_by_key: dict[str, dict[str, Any]] | None = None,
) -> bool:
    when = field.get("when")
    ...
    if provider is not None and hasattr(provider, "save_config"):
        try:
            from pcbdraft.agent.memory_provider import (
                MemoryProvider as _BaseMemoryProvider,
            )
        except Exception:
            provider.save_config(values, str(get_runtime_home()))
            return
...
def _discover_memory_provider_statuses() -> list[dict[str, Any]]:
    ...
    try:
        from pcbdraft.agent.memory_backends import discover_memory_providers
...
async def get_memory_provider_config(
    name: str, surface: str | None = None, profile: str | None = None
):
...
async def get_egress_status():
    ...
    from pcbdraft.interfaces.tui.proxy_cli import format_status_text
...
def get_model_info(profile: str | None = None):
    ...
    try:
        ...
        try:
            from pcbdraft.model.model_metadata import get_model_context_length
        ...
        caps = {}
        try:
            from pcbdraft.agent.models_dev import get_model_capabilities
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            from pcbdraft.model.auth import get_provider_auth_state
    ...
    try:
        ...
        from pcbdraft.model.catalog import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: str | None = None):
    ...
    try:
        from pcbdraft.interfaces.tui.moa_config import (
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from pcbdraft.model.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from pcbdraft.model.configuration import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
        k
...
async def set_env_var(body: EnvVarUpdate, profile: str | None = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from pcbdraft.model.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: str | None = None):
    ...
    the process-level PCBDRAFT_RUNTIME_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: str | None = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from pcbdraft.model.credential_lifecycle import (
...
def _messaging_platform_catalog() -> tuple[dict[str, Any], ...]:
    ...
    """
    from pcbdraft.services.messaging.config import Platform
...
from pcbdraft.interfaces.tui.setup_hidden_env import (
...
def _gateway_platform_config(platform_id: str):
    from pcbdraft.services.messaging.config import Platform, load_gateway_config
...
def _messaging_platform_payload(
    ...
    # PCBDRAFT_RUNTIME_HOME contextvar override (#56986 / #69143), so the profile's
...
async def apply_whatsapp_onboarding(
    pairing_id: str, body: WhatsAppOnboardingApply, profile: str | None = None
):
    with _whatsapp_onboarding_lock:
        _prune_whatsapp_onboarding_sessions()
        record = _whatsapp_onboarding_sessions.get(pairing_id)
...
async def get_messaging_platforms(profile: str | None = None):
    ...
    # load_env() honors the PCBDRAFT_RUNTIME_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    platform_id: str, requested_profile: str | None
) -> str | None:
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    """
    from pcbdraft.services.messaging.config import (
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized PCBDRAFT_RUNTIME_HOME)
...
def _resolve_provider_status(provider_id: str, status_fn) -> dict[str, Any]:
    ...
    try:
        import pcbdraft.model.auth as hauth
    ...
    def _run():
        with _profile_scope(profile):
            ...
            if provider_id == "anthropic":
                cleared = False
                try:
                    from pcbdraft.model.anthropic_adapter import _get_hermes_oauth_file
                ...
                try:
                    from pcbdraft.model.auth import clear_provider_auth
...
try:
    ...
    from pcbdraft.model.anthropic_adapter import _generate_pkce as _generate_pkce_pair
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        from pcbdraft.model.auth import PROVIDER_REGISTRY, _request_device_code
    ...
    if provider_id == "xai-oauth":
        ...
        from pcbdraft.model.auth import _xai_oauth_request_device_code
...
def _nous_poller(session_id: str) -> None:
    ...
    from pcbdraft.model.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from pcbdraft.model.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from pcbdraft.model.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
        ...
        from pcbdraft.model.auth import _save_codex_tokens
...
def _import_sessions_for_profile(
    profile: str | None, sessions: list[dict[str, Any]]
) -> dict[str, Any]:
...
from pcbdraft.interfaces.tui.web_routers.sessions import (
...
def _session_db_read_probe_statements() -> tuple:
    ...
    """
    from pcbdraft.services.session_db_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from pcbdraft.services.session_db import SessionDB, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: str | None) -> None:
    ...
    try:
        ...
        from pcbdraft.model.configuration import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    ...
    profile_home = (
        _cron_profile_home(body.profile)[1] if body.profile else get_runtime_home()
    )
...
async def get_logs(
    ...
    from pcbdraft.interfaces.tui.logs import LOG_FILES, _read_tail

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_runtime_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from pcbdraft.core.runtime_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (PCBDRAFT_RUNTIME_HOME already
    ...
    PCBDRAFT_RUNTIME_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: str | None) -> tuple[str, Path]:
    """Resolve a profile query value to (profile_name, PCBDRAFT_RUNTIME_HOME)."""
    from pcbdraft.interfaces.tui import profiles as profiles_mod
...
def _annotate_cron_job(job: dict[str, Any], profile: str, home: Path) -> dict[str, Any]:
    ...
    annotated["runtime_home"] = str(home)
...
def _call_cron_for_profile(target_profile: str | None, func_name: str, *args, **kwargs):
    ...
    token = set_runtime_home_override(str(home))
    try:
...
def _mutate_cron_for_profile(
    target_profile: str | None, func_name: str, *args, **kwargs
):
...
from pcbdraft.interfaces.tui.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from pcbdraft.interfaces.tui.web_routers.cron import (
    create_cron_job,
    cron_fire_webhook,
    delete_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # test_config_read_guard.py). The PCBDRAFT_RUNTIME_HOME override scopes
...
def _normalize_mcp_server_create(
    ...
    """
    from pcbdraft.interfaces.tui.mcp_config import (
...
from pcbdraft.interfaces.tui.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from pcbdraft.interfaces.tui.web_routers.mcp import (
    add_mcp_server,
    auth_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from pcbdraft.interfaces.tui.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.runtime_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from pcbdraft.tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from pcbdraft.tools.mcp_oauth import HermesTokenStorage, force_interactive_oauth
        from pcbdraft.tools.mcp_oauth_manager import get_manager

        home_token = set_runtime_home_override(flow.runtime_home)
        secret_token = set_secret_scope(
            build_profile_secret_scope(Path(flow.runtime_home))
        )
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        runtime_home=flow.runtime_home,
                    )
                    ...
                    if flow.reconnect_live:
                        from pcbdraft.tools.mcp_tool import reconnect_mcp_server
            ...
            reset_runtime_home_override(home_token)
    except Exception as exc:
        msg = str(exc)
        ...
        try:
            from pcbdraft.tools.mcp_oauth import humanize_oauth_registration_error
...
def _pairing_store(profile: str | None = None):
    ...
    the ``await`` boundary.
    """
    from pcbdraft.services.messaging.pairing import PairingStore
...
async def list_webhooks():
    import pcbdraft.interfaces.tui.webhook as wh
...
async def list_credential_pool():
    from pcbdraft.model.auth import read_credential_pool
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    """
    from pcbdraft.model.auth import suppress_credential_source
    from pcbdraft.model.credential_pool import load_pool
    from pcbdraft.model.credential_sources import find_removal_step
...
def _dashboard_backup_dir() -> Path:
    return get_runtime_home() / "backups"
...
async def list_hooks():
    ...
    def _run():
        from pcbdraft.agent import shell_hooks
        from pcbdraft.model.configuration import load_config as _load_config

        try:
            from pcbdraft.agent.extensions.manager import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <runtime_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_runtime_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
from pcbdraft.interfaces.tui.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from pcbdraft.interfaces.tui.web_routers.skills import (
    install_skill_hub,
...
def _installed_hub_identifiers(profile: str | None = None) -> dict:
    ...
    """
    try:
        from pcbdraft.tools.skills_hub import HubLockFile
...
def _fallback_profile_dicts(profiles_mod) -> list[dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_runtime_home()
            ...
            profiles.append(
                {
                    ...
                    "gateway_running": _safe(
                        lambda entry=entry_path: profiles_mod._check_gateway_running(
                            entry
                        ),
                        False,
                    ),
                    "description": _safe(
                        lambda entry=entry_path: profiles_mod.read_profile_meta(
                            entry
                        ).get("description", ""),
                        "",
                    ),
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local PCBDRAFT_RUNTIME_HOME override so the write lands in the target
    ...
    token = set_runtime_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
        cfg["model"] = _apply_main_model_assignment(
...
def _write_profile_mcp_servers(
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local PCBDRAFT_RUNTIME_HOME override (same mechanism as
    ...
    from pcbdraft.interfaces.tui.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: list[str]) -> int:
    ...
    install.) Scoped to the profile via the PCBDRAFT_RUNTIME_HOME override. Returns the
...
def _profile_scope(profile: str | None):
    ...
    1. ``load_config``/``save_config`` resolve ``get_runtime_home()`` at call
       time — the context-local override from ``set_runtime_home_override``
    ...
    retargeted to the *current* ``get_runtime_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a PCBDRAFT_RUNTIME_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    from pcbdraft.tools import skill_manager_tool as _skill_mgr
    from pcbdraft.tools import skills_tool as _skills_tool

    token = None
    if not requested or requested.lower() == "current":
        profile_dir = get_runtime_home()
    else:
        profile_dir = _resolve_profile_dir(requested)
        token = set_runtime_home_override(str(profile_dir))

    with _SKILLS_PROFILE_LOCK:
        old_home = _skills_tool.PCBDRAFT_RUNTIME_HOME
        old_skills_dir = _skills_tool.SKILLS_DIR
        old_mgr_home = _skill_mgr.PCBDRAFT_RUNTIME_HOME
        old_mgr_skills_dir = _skill_mgr.SKILLS_DIR
        _skills_tool.PCBDRAFT_RUNTIME_HOME = profile_dir
        _skills_tool.SKILLS_DIR = profile_dir / "skills"
        _skill_mgr.PCBDRAFT_RUNTIME_HOME = profile_dir
        _skill_mgr.SKILLS_DIR = profile_dir / "skills"
        try:
            yield profile_dir if token is not None else None
        finally:
            _skills_tool.PCBDRAFT_RUNTIME_HOME = old_home
            _skills_tool.SKILLS_DIR = old_skills_dir
            _skill_mgr.PCBDRAFT_RUNTIME_HOME = old_mgr_home
...
def _config_profile_scope(profile: str | None):
    ...
    ``set_runtime_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_runtime_home()`` at call time
...
def _clear_skills_prompt_cache() -> None:
    ...
    """
    try:
        from pcbdraft.agent.prompt_builder import clear_skills_system_prompt_cache
...
from pcbdraft.interfaces.tui.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from pcbdraft.interfaces.tui.web_routers.tools import (
...
def _find_toolset_provider_row(
    ts_key: str, config: dict, provider: str | None
) -> dict | None:
...
def _probe_modal_backend() -> tuple:
    try:
        from pcbdraft.tools.tool_backend_helpers import has_direct_modal_credentials
...
if sys.platform.startswith("win"):
    try:
        from pcbdraft.interfaces.tui.win_pty_bridge import PtyUnavailableError
        from pcbdraft.interfaces.tui.win_pty_bridge import WinPtyBridge as PtyBridge
    ...
    try:
        from pcbdraft.interfaces.tui.pty_bridge import PtyBridge, PtyUnavailableError
...
from pcbdraft.interfaces.tui.pty_session import (
    PtySessionRegistry,
    RegistryFull,
    run_reaper,  # noqa: E402
)
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[str | None, str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from pcbdraft.interfaces.tui.dashboard_auth.audit import AuditEvent, audit_log
        from pcbdraft.interfaces.tui.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``PCBDRAFT_RUNTIME_TUI_RESUME`` env var —
    matching what ``hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``PCBDRAFT_RUNTIME_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``PCBDRAFT_RUNTIME_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``PCBDRAFT_RUNTIME_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``PCBDRAFT_RUNTIME_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_runtime_home()`` from that env var at its own import, so the child
    ...
    ``PCBDRAFT_RUNTIME_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    # An explicit profile scope below still overrides PCBDRAFT_RUNTIME_HOME afterwards.
    from pcbdraft.tools.environments.local import build_subprocess_env
    ...
    env.setdefault("PCBDRAFT_RUNTIME_TUI_DISABLE_MOUSE", "1")
    env.setdefault("PCBDRAFT_RUNTIME_TUI_INLINE", "1")
    ...
    env["PCBDRAFT_RUNTIME_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["PCBDRAFT_RUNTIME_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["PCBDRAFT_RUNTIME_TUI_RESUME"] = resume

    if sidecar_url:
        env["PCBDRAFT_RUNTIME_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile PCBDRAFT_RUNTIME_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["PCBDRAFT_RUNTIME_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> str | None:
    ...
    1. Explicit ``PCBDRAFT_RUNTIME_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("PCBDRAFT_RUNTIME_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _console_profile_from_ws(ws: WebSocket) -> str | None:
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from pcbdraft.interfaces.tui.console_engine import HermesConsoleEngine
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("PCBDRAFT_RUNTIME_TUI_RESUME") or raw_resume
...
# PCBDRAFT_RUNTIME_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: str | None) -> str:
    ...
    """
    from pcbdraft.interfaces.tui.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    _headless = os.environ.get("PCBDRAFT_RUNTIME_SERVE_HEADLESS") == "1"
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``PCBDRAFT_RUNTIME_HOME/dashboard-themes/<name>.yaml``), inject a minimal
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_runtime_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``PCBDRAFT_RUNTIME_HOME``.
    """
    themes_dir = get_process_runtime_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    3. Project plugins: ./.hermes/plugins/  (only if PCBDRAFT_RUNTIME_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from pcbdraft.agent.extensions.manager import get_bundled_plugins_dir
    ...
    # PCBDRAFT_RUNTIME_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``PCBDRAFT_RUNTIME_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the hermes root (``~/.hermes/plugins``). Scan the default
    # root as well (``get_default_runtime_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``PCBDRAFT_RUNTIME_HOME`` unchanged when it *is* the root), mirroring how
    ...
    user_plugin_roots = [get_process_runtime_home() / "plugins"]
    root_plugins = get_default_runtime_root() / "plugins"
    ...
    if env_var_enabled("PCBDRAFT_RUNTIME_ENABLE_PROJECT_PLUGINS"):
...
_plugins_hub_cache: dict[str, Any] | None = None
...
def _schedule_check_fn_probe(fn) -> threading.Thread | None:
    ...
    def _probe():
        try:
            from pcbdraft.tools.registry import _check_fn_cached
...
def _merged_plugins_hub(force_refresh: bool = False) -> dict[str, Any]:
    ...
    plugins_root_resolved = (get_runtime_home() / "plugins").resolve()
    rows: list[dict[str, Any]] = []
    ...
    for name, version, description, source, dir_str, key in _discover_all_plugins():
        ...
        if provides_tools:
            try:
                from pcbdraft.tools.registry import get_cached_check_fn_result, registry
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from pcbdraft.interfaces.tui.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from pcbdraft.interfaces.tui.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pcbdraft.interfaces.tui.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pcbdraft.interfaces.tui.plugins_cmd import dashboard_remove_user_plugin
...
from pcbdraft.interfaces.tui.dashboard_auth.routes import (
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``PCBDRAFT_RUNTIME_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("PCBDRAFT_RUNTIME_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``PCBDRAFT_RUNTIME_PARENT_PID`` is the Electron Desktop PID, not necessarily this
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``PCBDRAFT_RUNTIME_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("PCBDRAFT_RUNTIME_PARENT_PID")
    start_marker = os.environ.get("PCBDRAFT_RUNTIME_PARENT_START_MARKER")
    nonce = os.environ.get("PCBDRAFT_RUNTIME_PARENT_NONCE")
    ...
    try:
        poll = max(
            0.5, float(os.environ.get("PCBDRAFT_RUNTIME_SERVE_WATCHDOG_POLL_S", "2.0"))
        )
    except (TypeError, ValueError):
        poll = 2.0
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``PCBDRAFT_RUNTIME_SERVE_HEADLESS``), so
    ...
    try:
        from pcbdraft.interfaces.tui.nous_auth_keepalive import (
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID).
            ...
            try:
                from pcbdraft.agent.extensions.bundled.dashboard_auth import (
                    nous as _nous_plugin,
                )
            ...
            # (#54489).
            try:
                from pcbdraft.interfaces.tui.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
    ...
    # PCBDRAFT_RUNTIME_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("PCBDRAFT_RUNTIME_DESKTOP") == "1":
                try:
                    from pcbdraft.interfaces.tui.dashboard_procs import (
            ...
            # for standalone `hermes serve` (no PCBDRAFT_RUNTIME_PARENT_PID env).
            ...
            ready_token = (
                "PCBDRAFT_RUNTIME_BACKEND_READY"
                if headless
                else "PCBDRAFT_RUNTIME_DASHBOARD_READY"
            )
