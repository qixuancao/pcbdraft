...
Env var fallbacks (used when the corresponding arg is not passed):
    - PCBDRAFT_RUNTIME_INFERENCE_MODEL
"""
...
def _validate_explicit_toolsets(
    ...
    if unresolved:
        try:
            from pcbdraft.interfaces.tui.tools_config import _parse_enabled_flag
...
def _write_usage_file(
    path: str | None, result: dict, failure: str | None = None
) -> None:
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to PCBDRAFT_RUNTIME_INFERENCE_MODEL
...
def _run_agent(
    ...
    from pcbdraft.model.catalog import detect_provider_for_model
    ...
    env_model = os.getenv("PCBDRAFT_RUNTIME_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                import pcbdraft.model.model_switch as _ms
    ...
    # TerminalApp._init_agent's wait — it builds AIAgent directly here, so the
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                PCBDRAFT_RUNTIME_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via PCBDRAFT_RUNTIME_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via PCBDRAFT_RUNTIME_YOLO_MODE=1
