    ...
    *,
    ...
    def copy_range(low: int, high: int) -> None:
        ...
        try:
            ...
            retry_low = (
                last_committed_rowid + 1 if last_committed_rowid is not None else low
            )
            if retry_low > high:
                return
            if retry_low == high:
...
def _recover_via_lost_and_found(
    *,
    ...
    from pcbdraft.interfaces.tui.session_lost_and_found import (
...
def recover_session_database(
    ...
    *,
    ...
    try:
        ...
        destination_db: SessionDB | None = None
        ...
        try:
            ...
            if state_meta_inspection.get("available"):
                state_meta_copy_function = (
                    _copy_state_meta_salvage if allow_partial else _copy_state_meta
                )
            ...
            orphan_cleanup = (
                _cleanup_partial_orphans(destination_conn) if allow_partial else None
            )
            derived_metadata = _finalize_derived_metadata(destination_conn)
        finally:
            source_conn.close()
