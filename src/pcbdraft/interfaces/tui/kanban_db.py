...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``PCBDRAFT_RUNTIME_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``PCBDRAFT_RUNTIME_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``PCBDRAFT_RUNTIME_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``PCBDRAFT_RUNTIME_KANBAN_DB``,
``PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT``, and ``PCBDRAFT_RUNTIME_KANBAN_BOARD`` into
...
from pcbdraft.tools.toolsets import get_toolset_names
...
def normalize_reasoning_effort(effort: str | None) -> str | None:
    ...
    """
    from pcbdraft.core.runtime_environment import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    """
    try:
        from pcbdraft.agent.delegation_context import is_delegated_child_process_context

        delegated = is_delegated_child_process_context()
    except Exception:
        delegated = bool(os.environ.get("PCBDRAFT_RUNTIME_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active PCBDRAFT_RUNTIME_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from pcbdraft.interfaces.tui.lifecycle import invoke_hook
...
# ``PCBDRAFT_RUNTIME_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: int | None = None) -> int:
    ...
    ``PCBDRAFT_RUNTIME_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``PCBDRAFT_RUNTIME_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``PCBDRAFT_RUNTIME_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "PCBDRAFT_RUNTIME_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
def kanban_home() -> Path:
    ...
    1. ``PCBDRAFT_RUNTIME_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_runtime_root()``, which already returns ``<root>``
       when ``PCBDRAFT_RUNTIME_HOME`` is ``<root>/profiles/<name>``, and returns
       ``PCBDRAFT_RUNTIME_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``PCBDRAFT_RUNTIME_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    ...
    env = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def kanban_db_path(board: str | None = None) -> Path:
    ...
    1. ``PCBDRAFT_RUNTIME_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: str | None = None) -> Path:
    ...
    ``PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: str | None = None) -> Path:
    ...
    ``PCBDRAFT_RUNTIME_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
class Task:
    ...
    # within an agent loop that propagated ``PCBDRAFT_RUNTIME_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``PCBDRAFT_RUNTIME_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("PCBDRAFT_RUNTIME_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``PCBDRAFT_RUNTIME_KANBAN_DB`` env → ``PCBDRAFT_RUNTIME_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        from pcbdraft.services.session_db import preflight_db_writability
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``PCBDRAFT_RUNTIME_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from pcbdraft.interfaces.tui.sqlite_safe_read import file_length_matches_header
...
def create_task(
    ...
    *,
    ...
    if project_id:
        from pcbdraft.interfaces.tui import projects_db as _pdb
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
# without thrashing. Overridable via ``PCBDRAFT_RUNTIME_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
def _system_memory_sample() -> dict:
    ...
    """
    try:
        from pcbdraft.services.messaging.lifecycle_ledger import sample_memory
...
def count_running_tasks_other_boards(board: str | None = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``PCBDRAFT_RUNTIME_KANBAN_DB``
...
def _memory_pressure_level(sample: Mapping[str, Any] | None = None) -> str:
    ...
    if not sample:
        return "unknown"
    try:
        from pcbdraft.services.messaging.memory_status import classify_pressure
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from pcbdraft.interfaces.tui.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from pcbdraft.interfaces.tui.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from pcbdraft.interfaces.tui.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_worker_cli_toolsets(runtime_home: str | None) -> list[str] | None:
    ...
    task-scoped kanban lifecycle tools when ``PCBDRAFT_RUNTIME_KANBAN_TASK`` is set.
    """
    if not runtime_home:
        return None
    try:
        ...
        token = set_runtime_home_override(runtime_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for PCBDRAFT_RUNTIME_HOME=%r (%s)",
            runtime_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    ``PCBDRAFT_RUNTIME_KANBAN_DB`` / ``PCBDRAFT_RUNTIME_KANBAN_BOARD`` / workspaces_root env
    ...
    from pcbdraft.agent.session_context import _VAR_MAP
    ...
    try:
        env["PCBDRAFT_RUNTIME_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via PCBDRAFT_RUNTIME_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # PCBDRAFT_RUNTIME_HOME never had profiles created.
        pass
    if task.tenant:
        env["PCBDRAFT_RUNTIME_TENANT"] = task.tenant
    env["PCBDRAFT_RUNTIME_KANBAN_TASK"] = task.id
    ...
    # ("work kanban task t_…").
    env["PCBDRAFT_RUNTIME_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["PCBDRAFT_RUNTIME_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["PCBDRAFT_RUNTIME_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["PCBDRAFT_RUNTIME_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["PCBDRAFT_RUNTIME_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["PCBDRAFT_RUNTIME_KANBAN_GOAL_MAX_TURNS"] = str(
    ...
    env["PCBDRAFT_RUNTIME_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["PCBDRAFT_RUNTIME_KANBAN_WORKSPACES_ROOT"])
    ...
    # A worker must NEVER boot the interactive TUI: an inherited PCBDRAFT_RUNTIME_TUI=1
    ...
    cmd = [
        *_resolve_pcbdraft_argv(),
        ...
        profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped PCBDRAFT_RUNTIME_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("PCBDRAFT_RUNTIME_HOME"))
    if worker_toolsets:
