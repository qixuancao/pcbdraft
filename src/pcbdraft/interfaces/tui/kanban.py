"""CLI for the PCBDraft Kanban board — ``internal kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/pcbdraft-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence(
    ...
    - ``running=True``: a gateway is alive for this PCBDRAFT_RUNTIME_HOME and its
    ...
    Used by ``internal kanban create`` (and callers) to warn when a task
    ...
    ``runtime_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different PCBDRAFT_RUNTIME_HOME than the profile the request
    ...
    try:
        from pcbdraft.core.runtime_process import (
            resolve_gateway_liveness,  # type: ignore
        )
    except Exception:
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(profile_dir=runtime_home, use_cache=False)
    except Exception:
...
def build_parser(
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across PCBDraft profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://github.com/qixuancao/pcbdraft#readme "
            "or docs/pcbdraft-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # PCBDRAFT_RUNTIME_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `pcbdraft --help` or the "
            "PCBDRAFT_RUNTIME_KANBAN_BOARD env var). Use `pcbdraft --help` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument(
        ...
        "deterministic branch. See `pcbdraft --help`.",
    )
    ...
    p_list.add_argument(
        "--mine",
        action="store_true",
        help="Filter by $PCBDRAFT_RUNTIME_PROFILE as assignee",
    )
    ...
    p_comment.add_argument(
        "--author",
        default=None,
        help="Author name (default: $PCBDRAFT_RUNTIME_PROFILE or 'user')",
    )
    p_comment.add_argument(
        "--max-len",
        type=int,
    ...
    p_attach.add_argument(
        "--author",
        default=None,
        help="uploaded_by label (default: $PCBDRAFT_RUNTIME_PROFILE or 'user')",
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `pcbdraft --help`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        "(union of ~/.pcbdraft/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        "(default: $PCBDRAFT_RUNTIME_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
        "(default: $PCBDRAFT_RUNTIME_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``internal kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: pcbdraft --help\n"
                "Run 'pcbdraft --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # Fast-fail for clearer CLI UX only. The durable trust boundary is lower in
    # pcbdraft.interfaces.tui.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using PCBDRAFT_RUNTIME_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `pcbdraft --help`.",
                file=sys.stderr,
            )
            return 1
    ...
    # PCBDRAFT_RUNTIME_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `internal kanban repair` into "could not initialize database"
...
def _profile_author() -> str:
    ...
    for env in ("PCBDRAFT_RUNTIME_PROFILE_NAME", "PCBDRAFT_RUNTIME_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("PCBDRAFT_RUNTIME_DELEGATED_CHILD_CONTEXT"))


# ---------------------------------------------------------------------------
# Boards management (internal kanban boards …)
# ---------------------------------------------------------------------------


def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``internal kanban boards <action>``.
    ...
    DB, so a fresh PCBDRAFT_RUNTIME_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `pcbdraft --help`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `pcbdraft --help`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print("  Use `pcbdraft --help` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `internal kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.pcbdraft/profiles/.")
        print("Create one with `pcbdraft --help` before assigning tasks.")
    print()
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `pcbdraft --help`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from pcbdraft.interfaces.tui import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``internal kanban show <id>``
...
def _goal_mode_handoff_rejection(task: kb.Task | None, evidence: str) -> str | None:
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real PCBDraft profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from pcbdraft.interfaces.tui import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from pcbdraft.interfaces.tui import kanban_decompose as decomp
