...
def resolve_bang_cwd(session_key: str | None = None) -> str | None:
    ...
    """
    try:
        from pcbdraft.tools.terminal_tool import _get_env_config, get_session_cwd
...
def check_bang_approval(command: str) -> dict:
    ...
    """
    try:
        from pcbdraft.tools.terminal_tool import _check_all_guards
    except Exception:
...
def _bang_env() -> dict:
    """Environment for a bang command, with PCBDraft-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the PCBDraft session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
