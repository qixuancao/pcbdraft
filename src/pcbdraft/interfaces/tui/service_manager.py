...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from pcbdraft.interfaces.tui.gateway import systemd_start

        systemd_start()

    def stop(self, name: str) -> None:
        from pcbdraft.interfaces.tui.gateway import systemd_stop

        systemd_stop()

    def restart(self, name: str) -> None:
        from pcbdraft.interfaces.tui.gateway import systemd_restart

        systemd_restart()

    def is_running(self, name: str) -> bool:
        from pcbdraft.interfaces.tui.gateway import _probe_systemd_service_running
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from pcbdraft.interfaces.tui.gateway import launchd_start

        launchd_start()

    def stop(self, name: str) -> None:
        from pcbdraft.interfaces.tui.gateway import launchd_stop

        launchd_stop()

    def restart(self, name: str) -> None:
        from pcbdraft.interfaces.tui.gateway import launchd_restart

        launchd_restart()

    def is_running(self, name: str) -> bool:
        from pcbdraft.interfaces.tui.gateway import _probe_launchd_service_running
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's PCBDRAFT_RUNTIME_HOME blindly;
    derive the shared profile root from the current PCBDRAFT_RUNTIME_HOME and map the
    ...
    runtime_home = Path(os.environ.get("PCBDRAFT_RUNTIME_HOME", "/opt/data"))
    if runtime_home.parent.name == "profiles":
        root = runtime_home.parent.parent
    else:
        root = runtime_home
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources PCBDRAFT_RUNTIME_HOME (and any extra env) via with-contenv —
             so e.g. ``-e PCBDRAFT_RUNTIME_HOME=/data/hermes`` is honored at run
        ...
        root PCBDRAFT_RUNTIME_HOME profile" (the implicit profile that exists at
        the top of $PCBDRAFT_RUNTIME_HOME, not under profiles/). It must be
        ...
        would instead look up ``$PCBDRAFT_RUNTIME_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``PCBDRAFT_RUNTIME_HOME``,
        ...
        # guard.
        lines.append("export PCBDRAFT_RUNTIME_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's PCBDRAFT_RUNTIME_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-PCBDRAFT_RUNTIME_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The PCBDRAFT_RUNTIME_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own PCBDRAFT_RUNTIME_HOME and s6 guarantees a
    ...
    def _render_finish_script() -> str:
        ...
        See #51228, #76435.
        """
        from pcbdraft.services.messaging.restart import GATEWAY_FATAL_CONFIG_EXIT_CODE
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${PCBDRAFT_RUNTIME_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the PCBDRAFT_RUNTIME_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e PCBDRAFT_RUNTIME_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{PCBDRAFT_RUNTIME_HOME:=/opt/data}}"\n'
            f'log_dir="$PCBDRAFT_RUNTIME_HOME/logs/gateways/{prof}"\n'
