...
def scan_approval_history(
    db_path: Path | None = None, days: int = 90
) -> list[tuple[str, str]]:
...
def normalize_command(command: str) -> str:
    ...
    """
    from pcbdraft.tools.approval import (
        _rewrite_resolved_runtime_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_runtime_home(command))
...
def derive_glob(normalized: str) -> str | None:
    ...
    unsafe root binary.
    """
    from pcbdraft.tools.approval import _has_allowlist_shell_operator
...
def apply_proposals(proposals: list[Proposal], indices: list[int]) -> set:
    ...
    import pcbdraft.tools.approval as approval_module
...
def approvals_command(args) -> int:
    ...
    if sub == "test":
        from pcbdraft.interfaces.tui.approvals_test import approvals_test_command
