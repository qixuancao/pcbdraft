...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(mcp_servers, dict) and len(mcp_servers) > 0:
            return True
        from pcbdraft.interfaces.tui.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local PCBDRAFT_RUNTIME_HOME override (profile
        ...
        def _discover() -> None:
            token = None
            try:
                from pcbdraft.core.runtime_environment import set_runtime_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``pcbdraft -z "..."`` / ``-q``), the larger
    ...
    try:
        from pcbdraft.model.configuration import DEFAULT_CONFIG, load_config
...
def _discover_mcp_tools_without_interactive_oauth() -> None:
    ...
    try:
        from pcbdraft.tools.mcp_oauth import suppress_interactive_oauth
    except Exception:
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``internal dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``pcbdraft -z``) can construct
