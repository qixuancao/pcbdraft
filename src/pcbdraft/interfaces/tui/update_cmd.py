    ...
    for mod_name in (
        ...
        if mod is not None:
            try:
                ...
                logger.debug(
                    "Could not reload %s for fresh post-update code: %s", mod_name, exc
                )
...
def _run_config_check_fresh() -> tuple:
    ...
    from pcbdraft.model.configuration import check_config_version

    return check_config_version()


def _run_migrate_config_fresh(
    *, interactive: bool = False, quiet: bool = False
) -> dict:
    ...
    """
    _reload_config_modules()
    from pcbdraft.model.configuration import migrate_config
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    """
    from pcbdraft.core.runtime_environment import FIRST_PARTY_MODULE_ROOTS
...
def _print_fts_optimize_available_notice() -> None:
    ...
    try:
        ...
        return
    db_path = get_runtime_home() / "state.db"
...
def _print_parked_branch_skip_warning(
    ...
    if reason == "dirty":
        ...
        why = f"the branch has {count} commit(s) not merged into origin/{target_branch}"
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("PCBDRAFT_RUNTIME_ACTION_ID", "")
...
def _read_project_version() -> str | None:
    ...
    try:
        ...
        with open(
            _m().PROJECT_ROOT / "pyproject.toml", "rb"
        ) as fh:  # windows-footgun: ok — binary mode, tomllib requires bytes
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    ...
    from pcbdraft.interfaces.tui.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from pcbdraft.model.model_catalog import seed_cache_from_checkout
    ...
    try:
        ...
        _state_path = get_runtime_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_runtime_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> str | None:
    ...
    if unmerged.stdout.strip():
        ...
        subprocess.run(git_cmd + ["reset"], check=False, cwd=cwd, capture_output=True)

    from datetime import datetime, timezone

    stash_name = datetime.now(UTC).strftime("hermes-update-autostash-%Y%m%d-%H%M%S")
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_runtime_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_runtime_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    default_home = get_default_runtime_root()
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        from pcbdraft.interfaces.tui import tools_config
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from pcbdraft.interfaces.tui.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from pcbdraft.interfaces.tui.psutil_android import (
        PSUTIL_URL,
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$PCBDRAFT_RUNTIME_HOME/bin/uv``, but on Termux the official
    ...
    """
    from pcbdraft.interfaces.tui.managed_uv import resolve_uv
...
def _npm_lockfile_changed(runtime_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = runtime_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(runtime_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from pcbdraft.core.runtime_environment import is_wsl
    ...
    shared_runtime_root = get_default_runtime_root()
    ...
    if not _m()._npm_lockfile_changed(shared_runtime_root):
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_runtime_root)
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        from pcbdraft.model.configuration import format_docker_update_message
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from pcbdraft.interfaces.tui.banner import _github_compare_behind
            ...
            if counted is not None:
                ...
                print(
                    f"⚕ Update available: {counted} {commits_word} behind {compare_branch}."
                )
            else:
...
def _run_pre_update_backup(args) -> str | None:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of PCBDRAFT_RUNTIME_HOME under
    ...
    try:
        ...
        # get_runtime_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from pcbdraft.model.configuration import get_runtime_home as _get_home
    ...
    try:
        from pcbdraft.interfaces.tui.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from pcbdraft.interfaces.tui.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_runtime_home so the user sees ~/.hermes/...
    try:
        ...
        home = get_runtime_home()
        try:
            display_path = f"{display_runtime_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _write_update_planned_stop_marker(profile_path: Path, pid: int) -> bool:
    ...
    try:
        ...
        from pcbdraft.core.runtime_process import _get_process_start_time
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from pcbdraft.interfaces.tui._scan_venv_blockers import _is_pausable_gateway
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from pcbdraft.interfaces.tui import gateway_windows
        from pcbdraft.interfaces.tui.gateway import find_gateway_pids
    except Exception as exc:
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    ``$PCBDRAFT_RUNTIME_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_runtime_home()) / "bootstrap-cache"
...
def _desktop_app_present(desktop_dir: Path) -> bool:
    ...
    return _m()._desktop_packaged_executable(
        desktop_dir
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_runtime_home added to hermes_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_runtime_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # seed_profile_skills() uses subprocess with an explicit PCBDRAFT_RUNTIME_HOME so
        # it is not affected by sync_skills()'s module-level PCBDRAFT_RUNTIME_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's PCBDRAFT_RUNTIME_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from pcbdraft.interfaces.tui.profiles import backfill_profile_envs
        ...
        # it and warn loudly.
        try:
            from pcbdraft.interfaces.tui.backup import restore_cron_jobs_if_emptied
        ...
        try:
            ...
            if (
                ...
                from pcbdraft.interfaces.tui.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_runtime_home() / ".update_exit_code"
            try:
