...
How it fits the existing auth framework:
  ...
  * :func:`token_auth_middleware` runs OUTERMOST (installed last in
    ...
    :class:`~pcbdraft.interfaces.tui.dashboard_auth.base.TokenPrincipal` to
...
from pcbdraft.interfaces.tui.dashboard_auth import list_token_providers
from pcbdraft.interfaces.tui.dashboard_auth.audit import AuditEvent, audit_log
from pcbdraft.interfaces.tui.dashboard_auth.base import ProviderError, TokenPrincipal
...
def authenticate_token(
    request: Request,
) -> tuple[TokenPrincipal | None, str | None]:
