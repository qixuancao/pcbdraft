...
from pcbdraft.interfaces.tui.dashboard_auth import list_session_providers
...
from pcbdraft.interfaces.tui.dashboard_auth.cookies import (
...
from pcbdraft.interfaces.tui.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /pcbdraft``, the
    ``login_url`` is prefixed (``/pcbdraft/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from pcbdraft.interfaces.tui.dashboard_auth.cookies import detect_https
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["pcbdraft.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``pcbdraft_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``pcbdraft_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from pcbdraft.interfaces.tui.dashboard_auth.cookies import clear_session_cookies
        from pcbdraft.interfaces.tui.dashboard_auth.prefix import prefix_from_request
