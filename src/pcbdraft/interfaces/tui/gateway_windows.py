...
def _preserve_runtime_home_path(path: str | Path) -> str:
    """Render Hermes-owned paths under the configured PCBDRAFT_RUNTIME_HOME spelling.
    ...
    path lives under PCBDRAFT_RUNTIME_HOME.
    """
    candidate = Path(path)
    try:
...
def _current_profile_cli_args() -> list[str]:
    ...
    from pcbdraft.interfaces.tui.gateway import _profile_arg
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("PCBDRAFT_RUNTIME_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get(
        "PCBDRAFT_RUNTIME_GATEWAY_INSTALL_START_ON_LOGIN"
    )
    old_handoff = os.environ.get("PCBDRAFT_RUNTIME_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["PCBDRAFT_RUNTIME_GATEWAY_INSTALL_START_NOW"] = (
                "1" if start_now else "0"
            )
        if start_on_login is not None:
            os.environ["PCBDRAFT_RUNTIME_GATEWAY_INSTALL_START_ON_LOGIN"] = (
                "1" if start_on_login else "0"
            )
        os.environ["PCBDRAFT_RUNTIME_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("PCBDRAFT_RUNTIME_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("PCBDRAFT_RUNTIME_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("PCBDRAFT_RUNTIME_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_script_path() -> Path:
    ...
    (or ``<PCBDRAFT_RUNTIME_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Hermes installs stay self-contained).
    """
    _assert_windows()
    from pcbdraft.model.configuration import get_runtime_home

    script_dir = Path(get_runtime_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``PCBDRAFT_RUNTIME_HOME`` whenever it
    ...
    source checkout only if ``PCBDRAFT_RUNTIME_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports PCBDRAFT_RUNTIME_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    lines.append(f'set "PCBDRAFT_RUNTIME_HOME={runtime_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "PCBDRAFT_RUNTIME_GATEWAY_DETACHED=1"')
    ...
    lines.append(f'set "VIRTUAL_ENV={_preserve_runtime_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_runtime_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_runtime_home_path(entry) for entry in extra_pythonpath],
    ]
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_runtime_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_runtime_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('PCBDRAFT_RUNTIME_HOME')}) = {_quote_vbs_string(runtime_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('PCBDRAFT_RUNTIME_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_runtime_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_runtime_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    runtime_home = str(Path(get_runtime_home()))
    profile_arg = _profile_arg(runtime_home)

    content = _build_gateway_cmd_script(
        python_path, working_dir, runtime_home, profile_arg
    )
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_runtime_home_path(get_python_path())
    )
    project_root = _preserve_runtime_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    runtime_home = str(Path(get_runtime_home()))
    profile_arg = _profile_arg(runtime_home)
    ...
    env_overlay = {
        ...
        "PCBDRAFT_RUNTIME_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_runtime_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (PCBDRAFT_RUNTIME_HOME, VIRTUAL_ENV,
    ...
    from pcbdraft.interfaces.tui.gateway import PROJECT_ROOT
    from pcbdraft.model.configuration import get_runtime_home
    ...
    try:
        runtime_home = str(Path(get_runtime_home()).resolve())
    except Exception:
        runtime_home = ""
    ...
    if runtime_home:
        env_overlay["PCBDRAFT_RUNTIME_HOME"] = runtime_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    log_dir = Path(get_runtime_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env(
        "PCBDRAFT_RUNTIME_GATEWAY_INSTALL_START_NOW"
    )
    ...
    from pcbdraft.interfaces.tui.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from pcbdraft.interfaces.tui.gateway import _profile_arg, find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"    type {Path(get_runtime_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_runtime_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from pcbdraft.model.configuration import get_runtime_home

    runtime_home = Path(get_runtime_home())
    print()
    print("Next steps:")
    print("  hermes gateway status                      # Check status")
    print(f"  type {runtime_home}\\logs\\gateway.log       # View logs")
...
def _print_deep_probes() -> None:
    ...
    if lock_present:
        try:
            from pcbdraft.core.runtime_process import is_gateway_runtime_lock_active
...
def _force_terminate_known_gateway_pids(pids: list[int]) -> int:
    ...
    try:
        from pcbdraft.core.runtime_process import _pid_exists, terminate_pid
    except ImportError:
        return 0
