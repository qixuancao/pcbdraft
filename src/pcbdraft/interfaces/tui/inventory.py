...
def build_models_payload(
    ...
    *,
    ...
    """
    from pcbdraft.model.model_switch import list_authenticated_providers
...
def format_aux_picker_entries(
    ...
    *,
    ...
    for row in rows:
        ...
        entries.append(
            (slug, f"{name}{model_hint}{marker}", list(row.get("models") or []))
        )
    return entries
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    """
    try:
        from pcbdraft.agent.models_dev import get_model_info
    except Exception:
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from pcbdraft.model.catalog import _PROVIDER_LABELS, CANONICAL_PROVIDERS
