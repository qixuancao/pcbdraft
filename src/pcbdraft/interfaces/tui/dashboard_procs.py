...
def _scan_dashboard_processes(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    environment variable ``PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID`` on the spawned
...
def _runtime_home_for_pid(pid: int) -> str | None:
    """Best-effort ``PCBDRAFT_RUNTIME_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("PCBDRAFT_RUNTIME_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"PCBDRAFT_RUNTIME_HOME="):
...
def _profile_key_for_respawn(argv: list[str], runtime_home: str | None = None) -> str:
    """Stable owner key: ``PCBDRAFT_RUNTIME_HOME`` when known, else ``--profile`` / ``-p``.

    ``PCBDRAFT_RUNTIME_HOME`` ending in ``profiles/<name>`` is normalized to
    ...
    if runtime_home:
        try:
            home_path = Path(runtime_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(runtime_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(runtime_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    Each candidate is ``(pid, argv, runtime_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``PCBDRAFT_RUNTIME_HOME``.
    ...
    for _pid, argv, runtime_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, runtime_home)
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    ...
    # backend child, it sets PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot PCBDRAFT_RUNTIME_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _runtime_home_for_pid(pid)
            ...
            if svc_name:
                ...
                if _m()._try_restart_systemd_service(svc_name, pid_cgroup.get(pid)):
                    ...
                    failed_restarts.append(
                        (svc_name, "systemctl restart returned non-zero")
                    )
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
def _runtime_home_dir() -> Path:
    """Resolved Hermes home (PCBDRAFT_RUNTIME_HOME override or ~/.hermes)."""
    override = os.environ.get("PCBDRAFT_RUNTIME_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    # only check the suffix so a relocated PCBDRAFT_RUNTIME_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{runtime_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = (
        ...
        else (_runtime_home_dir() / _REMOTE_LOCK_SUBDIR)
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    ...
    running under PCBDRAFT_RUNTIME_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never PCBDRAFT_RUNTIME_DESKTOP_CHILD_PID entries
