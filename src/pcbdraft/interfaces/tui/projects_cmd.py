"""``internal project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$PCBDRAFT_RUNTIME_HOME/projects.db`` store
(see :mod:`pcbdraft.interfaces.tui.projects_db`).
...
from pcbdraft.interfaces.tui import projects_db as pdb
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'PCBDraft Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``internal project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: pcbdraft --help\nRun 'pcbdraft --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `pcbdraft --help`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `pcbdraft --help`.",
            file=sys.stderr,
        )
        return 1
