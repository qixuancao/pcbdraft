...
def _root() -> Path:
    ...
    # helper's token, including custom PCBDRAFT_RUNTIME_HOME layouts.
    return get_default_runtime_root() / "desktop-ssh"
...
def _open(path: Path, access: int, creation: int, flags: int, share: int = 0):
    ...
    handle = win32file.CreateFile(
        str(path), access, share, _security_attributes(), creation, flags, None
    )
    try:
...
def _ensure_directory(path: Path) -> None:
    ...
    if path.parent != path:
        _ensure_directory(path.parent) if path.parent not in (
            Path(path.anchor),
            path,
        ) and not path.parent.exists() else None
    if not path.exists():
        try:
...
def read_token(path_value: str) -> str:
    ...
    if (
        ...
        or not _HEX32.fullmatch(relative.parts[0])
        or not re.fullmatch(r"[0-9a-f]{16}\.token", relative.parts[1])
    ):
    ...
    try:
        handle = _open(
            path,
            win32con.GENERIC_READ | win32con.READ_CONTROL | win32con.DELETE,
            win32con.OPEN_EXISTING,
            flags,
        )
...
def remove_artifact(path: Path) -> bool:
    ...
    try:
        handle = _open(
            path,
            win32con.DELETE | win32con.READ_CONTROL,
            win32con.OPEN_EXISTING,
...
def process_state(
    pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str
) -> dict[str, Any]:
    import psutil
    ...
    if actual_creation != creation_time_ns:
        return {
            ...
            "owned": False,
            "indeterminate": False,
            "reason": "creation-time",
            "actualCreationTimeNs": str(actual_creation),
            "expectedCreationTimeNs": str(creation_time_ns),
        }
    if not argv:
        return {
            "alive": True,
    ...
    launches_module = argv[1:3] == ["-m", "hermes_cli.main"] or (
        len(argv) > 2 and argv[1] == "-c" and "hermes_cli.main" in argv[2]
    )
    ...
    try:
        ...
        owned = (
            executable_match
            and "--isolated" in argv[serve + 1 :]
            and argv[owner + 1] == spawn_nonce
        )
    ...
    return {
        ...
        "owned": owned,
        "indeterminate": False,
        "creationTimeNs": str(actual_creation),
        "reason": "owned" if owned else "argv",
        "argv": argv[:20],
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    creation_time_ns = int(
        __import__("psutil").Process(process.pid).create_time() * 1_000_000_000
    )
    return {
        "pid": process.pid,
        "creationTimeNs": str(creation_time_ns),
...
def inspect_hermes(hermes_path: str) -> dict[str, Any]:
    ...
    return {
        ...
        "supported": "--ssh-session-token-file" in help_text
        and "--ssh-owner-nonce" in help_text,
    }
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        ...
        return {
            ...
            "hermesHome": str(get_default_runtime_root()),
    ...
    if operation == "terminate" and len(argv) == 5:
        return {
            "terminated": terminate_owned(int(argv[1]), int(argv[2]), argv[3], argv[4])
        }
