...
def normalize_export_only(only: str | None) -> ExportOnly | None:
...
def export_record_count(
    sessions: Iterable[dict[str, Any]], *, only: str | None = None
) -> tuple[int, str]:
...
def _render_jsonl(sessions: list[dict[str, Any]], *, only: ExportOnly | None) -> str:
...
def _render_markdown(sessions: list[dict[str, Any]], *, only: ExportOnly | None) -> str:
    if only == "user-prompts":
...
def normalize_save_format(fmt: str | None) -> str:
...
def render_session_for_save(session: dict[str, Any], fmt: str) -> str:
    ...
    if fmt == "html":
        from pcbdraft.interfaces.tui.session_export_html import generate_html_export
