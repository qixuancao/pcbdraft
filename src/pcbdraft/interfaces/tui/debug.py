...
def _pending_file() -> Path:
    ...
    """
    return get_runtime_home() / "pastes" / "pending.json"
...
def _primary_log_path(log_name: str) -> Path | None:
    ...
    from pcbdraft.interfaces.tui.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_runtime_home() / "logs" / filename) if filename else None
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    ...
    suggests ``--local``.
    """
    from pcbdraft.interfaces.tui.diagnostics_upload import share_to_nous
