...
def _normalize_skills(
    single_skill=None, skills: Iterable[str] | None = None
) -> list[str] | None:
    if skills is None:
    ...
    for job in jobs:
        ...
        if last_status:
            ...
            if last_status == "ok":
                ...
                if streak >= 2:
                    status_display += color(
                        f"  ({streak} failures in a row)", Colors.RED
                    )
...
def cron_create(args):
    ...
    if job_data.get("monitor_script"):
        print(
            f"  Monitor: {job_data['monitor_script']} (agent runs only on output change)"
        )
    if job_data.get("monitor_url"):
        print(
            f"  Monitor: {job_data['monitor_url']} (agent runs only on output change)"
        )
...
def cron_edit(args):
    ...
    if updated.get("monitor_script"):
        print(
            f"  Monitor: {updated['monitor_script']} (agent runs only on output change)"
        )
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (PCBDRAFT_RUNTIME_SESSION_KEY);
        ...
        try:
            from pcbdraft.agent.session_context import _SESSION_ASYNC_DELIVERY
    ...
    try:
        ...
        if action == "delete":
            ...
            if notepad.delete_note(job_id, key):
                print(
                    color(
                        f"Deleted notepad key '{key}' for job {job_id}.", Colors.GREEN
                    )
                )
                return 0
            print(color(f"No notepad key '{key}' for job {job_id}.", Colors.YELLOW))
