...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"PCBDraft could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `pcbdraft connect` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but PCBDraft cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`pcbdraft connect` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `pcbdraft connect`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"PCBDraft could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `pcbdraft connect` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"PCBDraft could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `pcbdraft connect` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `pcbdraft connect` to refresh PCBDraft."
        return message
...
def _no_paid_access_message(
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f" member spend cap at {billing_url}, then run `pcbdraft connect`"
            f" to refresh."
        )
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from pcbdraft.model.auth import _decode_jwt_claims
    except Exception:
        return None
...
def _paid_service_access_from_payload(value: Any) -> NousPaidServiceAccessInfo | None:
    ...
    return NousPaidServiceAccessInfo(
        ...
        member_spend_cap_remaining_usd=_coerce_float(
            value.get("member_spend_cap_remaining_usd")
        ),
    )
