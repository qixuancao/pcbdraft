...
from pcbdraft.interfaces.tui._subprocess_compat import suppress_platform_ver_console
...
from pcbdraft.interfaces.tui import _early_recovery as _early_recovery_mod
from pcbdraft.interfaces.tui import _startup_fast  # noqa: E402
...
def _cleanup_oneshot_runtime() -> None:
    ...
    try:
        from pcbdraft.tools.terminal_tool import cleanup_all_environments

        cleanup_all_environments()
    except Exception:
        pass
    try:
        from pcbdraft.tools.async_delegation import interrupt_all

        interrupt_all(reason="oneshot shutdown")
    except Exception:
        pass
    try:
        from pcbdraft.tools.browser_tool import _emergency_cleanup_all_sessions

        _emergency_cleanup_all_sessions()
    except Exception:
        pass
    try:
        from pcbdraft.tools.mcp_tool import shutdown_mcp_servers

        shutdown_mcp_servers()
    except BaseException:
        pass
    try:
        from pcbdraft.model.auxiliary_client import shutdown_cached_clients
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("PCBDRAFT_RUNTIME_HOME")
        if home:
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``PCBDRAFT_RUNTIME_TUI=1``, then a real-TTY gate (a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("PCBDRAFT_RUNTIME_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``PCBDRAFT_RUNTIME_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("PCBDRAFT_RUNTIME_TUI_NO_EARLY_DISABLE") == "1":
        return
...
def _active_profile_may_override_home_fast(runtime_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(runtime_root)
...
from pcbdraft.interfaces.tui.subcommands.acp import build_acp_parser
from pcbdraft.interfaces.tui.subcommands.approvals import build_approvals_parser
from pcbdraft.interfaces.tui.subcommands.auth import build_auth_parser
from pcbdraft.interfaces.tui.subcommands.backup import build_backup_parser
from pcbdraft.interfaces.tui.subcommands.claw import build_claw_parser
from pcbdraft.interfaces.tui.subcommands.config import build_config_parser
from pcbdraft.interfaces.tui.subcommands.console import build_console_parser
from pcbdraft.interfaces.tui.subcommands.cron import build_cron_parser
from pcbdraft.interfaces.tui.subcommands.dashboard import build_dashboard_parser
from pcbdraft.interfaces.tui.subcommands.debug import build_debug_parser
from pcbdraft.interfaces.tui.subcommands.doctor import build_doctor_parser
from pcbdraft.interfaces.tui.subcommands.dump import build_dump_parser
from pcbdraft.interfaces.tui.subcommands.gateway import build_gateway_parser
from pcbdraft.interfaces.tui.subcommands.gui import build_gui_parser
from pcbdraft.interfaces.tui.subcommands.hooks import build_hooks_parser
from pcbdraft.interfaces.tui.subcommands.import_agent import build_import_agent_parser
from pcbdraft.interfaces.tui.subcommands.import_cmd import build_import_cmd_parser
from pcbdraft.interfaces.tui.subcommands.insights import build_insights_parser
from pcbdraft.interfaces.tui.subcommands.login import build_login_parser
from pcbdraft.interfaces.tui.subcommands.logout import build_logout_parser
from pcbdraft.interfaces.tui.subcommands.logs import build_logs_parser
from pcbdraft.interfaces.tui.subcommands.mcp import build_mcp_parser
from pcbdraft.interfaces.tui.subcommands.memory import build_memory_parser
from pcbdraft.interfaces.tui.subcommands.model import build_model_parser
from pcbdraft.interfaces.tui.subcommands.monitoring import build_monitoring_parser
from pcbdraft.interfaces.tui.subcommands.pairing import build_pairing_parser
from pcbdraft.interfaces.tui.subcommands.pause import build_pause_parser
from pcbdraft.interfaces.tui.subcommands.plugins import build_plugins_parser
from pcbdraft.interfaces.tui.subcommands.profile import build_profile_parser
from pcbdraft.interfaces.tui.subcommands.prompt_size import build_prompt_size_parser
from pcbdraft.interfaces.tui.subcommands.security import build_security_parser
from pcbdraft.interfaces.tui.subcommands.setup import build_setup_parser
from pcbdraft.interfaces.tui.subcommands.skills import build_skills_parser
from pcbdraft.interfaces.tui.subcommands.skin import build_skin_parser
from pcbdraft.interfaces.tui.subcommands.slack import build_slack_parser
from pcbdraft.interfaces.tui.subcommands.status import build_status_parser
from pcbdraft.interfaces.tui.subcommands.sync import build_sync_parser
from pcbdraft.interfaces.tui.subcommands.tools import build_tools_parser
from pcbdraft.interfaces.tui.subcommands.uninstall import build_uninstall_parser
from pcbdraft.interfaces.tui.subcommands.update import build_update_parser
from pcbdraft.interfaces.tui.subcommands.verify import build_verify_parser
from pcbdraft.interfaces.tui.subcommands.version import build_version_parser
from pcbdraft.interfaces.tui.subcommands.webhook import build_webhook_parser
from pcbdraft.interfaces.tui.subcommands.whatsapp import build_whatsapp_parser
...
#
# Many modules cache PCBDRAFT_RUNTIME_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("PCBDRAFT_RUNTIME_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.hermes/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set PCBDRAFT_RUNTIME_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    # 1.5 If PCBDRAFT_RUNTIME_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.hermes/profiles/coder or
    # /opt/data/profiles/coder).  If PCBDRAFT_RUNTIME_HOME points to the hermes root
    # instead (e.g. systemd hardcodes PCBDRAFT_RUNTIME_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    runtime_home_env = os.environ.get("PCBDRAFT_RUNTIME_HOME", "")
    if profile_name is None and runtime_home_env:
        if Path(runtime_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the hermes root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as PCBDRAFT_RUNTIME_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # "the root PCBDRAFT_RUNTIME_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get(
        "PCBDRAFT_RUNTIME_S6_SUPERVISED_CHILD"
    ):
        try:
            from pcbdraft.core.runtime_environment import get_default_runtime_root

            active_path = get_default_runtime_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set PCBDRAFT_RUNTIME_HOME
    if profile_name is not None:
        try:
            from pcbdraft.interfaces.tui.profiles import resolve_profile_env

            runtime_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            runtime_home = _resolve_sudo_user_profile_env(profile_name)
            if not runtime_home:
            ...
            return
        os.environ["PCBDRAFT_RUNTIME_HOME"] = runtime_home
...
# Bridge security.redact_secrets from config.yaml → PCBDRAFT_RUNTIME_REDACT_SECRETS env
...
try:
    ...
    from pcbdraft.model.configuration import read_raw_config as _read_raw_early

    _cfg_path = get_runtime_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "PCBDRAFT_RUNTIME_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["PCBDRAFT_RUNTIME_REDACT_SECRETS"] = str(
                        _early_redact
                    ).lower()
...
try:
    from pcbdraft.core.runtime_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from pcbdraft.core.runtime_environment import (
            apply_ipv4_preference as _apply_ipv4,
        )
...
from pcbdraft.model.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_runtime_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("PCBDRAFT_RUNTIME_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _sync_bundled_skills_for_startup() -> bool:
    ...
    from pcbdraft.tools.skills_sync import sync_skills
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("PCBDRAFT_RUNTIME_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from pcbdraft.model.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from pcbdraft.model.configuration import split_model_config_default
    ...
    auth_file = get_runtime_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from pcbdraft.model.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from pcbdraft.interfaces.tui.terminal_breadcrumbs import (
                    resolve_breadcrumb_session,
                )
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``PCBDRAFT_RUNTIME_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("PCBDRAFT_RUNTIME_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``PCBDRAFT_RUNTIME_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("PCBDRAFT_RUNTIME_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "PCBDRAFT_RUNTIME_HOME": runtime_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(runtime_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("PCBDRAFT_RUNTIME_QUIET"):
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (PCBDRAFT_RUNTIME_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("PCBDRAFT_RUNTIME_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $PCBDRAFT_RUNTIME_HOME/node tree,
        ...
        if not path and bin == "node":
            try:
                from pcbdraft.interfaces.tui.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("PCBDRAFT_RUNTIME_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with PCBDRAFT_RUNTIME_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset PCBDRAFT_RUNTIME_TUI_DIR (e.g. `unset PCBDRAFT_RUNTIME_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not skip_install_for_fresh_termux_bundle and _tui_need_npm_install(tui_dir):
        ...
        if result.returncode != 0:
            ...
            from pcbdraft.interfaces.tui.npm_engine import maybe_repair_npm_engine
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from pcbdraft.interfaces.tui.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("PCBDRAFT_RUNTIME_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["PCBDRAFT_RUNTIME_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("PCBDRAFT_RUNTIME_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["PCBDRAFT_RUNTIME_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("PCBDRAFT_RUNTIME_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["PCBDRAFT_RUNTIME_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["PCBDRAFT_RUNTIME_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    from pcbdraft.tools.environments.local import build_subprocess_env
    ...
    env["PCBDRAFT_RUNTIME_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["PCBDRAFT_RUNTIME_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["PCBDRAFT_RUNTIME_MODEL"] = model
        env["PCBDRAFT_RUNTIME_INFERENCE_MODEL"] = model
    if provider:
        env["PCBDRAFT_RUNTIME_TUI_PROVIDER"] = provider
        env["PCBDRAFT_RUNTIME_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["PCBDRAFT_RUNTIME_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["PCBDRAFT_RUNTIME_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["PCBDRAFT_RUNTIME_TUI_SKILLS"] = value
    if query:
        env["PCBDRAFT_RUNTIME_TUI_QUERY"] = query
    if image:
        env["PCBDRAFT_RUNTIME_TUI_IMAGE"] = image
    if checkpoints:
        env["PCBDRAFT_RUNTIME_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["PCBDRAFT_RUNTIME_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["PCBDRAFT_RUNTIME_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["PCBDRAFT_RUNTIME_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["PCBDRAFT_RUNTIME_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["PCBDRAFT_RUNTIME_ACCEPT_HOOKS"] = "1"
    ...
    # PCBDRAFT_RUNTIME_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set PCBDRAFT_RUNTIME_TUI_RESUME themselves.
    env.pop("PCBDRAFT_RUNTIME_TUI_RESUME", None)
    if resume_session_id:
        env["PCBDRAFT_RUNTIME_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``PCBDRAFT_RUNTIME_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("PCBDRAFT_RUNTIME_KANBAN_BOARD"):
        return
    try:
        from pcbdraft.interfaces.tui.kanban_db import get_current_board

        os.environ["PCBDRAFT_RUNTIME_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      4. ``PCBDRAFT_RUNTIME_TUI=1`` env   → TUI
    ...
    try:
        ...
        return False
    if os.environ.get("PCBDRAFT_RUNTIME_TUI") == "1":
        return True
    try:
        from pcbdraft.model.configuration import load_config
...
def cmd_chat(args):
    ...
    if in_dir:
        ...
        from pcbdraft.tools.environments.local import _msys_to_windows_path
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        from pcbdraft.interfaces.tui.foreign_sessions import (
    ...
    try:
        from pcbdraft.interfaces.tui.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
        ...
        from pcbdraft.model.configuration import (
            load_config as _load_config_for_xai_check,
        )
    ...
    if _termux_should_prefetch_update_check():
        try:
            from pcbdraft.interfaces.tui.banner import (
    ...
    if getattr(args, "yolo", False):
        os.environ["PCBDRAFT_RUNTIME_YOLO_MODE"] = "1"
    ...
    if getattr(args, "ignore_user_config", False):
        os.environ["PCBDRAFT_RUNTIME_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["PCBDRAFT_RUNTIME_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["PCBDRAFT_RUNTIME_SESSION_SOURCE"] = args.source
    ...
    from pcbdraft.interfaces.tui.app import main as cli_main
...
def cmd_gateway(args):
    ...
    from pcbdraft.interfaces.tui.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from pcbdraft.interfaces.tui.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from pcbdraft.model.configuration import get_env_value, save_env_value
    ...
    from pcbdraft.services.messaging.platforms.whatsapp_common import (
    ...
    session_dir = get_runtime_home() / "whatsapp" / "session"
...
def cmd_whatsapp_cloud(args):
    ...
    from pcbdraft.interfaces.tui.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from pcbdraft.interfaces.tui.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from pcbdraft.model.catalog import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    effective_provider = (
        config_provider or os.getenv("PCBDRAFT_RUNTIME_INFERENCE_PROVIDER") or "auto"
    )
...
def _prompt_api_key(
    ...
    from pcbdraft.model.env_loader import format_secret_source_suffix
...
def cmd_login(args):
    ...
    from pcbdraft.model.auth import login_command
...
def cmd_logout(args):
    ...
    from pcbdraft.model.auth import logout_command
...
def cmd_auth(args):
    ...
    from pcbdraft.model.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from pcbdraft.interfaces.tui.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from pcbdraft.interfaces.tui.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub == "device":
        from pcbdraft.tools import skills_sync_client as ssc
    ...
    if sub in {"enable", "disable"}:
        from pcbdraft.tools.skill_usage import is_curation_eligible, set_sync
    ...
    if sub == "status":
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set PCBDRAFT_RUNTIME_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "PCBDRAFT_RUNTIME_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    if not ssc.resolve_sync_base_url():
        print(
            ...
            "or PCBDRAFT_RUNTIME_SYNC_BASE_URL).",
            file=sys.stderr,
        )
        return 1
...
def cmd_webhook(args):
    ...
    from pcbdraft.interfaces.tui.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from pcbdraft.interfaces.tui.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from pcbdraft.interfaces.tui.kanban import kanban_command
...
def cmd_project(args):
    ...
    from pcbdraft.interfaces.tui.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from pcbdraft.interfaces.tui.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from pcbdraft.interfaces.tui.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from pcbdraft.interfaces.tui.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    ...
    from pcbdraft.interfaces.tui.approvals_suggest import approvals_command
...
def cmd_debug(args):
    ...
    from pcbdraft.interfaces.tui.debug import run_debug
...
def cmd_config(args):
    ...
    from pcbdraft.model.configuration import config_command
...
def cmd_skin(args):
    ...
    from pcbdraft.interfaces.tui.skin_cmd import skin_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from pcbdraft.interfaces.tui.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from pcbdraft.interfaces.tui.backup import run_backup
...
def cmd_import(args):
    ...
    from pcbdraft.interfaces.tui.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from pcbdraft.interfaces.tui.slash_exec import CommandContext, execute_command
    from pcbdraft.model.configuration import detect_install_method
    ...
    try:
        ...
        from pcbdraft.model.configuration import recommended_update_command
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from pcbdraft.interfaces.tui.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from pcbdraft.interfaces.tui.uninstall import run_gui_uninstall
    ...
    from pcbdraft.interfaces.tui.uninstall import run_uninstall
...
# validated against. Lives next to the checkout (NOT in PCBDRAFT_RUNTIME_HOME) because
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $PCBDRAFT_RUNTIME_HOME."""
    from pcbdraft.core.runtime_environment import get_runtime_home

    return get_runtime_home() / "web-ui-build-stamp.json"
...
#
# Stamp file: $PCBDRAFT_RUNTIME_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $PCBDRAFT_RUNTIME_HOME."""
    from pcbdraft.core.runtime_environment import get_runtime_home

    return get_runtime_home() / "desktop-build-stamp.json"
...
def _windows_native_machine_from_iswow64() -> str | None:
...
def _rollback_desktop_from_backup(packaged_executable: Path) -> Path | None:
...
def _ensure_desktop_exe_launchable(
    desktop_dir: Path, packaged_executable: Path | None
) -> tuple:
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the PCBDRAFT_RUNTIME_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    ...
    """
    try:
        from pcbdraft.interfaces.tui.linux_desktop_entry import (
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from pcbdraft.core.runtime_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["PCBDRAFT_RUNTIME_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["PCBDRAFT_RUNTIME_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "runtime_root", None):
        env["PCBDRAFT_RUNTIME_DESKTOP_HERMES_ROOT"] = str(
            Path(args.runtime_root).expanduser().resolve()
        )
    if getattr(args, "cwd", None):
        env["PCBDRAFT_RUNTIME_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["PCBDRAFT_RUNTIME_DESKTOP_CWD"] = os.getcwd()
    ...
    # `PCBDRAFT_RUNTIME_DESKTOP_DISABLE_GPU=... hermes desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = (
        _desktop_launch_options()
    )
    if (
        config_disable_gpu != "auto"
        and "PCBDRAFT_RUNTIME_DESKTOP_DISABLE_GPU" not in os.environ
    ):
        env["PCBDRAFT_RUNTIME_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `PCBDRAFT_RUNTIME_DESKTOP_PASSWORD_STORE=... hermes desktop` keeps working.
    if (
        sys.platform == "linux"
        and "PCBDRAFT_RUNTIME_DESKTOP_PASSWORD_STORE" not in os.environ
    ):
        password_store = (
        ...
        if password_store:
            env["PCBDRAFT_RUNTIME_DESKTOP_PASSWORD_STORE"] = password_store
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    ...
    log_path = get_runtime_home() / "logs" / "dashboard-restart.log"
    try:
...
# Install-scoped breadcrumbs live next to the venv (not under $PCBDRAFT_RUNTIME_HOME)
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from pcbdraft.interfaces.tui import _install_repair as _ir
        ...
        from pcbdraft.interfaces.tui.managed_uv import ensure_uv
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from pcbdraft.core.runtime_environment import venv_bin_dir
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # hermes_cli.config.get_runtime_home to simulate setup failure.
        from pcbdraft.model.configuration import get_runtime_home as _get_runtime_home
...
def cmd_update(args):
    ...
    from pcbdraft.interfaces.tui.update_lock import (
...
def cmd_profile(args):
    ...
    from pcbdraft.interfaces.tui.profiles import (
        _get_wrapper_dir,
    ...
    if action == "list":
        ...
        try:
            ...
            if clone_config or clone_all:
                try:
                    from pcbdraft.agent.memory_backends.honcho.cli import (
        ...
        from pcbdraft.interfaces.tui import profiles as _profiles_mod
        ...
        from pcbdraft.interfaces.tui import profile_describer as _pd
        ...
        from pcbdraft.interfaces.tui.profiles import profile_exists, validate_alias_name
        ...
        from pcbdraft.interfaces.tui.profiles import rename_profile
        ...
        from pcbdraft.interfaces.tui.profiles import export_profile
        ...
        from pcbdraft.interfaces.tui.profiles import import_profile
        ...
        from pcbdraft.interfaces.tui.profile_distribution import (
            DistributionError,
...
def _render_distribution_plan(plan) -> None:
    ...
    from pcbdraft.interfaces.tui.profile_distribution import MANIFEST_FILENAME
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from pcbdraft.interfaces.tui.web_server import should_require_auth
    ...
    try:
        from pcbdraft.interfaces.tui.dashboard_auth import list_providers
    ...
    if choice == "2":
        ...
        print(
            ...
            "PCBDRAFT_RUNTIME_DASHBOARD_OAUTH_CLIENT_ID into ~/.hermes/.env for you.\n"
    ...
    try:
        from pcbdraft.agent.extensions.bundled.dashboard_auth.basic import hash_password
    except Exception as exc:
    ...
    try:
        from pcbdraft.interfaces.tui.plugins_cmd import (
            ensure_basic_auth_plugin_enabled_in_config,
        )
        from pcbdraft.model.configuration import load_config, save_config
    ...
    try:
        from pcbdraft.agent.extensions.manager import discover_plugins
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from pcbdraft.interfaces.tui.windows_ssh_runtime import read_token
    ...
    # expanded against the account's $HOME, independent of PCBDRAFT_RUNTIME_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_runtime_home(): a non-default sticky profile (or any PCBDRAFT_RUNTIME_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_runtime_home() and
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``PCBDRAFT_RUNTIME_WEB_DIST`` to ``.../app.asar/dist`` or
...
def cmd_dashboard(args):
    ...
    # Desktop Electron spawns its backend with PCBDRAFT_RUNTIME_DESKTOP=1 plus
    # PCBDRAFT_RUNTIME_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # PCBDRAFT_RUNTIME_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `hermes dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited PCBDRAFT_RUNTIME_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # PCBDRAFT_RUNTIME_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (PCBDRAFT_RUNTIME_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets PCBDRAFT_RUNTIME_SERVE_HEADLESS below.
    if os.environ.get("PCBDRAFT_RUNTIME_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("PCBDRAFT_RUNTIME_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("PCBDRAFT_RUNTIME_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("PCBDRAFT_RUNTIME_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → PCBDRAFT_RUNTIME_HOME points into
    ...
    if (
        ...
        and os.environ.get("PCBDRAFT_RUNTIME_DESKTOP") != "1"
    ):
        url = (
        ...
        # Exact env preservation: PCBDRAFT_RUNTIME_HOME is explicitly pinned to the
        ...
        # PCBDRAFT_RUNTIME_HOME.  We must resolve the root explicitly instead of just
        # dropping PCBDRAFT_RUNTIME_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV PCBDRAFT_RUNTIME_HOME=/opt/data`), so an unset
        # PCBDRAFT_RUNTIME_HOME falls back to $HOME/.hermes = /opt/data/.hermes — an
        ...
        # update-button guard also misfires).  get_default_runtime_root()
        ...
        try:
            ...
            env["PCBDRAFT_RUNTIME_HOME"] = str(get_default_runtime_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop PCBDRAFT_RUNTIME_HOME) rather than block the reroute.
            env.pop("PCBDRAFT_RUNTIME_HOME", None)
    ...
    from pcbdraft.interfaces.tui.resource_limits import apply_nofile_soft_limit
    ...
    if _headless_backend:
        ...
        os.environ["PCBDRAFT_RUNTIME_SERVE_HEADLESS"] = "1"
    elif "PCBDRAFT_RUNTIME_WEB_DIST" not in os.environ and not getattr(
        ...
        _dist_root = (
            Path(os.environ["PCBDRAFT_RUNTIME_WEB_DIST"])
            if "PCBDRAFT_RUNTIME_WEB_DIST" in os.environ
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom PCBDRAFT_RUNTIME_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "PCBDRAFT_RUNTIME_WEB_DIST" not in os.environ
            if _recoverable:
                print(
        ...
        # PCBDRAFT_RUNTIME_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["PCBDRAFT_RUNTIME_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(
                f"✗ PCBDRAFT_RUNTIME_WEB_DIST is set but no web dist found at: {_dist_root}"
            )
            print(
                "  Pre-build first:  npm install --workspace web && npm run build -w web"
            )
            print(
                "  Or unset PCBDRAFT_RUNTIME_WEB_DIST to build and use the default web UI dist."
            )
            sys.exit(1)
        # Write the expanded path back: web_server reads PCBDRAFT_RUNTIME_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["PCBDRAFT_RUNTIME_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from PCBDRAFT_RUNTIME_WEB_DIST: {_dist_root}")
    ...
    try:
        from pcbdraft.interfaces.tui.mcp_startup import start_background_mcp_discovery
    ...
    from pcbdraft.interfaces.tui.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from pcbdraft.interfaces.tui.dashboard_register import (
        cmd_dashboard_register as _impl,
    )
...
def cmd_gateway_enroll(args):
    ...
    from pcbdraft.interfaces.tui.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from pcbdraft.interfaces.tui.completion import (
        generate_bash,
        generate_fish,
        generate_zsh,
    )
...
def cmd_prompt_size(args):
    ...
    from pcbdraft.interfaces.tui.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from pcbdraft.interfaces.tui.logs import list_logs, tail_log
...
def cmd_console(args):
    ...
    from pcbdraft.interfaces.tui.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from pcbdraft.model.catalog import CANONICAL_PROVIDERS as _cp
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    if not command_name:
        return
    try:
        from pcbdraft.services.messaging.platform_registry import platform_registry
...
def _is_tui_chat_launch(args) -> bool:
    return bool(
        getattr(args, "tui", False) or os.environ.get("PCBDRAFT_RUNTIME_TUI") == "1"
    )
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that PCBDRAFT_RUNTIME_YOLO_MODE is set before ANY
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from pcbdraft.interfaces.tui.mcp_startup import (
                start_background_mcp_discovery,
            )
    ...
    if _run_inline_mcp_discovery:
        try:
            ...
            from pcbdraft.tools.mcp_tool import discover_mcp_tools
    ...
    try:
        from pcbdraft.agent.shell_hooks import register_from_config
        from pcbdraft.model.configuration import load_config

        _hooks_cfg = load_config()
        register_from_config(_hooks_cfg, accept_hooks=_accept_hooks)

        from pcbdraft.agent.outbound_webhooks import (
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["PCBDRAFT_RUNTIME_SAFE_MODE"] = "1"
    os.environ["PCBDRAFT_RUNTIME_IGNORE_USER_CONFIG"] = "1"
    os.environ["PCBDRAFT_RUNTIME_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("PCBDRAFT_RUNTIME_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from pcbdraft.model.configuration import get_container_exec_info
    ...
    from pcbdraft.interfaces.tui._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("PCBDRAFT_RUNTIME_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["PCBDRAFT_RUNTIME_DEFER_AGENT_STARTUP"] = "1"
            os.environ["PCBDRAFT_RUNTIME_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["PCBDRAFT_RUNTIME_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_runtime_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_runtime_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_runtime_home()}/memories/\n")
    else:
        from pcbdraft.interfaces.tui.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from pcbdraft.interfaces.tui.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from pcbdraft.interfaces.tui.tools_config import tools_command
...
def cmd_insights(args):
    db = None
    try:
        from pcbdraft.agent.insights import InsightsEngine
        from pcbdraft.services.session_db import SessionDB
...
def cmd_monitoring(args):
    ...
    if action == "status":
        from pcbdraft.agent.monitoring import otlp_exporter
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from pcbdraft.interfaces.tui.skills_config import (
        ...
        from pcbdraft.interfaces.tui.skills_hub import skills_command
...
def cmd_pairing(args):
    from pcbdraft.interfaces.tui.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from pcbdraft.interfaces.tui.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from pcbdraft.interfaces.tui.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from pcbdraft.interfaces.tui.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    ``PCBDRAFT_RUNTIME_AGENT`` is the Hermes-specific marker. setdefault: never
    clobber an outer harness (e.g. Hermes running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "hermes-agent")
    os.environ.setdefault("PCBDRAFT_RUNTIME_AGENT", "true")
...
def main():
    ...
    try:
        from pcbdraft.interfaces.tui.stdio import configure_windows_stdio
    ...
    from pcbdraft.interfaces.tui.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from pcbdraft.interfaces.tui.fallback_cmd import cmd_fallback
    ...
    from pcbdraft.interfaces.tui import onepassword_secrets_cli as _op_secrets_cli
    from pcbdraft.interfaces.tui import secrets_cli as _secrets_cli
    ...
    from pcbdraft.interfaces.tui import proxy_cli as _proxy_cli
    ...
    # migrate command
    # =========================================================================
    from pcbdraft.interfaces.tui.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    try:
        from pcbdraft.agent.lsp.cli import register_subparser as _lsp_register
    ...
    # =========================================================================
    from pcbdraft.interfaces.tui.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from pcbdraft.interfaces.tui.subcommands.peer import build_peer_parser
    ...
    # =========================================================================
    from pcbdraft.interfaces.tui.portal_cli import add_parser as _add_portal_parser
    ...
    # =========================================================================
    from pcbdraft.interfaces.tui.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from pcbdraft.interfaces.tui.projects_cmd import (
        build_parser as _build_project_parser,
    )
    ...
    from pcbdraft.interfaces.tui.checkpoints import (
        register_cli as _register_checkpoints_cli,
    )
    ...
    def cmd_import_agent(args):
        from pcbdraft.interfaces.tui.agent_import import import_agent_command
    ...
    from pcbdraft.interfaces.tui.bundles import bundles_command
    from pcbdraft.interfaces.tui.bundles import register_cli as _bundles_register
    ...
    try:
        from pcbdraft.interfaces.tui.curator import (
            register_cli as _register_curator_cli,
        )
    ...
    try:
        from pcbdraft.interfaces.tui.pets import register_cli as _register_pets_cli
    ...
    try:
        from pcbdraft.interfaces.tui.journey import (
            register_cli as _register_journey_cli,
        )
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            from pcbdraft.interfaces.tui.tools_config import _cua_driver_contract_status
            from pcbdraft.tools.computer_use.cua_backend import (
            ...
            override = _os.environ.get("PCBDRAFT_RUNTIME_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from pcbdraft.interfaces.tui.tools_config import _cua_driver_env
                    ...
                    pass
                from pcbdraft.interfaces.tui.tools_config import _cua_version_summary
                ...
                origin = (
                    " [custom binary from PCBDRAFT_RUNTIME_CUA_DRIVER_CMD]"
                    if override
                    else ""
                )
                if version:
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by PCBDRAFT_RUNTIME_CUA_DRIVER_CMD, or unset "
            ...
            return 1
        if action == "doctor":
            from pcbdraft.tools.computer_use.doctor import run_doctor
        ...
        if action == "permissions":
            ...
            if perms_action == "grant":
                from pcbdraft.tools.computer_use.permissions import (
            ...
            if perms_action == "status":
                ...
                from pcbdraft.tools.computer_use.permissions import computer_use_status
    ...
    # --yolo: set PCBDRAFT_RUNTIME_YOLO_MODE *before* plugin discovery.  The call to
