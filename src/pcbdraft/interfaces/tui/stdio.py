...
on Japanese locales, etc.).  PCBDraft's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``pcbdraft.interfaces.tui/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. PCBDraft is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``PCBDRAFT_RUNTIME_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("PCBDRAFT_RUNTIME_DISABLE_WINDOWS_UTF8") in {
        "1",
        "true",
        "True",
        "yes",
    ...
    # Augment PATH with the PCBDraft-managed Git install directories so
    ...
    # already-running shells don't see the change — which means pcbdraft
    ...
    # install without PCBDraft-managed PortableGit).
    ...
    # stdin is re-configured for completeness; PCBDraft's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and PCBDraft's
    ``internal config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching PCBDraft (User env var in Windows Settings, or
    export in a PowerShell profile) and PCBDraft picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known PCBDraft-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\pcbdraft\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``pcbdraft`` from right after install) retain their old PATH.

    Any subprocess PCBDraft spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known PCBDraft-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "pcbdraft", "git", "cmd"),
        os.path.join(local_appdata, "pcbdraft", "git", "bin"),
        os.path.join(local_appdata, "pcbdraft", "git", "usr", "bin"),
        # PCBDraft venv Scripts directory — host of the pcbdraft.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes pcbdraft, but harmless to include.
        os.path.join(local_appdata, "pcbdraft", "pcbdraft", "venv", "Scripts"),
