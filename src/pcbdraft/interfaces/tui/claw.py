"""internal claw — OpenClaw migration commands.

Usage:
    internal claw migrate              # Preview then migrate (always shows preview first)
    internal claw migrate --dry-run    # Preview only, no changes
    internal claw migrate --yes        # Skip confirmation prompt
    internal claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    internal claw migrate --no-backup  # Skip pre-migration snapshot
    internal claw cleanup              # Archive leftover OpenClaw directories
    internal claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "PCBDraft may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a PCBDraft gateway is running with connected platforms.
    ...
    """
    from pcbdraft.core.runtime_process import get_running_pid, read_runtime_status
    ...
    print()
    print_error(
        "PCBDraft gateway is running with active connections: " + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'pcbdraft --help'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_pcbdraft.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_pcbdraft", script_path)
...
def claw_command(args):
    """Route internal claw subcommands."""
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to PCBDraft")
        print(
            "  cleanup          Archive leftover OpenClaw directories after migration"
        )
        print()
        print("Run 'pcbdraft --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → PCBDraft migration."""
    ...
    # migrate-pcbdraft posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info(
            "Set a custom import path through the internal migration adapter. Public commands: pcbdraft --help"
        )
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.pcbdraft/. "
    ...
    # ── Phase 2b: Pre-apply backup of the PCBDraft home ─────────
    # Delegates to pcbdraft.interfaces.tui.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `internal import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            backup_archive = create_pre_migration_backup(runtime_home=runtime_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info("Restore with: pcbdraft --help")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the PCBDraft home."
            )
    ...
    # 'internal claw cleanup' separately.
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: pcbdraft --help")
                return
