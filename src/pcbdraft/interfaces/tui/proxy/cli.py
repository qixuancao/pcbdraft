"""CLI handlers for the ``internal proxy`` subcommand."""
...
from pcbdraft.interfaces.tui.proxy.adapters import ADAPTERS, get_adapter
from pcbdraft.interfaces.tui.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "pcbdraft --help. Run `pcbdraft setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", "pcbdraft connect")
        print(
    ...
    print(
        f"Starting PCBDraft proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("PCBDraft proxy upstream adapters\n")
    ...
    print("\nStart the proxy with: pcbdraft --help")
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``internal proxy <subcommand>``."""
