...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in PCBDRAFT_RUNTIME_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → PCBDRAFT_RUNTIME_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in PCBDRAFT_RUNTIME_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in PCBDRAFT_RUNTIME_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → PCBDRAFT_RUNTIME_HOME/skills/codex-imports/<name>/
...
class AgentImporter:
    ...
    def record(
        self, kind: str, source, destination, status: str, reason: str = "", **details
    ) -> None:
        item: dict[str, Any] = {
            "kind": kind,
    ...
    def load_target_config(
        self, kind: str, source, destination: Path
    ) -> dict[str, Any] | None:
    ...
    def _run_codex(self) -> None:
        ...
        self.import_mcp_servers(
            mcp if isinstance(mcp, dict) else {}, kind="mcp-servers"
        )
    ...
    def _load_claude_settings(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.record(
                "settings", path, None, "error", f"Could not parse settings.json: {exc}"
            )
            return {}
        if not isinstance(data, dict):
            self.record(
                "settings", path, None, "error", "settings.json is not a JSON object"
            )
            return {}
        return data
    ...
    def _load_codex_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.record(
                "config", path, None, "error", f"Could not parse config.toml: {exc}"
            )
            return {}
    ...
    def import_context_file(self, source: Path, kind: str) -> None:
        ...
        try:
            ...
            self.record(
                kind, source, destination, "error", f"Could not read file: {exc}"
            )
            return
        if not incoming:
            self.record(
    ...
    def import_memories_dir(self, memories_dir: Path) -> None:
        ...
        if not memories_dir.is_dir():
            self.record(
                "memories", None, destination, "skipped", "No memories directory found"
            )
            return
        incoming: list[str] = []
    ...
    def _merge_memory_entries(
        self, kind: str, source: Path, destination: Path, incoming: list[str]
    ) -> None:
        ...
        if self.execute:
            ...
            self.record(
                kind, source, destination, "imported", "Would merge entries", **details
            )
    ...
    def import_mcp_servers(self, servers: dict[str, Any], kind: str) -> None:
        ...
        for name, srv in servers.items():
            if not isinstance(srv, dict):
                self.record(
                    kind, name, None, "skipped", "Server entry is not a mapping"
                )
                continue
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → PCBDRAFT_RUNTIME_HOME/skills/<category>/<name>."""
        category = _SKILL_CATEGORY[self.agent]
        destination_root = self.target_root / "skills" / category
        if not source_root.is_dir():
            self.record(
                "skills", None, destination_root, "skipped", "No skills directory found"
            )
            return
        skill_dirs = [
            p
...
def import_agent_command(args) -> None:
    ...
    from pcbdraft.model.configuration import get_config_path, load_config, save_config
    ...
    print()
    print_header(
        f"Import Preview — {summary.get('imported', 0)} item(s) would be imported"
    )
...
def print_import_report(report: dict[str, Any], dry_run: bool) -> None:
    ...
    from pcbdraft.interfaces.tui.setup import Colors, color, print_header, print_info
