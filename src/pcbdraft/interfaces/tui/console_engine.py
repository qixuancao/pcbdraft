...
from pcbdraft.tools.ansi_strip import strip_ansi as _strip_ansi
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("pcbdraft.interfaces.tui.main")
    builder = getattr(module, builder_name)
...
def _sessions_stats(_engine: PCBDraftConsoleEngine, args: list[str]) -> str:
    ...
    try:
        ...
        listable = db.session_count(
            exclude_children=True, exclude_sources=["kanban", "tool"]
        )
...
def _config_set(_engine: PCBDraftConsoleEngine, args: list[str]) -> str:
    ...
    key = args[0]
    value = " ".join(args[1:])
    from pcbdraft.model.configuration import set_config_value
...
def _cron_list(_engine: PCBDraftConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from pcbdraft.interfaces.tui.cron import cron_list
