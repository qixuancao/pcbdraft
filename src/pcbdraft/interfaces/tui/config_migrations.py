...
#: PCBDRAFT_RUNTIME_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def _migrate_to_12(results: dict[str, Any], quiet: bool) -> None:
    ...
    _custom_provider_entry_to_provider_config = (
        _c._custom_provider_entry_to_provider_config
    )
...
def _migrate_to_21(results: dict[str, Any], quiet: bool) -> None:
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$PCBDRAFT_RUNTIME_HOME/plugins/`` for currently installed user plugins.
        grandfathered: list[str] = []
        try:
            user_plugins_dir = get_runtime_home() / "plugins"
