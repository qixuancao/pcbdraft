...
Pure logic: walks a PCBDraft config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `pcbdraft doctor` and a future `internal migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a PCBDraft config."""
...
def find_retired_xai_refs(config: dict[str, Any]) -> list[RetirementIssue]:
    """Walk all model slots in a PCBDraft config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `internal migrate xai` reports nothing to migrate
