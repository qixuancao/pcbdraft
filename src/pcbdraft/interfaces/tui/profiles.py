...
Each profile is a fully independent PCBDRAFT_RUNTIME_HOME directory with its own
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_runtime_home().resolve()
...
# Allow-list for ``export_profile("default")``: when PCBDRAFT_RUNTIME_HOME equals the
...
# artifacts* at the root of PCBDRAFT_RUNTIME_HOME; everything else is excluded.
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current PCBDRAFT_RUNTIME_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where PCBDRAFT_RUNTIME_HOME points outside
    ``~/.hermes``, profiles live under ``PCBDRAFT_RUNTIME_HOME/profiles/`` so
...
def _get_default_runtime_home() -> Path:
    """Return the default (pre-profile) PCBDRAFT_RUNTIME_HOME path.

    In standard deployments this is ``~/.hermes``.
    In Docker/custom deployments where PCBDRAFT_RUNTIME_HOME is outside ``~/.hermes``
    (e.g. ``/opt/data``), returns PCBDRAFT_RUNTIME_HOME directly.
    """
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its PCBDRAFT_RUNTIME_HOME directory."""
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        from pcbdraft.model.configuration import check_config_version, migrate_config
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``PCBDRAFT_RUNTIME_HOME``.
    """
    try:
...
def list_profiles() -> list[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_runtime_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, runtime_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own PCBDRAFT_RUNTIME_HOME. When
    ...
    The returned ``runtime_home`` is the path to pass to
    ``set_runtime_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: list[tuple[str, Path]] = [("default", _get_default_runtime_home())]
...
def create_profile(
    ...
    if not soul_path.exists():
        try:
            from pcbdraft.interfaces.tui.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> dict | None:
    ...
    Uses subprocess because sync_skills() caches PCBDRAFT_RUNTIME_HOME at module level.
...
def backfill_profile_envs(quiet: bool = False) -> list[str]:
    ...
    default_env = _get_default_runtime_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``PCBDRAFT_RUNTIME_HOME``
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if (
                not is_hermes
                and len(argv) >= 2
                and _python_interpreter_re.match(exe_name)
            ):
                ...
                script_stem = (
                    script_name.rsplit(".", 1)[0] if "." in script_name else script_name
                )
            ...
            # ...or by PCBDRAFT_RUNTIME_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("PCBDRAFT_RUNTIME_HOME", "")
...
def _stop_profile_backends(canon: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from pcbdraft.core.runtime_process import terminate_pid as _terminate_pid
    except Exception:
        return
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_runtime_home
    ...
    try:
        from pcbdraft.agent.memory_backends.holographic.store import (
        ...
        if _released:
            print(
                f"✓ Released {_released} memory-store connection(s) held by this process"
            )
    except Exception:
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``PCBDRAFT_RUNTIME_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from pcbdraft.interfaces.tui.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set PCBDRAFT_RUNTIME_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("PCBDRAFT_RUNTIME_HOME")
    try:
        os.environ["PCBDRAFT_RUNTIME_HOME"] = str(profile_dir)
        ...
        if old_home is not None:
            os.environ["PCBDRAFT_RUNTIME_HOME"] = old_home
        elif "PCBDRAFT_RUNTIME_HOME" in os.environ:
            del os.environ["PCBDRAFT_RUNTIME_HOME"]
...
def get_active_profile_name() -> str:
    """Infer the current profile name from PCBDRAFT_RUNTIME_HOME.

    Returns ``"default"`` if PCBDRAFT_RUNTIME_HOME is not set or points to ``~/.hermes``.
    Returns the profile name if PCBDRAFT_RUNTIME_HOME points into ``~/.hermes/profiles/<name>``.
    Returns ``"custom"`` if PCBDRAFT_RUNTIME_HOME is set to an unrecognized path.
    """
    from pcbdraft.core.runtime_environment import get_runtime_home
    ...
    default_resolved = _get_default_runtime_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      PCBDRAFT_RUNTIME_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hermes file the
      user may have lying alongside PCBDRAFT_RUNTIME_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``security.redact_secrets`` / ``PCBDRAFT_RUNTIME_REDACT_SECRETS`` — share archives
...
def export_profile(
    name: str, output_path: str, extra_files: dict[str, str] | None = None
) -> Path:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_runtime_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a PCBDRAFT_RUNTIME_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the PCBDRAFT_RUNTIME_HOME environment variable.

    When PCBDRAFT_RUNTIME_HOME is already set, the configured spelling IS the
    ...
    if env_home:
        ...
        # (mirrors get_default_runtime_root()).
        root = (
