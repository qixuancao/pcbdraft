"""Bounded product contract for the first PCBDraft shared-metrics slice."""
...
from pcbdraft.agent.relay_runtime import (
    LOGICAL_LLM_SCOPE,
...
SCHEMA_KEY = "pcbdraft.metrics.schema_version"
SCHEMA_VERSION = "pcbdraft.metrics.event.v2"
MODEL_CALL_SCOPE = "pcbdraft.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "pcbdraft.task_run"
TOOL_CALL_SCOPE = "pcbdraft.tool_call"
CLIENT_ACTIVE_MARK = "pcbdraft.client.active"
TOOL_APPROVAL_MARK = "pcbdraft.tool_approval"
SKILL_LIFECYCLE_MARK = "pcbdraft.skill.lifecycle"
SKILL_LOAD_MARK = "pcbdraft.skill.load"
SUBSCRIBER_NAME = "pcbdraft.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "pcbdraft.client.active"
LEGACY_MODEL_CALL_METRIC = "pcbdraft.model_call.count"
MODEL_ROUTE_METRIC = "pcbdraft.model_route.count"
TASK_STARTED_METRIC = "pcbdraft.task_run.started"
TASK_FINISHED_METRIC = "pcbdraft.task_run.finished"
TOOL_CALL_METRIC = "pcbdraft.tool_call.count"
TOOL_APPROVAL_METRIC = "pcbdraft.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "pcbdraft.skill.lifecycle.count"
SKILL_LOAD_METRIC = "pcbdraft.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted PCBDraft installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(pcbdraft_version or "").strip()
    ...
    return {
        ...
        "pcbdraft_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("pcbdraft_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its PCBDraft logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("pcbdraft.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from pcbdraft.interfaces.tui.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map PCBDraft terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map PCBDraft registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "pcbdraft-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal PCBDraft tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to PCBDraft."""
...
def _metric_identifier(value: Any, *, max_length: int) -> str:
    ...
    if (
        ...
        or any(
            character not in _METRIC_IDENTIFIER_CHARACTERS for character in identifier
        )
    ):
        return "unknown"
    return identifier
