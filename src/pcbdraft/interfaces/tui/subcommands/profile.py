"""``internal profile`` subcommand parser.
...
def build_profile_parser(subparsers, *, cmd_profile: Callable) -> None:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated PCBDraft instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `pcbdraft --help` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        ...
        "of profile name alone. Skip and add later via `pcbdraft --help`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a PCBDraft profile distribution. SOURCE can be a git URL "
