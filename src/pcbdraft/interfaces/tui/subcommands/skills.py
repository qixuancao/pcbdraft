"""``internal skills`` subcommand parser.
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser(
        "trust",
        help="Trust a project so its repo-local skills (./.pcbdraft/skills, ./.agents/skills) load",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.pcbdraft/skills/.bundled_manifest) "
            "so future 'pcbdraft --help' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `pcbdraft --help` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `pcbdraft --help` reports as user-modified and skips. "
            "Use `pcbdraft --help` to see changes and `pcbdraft --help"
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`pcbdraft --help`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `pcbdraft --help`. Pass --sync to re-seed now."
        ),
    )
