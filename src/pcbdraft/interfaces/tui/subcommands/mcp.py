"""``internal mcp`` subcommand parser.
...
from pcbdraft.interfaces.tui.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run PCBDraft as an MCP server",
        description=(
            "Manage MCP server connections and run PCBDraft as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'pcbdraft --help' to connect to a new server, or\n"
            "'pcbdraft --help' to expose PCBDraft conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run PCBDraft as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `internal mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `pcbdraft --help`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `pcbdraft --help`)",
    )
