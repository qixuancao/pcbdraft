"""``internal skin`` subcommand parser."""
...
def build_skin_parser(subparsers, *, cmd_skin: Callable) -> None:
    ...
    skin_parser = subparsers.add_parser(
        ...
        description="Manage PCBDraft skins. `set` tweaks one color of the active skin in place.",
    )
    ...
    skin_set = skin_subparsers.add_parser(
        "set",
        help="Set one color of the active skin (e.g. `skin set ui_tool '#00FFFF'`)",
    )
    ...
    skin_set.add_argument(
        "--skin", help="Target a specific skin instead of the active one"
    )
