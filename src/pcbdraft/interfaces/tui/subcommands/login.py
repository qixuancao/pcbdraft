"""``internal login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``internal login`` was removed in favor of ``internal auth`` / ``internal model``
    (the runtime handler in ``pcbdraft.interfaces.tui/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``internal login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``pcbdraft --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``internal login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``internal model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `pcbdraft connect` to manage credentials, "
            "`pcbdraft connect` to select a provider, or `pcbdraft setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `pcbdraft connect`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: pcbdraft-cli)",
    )
