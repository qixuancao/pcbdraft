"""On-demand supply-chain audit for PCBDraft Agent installs.

Scans three surfaces a PCBDraft user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The PCBDraft venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.pcbdraft/plugins``
...
def _discover_plugins(runtime_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.pcbdraft/plugins``.
...
def _discover_components(
    *,
    ...
    home = runtime_home or Path(get_runtime_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `internal security audit`."""
