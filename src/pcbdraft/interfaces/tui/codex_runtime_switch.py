...
Toggles `model.openai_runtime` between "auto" (= chat_completions, PCBDraft'
...
The actual runtime resolution happens in pcbdraft.interfaces.tui.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[str | None, list[str]]:
    ...
    if raw in {"off", "default", "disable", "pcbdraft"}:
        return "auto", []
...
def check_codex_binary_ok() -> tuple[bool, str | None]:
    ...
    try:
        from pcbdraft.model.transports.codex_app_server import check_codex_binary
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, pcbdraft-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate PCBDraft' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into PCBDraft for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from pcbdraft.interfaces.tui.codex_runtime_plugin_migration import migrate

            mig_report = migrate(config)
            # Tools/MCP servers (excluding the pcbdraft-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [s for s in mig_report.migrated if s != "pcbdraft-tools"]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + PCBDraft tool callback are always-on production
            ...
            if "pcbdraft-tools" in mig_report.migrated:
                msg_lines.append(
                    "PCBDraft tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default PCBDraft runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "PCBDraft tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default PCBDraft runtime.")
