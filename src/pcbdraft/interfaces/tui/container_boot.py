...
``$PCBDRAFT_RUNTIME_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $PCBDRAFT_RUNTIME_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$PCBDRAFT_RUNTIME_HOME``,
    ...
    ``$PCBDRAFT_RUNTIME_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        runtime_home: The container's PCBDRAFT_RUNTIME_HOME (typically /opt/data).
            Profiles live under ``<runtime_home>/profiles/<name>/``;
            the default profile lives at ``<runtime_home>`` itself.
    ...
    default_prior_state = legacy_default_state or _read_desired_state(runtime_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(runtime_home)
    ...
    actions.append(
        ReconcileAction(
            ...
            prior_exit=_read_prior_exit_label(runtime_home),
        )
    )
    ...
    if not dry_run:
        _write_reconcile_log(runtime_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = runtime_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("PCBDRAFT_RUNTIME_GATEWAY_NO_SUPERVISE", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    the gateway and dashboard containers share a bind-mounted PCBDRAFT_RUNTIME_HOME,
...
def _read_prior_exit_label(profile_dir: Path) -> str:
    ...
    (NS-608)."""
    try:
        from pcbdraft.services.messaging.lifecycle_ledger import read_prior_exit_label
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        run = tmp_dir / "run"
        run.write_text(
            S6ServiceManager._render_run_script(profile, extra_env={}), encoding="utf-8"
        )
        run.chmod(0o755)
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $PCBDRAFT_RUNTIME_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted PCBDRAFT_RUNTIME_HOME, both race to flock() the same s6-log lock
