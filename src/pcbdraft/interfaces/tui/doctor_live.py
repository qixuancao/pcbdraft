...
from pcbdraft.interfaces.tui.doctor import (
    _section,
    check_fail,
    check_info,
    check_ok,
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from pcbdraft.interfaces.tui.doctor import PCBDRAFT_RUNTIME_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (
            PCBDRAFT_RUNTIME_HOME / "node" / "bin",
            PCBDRAFT_RUNTIME_HOME / "node",
            PCBDRAFT_RUNTIME_HOME / "node_modules" / ".bin",
        ):
...
def _probe_fal(timeout: float) -> ProbeResult:
    ...
    resp = _http_get(
        FAL_MODELS_URL, headers={"Authorization": f"Key {key}"}, timeout=timeout
    )
...
def _audio_provider_probe(kind: str, provider: str, timeout: float) -> ProbeResult:
    ...
    if provider in _LOCAL_AUDIO_PROVIDERS:
        return ProbeResult(
            name,
            "skip",
            f"(provider '{provider or 'local'}' — no remote backend to probe)",
        )
    ...
    if entry is None:
        return ProbeResult(
            name, "skip", f"(provider '{provider}' — no live probe implemented)"
        )
    url, env_var, scheme = entry
    key = os.getenv(env_var, "").strip()
    if not key:
        return ProbeResult(
            name, "warn", f"(provider '{provider}' configured but {env_var} is not set)"
        )
    if scheme == "xi":
...
def _run_one(
    name: str, fn: Callable[[], ProbeResult], issues: list[str]
) -> ProbeResult:
...
def run_live_checks(issues: list[str]) -> list[ProbeResult]:
    ...
    try:
        timeout = float(
            (config.get("doctor") or {}).get(
                "live_probe_timeout", DEFAULT_PROBE_TIMEOUT
            )
        )
