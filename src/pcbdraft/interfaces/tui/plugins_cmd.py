...
def _scan_plugin_tree(
    ...
    from pcbdraft.tools.plugin_guard import (
        format_scan_report,
        scan_plugin,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_runtime_home() / "plugins"
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if not manifest_file.exists():
        ...
        if not portable_file.exists() and not portable_file.is_symlink():
            return {}
        try:
            from pcbdraft.interfaces.tui.agent_plugins import read_agent_plugin_manifest
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(
                f"\n[dim]  Skipped (you can set these later in {display_runtime_home()}/.env)[/dim]"
            )
            return
        ...
        if value:
            ...
            console.print(f"  [green]✓[/green] Saved to {display_runtime_home()}/.env")
        else:
            console.print(
                f"  [dim]  Skipped (set {name} in {display_runtime_home()}/.env later)[/dim]"
            )
...
def _install_metadata_path() -> Path:
    return get_runtime_home() / "plugins" / _INSTALL_METADATA_FILE
...
def _read_install_metadata() -> dict[str, dict[str, object]]:
    ...
    try:
        ...
        raise PluginOperationError(
            f"Could not read plugin install metadata: {exc}"
        ) from exc
...
def _resolve_index_name(identifier: str, console) -> tuple[str, str | None]:
    ...
    """
    from pcbdraft.interfaces.tui.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        resolve_name,
    )
...
def cmd_install(
    ...
    def _interactive_scan_decision(scan_result) -> bool:
        ...
        from pcbdraft.tools.plugin_guard import format_scan_report
...
def _declared_capabilities_from_manifest(
    ...
    from pcbdraft.agent.extensions.capabilities import parse_declared_capabilities
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from pcbdraft.agent.extensions.capabilities import CAPABILITY_REGISTRY
...
def _resolve_tool_override_grant(
    console, key: str, allow_tool_override: bool | None
) -> None:
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from pcbdraft.interfaces.tui.plugin_dev import doctor_plugin
...
def plugins_command(args) -> None:
    ...
    if action == "install":
        ...
        from pcbdraft.interfaces.tui.plugin_packs import pack_command
