...
from pcbdraft.tools.mcp_tool import _ENV_VAR_PATTERN, _env_ref_name
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from pcbdraft.interfaces.tui.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    """
    from pcbdraft.agent.secret_scope import current_secret_scope
    from pcbdraft.tools.mcp_tool import _interpolate_env_vars
        ...
        try:
            ...
            if details is not None:
                ...
                try:
                    ...
                    from pcbdraft.tools.mcp_tool import _convert_mcp_schema
...
def cmd_mcp_add(args):
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_runtime_home()}/config.yaml ({tool_count}/{total} tools enabled)"
        )
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    try:
        from pcbdraft.tools.mcp_oauth import force_interactive_oauth
...
def cmd_mcp_configure(args):
    ...
    try:
        from pcbdraft.tools.mcp_tool import matches_name_filter
    ...
    if include and isinstance(include, list):
        ...
        pre_selected = {
            i for i, tn in enumerate(tool_names) if matches_name_filter(tn, include_set)
        }
    elif exclude and isinstance(exclude, list):
...
def mcp_command(args):
    ...
    if action == "picker":
        ...
        return
    if action == "catalog":
        from pcbdraft.interfaces.tui.mcp_picker import show_catalog
    ...
    if action == "install":
        ...
        from pcbdraft.interfaces.tui.mcp_picker import install_by_name
