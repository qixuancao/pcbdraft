"""CLI subcommand: ``internal send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"PCBDraft send adapter: {file_path} is not a text file. --file reads the "
                ...
                f'  pcbdraft --help"MEDIA:{file_path}"\n'
                f'  pcbdraft --help"optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  pcbdraft --help"[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(
                f"PCBDraft send adapter: cannot read {file_path}: {exc}",
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"PCBDraft send adapter: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: str | None, *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(
            f"PCBDraft send adapter: failed to load channel directory: {exc}",
            file=sys.stderr,
        )
        return _FAILURE_EXIT
    ...
    try:
        ...
        print(
            f"PCBDraft send adapter: failed to read channel directory: {exc}",
            file=sys.stderr,
        )
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `internal send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"PCBDraft send adapter: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `pcbdraft --help`, or run the gateway once so")
        print("channel discovery can populate ~/.pcbdraft/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_pcbdraft_env() -> None:
    """Populate ``os.environ`` from ``~/.pcbdraft/.env`` AND bridge top-level
    ...
    startup that ``internal send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.pcbdraft/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.pcbdraft/config.yaml`` into
       ...
       via ``internal config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``internal send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.pcbdraft/.env and ~/.pcbdraft/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "PCBDraft send adapter: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            '  pcbdraft --help"hello"\n'
    ...
    if message is None or not message.strip():
        print(
            "PCBDraft send adapter: no message provided. Pass text as a positional "
    ...
    # Import lazily so `internal send --help` stays fast and does not pull in
    # the full tool registry / gateway config stack.
    from pcbdraft.tools.send_message_tool import send_message_tool
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform PCBDraft "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.pcbdraft/.env + ~/.pcbdraft/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            '  pcbdraft --help"deploy finished"\n'
            '  echo "RAM 92%" | pcbdraft --help\n'
            "  Public CLI commands: pcbdraft --help\n"
            '  pcbdraft --help"[CI]" --file build.log\n'
            '  pcbdraft --help"MEDIA:/tmp/chart.png"   # send a media attachment\n'
            "  pcbdraft --help\n"
            "  pcbdraft --help\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `pcbdraft --help`.",
    )
