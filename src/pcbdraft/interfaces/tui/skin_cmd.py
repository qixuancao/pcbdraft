...
def _use(name: str) -> None:
    ...
    config_command(
        argparse.Namespace(
            config_command="set", key="display.skin", value=name, force=True
        )
    )
...
def _skin_set(key: str, value: str, skin: str | None) -> int:
    ...
    if path.exists():
        ...
        from pcbdraft.interfaces.tui.skin_engine import load_skin
    ...
    print(
        f"✓ {key} = {value} in {display_runtime_home()}/skins/{target}.yaml (live within ~1s)"
    )
    return 0
...
def _skin_list() -> int:
    ...
    for s in list_skins():
        ...
        print(
            f"{mark} {s['name']:<16} {s.get('source', ''):<8} {s.get('description', '')}"
        )
    return 0
