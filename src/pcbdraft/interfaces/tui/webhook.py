"""internal webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    internal webhook subscribe <name> [options]
    internal webhook list
    internal webhook remove <name>
    internal webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.pcbdraft/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _runtime_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: pcbdraft --help
"""
...
def webhook_command(args):
    """Entry point for 'internal webhook' subcommand."""
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (pcbdraft --help).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: pcbdraft --help")
        return
...
def _cmd_test(args):
    ...
    payload = (
        args.payload
        or '{"test": true, "event_type": "test", "message": "Hello from PCBDraft webhook test"}'
    )
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (pcbdraft --help)")
