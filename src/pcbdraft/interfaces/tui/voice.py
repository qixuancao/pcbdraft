...
# lookup, and pcbdraft-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and pcbdraft-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when PCBDRAFT_RUNTIME_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("PCBDRAFT_RUNTIME_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
_continuous_on_stop_phrase: Callable[[str], None] | None = None
...
def _speak_text_streaming(text: str, stop_event: threading.Event | None = None) -> bool:
    ...
    from pcbdraft.tools.tts_tool import stream_tts_to_speaker
