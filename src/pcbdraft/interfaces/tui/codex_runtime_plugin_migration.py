...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (PCBDRAFT_RUNTIME_HOME, PYTHONPATH) could
...
def _query_codex_plugins(
    ...
    """
    try:
        from pcbdraft.model.transports.codex_app_server import CodexAppServerClient
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    env: dict[str, str] = {}
    # PCBDRAFT_RUNTIME_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_runtime_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever PCBDRAFT_RUNTIME_HOME its launcher sets
    ...
    # — that would override the launcher's PCBDRAFT_RUNTIME_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("PCBDRAFT_RUNTIME_HOME", tmp_path) would
    ...
    runtime_home = os.environ.get("PCBDRAFT_RUNTIME_HOME") or ""
    if runtime_home and _looks_like_test_tempdir(runtime_home):
        runtime_home = ""
    if runtime_home:
    ...
    env["PCBDRAFT_RUNTIME_QUIET"] = "1"
    env["PCBDRAFT_RUNTIME_REDACT_SECRETS"] = env.get(
        "PCBDRAFT_RUNTIME_REDACT_SECRETS", "true"
    )
