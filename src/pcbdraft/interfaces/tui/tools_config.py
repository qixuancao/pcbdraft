"""
Unified tool configuration for PCBDraft Agent.

`internal tools` and `pcbdraft setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.pcbdraft/config.yaml under
...
from pcbdraft.interfaces.tui.nous_account import format_nous_portal_entitlement_message
...
from pcbdraft.tools.tool_backend_helpers import fal_key_is_configured
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``internal tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`pcbdraft.interfaces.tui._subprocess_compat.windows_hide_flags`) suppresses it
...
from pcbdraft.interfaces.tui.cli_output import print_error as _print_error
from pcbdraft.interfaces.tui.cli_output import print_info as _print_info
from pcbdraft.interfaces.tui.cli_output import print_success as _print_success
from pcbdraft.interfaces.tui.cli_output import print_warning as _print_warning
from pcbdraft.interfaces.tui.cli_output import prompt as _prompt
...
CONFIGURABLE_TOOLSETS = [
    ...
    (
        "stt",
        "🎙️ Speech-to-Text",
        "voice transcription (gateway voice messages + voice mode)",
    ),
    ...
    (
        "pcbdraft",
        "🔌 PCB Design",
        "pcb_plan_request, pcb_generate_candidate, pcb_validate, pcb_repair_candidate",
    ),
    (
        "cronjob",
        "⏰ Cron Jobs",
...
# They're still in _PCBDRAFT_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `internal tools` → Video Generation, which walks
...
# `internal tools` → X (Twitter) Search setup walks users through credential
...
# Config-only capabilities: they appear in `internal tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from pcbdraft.model.auth import _read_xai_oauth_tokens
        ...
        pass
    try:
        from pcbdraft.tools.xai_http import get_env_value as _xai_get_env_value
...
# Platform-scoped toolsets: only appear in the `internal tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``internal tools`` → "reconfigure existing" would
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from pcbdraft.agent.extensions.manager import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> set[str]:
    """Return the toolset keys the ``internal tools`` checklist actually offers
    for ``platform``.
    ...
    so ``internal tools`` never claims to add or remove a toolset the user was
...
from pcbdraft.interfaces.tui.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "PCBDraft routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set PCBDRAFT_RUNTIME_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {
                        "key": "PCBDRAFT_RUNTIME_LANGFUSE_PUBLIC_KEY",
                        "prompt": "Langfuse public key (pk-lf-...)",
                        "url": "https://cloud.langfuse.com",
                    },
                    {
                        "key": "PCBDRAFT_RUNTIME_LANGFUSE_SECRET_KEY",
            ...
            },
            {
                ...
                "env_vars": [
                    ...
                    {
                        "key": "PCBDRAFT_RUNTIME_LANGFUSE_BASE_URL",
...
# provider+model picker like `internal model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("PCBDRAFT_RUNTIME_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``PCBDRAFT_RUNTIME_CUA_DRIVER_CMD`` is not obliged to answer
...
def _resolved_cua_driver_cmd() -> str | None:
    ...
    from pcbdraft.tools.computer_use.cua_backend import resolve_cua_driver_cmd

    return resolve_cua_driver_cmd()


def _cua_driver_env() -> dict:
    """cua-driver child env with the PCBDraft telemetry policy applied.
    ...
    """
    try:
        from pcbdraft.tools.computer_use.cua_backend import cua_driver_child_env
...
def _cua_driver_contract_status(binary: str | None = None) -> dict:
    """Inspect whether an installed driver supports PCBDraft' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $PCBDRAFT_RUNTIME_HOME/bin is never on PATH, so a bare which()
    # misses the uv PCBDraft installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``internal update`` and
      by ``internal computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``internal update`` sets
    ...
    a further ~600s wait on Windows). ``internal computer-use install
    ...
    ``internal update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `internal update` calls this
    ...
    override = os.environ.get("PCBDRAFT_RUNTIME_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    PCBDRAFT_RUNTIME_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails PCBDraft' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # PCBDraft' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `internal update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but PCBDraft cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("PCBDRAFT_RUNTIME_CUA_DRIVER_CMD", "").strip():
            _print_info(
                "    Update the binary selected by PCBDRAFT_RUNTIME_CUA_DRIVER_CMD, or unset "
                "the override and run: pcbdraft --help"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info("    Run `pcbdraft --help` from an admin account to update it.")
        return bool(binary)
    ...
    # caller: `internal update` (require_confirmed_update=True) keeps the
    ...
    # `internal computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    confirmed_version = None
    if binary and not repair_existing:
        _state = None
        try:
            from pcbdraft.tools.computer_use.cua_backend import cua_driver_update_check
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info("    Force a refresh with: pcbdraft --help")
            return True
...
def _cua_install_home() -> "Path":
    ...
    return Path(
        os.environ.get("CUA_DRIVER_RS_HOME") or str(Path.home() / ".cua-driver")
    )
    ...
    try:
        ...
        if holder_pid is not None:
            try:
                os.kill(
                    holder_pid, 0
                )  # windows-footgun: ok — function early-returns on win32
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    def _kill_installer_tree(proc):
        ...
        try:
            if not is_windows:
                os.killpg(
                    os.getpgid(proc.pid), _signal.SIGKILL
                )  # windows-footgun: ok — POSIX branch only
            else:
    ...
    try:
        # When not verbose (e.g. `internal update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `internal update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.pcbdraft/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("    Both must allow the terminal / PCBDraft process.")
            return True
        _print_warning(
        ...
        _print_warning(
            f"    cua-driver {label.lower()} timed out after {_CUA_INSTALLER_TIMEOUT}s."
        )
        if not is_windows:
            _print_info(
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the PCBDraft-managed ``$PCBDRAFT_RUNTIME_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary PCBDraft installs
    and updates rather than a user-level side install.
    """
    _print_info("    Ensuring browser-use CLI (managed install)...")
    try:
        from pcbdraft.tools.browser_use_cli import install_cli
    ...
    if ok:
        ...
        if shutil.which("uvx"):
            ...
            _print_info(
                "    Install manually: uv tool install browser-use  (https://docs.astral.sh/uv/)"
            )
    if verbose_hints:
        _print_info(
            "    Local Chrome needs remote debugging: chrome://inspect/#remote-debugging"
        )
        _print_info(
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/PCBDraft-managed node -> npx) rather than a bare
        # shutil.which — PCBDraft-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      Use an explicitly configured container image, then run pcbdraft doctor."
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # PCBDraft-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — PCBDraft-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import pcbdraft.tools.browser_tool as _bt
        ...
        try:
            ...
            pass
        _print_info(
            "    Installing faster-whisper (model ~150MB downloads on first use)..."
        )
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.pcbdraft/config.yaml")
            else:
        ...
        _print_info(
            "    Switch voices by setting tts.piper.voice in ~/.pcbdraft/config.yaml"
        )
        ...
        # Run the full `internal auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.pcbdraft/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace

        try:
            from pcbdraft.model.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: pcbdraft connect")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `internal auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: pcbdraft connect")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: pcbdraft --help")
        _print_info("    Restart PCBDraft for tracing to take effect.")
        _print_info("    Verify: pcbdraft --help")
        ...
        try:
            ...
            from pcbdraft.interfaces.tui.setup import prompt as _setup_prompt
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `pcbdraft connect`",
            ],
            default=0,
        )
        if idx == 0:
            ...
            if api_key:
                ...
                _print_warning("    No API key provided. Run later: pcbdraft connect")
        else:
            _print_info(
...
def valid_post_setup_keys() -> set[str]:
    ...
    ``post_setup``). This is the allowlist the ``internal tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``internal tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: pcbdraft --help")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``pcbdraft-discord``, which contains those
...
#:
#: Saving ``internal tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[pcbdraft-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``internal tools``
    ...
    """
    from pcbdraft.tools.toolsets import resolve_toolset
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"pcbdraft-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (pcbdraft-acp, pcbdraft-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    from pcbdraft.tools.toolsets import TOOLSETS, resolve_toolset
    ...
    # ``pcbdraft-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"pcbdraft-{platform}"
    ...
    # explicit config like ``[pcbdraft-cli, a2a]`` must survive filtering just
    ...
    # "pcbdraft-cli" (which include all _PCBDRAFT_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[pcbdraft-cli, spotify]`` after enabling Spotify via ``pcbdraft
        ...
        # (e.g. "pcbdraft-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``internal tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `internal tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = (
        _plat_info["default_toolset"] if _plat_info else f"pcbdraft-{platform}"
    )
    ...
    skip |= {k for k in TOOLSETS if k.startswith("pcbdraft-")}
    ...
    # shipped as a bundled plugin but user must opt in via `internal tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `internal tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by internal tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `internal config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `pcbdraft` instead of
    # `pcbdraft-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `internal update`/`pcbdraft doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `pcbdraft --help` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: set[str]):
    ...
    # Also exclude platform default toolsets (pcbdraft-cli, pcbdraft-telegram, etc.)
    ...
    # Opening `internal tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key == "vision":
        try:
            from pcbdraft.model.auxiliary_client import resolve_vision_provider_client
...
_tool_token_cache: dict[tuple[str, int], dict[str, int]] | None = None
...
def _estimate_tool_tokens() -> dict[str, int]:
    ...
    scope = runtime_home_key()

    try:
        # Trigger full tool discovery (imports all tool modules).
        import pcbdraft.tools.dispatch as model_tools  # noqa: F401
...
def web_provider_capabilities(backend: str) -> list:
    ...
    """
    try:
        from pcbdraft.agent.web_search_registry import get_provider
...
def _plugin_browser_providers() -> list[dict]:
    ...
    """
    try:
        from pcbdraft.agent.browser_registry import (
            list_providers as _list_browser_providers,
        )
        from pcbdraft.agent.extensions.manager import _ensure_plugins_discovered
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `internal tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``internal tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``internal tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from pcbdraft.interfaces.tui.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``internal tools post-setup`` process,
    ...
    bt = sys.modules.get("pcbdraft.tools.browser_tool")
    if bt is not None:
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from pcbdraft.interfaces.tui.nous_subscription import _has_agent_browser
...
def _is_provider_active(
    ...
    *,
    ...
    if plugin_name and not provider.get("managed_nous_feature"):
        ...
        if not (
            isinstance(image_cfg, dict) and image_cfg.get("provider") == plugin_name
        ):
            return False
    ...
    if provider.get("browser_backend"):
        ...
        try:
            from pcbdraft.tools.browser_use_cli import (
                _find_cli,
            ...
            if is_legacy_browser_use_cloud_config(browser_cfg or {}):
                return True
            try:
                from pcbdraft.tools.browser_camofox import is_camofox_mode
...
def _fal_model_catalog():
    ...
    from pcbdraft.tools.image_generation_tool import DEFAULT_MODEL, FAL_MODELS
...
# Kept in sync with the dashboard selects (pcbdraft.interfaces.tui/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `internal model`).
    if managed_feature:
        from pcbdraft.interfaces.tui.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
    ...
    if not env_vars:
        ...
        if plugin_name:
            _select_plugin_image_gen_provider(
                plugin_name, config, use_gateway=bool(managed_feature)
            )
            return
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``internal model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``internal model`` aux-task
    ...
    if not providers:
        _print_warning(
            ...
            "with `pcbdraft connect`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    if provider.get("computer_use_backend"):
        ...
        cu_cfg["backend"] = provider["computer_use_backend"]
        _print_success(
            f"  Computer Use backend set to: {provider['computer_use_backend']}"
        )
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `internal model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict | None = None):
    """Entry point for `internal tools` and `pcbdraft setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ PCBDraft Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print(
        color(
            "  Guide: https://github.com/qixuancao/pcbdraft#readme",
            Colors.DIM,
        )
    )
    print()
    ...
    while True:
        ...
        if idx == _global_idx:
            ...
            if selected_to_configure:
                ...
                print(
                    color(
                        f"  Configuring {len(selected_to_configure)} selected tool(s):",
                        Colors.YELLOW,
                    )
                )
        ...
        if selected_to_configure:
            ...
            for ts_key in selected_to_configure:
                ...
                print(color(f"    • {label}", Colors.DIM))
            print(
                color("  You can skip any tool you don't need right now.", Colors.DIM)
            )
            print()
            for ts_key in selected_to_configure:
                _configure_toolset(ts_key, config)
    ...
    print(
        color(
            f"  Tool configuration saved to {display_runtime_home()}/config.yaml",
            Colors.DIM,
        )
    )
    print(
        color(
            "  Changes take effect on next 'pcbdraft' or gateway restart.", Colors.DIM
        )
    )
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    try:
        from pcbdraft.tools.mcp_tool import probe_mcp_server_tools
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, internal mcp
