"""Client for uploading ``pcbdraft doctor`` bundles to Nous-internal S3.

This is the opt-in (``--nous``) destination for ``pcbdraft doctor``.
...
NAS_BASE = os.environ.get(
    "PCBDRAFT_RUNTIME_DIAGNOSTICS_BASE_URL", "https://portal.nousresearch.com"
)
...
_USER_AGENT = "pcbdraft/debug-share"
...
def request_upload_url(
    ...
    if not isinstance(result, dict) or not result.get("uploadUrl"):
        raise RuntimeError(
            f"diagnostics upload-url response missing 'uploadUrl': {body[:200]}"
        )
    return result
