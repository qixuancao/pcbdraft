...
PCBDraft is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_PCBDRAFT_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `pcbdraft
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from PCBDraft' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    PCBDraft shells out to git from many non-interactive contexts — MCP catalog
...
def kill_process_tree(proc: subprocess.Popen) -> None:
    ...
    if not IS_WINDOWS:
        ...
        try:
            ...
            if pgid == proc.pid:
                os.killpg(
                    pgid, _signal.SIGKILL
                )  # windows-footgun: ok — inside `if not IS_WINDOWS` gate
        except Exception:
            pass
    try:
        proc.kill()
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``internal update`` on slow-WMI machines
