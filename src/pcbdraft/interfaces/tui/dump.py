...
def _gateway_status() -> str:
    ...
    try:
        from pcbdraft.interfaces.tui.gateway import get_gateway_runtime_snapshot
...
def _count_skills(runtime_home: Path) -> int:
...
def _cron_summary(runtime_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = runtime_home / "cron" / "jobs.json"
...
def _config_overrides(config: dict) -> dict[str, str]:
    ...
    """
    from pcbdraft.model.configuration import DEFAULT_CONFIG
...
def run_dump(args):
    ...
    lines.append(f"runtime_home:      {display_runtime_home()}")
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(runtime_home)}")
    lines.append(f"  skills:             {_count_skills(runtime_home)}")
