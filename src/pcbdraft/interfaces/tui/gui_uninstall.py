...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hermes desktop`` / ``hermes gui``)
     Built inside the agent checkout under ``$PCBDRAFT_RUNTIME_HOME/hermes-agent/``:
       ...
       - ``$PCBDRAFT_RUNTIME_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$PCBDRAFT_RUNTIME_HOME``:
...
def _agent_root(runtime_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return runtime_home / "hermes-agent"
...
def source_built_gui_artifacts(runtime_home: Path) -> "list[Path]":
    ...
    """
    agent_root = _agent_root(runtime_home)
    ...
    return [
        ...
        runtime_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        # ``uninstall_gui``.
        from pcbdraft.interfaces.tui.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(runtime_home: Path) -> bool:
    """Return True when a usable Python agent install exists under PCBDRAFT_RUNTIME_HOME.
...
def gui_is_installed(runtime_home: Path) -> bool:
    ...
    for p in source_built_gui_artifacts(runtime_home):
        if p.exists():
            return True
...
def uninstall_gui(
    runtime_home: "Path | None" = None, *, remove_userdata: bool = True
) -> "list[Path]":
    ...
    config / sessions / .env under ``$PCBDRAFT_RUNTIME_HOME``.
