"""``internal journey`` — what PCBDraft has learned, on a timeline.
...
def _primary_hex() -> str:
    ...
    try:
        ...
        return skin.get_color("ui_primary", "") or skin.get_color(
            "banner_title", "#FFD700"
        )
    except Exception:
        return "#FFD700"
...
def _palette() -> dict[str, str]:
    from pcbdraft.agent.learning_graph_render import derive_palette

    return derive_palette(_primary_hex(), dark=True)


def _fade(base: str | None, alpha: float) -> str | None:
    from pcbdraft.agent.learning_graph_render import hex_to_rgb, mix_rgb, rgb_to_hex
...
def _frame_renderable(payload, *, cols, rows, reveal, color):
    ...
    from pcbdraft.agent import learning_graph_render as render
    ...
    title.append(
        "· learned skills & memories over time", style="grey62" if color else None
    )
    parts.append(title)
    ...
    for i, item in enumerate(legend):
        if i:
            legend_line.append("   ")
        legend_line.append(
            item["glyph"] + " ", style=_resolve(item["style"], 1.0) if color else None
        )
    ...
    if categories:
        ...
        for i, item in enumerate(categories):
            ...
            cat_line.append(
                item["glyph"] + " ",
                style=_fade(item.get("color"), 1.0) if color else None,
            )
    ...
    foot.append(
        f"   {frame['visible']}/{count} revealed · {pct}%",
        style="grey54" if color else None,
    )
    parts.append(foot)
    ...
    if labels:
        ...
        def label_row(item) -> Text:
            ...
            row.append(
                f"{item['glyph']} ",
                style=_resolve(item["style"], float(item.get("alpha", 1.0)))
                if color
                else None,
            )
            row.append(
            ...
            meta = str(item["meta"])
            row.append(
                f"  {meta if len(meta) <= 32 else meta[:29] + '…'}",
                style="grey54" if color else None,
            )
            return row
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    console = _console(
        color=color, width=cols, force=bool(getattr(args, "force_color", False))
    )

    if not payload.get("nodes"):
        console.print(
            "[grey62]No learning yet — use PCBDraft a while and your learned skills and "
    ...
    console.print(
        _frame_renderable(payload, cols=cols, rows=rows, reveal=reveal, color=color)
    )
    return 0
...
def _play(console, payload, *, cols, rows, color, fps: int) -> int:
    ...
    try:
        with Live(
            console=console, refresh_per_second=max(1, fps), screen=False
        ) as live:
            for i in range(frames):
                reveal = i / (frames - 1)
                live.update(
                    _frame_renderable(
                        payload, cols=cols, rows=rows, reveal=reveal, color=color
                    )
                )
                time.sleep(delay)
            live.update(
                _frame_renderable(
                    payload, cols=cols, rows=rows, reveal=1.0, color=color
                )
            )
...
def _cmd_list(args: argparse.Namespace) -> int:
    from pcbdraft.agent.learning_graph_render import format_date

    console = _console(
        color=not bool(getattr(args, "no_color", False)),
        force=bool(getattr(args, "force_color", False)),
    )
    nodes = sorted(
        _build_payload().get("nodes", []), key=lambda n: n.get("timestamp") or 0
    )
    if not nodes:
    ...
    for node in nodes:
        ...
        console.print(
            f"[grey54]{node['id']}[/grey54]  {glyph} {node.get('label', '')}  [grey54]{date}[/grey54]"
        )
    return 0


def _cmd_delete(args: argparse.Namespace) -> int:
    from pcbdraft.agent.learning_mutations import delete_node, node_detail
    ...
    if not getattr(args, "yes", False):
        try:
            if input(f"  Delete {detail['label']!r}? [y/N] ").strip().lower() not in (
                "y",
                "yes",
            ):
                print("  aborted")
...
def _cmd_edit(args: argparse.Namespace) -> int:
    from pcbdraft.agent.learning_mutations import edit_node, node_detail
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    parent.add_argument(
        "--play",
        action="store_true",
        help="Animate the build-up over time (Ctrl-C to stop).",
    )
    parent.add_argument(
        "--fps",
        type=int,
        default=12,
        help="Animation frames per second for --play (default 12).",
    )
    parent.add_argument(
        "--width", type=int, default=None, help="Override render width in columns."
    )
    parent.add_argument(
        "--height", type=int, default=None, help="Override render height in rows."
    )
    ...
    parent.add_argument(
        "--json",
        action="store_true",
        help="Print the raw graph payload as JSON and exit.",
    )
    ...
    p_del = sub.add_parser(
        "delete", help="Delete a learned skill (archived) or memory by node id."
    )
    p_del.add_argument(
        "node",
        help="Node id (skill name or memory:<source>:<index>; see `journey list`).",
    )
    p_del.add_argument(
    ...
    p_edit = sub.add_parser(
        "edit", help="Edit a learned skill or memory by node id in $EDITOR."
    )
    p_edit.add_argument(
        "node",
        help="Node id (skill name or memory:<source>:<index>; see `journey list`).",
...
if __name__ == "__main__":
    _p = argparse.ArgumentParser(prog="pcbdraft --help")
    register_cli(_p)
    _a = _p.parse_args()
    sys.exit(_a.func(_a))
