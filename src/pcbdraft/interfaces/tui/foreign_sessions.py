...
class ForeignSession:
    ...
    turn_count: int = 0
    session_id: str | None = None  # the foreign tool's own id
...
def list_claude_sessions(root: Path | None = None) -> list[ForeignSession]:
...
def list_codex_sessions(root: Path | None = None) -> list[ForeignSession]:
...
def _first_user_line(turns: list[tuple[str, str]]) -> str | None:
...
def import_foreign_session(source: str, path, db=None) -> str:
    ...
    parsed = (
        parse_claude_session(path) if source == "claude" else parse_codex_session(path)
    )
    turns = parsed["turns"]
    if not turns:
    ...
    first_user = (
        _first_user_line([(t["role"], t["content"]) for t in turns]) or path.stem
    )
