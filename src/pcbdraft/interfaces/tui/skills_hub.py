...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    """
    from pcbdraft.tools.skills_hub import unified_search
...
def _print_tier1_advisory(skill_dir, console) -> None:
    ...
    """
    try:
        from pcbdraft.tools.skillevaluator_scan import (
            format_tier1_report,
            run_tier1_scan,
...
def _existing_categories() -> list[str]:
    ...
    """
    from pcbdraft.tools.skills_hub import SKILLS_DIR, _category_skill_dirs
...
def do_search(
    ...
    """
    from pcbdraft.tools.skills_hub import (
        GitHubAuth,
...
def do_install(
    ...
    try:
        ...
        from pcbdraft.tools.skills_hub import append_audit_log
    ...
    from pcbdraft.tools.skills_hub import HUB_DIR, source_url_for_bundle
    ...
    try:
        ...
        return
    from pcbdraft.tools.skills_hub import SKILLS_DIR

    c.print(
        f"[bold green]Installed:[/] {install_dir.resolve().relative_to(Path(SKILLS_DIR).resolve()).as_posix()}"
    )
    ...
    try:
        from pcbdraft.tools.blueprints import (
            BlueprintError,
...
def do_inspect(identifier: str, console: Console | None = None) -> None:
    ...
    from pcbdraft.tools.skills_hub import GitHubAuth, create_source_router
...
def do_list(
    ...
    from pcbdraft.tools.skills_hub import HubLockFile, ensure_hub_dirs
    from pcbdraft.tools.skills_sync import _read_manifest
...
def do_check(name: str | None = None, console: Console | None = None) -> None:
    ...
    from pcbdraft.tools.skills_hub import check_for_skill_updates
...
def do_update(
    name: str | None = None, console: Console | None = None, force: bool = False
) -> None:
    ...
    """
    from pcbdraft.tools.skills_guard import content_hash
    from pcbdraft.tools.skills_hub import (
        SKILLS_DIR,
        HubLockFile,
...
def do_audit(
    name: str | None = None, console: Console | None = None, deep: bool = False
) -> None:
    ...
    are unchanged).
    """
    from pcbdraft.tools.skills_guard import format_scan_report, scan_skill
    from pcbdraft.tools.skills_hub import SKILLS_DIR, HubLockFile
    ...
    if deep:
        from pcbdraft.tools.skills_ast_audit import ast_scan_path, format_ast_report
...
def do_uninstall(
    ...
    from pcbdraft.tools.skills_hub import uninstall_skill
...
def do_reset(
    ...
    from pcbdraft.tools.skills_sync import reset_bundled_skill
...
def do_diff(name: str, console: Console | None = None) -> None:
    ...
    from pcbdraft.tools.skills_sync import diff_bundled_skill
...
def do_opt_out(
    ...
    never touched.
    """
    from pcbdraft.tools.skills_sync import (
...
def do_opt_in(
    sync: bool = False, console: Console | None = None, invalidate_cache: bool = True
) -> None:
...
def do_repair_official(
    ...
    from pcbdraft.tools.skills_sync import restore_official_optional_skill
...
def do_tap(action: str, repo: str = "", console: Console | None = None) -> None:
    ...
    from pcbdraft.tools.skills_hub import TapsManager
...
def do_publish(
    ...
    from pcbdraft.tools.skills_guard import format_scan_report, scan_skill
    from pcbdraft.tools.skills_hub import SKILLS_DIR, GitHubAuth
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print(
                ...
                f"Set GITHUB_TOKEN in {display_runtime_home()}/.env or run 'gh auth login'.\n"
            )
            return
...
def do_snapshot_export(output_path: str, console: Console | None = None) -> None:
    ...
    from pcbdraft.tools.skills_hub import HubLockFile, TapsManager
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        do_update(name=getattr(args, "name", None), force=getattr(args, "force", False))
    elif action == "audit":
