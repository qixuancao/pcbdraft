...
remains ``internal cron`` / the ``cronjob`` tool, which runs in isolated
...
def _get_session_db() -> Any | None:
    # Reuse the goals module's per-PCBDRAFT_RUNTIME_HOME cached SessionDB so both
    # features share one connection instead of thrashing the file.
    try:
        from pcbdraft.interfaces.tui.goals import _get_session_db as _goals_db
...
class HeartbeatManager:
    ...
    def status_line(self) -> str:
        ...
        if s.status == "active":
            ...
            return (
                f"♥ Heartbeat (every {every}, next in ~{next_in}s{fired}): {s.prompt}"
            )
        if s.status == "paused":
