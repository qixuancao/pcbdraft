...
def run_verify_command(args) -> int:
    from pcbdraft.agent.verify import (
        load_or_detect,
        manifest_path,
        run_verify,
        save_manifest,
...
def _merge_project_facts_commands(root: Path, recipe) -> None:
    ...
    """
    try:
        from pcbdraft.agent.coding_context import detect_project_facts

        facts_commands = list(detect_project_facts(root).verify_commands)
    except Exception:
        return
    existing = {
        c.strip() for c in (*recipe.bootstrap, *recipe.build, *recipe.test) if c
    }
...
def _record_evidence(root: Path, recipe, result, *, args) -> None:
    ...
    try:
        ...
        if result.readiness is not None:
            r = result.readiness
            readiness_line = f"[start] {recipe.start} -> " + (
                f"ready (HTTP {r.status_code})"
                if r.ready
                else f"not ready ({r.error or 'timeout'})"
            )
...
def _print_human_report(recipe, source, result) -> None:
    ...
    if result.phases:
        ...
        for p in result.phases:
            ...
            print(
                f"  {p.phase.ljust(width)}  {status:<7}  {p.duration:6.1f}s  {p.command}"
            )
    else:
    ...
    if result.readiness is not None:
        ...
        print(
            f"  {'start'.ljust(max(5, max((len(p.phase) for p in result.phases), default=5)))}  "
    ...
    if (
        result.readiness is not None
        and not result.readiness.ready
        and result.readiness.output_tail
    ):
        print("\n--- output tail: start ---")
