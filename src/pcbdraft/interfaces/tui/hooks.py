"""internal hooks — inspect and manage shell-script hooks.

Usage::

    internal hooks list
    internal hooks test <event> [--for-tool X] [--payload-file F]
    internal hooks revoke <command>
    internal hooks doctor

Consent records live under ``~/.pcbdraft/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.pcbdraft/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``internal hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: pcbdraft --help")
        print("Run 'pcbdraft --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    from pcbdraft.agent import outbound_webhooks, shell_hooks
    ...
    if not specs and not outbound:
        print(
            "No shell hooks or outbound webhooks configured in ~/.pcbdraft/config.yaml."
        )
        print("See `pcbdraft --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.pcbdraft/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `pcbdraft --help` to re-validate"
                            )
            print()
...
# stdin a script sees under `internal hooks test` and `internal hooks doctor`
...
def _print_run_result(result: dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (PCBDraft wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(
                ...
                f"then `pcbdraft --help` + re-approve to refresh"
            )
    ...
    # is already allowlisted.  Otherwise `internal hooks doctor` would execute
