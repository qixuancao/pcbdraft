"""``internal gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.pcbdraft/auth.json``) — the same path ``internal dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.pcbdraft/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_connector_url(override: str | None) -> str | None:
    ...
    if not raw:
        try:
            from pcbdraft.services.messaging.run import (
                _load_gateway_config,  # late import to avoid cycle
            )
...
def _resolve_identity_token() -> str:
    ...
    """
    from pcbdraft.services.messaging.relay import _resolve_relay_identity_token
...
def _post_enroll(
    *,
    ...
    data = json.dumps(
        {"enrollmentToken": enrollment_token, "gatewayId": gateway_id}
    ).encode("utf-8")
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `pcbdraft connect` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from pcbdraft.model.configuration import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `pcbdraft --help` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `pcbdraft setup` (or `pcbdraft connect`) first, then retry.")
        else:
    ...
    #    save_env_value writes them to ~/.pcbdraft/.env (0600 dir) and never logs.
    to_write = {
