...
Hermes owns its own uv binary at ``$PCBDRAFT_RUNTIME_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$PCBDRAFT_RUNTIME_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    ...
    ``$PCBDRAFT_RUNTIME_HOME/bin/uv`` on POSIX, ``$PCBDRAFT_RUNTIME_HOME\\bin\\uv.exe`` on
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    and, offline, an unbounded hang risk. A stamp file under PCBDRAFT_RUNTIME_HOME
    caches the last successful self-update time.
    """
    try:
        from pcbdraft.core.runtime_environment import get_runtime_home

        stamp = get_runtime_home() / "cache" / ".uv_self_update_stamp"
    ...
    *,
    ...
    if force or not _uv_self_update_is_fresh():
        try:
            ...
            logger.debug(
                "uv self update timed out after %ss", UV_SELF_UPDATE_TIMEOUT_SECONDS
            )
            result = None
...
def _reload_hermes_constants():
    ...
    """
    import pcbdraft.core.runtime_environment as hermes_constants
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("pcbdraft.interfaces.tui.main")
...
def _install_uv(target: Path) -> None:
    ...
    ``$PCBDRAFT_RUNTIME_HOME/bin/`` instead of ``~/.local/bin/``.
    """
