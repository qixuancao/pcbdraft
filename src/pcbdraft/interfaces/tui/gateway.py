...
# display_runtime_home is imported lazily at call sites to avoid ImportError
...
def probe_gateway_loop_liveness(
    ...
    *,
    ...
    try:
        from pcbdraft.services.messaging.shutdown_watchdog import (
...
def _scan_gateway_pids(
    ...
    current_home = str(get_runtime_home().resolve())
    # Forward slashes on both sides of the PCBDRAFT_RUNTIME_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"runtime_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. PCBDRAFT_RUNTIME_HOME
        ...
        # PCBDRAFT_RUNTIME_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "runtime_home=" in command_lc
            and f"runtime_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # watchdog probe.
            from pcbdraft.interfaces.tui._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    if not all_profiles:
        try:
            from pcbdraft.core.runtime_process import get_running_pid
...
def find_profile_gateway_processes(
    ...
    try:
        from pcbdraft.core.runtime_process import get_running_pid
        from pcbdraft.interfaces.tui.profiles import list_profiles
    except Exception:
        return processes
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    try:
        from pcbdraft.core.runtime_process import looks_like_gateway_command_line
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose PCBDRAFT_RUNTIME_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # interpreter and captures the stable cwd + env overlay (PCBDRAFT_RUNTIME_HOME,
    ...
    if sys.platform == "win32":
        try:
            from pcbdraft.interfaces.tui.gateway_windows import (
    ...
    watcher = (
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / PCBDRAFT_RUNTIME_HOME) the windowless
...
def _runtime_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``PCBDRAFT_RUNTIME_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("PCBDRAFT_RUNTIME_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_runtime_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``PCBDRAFT_RUNTIME_HOME``.

    Under ``sudo``, ``PCBDRAFT_RUNTIME_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_runtime_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``PCBDRAFT_RUNTIME_HOME`` for the actual gateway
    ...
    unit_home = (_runtime_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = (
            _read_systemd_unit_environment(system=True)
            .get("PCBDRAFT_RUNTIME_HOME", "")
            .strip()
        )
    if not unit_home:
        return
    current = os.environ.get("PCBDRAFT_RUNTIME_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["PCBDRAFT_RUNTIME_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from pcbdraft.interfaces.tui.service_manager import (
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            ...
            from pcbdraft.interfaces.tui.gateway_windows import get_task_name
    ...
    from pcbdraft.core.runtime_process import _pid_exists, write_planned_stop_marker
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (PCBDRAFT_RUNTIME_HOME-scoped).
    ...
    """
    try:
        from pcbdraft.core.runtime_process import get_running_pid, remove_pid_file
    except ImportError:
        return False
    ...
    try:
        from pcbdraft.core.runtime_process import write_planned_stop_marker
...
from pcbdraft.core.runtime_environment import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    ``PCBDRAFT_RUNTIME_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("PCBDRAFT_RUNTIME_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current PCBDRAFT_RUNTIME_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (PCBDRAFT_RUNTIME_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    home = get_runtime_home().resolve()
    default = get_default_runtime_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary PCBDRAFT_RUNTIME_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(
    runtime_home: str | None = None, default_root: str | Path | None = None
) -> str:
    """Return ``--profile <name>`` only when PCBDRAFT_RUNTIME_HOME is a named profile.

    For ``~/.hermes/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        runtime_home: Optional explicit PCBDRAFT_RUNTIME_HOME path. Defaults to the current
            ``get_runtime_home()`` value. Should be passed when generating a
        ...
        default_root: Optional Hermes root to compare against. Used when
            ...
            process, where ``Path.home()`` and ``get_default_runtime_root()``
    ...
    home = Path(runtime_home or str(get_runtime_home())).resolve()
    default = (
        Path(default_root).resolve()
        if default_root
        else get_default_runtime_root().resolve()
    )
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def _profile_arg_for_target_user(runtime_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".hermes"
    try:
        Path(runtime_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(runtime_home, default_root=target_root)
    except ValueError:
        return _profile_arg(runtime_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this PCBDRAFT_RUNTIME_HOME.

    Default ``~/.hermes`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.hermes/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other PCBDRAFT_RUNTIME_HOME appends a short hash for uniqueness.
    """
...
def _runtime_dir_is_ours(runtime_dir: str) -> bool:
    ...
    try:
        return (
            Path(runtime_dir).stat().st_uid == os.getuid()
        )  # windows-footgun: ok — POSIX systemd helper, never invoked on Windows
    except OSError:
        return False
...
def _user_systemd_socket_ready() -> bool:
    ...
    """
    return _path_exists_safe(_user_dbus_socket_path()) or _path_exists_safe(
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            from pcbdraft.interfaces.tui.managed_uv import _reload_hermes_constants
...
def _runtime_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current PCBDRAFT_RUNTIME_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_runtime_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes_raw = os.environ.get("PCBDRAFT_RUNTIME_HOME", "").strip()
    current_hermes = (
    ...
    # different PCBDRAFT_RUNTIME_HOME into the generated service unit.
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    hermes_node = runtime_home / "node" / "bin"
    if _is_dir(hermes_node):
        candidates.append(str(hermes_node))
    hermes_nm = runtime_home / "node_modules" / ".bin"
...
def _stable_service_working_dir() -> str:
    ...
    ``PCBDRAFT_RUNTIME_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if PCBDRAFT_RUNTIME_HOME
...
def _systemd_watchdog_seconds(runtime_home: str | Path | None = None) -> int:
    ...
    if runtime_home is not None:
        ...
        override_token = set_runtime_home_override(runtime_home)
        reset_home_override = reset_runtime_home_override
    try:
...
def _systemd_watchdog_service_fields(
    ...
    seconds = _systemd_watchdog_seconds(runtime_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], runtime_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Hermes-managed Node under ``$PCBDRAFT_RUNTIME_HOME/node`` goes first when it
    ...
    *runtime_root* is the Hermes home the unit will run against. System units
    ...
    managed_node_present = hermes_managed_node_tree_present(runtime_root)
    for directory in (
        iter_hermes_node_dirs(runtime_root) if managed_node_present else ()
    ):
        entry = str(directory)
        try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        runtime_home = _runtime_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            runtime_home
        )
        profile_arg = _profile_arg_for_target_user(runtime_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's PCBDRAFT_RUNTIME_HOME (stable, always exists)
        ...
        # the remapped runtime_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(runtime_home) if runtime_home else None
        )
        path_entries = [
...
Environment="PCBDRAFT_RUNTIME_HOME={runtime_home}"
Restart=always
RestartSec=5
...
"""
    ...
    runtime_home = str(get_runtime_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        runtime_home
    )
    profile_arg = _profile_arg(runtime_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    # ── PCBDRAFT_RUNTIME_HOME sync chokepoint ──────────────────────────────────────
    ...
    # pinned PCBDRAFT_RUNTIME_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo hermes gateway … --system``, PCBDRAFT_RUNTIME_HOME is often stripped
    # and falls back to ``/root/.hermes``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / PCBDRAFT_RUNTIME_HOME comparisons
    ...
    _sync_runtime_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir PCBDRAFT_RUNTIME_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved PCBDRAFT_RUNTIME_HOME
    ...
    exported a throwaway ``PCBDRAFT_RUNTIME_HOME=/tmp/...`` — writing it to the real
    ...
    with ``PCBDRAFT_RUNTIME_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="PCBDRAFT_RUNTIME_HOME=..."`` lines and launchd
    ``<key>PCBDRAFT_RUNTIME_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'PCBDRAFT_RUNTIME_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>PCBDRAFT_RUNTIME_HOME</key>\s*<string>(.*?)</string>",
        definition,
        flags=re.S,
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp PCBDRAFT_RUNTIME_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: PCBDRAFT_RUNTIME_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported PCBDRAFT_RUNTIME_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single PCBDRAFT_RUNTIME_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only PCBDRAFT_RUNTIME_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp PCBDRAFT_RUNTIME_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="PCBDRAFT_RUNTIME_HOME=..."``. Writing that to the developer's
    ...
    # PCBDRAFT_RUNTIME_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("PCBDRAFT_RUNTIME_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("PCBDRAFT_RUNTIME_RESTART_AFTER_TURN_TIMEOUT")
    ...
    if isinstance(agent_cfg, dict) and "restart_after_turn_timeout" in agent_cfg:
        return parse_restart_after_turn_timeout(
            agent_cfg.get("restart_after_turn_timeout")
        )
...
def systemd_install(
    ...
    # Existing system units already pin PCBDRAFT_RUNTIME_HOME; adopt it before any
...
def systemd_start(system: bool = False):
    ...
    # PCBDRAFT_RUNTIME_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_runtime_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_runtime_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    ...
    Profile-aware: honors the active PCBDRAFT_RUNTIME_HOME via `_profile_arg()` so the
...
def _spawn_detached_gateway() -> bool:
    ...
    working.
    """
    from pcbdraft.interfaces.tui._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_runtime_home() / "logs"
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from pcbdraft.interfaces.tui.resource_limits import configured_nofile_soft_limit
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        reload_log_path = get_runtime_home() / "logs" / "launchd-reload.log"
        try:
...
def _wait_for_gateway_exit(
    ...
    PCBDRAFT_RUNTIME_HOME directories.
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_runtime_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``PCBDRAFT_RUNTIME_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        ...
        from pcbdraft.core.runtime_process import (
            get_running_pid as _default_running_pid,
        )  # noqa
    except Exception:
        return
    ...
    try:
        ...
        from pcbdraft.core.runtime_process import _read_pid_record  # type: ignore
        ...
        if not rec:
            return
        from pcbdraft.core.runtime_process import _pid_exists, _pid_from_record
        ...
        # to config.yaml.
        from pcbdraft.services.messaging.config import _env_multiplex_profiles_override
        ...
        from pcbdraft.interfaces.tui.profiles import normalize_profile_name
        from pcbdraft.services.messaging.config import (
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    unrelated gateway processes from other PCBDRAFT_RUNTIME_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("PCBDRAFT_RUNTIME_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$PCBDRAFT_RUNTIME_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set PCBDRAFT_RUNTIME_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(
    ...
    # Service-style launchers set PCBDRAFT_RUNTIME_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = os.getenv(
        "PCBDRAFT_RUNTIME_GATEWAY_DETACHED", ""
    ...
    # PCBDRAFT_RUNTIME_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("PCBDRAFT_RUNTIME_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``PCBDRAFT_RUNTIME_GATEWAY_MAX_STARTS`` /
    # ``PCBDRAFT_RUNTIME_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    # Set max_starts <= 0 to disable. Best-effort: a bookkeeping failure must
    # never block startup.
    try:
        import time as _time

        from pcbdraft.core.runtime_process import record_start_and_check_storm
        ...
        try:
            _env_starts = os.getenv("PCBDRAFT_RUNTIME_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("PCBDRAFT_RUNTIME_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        ...
        from pcbdraft.services.messaging.run import _exit_after_graceful_shutdown
...
def _platform_status(platform: dict) -> str:
    ...
    if entry is not None:
        ...
        if entry.is_connected is not None:
            try:
                from pcbdraft.services.messaging.config import PlatformConfig
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_runtime_home() / "whatsapp" / "session" / "creds.json"
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        from pcbdraft.core.runtime_process import parse_active_agents
...
def _setup_weixin():
    ...
    try:
        from pcbdraft.services.messaging.platforms.weixin import (
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_runtime_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_qqbot():
    ...
    if method_idx == 0:
        ...
        try:
            from pcbdraft.services.messaging.platforms.qqbot import qr_register
...
def _builtin_setup_fn(key: str):
    ...
    """
    from pcbdraft.interfaces.tui import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    if profile is None:
        ...
        # PCBDRAFT_RUNTIME_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      2. ``PCBDRAFT_RUNTIME_S6_SUPERVISED_CHILD`` is exported by
         ...
         → ...).
      3. ``--no-supervise`` (or ``PCBDRAFT_RUNTIME_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or os.environ.get(
        "PCBDRAFT_RUNTIME_GATEWAY_NO_SUPERVISE", ""
    ).lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("PCBDRAFT_RUNTIME_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${PCBDRAFT_RUNTIME_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if PCBDRAFT_RUNTIME_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or PCBDRAFT_RUNTIME_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from pcbdraft.interfaces.tui.service_manager import detect_service_manager
