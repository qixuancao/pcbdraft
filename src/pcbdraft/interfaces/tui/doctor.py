...
from pcbdraft.core.runtime_environment import is_termux as _is_termux
...
def _pcbdraft_database_paths(runtime_home: Path) -> list[tuple[str, Path]]:
    ...
    entries = [
        (name, runtime_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((runtime_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(runtime_home)), board_db))
    return entries
...
def _format_db_size(db_path: Path) -> str:
    ...
    from pcbdraft.interfaces.tui.backup import _format_size
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("PCBDRAFT_RUNTIME_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from pcbdraft.model.auth import get_minimax_oauth_auth_status
...
from pcbdraft.interfaces.tui.sizefmt import format_bytes as _human_bytes
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. PCBDRAFT_RUNTIME_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    (
        "PCBDRAFT_RUNTIME_TOOL_PROGRESS",
        "display.tool_progress in config.yaml — ignored/unsupported since config floor v12",
    ),
    ("PCBDRAFT_RUNTIME_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    """
    try:
        from pcbdraft.interfaces.tui import __version__ as init_version
    except Exception:
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    """
    try:
        from pcbdraft.agent.errors import SSLConfigurationError
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        from pcbdraft.model.provider_profiles.base import ProviderProfile as _PP
...
def managed_scope_check() -> None:
    ...
    resolved from the PCBDRAFT_RUNTIME_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("PCBDRAFT_RUNTIME_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via PCBDRAFT_RUNTIME_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    if ack_target:
        from pcbdraft.interfaces.tui.security_advisories import ADVISORIES, ack_advisory
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from pcbdraft.model.configuration import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from pcbdraft.model.configuration import atomic_config_write
        ...
        # Detect stale PCBDRAFT_RUNTIME_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives PCBDRAFT_RUNTIME_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            env_ghost = load_env().get("PCBDRAFT_RUNTIME_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                ...
                if should_fix:
                    if remove_env_value("PCBDRAFT_RUNTIME_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale PCBDRAFT_RUNTIME_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn(
                            "Could not remove PCBDRAFT_RUNTIME_MAX_ITERATIONS from .env"
                        )
                        manual_issues.append(
                            "Manually delete the PCBDRAFT_RUNTIME_MAX_ITERATIONS line from "
        ...
        try:
            from pcbdraft.model.configuration import load_env as _load_env_depr
            from pcbdraft.model.configuration import (
                read_user_config_raw as _read_raw_depr,
            )
    ...
    runtime_home = PCBDRAFT_RUNTIME_HOME
    ...
    for subdir_name in expected_subdirs:
        subdir_path = runtime_home / subdir_name
    ...
    soul_path = runtime_home / "SOUL.md"
    if soul_path.exists():
    ...
    memories_dir = runtime_home / "memories"
    ...
    state_db_path = runtime_home / "state.db"
    ...
    wal_path = runtime_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    try:
        from pcbdraft.core.runtime_environment import is_container as _is_container
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                if warm_agent_browser_npx_cache():
                    ...
                    check_info(
                        "  Could not warm npx cache (offline or npx unavailable)"
                    )
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable PCBDRAFT_RUNTIME_HOME mirror
    ...
    def _probe_apikey_provider(
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from pcbdraft.model.auxiliary_client import _to_openai_base_url
    ...
    _section("Skills Hub")
    hub_dir = PCBDRAFT_RUNTIME_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        from pcbdraft.model.configuration import read_user_config_raw as _read_raw_mem

        _mem_cfg_path = PCBDRAFT_RUNTIME_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            from pcbdraft.agent.memory_backends.mem0 import (
                _load_config as _load_mem0_config,
            )
