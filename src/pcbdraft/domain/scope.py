...
from pcbdraft.domain.ir import Design, Scope
...
def evaluate_scope(scope: Scope) -> ScopeDecision:
    ...
    if complex_domains:
        warnings.append(
            ...
            "but PCBDraft does not validate domain-specific electrical, regulatory, "
    ...
    if scope.max_voltage_v > 60:
        warnings.append(
            "declared voltage exceeds 60 V; PCBDraft does not validate clearance, "
    ...
    if scope.risk_class not in {"prototype", "non_safety_critical", "unspecified"}:
        warnings.append(
            ...
            "PCBDraft safety or compliance claim"
        )
