...
def apply_kicad_import(
    ...
    *,
    ...
    transaction_parent = project.root.parent / ".pcbdraft-transactions"
    ...
    receipt: dict[str, Any] = {
        "schema": "pcbdraft-project-transaction",
        "version": 1,
        "kind": "kicad_import",
        "status": "preparing",
    ...
    try:
        ...
        atomic_write_json(receipt_path, receipt)
        lock_parent = project.root.parent / ".pcbdraft-locks"
...
def undo_kicad_import(transaction_value: str | Path) -> Path:
    ...
    if (
        ...
        or receipt.get("schema") != "pcbdraft-project-transaction"
    ):
    ...
    with ResourceLock(project, project.parent / ".pcbdraft-locks", timeout=10.0):
