...
class SessionSearchMixin:
    ...
    def _search_message_fields(
        ...
        if unknown:
            raise ValueError(
                f"unknown search result field(s): {', '.join(sorted(unknown))}"
            )
        return tuple(
    ...
    def _fts_external_index_empty_with_messages(self, conn) -> bool:
        ...
        """
        try:
            has_msg = conn.execute("SELECT EXISTS(SELECT 1 FROM messages)").fetchone()[
                0
            ]
            if not has_msg:
                return False
    ...
    def optimize_fts_storage(
        ...
        *,
        ...
        if still_pending or still_trash or empty_index:
            reason = (
                "backfill_incomplete"
                if still_pending or empty_index
    ...
    def _trigram_eligible_tokens(query: str) -> bool:
        ...
        tokens = [
            t
            for t in query.strip('"').strip().split()
    ...
    def search_messages(
        ...
        Threshold: PCBDRAFT_RUNTIME_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("PCBDRAFT_RUNTIME_SEARCH_SLOW_MS", "1000"))
    ...
    def _search_messages_like_fallback(
        ...
        *,
        ...
        """
        with self._read_ctx() as conn:
            rows = conn.execute(sql, [snippet_term, *params, limit, offset]).fetchall()
    ...
    def _finalize_search_matches(
        ...
        result_fields: Collection[str] | None = None,
    ) -> list[dict[str, Any]]:
        ...
        if is_cjk:
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        "messages_fts_cjk query failed; falling back to trigram/LIKE",
                        exc_info=True,
                    )
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``PCBDRAFT_RUNTIME_DISABLE_FTS_TRIGRAM`` or not yet created), so
    ...
    def _merge_fts_incrementally(
        self, *, max_pages: int, max_commands: int | None = None
    ) -> int:
        ...
        with self._lock:
            for tbl in self._FTS_TABLES:
                ...
                if not getattr(self, "_fts_usermerge_floor_applied", False):
                    self._conn.execute(
                        f"INSERT INTO {tbl}({tbl}, rank) VALUES('usermerge', 2)"
                    )
