...
from pcbdraft.services import (
    application_part_registration as _application_part_registration,
)
...
_application_part_registration._configure_legacy_application_hooks(
    validation_error_hook=lambda message: ValidationError(message),
    resource_lock_hook=lambda *args, **kwargs: ResourceLock(*args, **kwargs),
    open_managed_project_hook=lambda *args, **kwargs: open_managed_project(
        *args, **kwargs
    ),
    canonical_json_bytes_hook=lambda value: canonical_json_bytes(value),
    new_run_id_hook=lambda: new_run_id(),
    make_directory_hook=lambda path: make_directory(path),
    utc_timestamp_hook=lambda: utc_timestamp(),
    attach_progress_hook=lambda *args, **kwargs: _attach_progress(*args, **kwargs),
    atomic_write_json_hook=lambda *args, **kwargs: atomic_write_json(*args, **kwargs),
    load_generation_request_hook=lambda path: load_generation_request(path),
    materialize_managed_design_hook=lambda *args, **kwargs: materialize_managed_design(
        *args, **kwargs
    ),
    ...
    sanitize_secret_text_hook=lambda value: _sanitize_secret_text(value),
    pcb_operation_postcondition_error_hook=lambda *args, **kwargs: (
        _PCBOperationPostconditionError(*args, **kwargs)
    ),
    design_from_dict_hook=lambda value: Design.from_dict(value),
    installed_kicad_part_hook=lambda *args, **kwargs: PartGraph.installed_kicad_part(
        *args, **kwargs
    ),
    pcbdraft_error_type_hook=lambda: PCBDraftError,
)
...
class ApplicationService(
    ...
    def _modification_apply_unknown_progress(revision: int) -> Any:
        return ProgressVector.unknown(revision)

    @staticmethod
    def _modification_apply_not_started_stage() -> Any:
        return EngineeringStage.NOT_STARTED

    @staticmethod
    def _modification_apply_stage_projection(
        *args: Any,
        **kwargs: Any,
    ) -> Any:
        return StageProjection(*args, **kwargs)
