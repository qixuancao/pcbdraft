...
from pcbdraft.services import session_db_lifecycle as _session_db_lifecycle
...
from pcbdraft.services.session_db_compression_health import (
    SessionCompressionHealthMixin,
)
from pcbdraft.services.session_db_compression_lease import (
    SessionCompressionLeaseMixin,
)
...
from pcbdraft.services.session_db_gateway_queries import SessionGatewayQueryMixin
from pcbdraft.services.session_db_gateway_routing import SessionGatewayRoutingMixin
...
from pcbdraft.services.session_db_turn_lease import SessionTurnLeaseMixin
...
_session_db_lifecycle.configure_session_lifecycle_hooks(
    _session_db_lifecycle.SessionLifecycleHooks(
        system_prompt_hash=lambda system_prompt: _system_prompt_hash(system_prompt),
        reset_end_reasons=lambda: _RESET_END_REASONS,
        legacy_reset_child_sql=lambda alias, reasons_sql: _legacy_reset_child_sql(
            alias, reasons_sql
        ),
        now=lambda: time.time(),
        json_dumps=lambda value, **kwargs: json.dumps(value, **kwargs),
        json_loads=lambda value: json.loads(value),
        compression_busy_error=lambda message: CompressionSessionBusyError(message),
        compression_closed_error=lambda session_id: CompressionSessionClosedError(
            session_id
        ),
        compression_in_progress_error=lambda message: SessionCompressionInProgressError(
            message
        ),
        log_debug=lambda message, *args, **kwargs: logger.debug(
            message, *args, **kwargs
...
class SessionDB(
    ...
    SessionCompressionHealthMixin,
    SessionCompressionLeaseMixin,
    SessionTurnLeaseMixin,
    SessionLifecycleMixin,
    SessionGatewayRoutingMixin,
    SessionGatewayQueryMixin,
    ...
    # Compatibility hooks for compression health state moved to a mixin.
    # Resolve legacy runtime globals at call time so established time, logger,
    # and SQLite monkeypatch paths remain effective without a reverse import.
    @staticmethod
    def _compression_health_now() -> float:
        return time.time()

    @staticmethod
    def _compression_health_log_warning(message: str, *args) -> None:
        logger.warning(message, *args)

    @staticmethod
    def _compression_health_is_sqlite_row(row) -> bool:
        return isinstance(row, sqlite3.Row)

    @staticmethod
    def _compression_health_sqlite_error_type():
        return sqlite3.Error

    # Compatibility hook shared by compression and session-turn leases.
    # Resolve the legacy helper at call time so existing monkeypatch paths
    # remain effective without a reverse import from the compression mixin.
    @staticmethod
    def _compression_lease_holder_process_is_dead(holder: str) -> bool:
        return _compression_lock_holder_process_is_dead(holder)

    # Compatibility hooks for session-turn leases moved to a mixin. Resolve
    # legacy runtime globals at call time so time, SQLite, logger, liveness,
    # and persistence-classifier monkeypatch paths remain effective.
    @staticmethod
    def _turn_lease_now() -> float:
        return time.time()

    @staticmethod
    def _turn_lease_monotonic() -> float:
        return time.monotonic()

    @staticmethod
    def _turn_lease_sleep(seconds: float) -> None:
        time.sleep(seconds)

    @staticmethod
    def _turn_lease_holder_process_is_dead(holder: str) -> bool:
        return _compression_lock_holder_process_is_dead(holder)

    @staticmethod
    def _turn_lease_log_debug(message: str, *args, **kwargs) -> None:
        logger.debug(message, *args, **kwargs)

    @staticmethod
    def _turn_lease_sqlite_error_type():
        return sqlite3.Error

    @staticmethod
    def _turn_lease_classify_persistence_error(exc: BaseException) -> str:
        return classify_persistence_error(exc)
    ...
    # Compatibility hooks for gateway peer queries moved to a mixin. Resolve
    # legacy SQL helpers at call time so existing monkeypatch paths remain live.
    @staticmethod
    def _gateway_queries_last_active_sql(alias: str) -> str:
        return _sql_session_last_active(alias)

    @staticmethod
    def _gateway_queries_reset_end_reasons_sql() -> str:
        return _RESET_END_REASONS_SQL

    # Compatibility hooks for gateway routing state moved to a mixin. Resolve
    # legacy module globals at call time so existing time, JSON, logger, and
    # exception-info monkeypatch paths remain effective without a reverse import.
    @staticmethod
    def _gateway_routing_now() -> float:
        return time.time()

    @staticmethod
    def _gateway_routing_json_loads(value: str) -> Any:
        return json.loads(value)

    @staticmethod
    def _gateway_routing_log_debug(message: str, *args, **kwargs) -> None:
        logger.debug(message, *args, **kwargs)

    @staticmethod
    def _gateway_routing_exception_info():
        return _exception_info_without_values()
    ...
    # =========================================================================
    # Gateway orphan adoption
    # =========================================================================

    # Write half of orphaned gateway-session repair; candidate discovery stays
    # read-only in SessionGatewayQueryMixin.
