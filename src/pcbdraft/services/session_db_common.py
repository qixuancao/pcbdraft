...
Originally extracted from upstream hermes_state.py. Shared by the SessionDB
search, schema and portability mixins without
importing session_db (which would be a cycle). The session_db module
...
# optimized via the offline storage helper. A legacy DB sits at
...
# The offline storage helper rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into storage optimization. This is
