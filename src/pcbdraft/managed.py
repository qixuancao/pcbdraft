...
from .component_qualification import (
    COMPONENT_QUALIFICATION_NAME,
...
from .ir import Design, canonical_json_bytes, load_design
...
MANAGED_SCHEMA = "pcbdraft-managed-project"
MANAGED_VERSION = 1
MANAGED_MANIFEST = "project.pcbdraft.json"
...
class ManagedProject:
    ...
    qualification: ComponentQualificationReport | None
...
def materialize_managed_design(
    ...
    *,
    ...
    qualification: ComponentQualificationReport | None = None
    if isinstance(requirements, AgentDesignRequest):
        ...
        qualification = qualify_components(design, resolved_graph)
        if qualification.pad_mapping_failures:
            raise ValidationError(
                "component qualification contains invalid local pad mappings"
            )
        if (
            design.metadata.get("component_qualification_schema")
            != COMPONENT_QUALIFICATION_SCHEMA
            or design.metadata.get("component_qualification_hash")
            != qualification.sha256()
        ):
            raise ValidationError(
                "semantic design does not match its component qualification evidence"
            )
    ...
    with ResourceLock(target, parent / ".pcbdraft-locks", timeout=lock_timeout):
        ...
        try:
            ...
            if qualification is not None:
                atomic_write_bytes(
                    temporary / COMPONENT_QUALIFICATION_NAME,
            ...
            if qualification is not None:
                files["component_qualification"] = COMPONENT_QUALIFICATION_NAME
            hashes = {
...
def open_managed_project(value: str | Path) -> ManagedProject:
    ...
    qualification_path = paths.get("component_qualification")
    qualification = (
        ComponentQualificationReport.from_dict(
            load_json_limited(qualification_path, MANAGED_MANIFEST_LIMIT)
        )
    ...
    if isinstance(generation_request, AgentDesignRequest):
        ...
        expected_qualification_hash = design.metadata.get(
            "component_qualification_hash"
        )
        if qualification is None:
            if expected_qualification_hash is not None:
                raise ValidationError(
                    "managed generic project is missing component qualification evidence"
                )
        elif (
            qualification.design_id != design.design_id
            or qualification.part_catalog_hash
            != hashlib.sha256(canonical_json_bytes(graph.to_dict())).hexdigest()
            or qualification.sha256() != expected_qualification_hash
        ):
            raise ValidationError(
                "managed component qualification does not match project provenance"
            )
    elif plan is not None:
        raise ValidationError(
        ...
        raise ValidationError(
            "legacy managed project cannot contain component qualification evidence"
        )
    project = ManagedProject(
        root=root,
...
def load_generation_request(path: str | Path) -> GenerationRequest:
    ...
    if schema == "pcbdraft-requirements":
