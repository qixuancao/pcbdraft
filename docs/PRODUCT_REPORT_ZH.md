...
|---|---:|---|---|---|
| `low_voltage_i2c_controller_v1` | 2 | `5ce0faeded134f02b8adce95ec1d66919b7a9bef259ea026e2a61eac3fb7d722` | L0–L3 pass；真实 ERC/DRC 无违规 | `true` / `false` |
| `low_voltage_spi_environment_v1` | 2 | `7ee307abb58b16fcc5c7043149df544686a26851abc720cd4e6b7d395719b9bf` | L0–L3 pass；真实 ERC/DRC 无违规 | `true` / `false` |
| `low_voltage_uart_ldo_controller_v1` | 4 | `8330d3b74ae722d0ec7ef38b8045e934f14abea8d3847b0ee8c3c98aac70fa5e` | L0–L3 pass；真实 ERC/DRC 无违规 | `true` / `false` |
...
2→4 层语义预览和应用、原哈希
`e1ac4515d3c5c14d1134e5ee49b496f1f70c5f22059a2ea852ccf7e5c86029dc`、应用哈希
`2966634f19a44f8bba3758a01273cf1677b9050315a2e88ae0225ba61eac60e2`、撤销精确
恢复、L0–L7 共 8 层、候选就绪、生产未就绪、193,182 字节发布 ZIP 离线验证成功。
...
- `scripts/test.sh`：115 tests，175.541 s，`OK`；包含单元、集成、安全、
  打包契约、真实 KiCad、三配置文件和候选发布链。
- `scripts/python-matrix.sh`：Python 3.11/3.12/3.13/3.14 各 44 tests，分别
...
- 当前五次重复 benchmark：70/70 故障检出、20/20 clean 判定、0 假阳性、0 假
  ...
  3.394265 ms、median 2.890818 ms、p95 4.987415 ms。当前确定性运行没有把未请求
...
产品层提交范围为 `1c8b78d..8d5e8d6`：起点是共享对话式应用基础，终点
`8d5e8d6` 是通过上述 hard gate 的 1.0.0 产品发布实现。本报告的最终证据修订作为
其后的本地文档提交交付；工作树经检查为干净，且没有执行 push 或修改 GitHub 私有性。
