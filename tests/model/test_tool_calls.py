...
class ConfiguredPCBCallProducerTests(unittest.TestCase):
    ...
    def test_native_decision_marks_model_source_and_replays_completed_journal(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                ...
                first = ConfiguredPCBCallProducer(  # type: ignore[arg-type]
                    service
                ).next_call(record, _view(), timeout=5)
        ...
        self.assertEqual(first.source, "model")
        self.assertEqual(first.tool_call_id, "provider-call-model")
        self.assertEqual(first.arguments, {"message": record.user_message})
    ...
    def test_completed_journal_cannot_inject_a_model_hidden_tool(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                ...
                ConfiguredPCBCallProducer(service).next_call(  # type: ignore[arg-type]
    ...
    def test_dispatched_journal_fails_closed_to_deterministic_fallback(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            class CrashingClient:
                ...
                def request_tool_call(self, **_kwargs: Any) -> Any:
                    nonlocal client_calls
                    client_calls += 1
            ...
            with patch(
                ...
                fallback = ConfiguredPCBCallProducer(  # type: ignore[arg-type]
                    service
                ).next_call(record, _view(), timeout=5)
        ...
        self.assertEqual(fallback.source, "runtime_policy")
        self.assertEqual(fallback.arguments, {"message": record.user_message})
    ...
    def test_failed_journal_uses_deterministic_fallback_without_retry(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                ...
                first = producer.next_call(record, _view(), timeout=5)
                replayed = producer.next_call(record, _view(), timeout=5)
        ...
        self.assertIsNotNone(first)
        assert first is not None
        self.assertEqual(first.source, "runtime_policy")
    ...
    def test_model_cannot_rewrite_plan_message(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                ...
                proposal = ConfiguredPCBCallProducer(  # type: ignore[arg-type]
                    service
                ).next_call(record, _view(), timeout=5)
