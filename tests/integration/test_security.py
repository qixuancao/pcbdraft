...
from pcbdraft.core.project import TREE_MEMBER_LIMIT, validate_agent_tree
from pcbdraft.interfaces.api import serve
from pcbdraft.services.managed import generate_managed_project, open_managed_project
from pcbdraft.verification.benchmark import bundled_requirements_path
from pcbdraft.verification.evidence import MAX_ARTIFACTS, record_external_evidence
from pcbdraft.verification.release import build_manufacturing_release


class HostileInputSecurityTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory(prefix="pcbdraft-security-")
    ...
    def test_managed_manifest_path_escape_and_hardlink_are_rejected(self) -> None:
        ...
        manifest_path = escaped / "project.pcbdraft.json"
    ...
    def test_api_oversize_line_is_rejected_without_dispatch(self) -> None:
        ...
        with patch("pcbdraft.interfaces.api.handle_request") as handler:
    ...
    def test_missing_system_pcbnew_python_fails_closed(self) -> None:
        with self.assertRaisesRegex(PCBDraftError, "unavailable"):
