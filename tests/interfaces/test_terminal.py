...
class SlashHandlerTests(unittest.TestCase):
    ...
    def test_deferred_wizard_runs_on_main_thread_and_relaunches(self) -> None:
        ...
        def run_terminal(**_kwargs: Any) -> None:
            nonlocal launches
            launches += 1
            if launches == 1:
                import pcbdraft.interfaces.tui.app as hermes_cli_module
        ...
        with (
            ...
            patch(
                "pcbdraft.interfaces.terminal.connection_status",
                return_value=connected,
            ),
            patch.object(terminal_module, "main", side_effect=run_terminal) as terminal,
            patch(
                "pcbdraft.interfaces.terminal.connect", side_effect=run_wizard
            ) as wizard,
            patch("pcbdraft.agent.tool_bindings.refresh_service_provider") as refresh,
        ):
    ...
    def test_deferred_wizard_error_is_reported_before_relaunch(self) -> None:
        ...
        with (
            ...
            patch.object(terminal_module, "main", side_effect=run_terminal) as terminal,
            patch(
                "pcbdraft.interfaces.terminal.connect",
    ...
    def test_normal_exit_clears_stale_request_without_relaunching(self) -> None:
        ...
        with (
            ...
            patch.object(terminal_module, "main") as terminal,
            patch("pcbdraft.interfaces.terminal.connect") as wizard,
        ):
            self.assertEqual(launch_cli([]), 0)
...
class RepositoryInvariantTests(unittest.TestCase):
    def test_projects_stay_under_the_repository_and_runtime_home_does_not(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
